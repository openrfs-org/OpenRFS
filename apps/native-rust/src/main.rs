...
use openrfs::{File, OPEN_CREATE, OPEN_TRUNCATE, OPEN_WRITE, Startup, Thread,
...
fn application(startup: Startup) -> i32 {
    ...
    if console_write(b"OPENRFS RUST PASS alloc file time entropy thread\n").is_err() {
        return 19;
    }
    0
}

    openrfs::openrfs_main!(application);
