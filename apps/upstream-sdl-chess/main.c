...
/* Bounded OpenRFS harness for SDL 2.32.10's testdrawchessboard application. */

#define SDL_MAIN_HANDLED
#define main openrfs_upstream_sdl_chess_main
#include "upstream.c"
#undef main
...
static int persist_launch(void)
{
    ...
    char state_path[96];
    char *preference_path = SDL_GetPrefPath("OpenRFS", "SDL Chess");
    SDL_RWops *state;
    ...
    return written == sizeof(proof) - 1U && closed == 0 &&
        openrfs_volume_sync(OPENRFS_VOLUME_DATA) == 0 ? 0 : -1;
}
...
int main(int argc, char **argv)
{
    ...
    if (SDL_Init(SDL_INIT_VIDEO) != 0 ||
            SDL_strcmp(SDL_GetPlatform(), "OpenRFS") != 0 ||
            SDL_strcmp(SDL_GetCurrentVideoDriver(), "openrfs") != 0) {
    ...
    printf("OPENRFS SDL CHESS PASS upstream=release-2.32.10 "
