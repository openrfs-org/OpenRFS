...
/* Bounded Trait OS harness for SDL 2.32.10's testdrawchessboard application. */

#define SDL_MAIN_HANDLED
#define main trait_upstream_sdl_chess_main
#include "upstream.c"
#undef main
...
static int persist_launch(void)
{
    ...
    char state_path[96];
    char *preference_path = SDL_GetPrefPath("Trait OS", "SDL Chess");
    SDL_RWops *state;
    ...
    return written == sizeof(proof) - 1U && closed == 0 &&
        trait_volume_sync(TRAIT_VOLUME_DATA) == 0 ? 0 : -1;
}
...
int main(int argc, char **argv)
{
    ...
    if (SDL_Init(SDL_INIT_VIDEO) != 0 ||
            SDL_strcmp(SDL_GetPlatform(), "Trait OS") != 0 ||
            SDL_strcmp(SDL_GetCurrentVideoDriver(), "trait") != 0) {
    ...
    printf("TRAIT SDL CHESS PASS upstream=release-2.32.10 "
