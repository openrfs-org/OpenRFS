...
static int persistent_run(char **preference_path)
{
    ...
    *preference_path = SDL_GetPrefPath("OpenRFS", "SDL proof");
    ...
    if (written != 1U || close_status != 0 ||
        openrfs_volume_sync(OPENRFS_VOLUME_DATA) != 0) {
        return -1;
    }
    return (int)run;
}
...
int main(int argc, char **argv, char **environment)
{
    ...
    if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO | SDL_INIT_TIMER | SDL_INIT_EVENTS) != 0 ||
        strcmp(SDL_GetPlatform(), "OpenRFS") != 0 ||
        strcmp(SDL_GetCurrentVideoDriver(), "openrfs") != 0) {
        printf("OPENRFS SDL FAIL init=%s\n", SDL_GetError());
        goto cleanup;
    }
    run = persistent_run(&preference_path);
    window = SDL_CreateWindow("OpenRFS SDL 2 proof", SDL_WINDOWPOS_CENTERED,
        SDL_WINDOWPOS_CENTERED, 360, 240, SDL_WINDOW_SHOWN);
    if (run <= 0 || window == NULL || draw_window(window, &renderer) != 0 ||
        play_audio(&audio) != 0) {
        printf("OPENRFS SDL FAIL setup=%s\n", SDL_GetError());
        goto cleanup;
    }
    printf("OPENRFS SDL READY run=%d video=%s audio=%s pref=%s\n", run,
        SDL_GetCurrentVideoDriver(), SDL_GetCurrentAudioDriver(),
        preference_path);
    if ((run == 1 && await_input() != 0) || wait_for_audio(audio) != 0) {
        printf("OPENRFS SDL FAIL evidence=%s\n", SDL_GetError());
        goto cleanup;
    }
    printf("OPENRFS SDL PASS run=%d present=partial input=%s audio=non-silent persistent=yes\n",
