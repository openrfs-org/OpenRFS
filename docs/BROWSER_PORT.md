...
Trait OS does not ship a browser engine. NetSurf's framebuffer frontend is the
...
1. **Freestanding build.** Build the NetSurf core, framebuffer frontend,
   ...
   application.
2. **Frontend surface.** Use a bounded Trait OS window, damage rectangles, and
...
descriptors are not part of the Trait OS native ABI.
