...
OpenGAT has fifteen bounded probes for NVIDIA graphics and audio functions.
...
The QEMU model in `tools/qemu/opengat_nvidia_model.c` exercises the complete bind
...
No probe enables bus mastering or allocates DMA. OpenGAT has no IOMMU, so these
