...
`tools/openrfs-package.py` builds, inspects, and installs OpenRFS application
...
The binary manifest starts with `OPENRFS1`, format version and size, ABI
...
```sh
python3 tools/openrfs-package.py build \
    --spec apps/my-app/manifest.json \
    --executable build/my-app/MYAPP.APP \
    --output build/my-app/MYAPP.SPK
python3 tools/openrfs-package.py inspect build/my-app/MYAPP.SPK
python3 tools/openrfs-package.py install-system \
...
```json
{
  ...
  "abi_min": 1,
  "abi_max": 1,
  "identifier": "org.openrfs.example",
  "name": "Example",
  "version": "1.0.0",
...
extraction views, and the OpenRFS client drives their install/update/remove/repair
