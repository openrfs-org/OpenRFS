...
`tools/trait-package.py` builds, inspects, and installs Trait OS application
...
The binary manifest starts with `TRAITAPP`, format version and size, ABI
...
An `.SPK` file contains a 64-byte `TRTPKG01` header, the 1,024-byte manifest,
...
```sh
python3 tools/trait-package.py build \
    --spec apps/my-app/manifest.json \
    --executable build/my-app/MYAPP.APP \
    --output build/my-app/MYAPP.SPK
python3 tools/trait-package.py inspect build/my-app/MYAPP.SPK
python3 tools/trait-package.py install-system \
...
header. It uses the same `TRTPKG01` magic,
...
extraction views, and the Trait client drives their install/update/remove/repair
