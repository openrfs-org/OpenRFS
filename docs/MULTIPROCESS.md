...
OpenGAT can hold up to four user processes and schedule them cooperatively.

## Process state

OpenGAT holds up to four user processes at once. Each owns:
...
- `include/opengat/multiprocess.h` and `src/kernel/multiprocess.c` hold the
