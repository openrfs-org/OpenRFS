...
Trait OS v2.0.0 has one bounded FAT32 implementation for two ordinary emulated
...
The immutable system image is `trait-system-fat32.raw`. It has volume ID
`0x20000001`, label `TRAITSYS`, and contains `BUSYBOX`, `UNAMEBOX`, and
`CATBOX`. The mount is read-only below Trait OS. Each program still has an
...
The writable data image is `trait-data-fat32.raw`. It has volume ID
`0x20000002`, label `TRAITDATA`, and is Trait OS's user-data filesystem.
...
does not prevent the system image, kernel, or Trait OS from working.
...
and next-free values are hints only; Trait OS scans the FAT and replaces them
with measured values.

Before publishing a mount, Trait OS walks the complete live tree. The walk checks
...
by Trait OS contains both, and this bounded subset records explicit cluster
...
`include/trait/fat32_fs.h` exposes mount, unmount, sync, open, close, read,
...
```sh
make fat32-images
python3 tools/fat32_image.py inspect build/userspace/trait-data-fat32.raw
python3 tools/fat32_image.py verify data build/userspace/trait-data-fat32.raw
make verify
make qemu-tests
```

All production evidence uses the normal NVMe submission/completion path and
Trait OS commands. Host inspection is verification tooling, not a substitute
