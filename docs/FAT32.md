...
OpenRFS v2.0.0 has one bounded FAT32 implementation for two ordinary emulated
...
The immutable system image is `openrfs-system-fat32.raw`. It has volume ID
`0x20000001`, label `OPENRFSSYS`, and contains `BUSYBOX`, `UNAMEBOX`, and
`CATBOX`. The mount is read-only below OpenRFS. Each program still has an
...
The writable data image is `openrfs-data-fat32.raw`. It has volume ID
`0x20000002`, label `OPENRFSDATA`, and is OpenRFS's user-data filesystem.
...
does not prevent the system image, kernel, or OpenRFS from working.
...
and next-free values are hints only; OpenRFS scans the FAT and replaces them
with measured values.

Before publishing a mount, OpenRFS walks the complete live tree. The walk checks
...
by OpenRFS contains both, and this bounded subset records explicit cluster
...
`include/openrfs/fat32_fs.h` exposes mount, unmount, sync, open, close, read,
...
```sh
make fat32-images
python3 tools/fat32_image.py inspect build/userspace/openrfs-data-fat32.raw
python3 tools/fat32_image.py verify data build/userspace/openrfs-data-fat32.raw
make verify
make qemu-tests
```

All production evidence uses the normal NVMe submission/completion path and
OpenRFS commands. Host inspection is verification tooling, not a substitute
