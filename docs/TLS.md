...
OpenGAT's first transport-TLS profile uses the pinned BearSSL 0.6 archive in the
SDK. `opengat/tls.h` is a native userspace client API; it uses only public OpenGAT
...
`opengat_tls_client_open()` refuses an empty or malformed trust store. It passes
...
engine. That call bypasses OpenGAT's non-cryptographic generator, samples
...
`opengat_tls_client_close()` attempts authenticated
...
caller may therefore release its input after `opengat_tls_client_open()`
...
`opengat_tls_client_open_diagnostic()` preserves the BearSSL and OpenGAT
transport refusal values even though a failed open returns no client object.
`opengat_tls_client_cancel()` atomically publishes cancellation and routes it to
the underlying OpenGAT stream handle, so a second thread can interrupt a
