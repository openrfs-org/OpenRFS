...
Trait OS's first transport-TLS profile uses the pinned BearSSL 0.6 archive in the
SDK. `trait/tls.h` is a native userspace client API; it uses only public Trait OS
...
`trait_tls_client_open()` refuses an empty or malformed trust store. It passes
...
engine. That call bypasses Trait OS's non-cryptographic generator, samples
...
`trait_tls_client_close()` attempts authenticated
...
caller may therefore release its input after `trait_tls_client_open()`
...
`trait_tls_client_open_diagnostic()` preserves the BearSSL and Trait OS
transport refusal values even though a failed open returns no client object.
`trait_tls_client_cancel()` atomically publishes cancellation and routes it to
the underlying Trait OS stream handle, so a second thread can interrupt a
