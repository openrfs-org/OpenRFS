...
`tools/trait-transaction.py` defines a bounded installed-database format, an
...
The database is at most 32 MiB and starts with `TRTDB001`. Integers are
...
The authority record is exactly 128 bytes and starts with `TRTGN001`.
...
`pkgstate/repo.bin` is a checksummed 128-byte `TRAITREP1` record containing the
...
`include/trait/package_state.h` and `src/kernel/package_state.c` provide the
...
establish that proof. `include/trait/package_service.h` and
...
```sh
python3 tools/trait-transaction.py build-database \
    --spec build/installed-state.json --output build/installed.db
python3 tools/trait-transaction.py inspect-database build/installed.db
python3 tools/trait-transaction.py inspect-authority build/installed.authority
python3 tools/trait-transaction.py inspect-journal build/installed.journal

python3 tools/trait_transaction_host_test.py
...
client and desktop package manager without exposing private staging paths. It consumes only sealed
...
staging. The `native-trait` QEMU path exercises this endpoint as a real Ring 3
...
snapshot and launch quarantine it, and uses `trait repair` to authenticate and
...
retained image with `e2fsck`. Removal is exposed by the Trait client, and the
Trait OS DE package manager's signed SDL Chess listing queues that same bounded client for
