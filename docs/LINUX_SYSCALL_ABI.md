...
OpenGAT runs three pinned static BusyBox programs as bounded compatibility
proofs:

- v0.8.0: `busybox echo OPENGAT`;
...
This is not POSIX, a native OpenGAT ABI, or a general Linux personality. Each
...
`-EFAULT` with no partial output. The record is immutable and OpenGAT-owned;
...
the line supplied by OpenGAT. Those bytes are copied back from userspace
...
frame, restores the kernel CR3 and launch stack, and yields to the OpenGAT
...
The v2.0.0 OpenGAT path uses one deterministic read-only 64 MiB FAT32 system
...
The OpenGAT owner assigns a fresh generation, invokes only the selected
...
linking, PIE, sockets, native OpenGAT syscalls, general mappings, general
