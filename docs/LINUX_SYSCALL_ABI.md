...
OpenRFS runs three pinned static BusyBox programs as bounded compatibility
proofs:

- v0.8.0: `busybox echo OPENRFS`;
...
This is not POSIX, a native OpenRFS ABI, or a general Linux personality. Each
...
`-EFAULT` with no partial output. The record is immutable and OpenRFS-owned;
...
the line supplied by OpenRFS. Those bytes are copied back from userspace
...
frame, restores the kernel CR3 and launch stack, and yields to the OpenRFS
...
The v2.0.0 OpenRFS path uses one deterministic read-only 64 MiB FAT32 system
...
The OpenRFS owner assigns a fresh generation, invokes only the selected
...
linking, PIE, sockets, native OpenRFS syscalls, general mappings, general
