...
Trait OS runs three pinned static BusyBox programs as bounded compatibility
proofs:

- v0.8.0: `busybox echo TRAIT`;
...
This is not POSIX, a native Trait OS ABI, or a general Linux personality. Each
...
`-EFAULT` with no partial output. The record is immutable and Trait-OS-owned;
...
the line supplied by Trait OS. Those bytes are copied back from userspace
...
frame, restores the kernel CR3 and launch stack, and yields to the Trait OS
...
The v2.0.0 Trait OS path uses one deterministic read-only 64 MiB FAT32 system
...
The Trait OS owner assigns a fresh generation, invokes only the selected
...
linking, PIE, sockets, native Trait OS syscalls, general mappings, general
