...
Trait OS's kernel build is self-contained and makes no registry or network access.
...
commit date, Git tree, and license; `vendor/ext4plus/TRAIT-PORT.md` records the
Trait OS delta and the read-write safety gate.
...
single upstream implementation. Trait OS disables ext4plus's async and
...
Trait OS disables BearSSL's host `/dev/urandom`, Win32 random, host-time, SSE2,
AES-NI, and POWER8 compile-time paths. A Trait OS client must inject kernel-backed
...
Trait OS compiles those sources freestanding with MMX/SSE disabled. The package
...
caveat are recorded in `vendor/monocypher/TRAIT-PORT.md`.
...
not code linked into or distributed with Trait OS. The Linux verification image
