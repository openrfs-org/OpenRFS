...
OpenGAT's kernel build is self-contained and makes no registry or network access.
...
commit date, Git tree, and license; `vendor/ext4plus/OPENGAT-PORT.md` records the
OpenGAT delta and the read-write safety gate.
...
| async-opengat | 0.1.92 | MIT OR Apache-2.0 |
...
single upstream implementation. OpenGAT disables ext4plus's async and
...
OpenGAT disables BearSSL's host `/dev/urandom`, Win32 random, host-time, SSE2,
AES-NI, and POWER8 compile-time paths. A OpenGAT client must inject kernel-backed
...
OpenGAT compiles those sources freestanding with MMX/SSE disabled. The package
...
caveat are recorded in `vendor/monocypher/OPENGAT-PORT.md`.
...
not code linked into or distributed with OpenGAT. The Linux verification image
