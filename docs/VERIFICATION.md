...
OpenRFS checks the kernel on the host and in QEMU. Hardware coverage is limited
...
The `native-openrfs` scenario serves a deterministic sequence of root-signed
repositories and publisher-signed v3 packages over that same authenticated
HTTPS path. Across four boots, the Ring 3 `openrfs` client installs version 1,
...
- framebuffer, cached surfaces, keyboard, pointer, shell, and OpenRFS;
...
storage, networking, drivers, and OpenRFS capture. BusyBox binaries
...
The desktop capture opens Files, Terminal, Task Manager, Packages, and Settings
through the guest keyboard path. Screenshots and videos confirm visible guest
interaction. Serial output,
