...
OpenGAT can execute a bounded x86-64 PIE root with authenticated shared-object
...
no publisher signature. OpenGAT's Ed25519-signed format-v3 repository and trust
...
graphs. `tools/opengat_package_host_test.py` covers catalog/resource digest
...
- This does not add a hosted libc ABI or make arbitrary Linux shared objects
  compatible with OpenGAT's native ABI.
