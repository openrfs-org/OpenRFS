...
OpenRFS can execute a bounded x86-64 PIE root with authenticated shared-object
...
no publisher signature. OpenRFS's Ed25519-signed format-v3 repository and trust
...
graphs. `tools/openrfs_package_host_test.py` covers catalog/resource digest
...
- This does not add a hosted libc ABI or make arbitrary Linux shared objects
  compatible with OpenRFS's native ABI.
