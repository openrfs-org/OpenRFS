...
`tools/opengat-repository.py` defines OpenGAT's deterministic repository index and
...
and Ed25519 callbacks. The platform table provisions those keys, the OpenGAT
...
An index starts with the eight-byte `OGTIDX01` magic and a fixed 512-byte header.
...
```sh
python3 tools/opengat-repository.py build \
    --spec repository/index.json \
    --signing-key keys/repository-root-private.pem \
    --output build/repository/index.sri

python3 tools/opengat-repository.py inspect \
    --trusted-root keys/repository-root-public.pem \
    --minimum-version 42 \
    build/repository/index.sri

python3 tools/opengat-repository.py resolve \
    --trusted-root keys/repository-root-public.pem \
    --minimum-version 42 --abi 1 \
    --lock-output build/repository/desktop.lock \
    build/repository/index.sri org.opengat.desktop@^3.0.0

python3 tools/opengat-repository.py verify-download \
    --trusted-root keys/repository-root-public.pem \
    --identifier org.opengat.desktop --version 3.0.0 \
