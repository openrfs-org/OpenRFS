...
OpenGAT includes a pinned BearSSL TLS 1.2 SDK client for bounded,
...
all context and record buffers, which is compatible with OpenGAT's explicit
bounds.

BearSSL supplies the TLS 1.2 engine, not the surrounding web stack. OpenGAT owns
