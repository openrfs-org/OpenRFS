...
Trait OS includes a pinned BearSSL TLS 1.2 SDK client for bounded,
...
all context and record buffers, which is compatible with Trait OS's explicit
bounds.

BearSSL supplies the TLS 1.2 engine, not the surrounding web stack. Trait OS owns
