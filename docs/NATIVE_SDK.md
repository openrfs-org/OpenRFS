...
public ABI and runtime headers, `crt0.o`, `libtrait.a`, the static linker
script, and `bin/trait-cc`. `make reproducible-sdk` builds the complete tree
...
pthread/TLS/futex support, Trait OS surfaces/input, and typed networking
wrappers. `<trait/audio.h>` exposes the fixed 48 kHz S16LE stereo output
handle, whole-chunk submission, Q15 volume, drain, cancellation and close.
Familiar names are a source-porting layer; the kernel remains the Trait OS ABI
rather than a Linux personality.

The installed SDK also provides zlib 1.3.2 as `libz.a` and installs its public
`zlib.h`/`zconf.h`. Trait OS builds the upstream `Z_SOLO` profile: checksums and
streaming deflate/inflate are available, while `gz*`, `compress*`, and
`uncompress*` are not. Include `<trait/zlib.h>` and call
`trait_zlib_stream_prepare()` before `deflateInit*` or `inflateInit*` to use
the checked SDK allocation adapter. Every successful initialization still
requires its matching `deflateEnd` or `inflateEnd` call. `trait-cc` supplies
