...
public ABI and runtime headers, `crt0.o`, `libopengat.a`, the static linker
script, and `bin/opengat-cc`. `make reproducible-sdk` builds the complete tree
...
2. Compile every source with `$(SDK_CC) $(SDK_CFLAGS)` and link `crt0.o`, the
   objects, and `libopengat.a` using `$(SDK_LDFLAGS)`.
3. Add a version 1 manifest with the minimum capabilities and realistic memory,
   handle, and thread limits.
4. Build and inspect the package with `tools/opengat-package.py`.
...
pthread/TLS/futex support, OpenGAT surfaces/input, and typed networking
wrappers. `<opengat/audio.h>` exposes the fixed 48 kHz S16LE stereo output
handle, whole-chunk submission, Q15 volume, drain, cancellation and close.
Familiar names are a source-porting layer; the kernel remains the OpenGAT ABI
rather than a Linux personality.

The installed SDK also provides zlib 1.3.2 as `libz.a` and installs its public
`zlib.h`/`zconf.h`. OpenGAT builds the upstream `Z_SOLO` profile: checksums and
streaming deflate/inflate are available, while `gz*`, `compress*`, and
`uncompress*` are not. Include `<opengat/zlib.h>` and call
`opengat_zlib_stream_prepare()` before `deflateInit*` or `inflateInit*` to use
the checked SDK allocation adapter. Every successful initialization still
requires its matching `deflateEnd` or `inflateEnd` call. `opengat-cc` supplies
