...
| 26. SDL 2 application lifecycle | `native-sdl` | Runs the real SDL Phipia video, event, timer, thread, filesystem, and audio backends twice; requires partial presentation, injected key and pointer input, non-silent queued PCM, synchronized preference state on relaunch, and a clean resource census. |
