...
Trait OS 2.2.0 has a bounded IPv4 networking foundation for one modern
...
This is not an Internet-security claim. Trait OS has no IPv6, general-purpose
...
Until 2.2.0 Trait OS could only be a TCP client. It can now also be the side that
...
`include/trait/network.h` is native ABI version 1. It exposes explicit owners,
...
`include/trait/network_syscall.h` is an experimental Trait-OS-private ABI version
...
a port Trait OS is listening on or to one with no listener. It writes
