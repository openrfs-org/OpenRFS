...
The Trait OS native ABI is the kernel contract for static Ring 3 applications.
It is separate from the three measured Linux/BusyBox profiles. The canonical
machine-readable definitions are under `include/trait/abi/`; the SDK installs
the same headers under `sdk/include/trait/abi/`.
...
negative `trait_errno` value is failure. Unknown numbers return `-ENOSYS`.
...
### Trait OS window, surface, and input
...
payload upload. `trait repair` uses the repair flag with a signed repository and
...
`include/trait/abi/base.h` is the syscall-number and error-number registry.
