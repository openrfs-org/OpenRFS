...
OpenGAT has bounded identification drivers for thirteen PCI devices from Intel,
...
These drivers keep bus mastering disabled. OpenGAT has no IOMMU, so a
...
`include/opengat/driver.h` defines their interface. Two Boot Ledger stages cover
the matrix:
