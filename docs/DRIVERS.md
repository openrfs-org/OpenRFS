...
Trait OS has bounded identification drivers for thirteen PCI devices from Intel,
...
These drivers keep bus mastering disabled. Trait OS has no IOMMU, so a
...
`include/trait/driver.h` defines their interface. Two Boot Ledger stages cover
the matrix:
