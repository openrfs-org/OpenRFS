...
Rust has two separate roles in OpenRFS: validating selected byte
...
| `logo.rs` | the deterministic runtime OpenRFS S-mark stream |
...
`rust/openrfs` is a separate `#![no_std]` application crate. It supplies the
...
entropy, event waits, OpenRFS windows and surfaces, DNS, TCP, threads, futexes,
