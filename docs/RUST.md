...
Rust has two separate roles in OpenGAT: validating selected byte
...
| `logo.rs` | the deterministic runtime OpenGAT S-mark stream |
...
`rust/opengat` is a separate `#![no_std]` application crate. It supplies the
...
entropy, event waits, OpenGAT windows and surfaces, DNS, TCP, threads, futexes,
