...
`trait_https_get()` and `trait_https_get_stream()` are bounded HTTP/1.1
download operations over Trait OS's BearSSL TLS 1.2 client. They are Ring 3 SDK
facilities; they reach the network only through `trait_dns_resolve()` and the
Trait OS stream API. The first fills one caller buffer. The streaming form uses a
...
`trait_package_fetch_stage()` applies that pattern to the Data volume. It
...
and close failure. `trait_https_response` retains the BearSSL and raw Trait OS
...
TLS truncation; Trait OS ABI v1 otherwise exposes orderly TCP closure and reset
through overlapping stream errors. Cancellation is also available directly
through `trait_tls_client_cancel()` on the lower-level client. The synchronous
`trait_https_get()` helper does not expose its internal client, so callers
...
signed repository record before installation; the `native-trait` proof carries
...
This is an in-guest TLS 1.2/HTTPS download over Trait OS DNS and TCP. The client
