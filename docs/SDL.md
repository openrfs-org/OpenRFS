...
Trait OS carries the official SDL 2.32.10 source and a first-class `__TRAIT__`
...
video or audio drivers. Trait OS owns the window surface and receives bounded
damage rectangles through the public native window ABI. Input becomes normal
SDL keyboard, text, pointer, focus, and quit events. Event waits use Trait OS's
public multi-handle wait call, while a native timer handle implements SDL's
wakeup hook without polling.

Audio opens Trait OS's public PCM service and submits ordinary SDL mixed buffers.
...
detach blocks until join because Trait OS ABI v1 cannot reclaim a detached native
...
Trait OS harness runs its original event and software-render loop for eight
...
The Trait OS event pump treats the ABI's empty-queue `-EAGAIN` as normal after it
