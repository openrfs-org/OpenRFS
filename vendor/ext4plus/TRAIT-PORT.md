# Trait OS ext4plus port boundary

Trait OS vendors the `src/` tree from ext4plus commit
...
Trait OS therefore exposes read-write access only through the local retained
...
Trait OS's local journaling delta supplies that admission boundary.
...
admits Trait OS's 4 KiB/checksum-v3/64-bit profile, emits checksummed clean/live
...
to discard it. This narrow API is for Trait OS's coordinator after it has made the
...
to the same revoke/64-bit/checksum-v3 profile that Trait OS admits, then verifies
the clean start, UUID, geometry, feature masks, checksum type, and CRC32C before
the Rust handoff.

`recover_committed_ring` scans Trait OS's bounded single-descriptor transaction
...
Trait OS's mount adapter implements that boundary with native NVMe writes and
...
callback. Trait OS reloads every clean admitted mount with the retained stage as
...
Trait OS's directory delta initializes new directory inodes with checksummed
...
open file can report the initialized physical block at a byte offset so Trait OS
...
Trait OS also tightens upstream writer admission: an image carrying ext4's
...
Trait-OS-specific changes stay in reviewable commits and are summarized here as
