# OpenGAT ext4plus port boundary

OpenGAT vendors the `src/` tree from ext4plus commit
...
OpenGAT therefore exposes read-write access only through the local retained
...
OpenGAT's local journaling delta supplies that admission boundary.
...
admits OpenGAT's 4 KiB/checksum-v3/64-bit profile, emits checksummed clean/live
...
to discard it. This narrow API is for OpenGAT's coordinator after it has made the
...
to the same revoke/64-bit/checksum-v3 profile that OpenGAT admits, then verifies
the clean start, UUID, geometry, feature masks, checksum type, and CRC32C before
the Rust handoff.

`recover_committed_ring` scans OpenGAT's bounded single-descriptor transaction
...
OpenGAT's mount adapter implements that boundary with native NVMe writes and
...
callback. OpenGAT reloads every clean admitted mount with the retained stage as
...
OpenGAT's directory delta initializes new directory inodes with checksummed
...
open file can report the initialized physical block at a byte offset so OpenGAT
...
OpenGAT also tightens upstream writer admission: an image carrying ext4's
...
OpenGAT-specific changes stay in reviewable commits and are summarized here as
