// Copyright 2026 OpenRFS contributors
//
...
/// The only journal block size admitted by OpenRFS's ext4 profile.
...
pub enum JournalTransactionError {
    ...
    TooManyBlocks,
    /// A supplied image was not exactly one OpenRFS filesystem block.
...
/// A checksum-validated JBD2 superblock for OpenRFS's bounded 4 KiB profile.
///
/// The image preserves every admitted byte when its sequence/start state is
/// changed. OpenRFS admits logical journal data beginning at block one; callers
