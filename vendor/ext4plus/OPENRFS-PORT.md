# OpenRFS ext4plus port boundary

OpenRFS vendors the `src/` tree from ext4plus commit
...
OpenRFS therefore exposes read-write access only through the local retained
...
OpenRFS's local journaling delta supplies that admission boundary.
...
admits OpenRFS's 4 KiB/checksum-v3/64-bit profile, emits checksummed clean/live
...
to discard it. This narrow API is for OpenRFS's coordinator after it has made the
...
to the same revoke/64-bit/checksum-v3 profile that OpenRFS admits, then verifies
the clean start, UUID, geometry, feature masks, checksum type, and CRC32C before
the Rust handoff.

`recover_committed_ring` scans OpenRFS's bounded single-descriptor transaction
...
OpenRFS's mount adapter implements that boundary with native NVMe writes and
...
callback. OpenRFS reloads every clean admitted mount with the retained stage as
...
open file can report the initialized physical block at a byte offset so OpenRFS
...
OpenRFS also tightens upstream writer admission: an image carrying ext4's
