...
handles. Vendoring alone does not integrate an OpenRFS shared library or promise
compression support to applications.

The initial x86_64 OpenRFS build should compile exactly these sources:
...
library and every consumer of `zlib.h`. On OpenRFS's x86_64 LP64 target, also
...
The two explicit `HAVE_*` values keep OpenRFS's `-Wundef -Werror` policy while
...
returns `Z_STREAM_ERROR`. OpenRFS's adapter must check multiplication and size
...
`src/`; carry OpenRFS adapters outside the upstream tree and refresh provenance
