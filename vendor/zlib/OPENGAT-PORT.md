...
handles. Vendoring alone does not integrate a OpenGAT shared library or promise
compression support to applications.

The initial x86_64 OpenGAT build should compile exactly these sources:
...
library and every consumer of `zlib.h`. On OpenGAT's x86_64 LP64 target, also
...
The two explicit `HAVE_*` values keep OpenGAT's `-Wundef -Werror` policy while
...
returns `Z_STREAM_ERROR`. OpenGAT's adapter must check multiplication and size
...
`src/`; carry OpenGAT adapters outside the upstream tree and refresh provenance
