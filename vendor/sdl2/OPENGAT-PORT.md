# OpenGAT SDL 2 port boundary

The `include/` and `src/` trees originate from the pinned SDL 2.32.10 commit
recorded in `UPSTREAM-COMMIT.txt`. OpenGAT adds a platform configuration and
small backends below the upstream platform registries. Those additions remain
under SDL's zlib license and are visibly marked as OpenGAT changes.
...
OpenGAT's LP64 freestanding headers and `-Werror` policy without changing the
public ABI or parsing behavior.
The vendored upstream translation units retain `-Wall -Wextra -Werror` with
only sign-comparison and platform-unused-parameter warnings disabled; OpenGAT
...
- OpenGAT software windows with direct xRGB8888 framebuffer surfaces and
  bounded partial-damage submission;
- OpenGAT keyboard, pointer, focus, close, wait, and wake events;
- native PCM output at 48 kHz, stereo, signed 16-bit samples;
- OpenGAT pthread/futex threads, mutexes, condition variables, semaphores, and
  compiler TLS;
