/*
  ...
  OpenRFS thread backend addition for SDL 2.32.10. This file is distributed
  under the same zlib license as SDL.
*/

#include "../../SDL_internal.h"

#ifdef SDL_THREAD_OPENRFS
...
static _Thread_local SDL_TLSData *openrfs_tls_data;

static uint64_t OPENRFS_TimeoutDeadline(Uint32 timeout)
{
...
static long OPENRFS_FutexWait(volatile Uint32 *word, Uint32 expected,
                             uint64_t deadline)
{
    ...
    return openrfs_syscall1(OPENRFS_SYS_FUTEX_WAIT,
        (uint64_t)(uintptr_t)&request);
}

static void OPENRFS_FutexWake(volatile Uint32 *word, Uint32 count)
{
    const struct openrfs_futex_request request = {
        sizeof(request), OPENRFS_ABI_VERSION,
...
static void *OPENRFS_RunThread(void *argument)
{
...
int SDL_SYS_CreateThread(SDL_Thread *thread)
{
    ...
    if (status == 0) {
        status = pthread_create(&thread->handle, &attributes,
            OPENRFS_RunThread, thread);
    }
    if (attributes_initialized) {
        (void)pthread_attr_destroy(&attributes);
    }
    return status == 0 ? 0 : SDL_SetError("OpenRFS thread creation failed: %d",
        status);
}
...
{
    /* OpenRFS v1 has no detached-thread reclamation primitive. Joining here
...
{
    return openrfs_tls_data;
}

int SDL_SYS_SetTLSData(SDL_TLSData *data)
{
    openrfs_tls_data = data;
    return 0;
}

void SDL_SYS_QuitTLSData(void)
{
    openrfs_tls_data = NULL;
}
...
SDL_mutex *SDL_CreateMutex(void)
{
    ...
    if (pthread_mutex_init(&mutex->native, NULL) != 0) {
        SDL_free(mutex);
        SDL_SetError("OpenRFS mutex initialization failed");
        return NULL;
    }
    return mutex;
}
    ...
    if (mutex != NULL) {
        if (pthread_mutex_destroy(&mutex->native) != 0) {
            SDL_LogError(SDL_LOG_CATEGORY_SYSTEM,
                "OpenRFS destroyed a locked SDL mutex");
        }
        SDL_free(mutex);
    }
}
...
int SDL_LockMutex(SDL_mutex *mutex)
{
    ...
    if (mutex->owner == current) {
        if (mutex->recursion == UINT32_MAX) {
            return SDL_SetError("OpenRFS SDL mutex recursion overflow");
        }
        ++mutex->recursion;
        return 0;
    }
    if (pthread_mutex_lock(&mutex->native) != 0) {
        return SDL_SetError("OpenRFS mutex lock failed");
    }
    mutex->owner = current;
    mutex->recursion = 0U;
    return 0;
...
int SDL_TryLockMutex(SDL_mutex *mutex)
{
    ...
    if (status != 0) {
        return SDL_SetError("OpenRFS mutex try-lock failed: %d", status);
    }
    mutex->owner = current;
    mutex->recursion = 0U;
    return 0;
...
int SDL_UnlockMutex(SDL_mutex *mutex)
{
    ...
    if (mutex->owner != SDL_ThreadID()) {
        return SDL_SetError("OpenRFS SDL mutex is not owned by this thread");
    }
    ...
    return pthread_mutex_unlock(&mutex->native) == 0 ? 0 :
        SDL_SetError("OpenRFS mutex unlock failed");
}
...
int SDL_SemWaitTimeout(SDL_sem *semaphore, Uint32 timeout)
{
    const uint64_t deadline = timeout == SDL_MUTEX_MAXWAIT ? 0U :
        OPENRFS_TimeoutDeadline(timeout);
    ...
    for (;;) {
        ...
        result = OPENRFS_FutexWait(&semaphore->count, 0U, deadline);
        if (result == -OPENRFS_ETIMEDOUT) {
            return SDL_MUTEX_TIMEDOUT;
        }
        if (result < 0 && result != -OPENRFS_EAGAIN) {
            return SDL_SetError("OpenRFS semaphore wait failed: %ld", result);
        }
    }
}
...
int SDL_SemPost(SDL_sem *semaphore)
{
    ...
    for (;;) {
        if (observed == UINT32_MAX) {
            return SDL_SetError("OpenRFS semaphore count overflow");
        }
        if (__atomic_compare_exchange_n(&semaphore->count, &observed,
                observed + 1U, SDL_FALSE, __ATOMIC_RELEASE,
                __ATOMIC_ACQUIRE)) {
            OPENRFS_FutexWake(&semaphore->count, 1U);
            return 0;
        }
    }
}
...
int SDL_CondSignal(SDL_cond *condition)
{
    ...
    OPENRFS_FutexWake(&condition->sequence, 1U);
    return 0;
}
...
int SDL_CondBroadcast(SDL_cond *condition)
{
    ...
    (void)__atomic_add_fetch(&condition->sequence, 1U, __ATOMIC_RELEASE);
    OPENRFS_FutexWake(&condition->sequence, UINT32_MAX);
    return 0;
}
...
int SDL_CondWaitTimeout(SDL_cond *condition, SDL_mutex *mutex, Uint32 timeout)
{
    ...
    result = OPENRFS_FutexWait(&condition->sequence, sequence, deadline);
    ...
    return result < 0 && result != -OPENRFS_EAGAIN ?
        SDL_SetError("OpenRFS condition wait failed: %ld", result) : 0;
}
