/*
  ...
  Trait OS thread backend addition for SDL 2.32.10. This file is distributed
  under the same zlib license as SDL.
*/

#include "../../SDL_internal.h"

#ifdef SDL_THREAD_TRAIT
...
static _Thread_local SDL_TLSData *trait_tls_data;

static uint64_t TRAIT_TimeoutDeadline(Uint32 timeout)
{
...
static long TRAIT_FutexWait(volatile Uint32 *word, Uint32 expected,
                             uint64_t deadline)
{
    ...
    return trait_syscall1(TRAIT_SYS_FUTEX_WAIT,
        (uint64_t)(uintptr_t)&request);
}

static void TRAIT_FutexWake(volatile Uint32 *word, Uint32 count)
{
    const struct trait_futex_request request = {
        sizeof(request), TRAIT_ABI_VERSION,
...
static void *TRAIT_RunThread(void *argument)
{
...
int SDL_SYS_CreateThread(SDL_Thread *thread)
{
    ...
    if (status == 0) {
        status = pthread_create(&thread->handle, &attributes,
            TRAIT_RunThread, thread);
    }
    if (attributes_initialized) {
        (void)pthread_attr_destroy(&attributes);
    }
    return status == 0 ? 0 : SDL_SetError("Trait OS thread creation failed: %d",
        status);
}
...
{
    /* Trait OS v1 has no detached-thread reclamation primitive. Joining here
...
SDL_mutex *SDL_CreateMutex(void)
{
    ...
    if (pthread_mutex_init(&mutex->native, NULL) != 0) {
        SDL_free(mutex);
        SDL_SetError("Trait OS mutex initialization failed");
        return NULL;
    }
    return mutex;
}
    ...
    if (mutex != NULL) {
        if (pthread_mutex_destroy(&mutex->native) != 0) {
            SDL_LogError(SDL_LOG_CATEGORY_SYSTEM,
                "Trait OS destroyed a locked SDL mutex");
        }
        SDL_free(mutex);
    }
}
...
int SDL_LockMutex(SDL_mutex *mutex)
{
    ...
    if (mutex->owner == current) {
        if (mutex->recursion == UINT32_MAX) {
            return SDL_SetError("Trait OS SDL mutex recursion overflow");
        }
        ++mutex->recursion;
        return 0;
    }
    if (pthread_mutex_lock(&mutex->native) != 0) {
        return SDL_SetError("Trait OS mutex lock failed");
    }
    mutex->owner = current;
    mutex->recursion = 0U;
    return 0;
...
int SDL_TryLockMutex(SDL_mutex *mutex)
{
    ...
    if (status != 0) {
        return SDL_SetError("Trait OS mutex try-lock failed: %d", status);
    }
    mutex->owner = current;
    mutex->recursion = 0U;
    return 0;
...
int SDL_UnlockMutex(SDL_mutex *mutex)
{
    ...
    if (mutex->owner != SDL_ThreadID()) {
        return SDL_SetError("Trait OS SDL mutex is not owned by this thread");
    }
    ...
    return pthread_mutex_unlock(&mutex->native) == 0 ? 0 :
        SDL_SetError("Trait OS mutex unlock failed");
}
...
int SDL_SemWaitTimeout(SDL_sem *semaphore, Uint32 timeout)
{
    ...
    for (;;) {
        ...
        result = TRAIT_FutexWait(&semaphore->count, 0U, deadline);
        if (result == -TRAIT_ETIMEDOUT) {
            return SDL_MUTEX_TIMEDOUT;
        }
        if (result < 0 && result != -TRAIT_EAGAIN) {
            return SDL_SetError("Trait OS semaphore wait failed: %ld", result);
        }
    }
}
...
int SDL_SemPost(SDL_sem *semaphore)
{
    ...
    for (;;) {
        if (observed == UINT32_MAX) {
            return SDL_SetError("Trait OS semaphore count overflow");
        }
        if (__atomic_compare_exchange_n(&semaphore->count, &observed,
                observed + 1U, SDL_FALSE, __ATOMIC_RELEASE,
                __ATOMIC_ACQUIRE)) {
            TRAIT_FutexWake(&semaphore->count, 1U);
            return 0;
        }
    }
}
...
int SDL_CondSignal(SDL_cond *condition)
{
    ...
    TRAIT_FutexWake(&condition->sequence, 1U);
    return 0;
}
...
int SDL_CondBroadcast(SDL_cond *condition)
{
    ...
    (void)__atomic_add_fetch(&condition->sequence, 1U, __ATOMIC_RELEASE);
    TRAIT_FutexWake(&condition->sequence, UINT32_MAX);
    return 0;
}
...
int SDL_CondWaitTimeout(SDL_cond *condition, SDL_mutex *mutex, Uint32 timeout)
{
    ...
    result = TRAIT_FutexWait(&condition->sequence, sequence, deadline);
    ...
    return result < 0 && result != -TRAIT_EAGAIN ?
        SDL_SetError("Trait OS condition wait failed: %ld", result) : 0;
}
