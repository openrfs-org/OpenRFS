...
unsafe fn syscall0(number: u64) -> i64 {
    let result: i64;
    // SAFETY: the OpenRFS ABI declares RCX and R11 clobbered by SYSCALL.
    unsafe {
...
#[macro_export]
macro_rules! openrfs_main {
    ($main:path) => {
        #[unsafe(no_mangle)]
        pub extern "C" fn openrfs_rust_start(argc: usize, argv: *const *const u8,
            environment: *const *const u8) -> ! {
...
fn panic(_information: &PanicInfo<'_>) -> ! {
    let _ = console_write(b"OpenRFS Rust application panic\n");
    exit(127)
}
