...
def verify_files(image, output, expected):
    ...
    image = output / "disposable.raw"
    shutil.copyfile(preserved_image, image)
    if digest(image) != before:
        raise RuntimeError("disposable interoperability copy differs")
    commands = []
    mounted = False
    ...
    after = digest(image)
    image.unlink()
    if digest(preserved_image) != before:
        raise RuntimeError("preserved guest image changed")
    if after != before:
