 ...
 * capabilities used by OpenGAT. It has no graphics, display, interrupt, or
...
#define TYPE_OPENGAT_NVIDIA_MODEL "opengat-nvidia-model"
OBJECT_DECLARE_SIMPLE_TYPE(OpenGATOSNvidiaState, OPENGAT_NVIDIA_MODEL)
 ...
 */
#define OPENGAT_NVIDIA_PM_OFFSET  0x40
#define OPENGAT_NVIDIA_MSI_OFFSET 0x50
#define OPENGAT_NVIDIA_EXPRESS_OFFSET 0x60
#define OPENGAT_NVIDIA_MSI_VECTORS 1
 ...
 */
#define OPENGAT_NVIDIA_DEFECT_NO_POWER_MANAGEMENT 0x1
#define OPENGAT_NVIDIA_DEFECT_MESSAGE_INTERRUPTS  0x2
#define OPENGAT_NVIDIA_DEFECT_ROOT_PORT           0x4
#define OPENGAT_NVIDIA_DEFECT_ALIASED_SCALE       0x8

#define OPENGAT_NVIDIA_BAR_BYTES  (16 * MiB)
/*
 ...
 */
#define OPENGAT_NVIDIA_FB_BYTES   (256 * MiB)
#define NVIDIA_VENDOR_ID         0x10DE

struct OpenGATOSNvidiaState {
...
static uint64_t opengat_nvidia_read(void *opaque, hwaddr addr, unsigned size)
{
    OpenGATOSNvidiaState *state = opaque;
    ...
    case NV_PTIMER_NUMERATOR:
         ...
         */
        if (state->defects & OPENGAT_NVIDIA_DEFECT_ALIASED_SCALE) {
...
static void opengat_nvidia_write(void *opaque, hwaddr addr, uint64_t value,
                                unsigned size)
{
    OpenGATOSNvidiaState *state = opaque;
...
static const MemoryRegionOps opengat_nvidia_ops = {
    .read = opengat_nvidia_read,
    .write = opengat_nvidia_write,
...
static uint32_t opengat_nvidia_config_read(PCIDevice *dev, uint32_t address,
                                          int len)
{
    OpenGATOSNvidiaState *state = OPENGAT_NVIDIA_MODEL(dev);
    uint32_t value = pci_default_read_config(dev, address, len);

    if (state->defects & OPENGAT_NVIDIA_DEFECT_MESSAGE_INTERRUPTS) {
        value = patch_field(value, address, len,
                            OPENGAT_NVIDIA_MSI_OFFSET + PCI_MSI_FLAGS,
                            0, PCI_MSI_FLAGS_ENABLE);
    }
    if (state->defects & OPENGAT_NVIDIA_DEFECT_ROOT_PORT) {
        value = patch_field(value, address, len,
                            OPENGAT_NVIDIA_EXPRESS_OFFSET + PCI_EXP_FLAGS,
                            PCI_EXP_FLAGS_TYPE,
...
static void opengat_nvidia_realize(PCIDevice *dev, Error **errp)
{
    OpenGATOSNvidiaState *state = OPENGAT_NVIDIA_MODEL(dev);
    ...
    memory_region_init_io(&state->registers, OBJECT(state),
                          &opengat_nvidia_ops, state,
                          "opengat-nvidia-model/registers",
                          OPENGAT_NVIDIA_BAR_BYTES);
    ...
    memory_region_init_ram(&state->framebuffer, OBJECT(state),
                           "opengat-nvidia-model/framebuffer",
                           OPENGAT_NVIDIA_FB_BYTES, &error_fatal);
     ...
     */
    if (!(state->defects & OPENGAT_NVIDIA_DEFECT_NO_POWER_MANAGEMENT)) {
        if (pci_add_capability(dev, PCI_CAP_ID_PM, OPENGAT_NVIDIA_PM_OFFSET,
                               PCI_PM_SIZEOF, errp) < 0) {
            return;
        }
        pci_set_word(dev->config + OPENGAT_NVIDIA_PM_OFFSET + PCI_PM_PMC,
                     PCI_PM_CAP_VER_1_2);
        pci_set_word(dev->wmask + OPENGAT_NVIDIA_PM_OFFSET + PCI_PM_CTRL,
     ...
     */
    if (msi_init(dev, OPENGAT_NVIDIA_MSI_OFFSET, OPENGAT_NVIDIA_MSI_VECTORS,
                 true, false, errp) < 0) {
        return;
    }
     ...
     */
    if (pcie_endpoint_cap_init(dev, OPENGAT_NVIDIA_EXPRESS_OFFSET) < 0) {
        error_setg(errp, "could not install the PCI Express capability");
        return;
    }
}

static void opengat_nvidia_exit(PCIDevice *dev)
{
    msi_uninit(dev);
}

static Property opengat_nvidia_properties[] = {
    DEFINE_PROP_UINT32("boot0", OpenGATOSNvidiaState, boot0, 0x134000A1),
    DEFINE_PROP_UINT32("boot1", OpenGATOSNvidiaState, boot1, 0x00000000),
    DEFINE_PROP_UINT32("straps", OpenGATOSNvidiaState, straps, 0x0000042C),
    DEFINE_PROP_UINT32("enable", OpenGATOSNvidiaState, enable, 0x11111111),
    DEFINE_PROP_UINT32("subsystem", OpenGATOSNvidiaState, subsystem, 0x87651043),
    DEFINE_PROP_UINT32("numerator", OpenGATOSNvidiaState, numerator, 27),
    DEFINE_PROP_UINT32("denominator", OpenGATOSNvidiaState, denominator, 1000),
    DEFINE_PROP_UINT32("rom", OpenGATOSNvidiaState, rom_declaration, 0),
    DEFINE_PROP_UINT32("defects", OpenGATOSNvidiaState, defects, 0),
    DEFINE_PROP_STRING("vbios", OpenGATOSNvidiaState, vbios_path),
    DEFINE_PROP_END_OF_LIST(),
};

static void opengat_nvidia_class_init(ObjectClass *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);
    PCIDeviceClass *pci = PCI_DEVICE_CLASS(klass);

    pci->realize = opengat_nvidia_realize;
    pci->exit = opengat_nvidia_exit;
    pci->config_read = opengat_nvidia_config_read;
    ...
    device_class_set_props(dc, opengat_nvidia_properties);
    set_bit(DEVICE_CATEGORY_MISC, dc->categories);
}

static const TypeInfo opengat_nvidia_info = {
    .name = TYPE_OPENGAT_NVIDIA_MODEL,
    .parent = TYPE_PCI_DEVICE,
    .instance_size = sizeof(OpenGATOSNvidiaState),
    .class_init = opengat_nvidia_class_init,
...
static void opengat_nvidia_register_types(void)
{
    type_register_static(&opengat_nvidia_info);
}

type_init(opengat_nvidia_register_types)
