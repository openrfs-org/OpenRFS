 ...
 * capabilities used by Trait OS. It has no graphics, display, interrupt, or
...
#define TYPE_TRAIT_NVIDIA_MODEL "trait-nvidia-model"
OBJECT_DECLARE_SIMPLE_TYPE(TraitOSNvidiaState, TRAIT_NVIDIA_MODEL)
 ...
 */
#define TRAIT_NVIDIA_PM_OFFSET  0x40
#define TRAIT_NVIDIA_MSI_OFFSET 0x50
#define TRAIT_NVIDIA_EXPRESS_OFFSET 0x60
#define TRAIT_NVIDIA_MSI_VECTORS 1
 ...
 */
#define TRAIT_NVIDIA_DEFECT_NO_POWER_MANAGEMENT 0x1
#define TRAIT_NVIDIA_DEFECT_MESSAGE_INTERRUPTS  0x2
#define TRAIT_NVIDIA_DEFECT_ROOT_PORT           0x4
#define TRAIT_NVIDIA_DEFECT_ALIASED_SCALE       0x8

#define TRAIT_NVIDIA_BAR_BYTES  (16 * MiB)
/*
 ...
 */
#define TRAIT_NVIDIA_FB_BYTES   (256 * MiB)
#define NVIDIA_VENDOR_ID         0x10DE

struct TraitOSNvidiaState {
...
static uint64_t trait_nvidia_read(void *opaque, hwaddr addr, unsigned size)
{
    TraitOSNvidiaState *state = opaque;
    ...
    case NV_PTIMER_NUMERATOR:
         ...
         */
        if (state->defects & TRAIT_NVIDIA_DEFECT_ALIASED_SCALE) {
...
static void trait_nvidia_write(void *opaque, hwaddr addr, uint64_t value,
                                unsigned size)
{
    TraitOSNvidiaState *state = opaque;
...
static const MemoryRegionOps trait_nvidia_ops = {
    .read = trait_nvidia_read,
    .write = trait_nvidia_write,
...
static uint32_t trait_nvidia_config_read(PCIDevice *dev, uint32_t address,
                                          int len)
{
    TraitOSNvidiaState *state = TRAIT_NVIDIA_MODEL(dev);
    uint32_t value = pci_default_read_config(dev, address, len);

    if (state->defects & TRAIT_NVIDIA_DEFECT_MESSAGE_INTERRUPTS) {
        value = patch_field(value, address, len,
                            TRAIT_NVIDIA_MSI_OFFSET + PCI_MSI_FLAGS,
                            0, PCI_MSI_FLAGS_ENABLE);
    }
    if (state->defects & TRAIT_NVIDIA_DEFECT_ROOT_PORT) {
        value = patch_field(value, address, len,
                            TRAIT_NVIDIA_EXPRESS_OFFSET + PCI_EXP_FLAGS,
                            PCI_EXP_FLAGS_TYPE,
...
static void trait_nvidia_realize(PCIDevice *dev, Error **errp)
{
    TraitOSNvidiaState *state = TRAIT_NVIDIA_MODEL(dev);
    ...
    memory_region_init_io(&state->registers, OBJECT(state),
                          &trait_nvidia_ops, state,
                          "trait-nvidia-model/registers",
                          TRAIT_NVIDIA_BAR_BYTES);
    ...
    memory_region_init_ram(&state->framebuffer, OBJECT(state),
                           "trait-nvidia-model/framebuffer",
                           TRAIT_NVIDIA_FB_BYTES, &error_fatal);
     ...
     */
    if (!(state->defects & TRAIT_NVIDIA_DEFECT_NO_POWER_MANAGEMENT)) {
        if (pci_add_capability(dev, PCI_CAP_ID_PM, TRAIT_NVIDIA_PM_OFFSET,
                               PCI_PM_SIZEOF, errp) < 0) {
            return;
        }
        pci_set_word(dev->config + TRAIT_NVIDIA_PM_OFFSET + PCI_PM_PMC,
                     PCI_PM_CAP_VER_1_2);
        pci_set_word(dev->wmask + TRAIT_NVIDIA_PM_OFFSET + PCI_PM_CTRL,
     ...
     */
    if (msi_init(dev, TRAIT_NVIDIA_MSI_OFFSET, TRAIT_NVIDIA_MSI_VECTORS,
                 true, false, errp) < 0) {
        return;
    }
     ...
     */
    if (pcie_endpoint_cap_init(dev, TRAIT_NVIDIA_EXPRESS_OFFSET) < 0) {
        error_setg(errp, "could not install the PCI Express capability");
        return;
    }
}

static void trait_nvidia_exit(PCIDevice *dev)
{
    msi_uninit(dev);
}

static Property trait_nvidia_properties[] = {
    DEFINE_PROP_UINT32("boot0", TraitOSNvidiaState, boot0, 0x134000A1),
    DEFINE_PROP_UINT32("boot1", TraitOSNvidiaState, boot1, 0x00000000),
    DEFINE_PROP_UINT32("straps", TraitOSNvidiaState, straps, 0x0000042C),
    DEFINE_PROP_UINT32("enable", TraitOSNvidiaState, enable, 0x11111111),
    DEFINE_PROP_UINT32("subsystem", TraitOSNvidiaState, subsystem, 0x87651043),
    DEFINE_PROP_UINT32("numerator", TraitOSNvidiaState, numerator, 27),
    DEFINE_PROP_UINT32("denominator", TraitOSNvidiaState, denominator, 1000),
    DEFINE_PROP_UINT32("rom", TraitOSNvidiaState, rom_declaration, 0),
    DEFINE_PROP_UINT32("defects", TraitOSNvidiaState, defects, 0),
    DEFINE_PROP_STRING("vbios", TraitOSNvidiaState, vbios_path),
    DEFINE_PROP_END_OF_LIST(),
};

static void trait_nvidia_class_init(ObjectClass *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);
    PCIDeviceClass *pci = PCI_DEVICE_CLASS(klass);

    pci->realize = trait_nvidia_realize;
    pci->exit = trait_nvidia_exit;
    pci->config_read = trait_nvidia_config_read;
    ...
    device_class_set_props(dc, trait_nvidia_properties);
    set_bit(DEVICE_CATEGORY_MISC, dc->categories);
}

static const TypeInfo trait_nvidia_info = {
    .name = TYPE_TRAIT_NVIDIA_MODEL,
    .parent = TYPE_PCI_DEVICE,
    .instance_size = sizeof(TraitOSNvidiaState),
    .class_init = trait_nvidia_class_init,
...
static void trait_nvidia_register_types(void)
{
    type_register_static(&trait_nvidia_info);
}

type_init(trait_nvidia_register_types)
