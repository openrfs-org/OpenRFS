 ...
 * capabilities used by OpenRFS. It has no graphics, display, interrupt, or
...
#define TYPE_OPENRFS_NVIDIA_MODEL "openrfs-nvidia-model"
OBJECT_DECLARE_SIMPLE_TYPE(OpenRFSOSNvidiaState, OPENRFS_NVIDIA_MODEL)
 ...
 */
#define OPENRFS_NVIDIA_PM_OFFSET  0x40
#define OPENRFS_NVIDIA_MSI_OFFSET 0x50
#define OPENRFS_NVIDIA_EXPRESS_OFFSET 0x60
#define OPENRFS_NVIDIA_MSI_VECTORS 1
 ...
 */
#define OPENRFS_NVIDIA_DEFECT_NO_POWER_MANAGEMENT 0x1
#define OPENRFS_NVIDIA_DEFECT_MESSAGE_INTERRUPTS  0x2
#define OPENRFS_NVIDIA_DEFECT_ROOT_PORT           0x4
#define OPENRFS_NVIDIA_DEFECT_ALIASED_SCALE       0x8

#define OPENRFS_NVIDIA_BAR_BYTES  (16 * MiB)
/*
 ...
 */
#define OPENRFS_NVIDIA_FB_BYTES   (256 * MiB)
#define NVIDIA_VENDOR_ID         0x10DE

struct OpenRFSOSNvidiaState {
...
static uint64_t openrfs_nvidia_read(void *opaque, hwaddr addr, unsigned size)
{
    OpenRFSOSNvidiaState *state = opaque;
    ...
    case NV_PTIMER_NUMERATOR:
         ...
         */
        if (state->defects & OPENRFS_NVIDIA_DEFECT_ALIASED_SCALE) {
...
static void openrfs_nvidia_write(void *opaque, hwaddr addr, uint64_t value,
                                unsigned size)
{
    OpenRFSOSNvidiaState *state = opaque;
...
static const MemoryRegionOps openrfs_nvidia_ops = {
    .read = openrfs_nvidia_read,
    .write = openrfs_nvidia_write,
...
static uint32_t openrfs_nvidia_config_read(PCIDevice *dev, uint32_t address,
                                          int len)
{
    OpenRFSOSNvidiaState *state = OPENRFS_NVIDIA_MODEL(dev);
    uint32_t value = pci_default_read_config(dev, address, len);

    if (state->defects & OPENRFS_NVIDIA_DEFECT_MESSAGE_INTERRUPTS) {
        value = patch_field(value, address, len,
                            OPENRFS_NVIDIA_MSI_OFFSET + PCI_MSI_FLAGS,
                            0, PCI_MSI_FLAGS_ENABLE);
    }
    if (state->defects & OPENRFS_NVIDIA_DEFECT_ROOT_PORT) {
        value = patch_field(value, address, len,
                            OPENRFS_NVIDIA_EXPRESS_OFFSET + PCI_EXP_FLAGS,
                            PCI_EXP_FLAGS_TYPE,
...
static void openrfs_nvidia_realize(PCIDevice *dev, Error **errp)
{
    OpenRFSOSNvidiaState *state = OPENRFS_NVIDIA_MODEL(dev);
    ...
    memory_region_init_io(&state->registers, OBJECT(state),
                          &openrfs_nvidia_ops, state,
                          "openrfs-nvidia-model/registers",
                          OPENRFS_NVIDIA_BAR_BYTES);
    ...
    memory_region_init_ram(&state->framebuffer, OBJECT(state),
                           "openrfs-nvidia-model/framebuffer",
                           OPENRFS_NVIDIA_FB_BYTES, &error_fatal);
     ...
     */
    if (!(state->defects & OPENRFS_NVIDIA_DEFECT_NO_POWER_MANAGEMENT)) {
        if (pci_add_capability(dev, PCI_CAP_ID_PM, OPENRFS_NVIDIA_PM_OFFSET,
                               PCI_PM_SIZEOF, errp) < 0) {
            return;
        }
        pci_set_word(dev->config + OPENRFS_NVIDIA_PM_OFFSET + PCI_PM_PMC,
                     PCI_PM_CAP_VER_1_2);
        pci_set_word(dev->wmask + OPENRFS_NVIDIA_PM_OFFSET + PCI_PM_CTRL,
     ...
     */
    if (msi_init(dev, OPENRFS_NVIDIA_MSI_OFFSET, OPENRFS_NVIDIA_MSI_VECTORS,
                 true, false, errp) < 0) {
        return;
    }
     ...
     */
    if (pcie_endpoint_cap_init(dev, OPENRFS_NVIDIA_EXPRESS_OFFSET) < 0) {
        error_setg(errp, "could not install the PCI Express capability");
        return;
    }
}

static void openrfs_nvidia_exit(PCIDevice *dev)
{
    msi_uninit(dev);
}

static Property openrfs_nvidia_properties[] = {
    DEFINE_PROP_UINT32("boot0", OpenRFSOSNvidiaState, boot0, 0x134000A1),
    DEFINE_PROP_UINT32("boot1", OpenRFSOSNvidiaState, boot1, 0x00000000),
    DEFINE_PROP_UINT32("straps", OpenRFSOSNvidiaState, straps, 0x0000042C),
    DEFINE_PROP_UINT32("enable", OpenRFSOSNvidiaState, enable, 0x11111111),
    DEFINE_PROP_UINT32("subsystem", OpenRFSOSNvidiaState, subsystem, 0x87651043),
    DEFINE_PROP_UINT32("numerator", OpenRFSOSNvidiaState, numerator, 27),
    DEFINE_PROP_UINT32("denominator", OpenRFSOSNvidiaState, denominator, 1000),
    DEFINE_PROP_UINT32("rom", OpenRFSOSNvidiaState, rom_declaration, 0),
    DEFINE_PROP_UINT32("defects", OpenRFSOSNvidiaState, defects, 0),
    DEFINE_PROP_STRING("vbios", OpenRFSOSNvidiaState, vbios_path),
    DEFINE_PROP_END_OF_LIST(),
};

static void openrfs_nvidia_class_init(ObjectClass *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);
    PCIDeviceClass *pci = PCI_DEVICE_CLASS(klass);

    pci->realize = openrfs_nvidia_realize;
    pci->exit = openrfs_nvidia_exit;
    pci->config_read = openrfs_nvidia_config_read;
    ...
    device_class_set_props(dc, openrfs_nvidia_properties);
    set_bit(DEVICE_CATEGORY_MISC, dc->categories);
}

static const TypeInfo openrfs_nvidia_info = {
    .name = TYPE_OPENRFS_NVIDIA_MODEL,
    .parent = TYPE_PCI_DEVICE,
    .instance_size = sizeof(OpenRFSOSNvidiaState),
    .class_init = openrfs_nvidia_class_init,
...
static void openrfs_nvidia_register_types(void)
{
    type_register_static(&openrfs_nvidia_info);
}

type_init(openrfs_nvidia_register_types)
