#!/usr/bin/env python3
"""Canonical OpenRFS installed-state and transaction-recovery reference model.
...
PACKAGE_TOOL_PATH = Path(__file__).with_name("openrfs-package.py")
PACKAGE_TOOL_SPEC = importlib.util.spec_from_file_location(
    "openrfs_transaction_package_tool", PACKAGE_TOOL_PATH)
if PACKAGE_TOOL_SPEC is None or PACKAGE_TOOL_SPEC.loader is None:
    raise RuntimeError("could not load OpenRFS package tool")
...
AUTHORITY_MAGIC = b"ORFSGN01"
AUTHORITY_BYTES = 128

JOURNAL_MAGIC = b"ORFSTX01"
JOURNAL_BYTES = 512
...
def main() -> int:
    ...
    try:
        ...
        print(f"OpenRFS transaction state refused: {error}", file=sys.stderr)
        return 1
    return 0
