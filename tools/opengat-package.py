#!/usr/bin/env python3
"""Build, inspect, and install deterministic OpenGAT application packages."""
...
PACKAGE_MAGIC = b"OGTPKG01"
MANIFEST_MAGIC = b"OPENGAT1"
V3_HEADER_BYTES = 512
...
def inspect_dynamic_catalog(report: dict[str, Any],
    ...
    if len(catalog) != 2048 or catalog[:8] != b"OGTDYN01":
...
def _v3_file_records(files: tuple[dict[str, Any], ...],
    ...
    _reject_file_ancestors(
        paths, "a packaged file cannot be another file's ancestor")
...
def main() -> int:
    try:
        ...
        print(f"OpenGAT package refused: {error}", file=sys.stderr)
        return 1
    return 0
