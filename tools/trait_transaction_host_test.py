...
SPEC = importlib.util.spec_from_file_location(
    "trait_transaction", ROOT / "tools" / "trait-transaction.py")
...
def test_canonical_database_authority_and_journal() -> None:
    lib = package("org.trait.lib", "1.0.0", "lib/libx.so.1", b"library",
                  explicit=False)
    app = package(
        "org.trait.app", "1.0.0", "bin/app", b"application", explicit=True,
        dependencies=[dependency("org.trait.lib")], kind="executable")
...
def test_interrupted_commit_selects_only_complete_old_or_new() -> None:
    candidate = package(
        "org.trait.app", "1.0.0", "bin/app", b"v1", explicit=True,
        kind="executable")
...
def test_update_rollback_and_cancellation_cleanup() -> None:
    store = TRANSACTION.create_store(capacity_bytes=1_000_000)
    version_one = package(
        "org.trait.app", "1.0.0", "bin/app", b"version one", explicit=True,
        kind="executable")
    ...
    version_two = package(
        "org.trait.app", "2.0.0", "bin/app", b"version two", explicit=True,
        kind="executable")
...
def test_remove_keeps_shared_dependency_and_preserves_user_data() -> None:
    user_data = {"org.trait.app-a/settings.db": b"personal settings"}
    store = TRANSACTION.create_store(
        capacity_bytes=2_000_000, user_data=user_data)
    lib = package("org.trait.lib", "1.0.0", "lib/libshared.so.1", b"shared",
                  explicit=False)
    app_a = package(
        "org.trait.app-a", "1.0.0", "bin/app-a", b"app a", explicit=True,
        dependencies=[dependency("org.trait.lib")], kind="executable")
    app_b = package(
        "org.trait.app-b", "1.0.0", "bin/app-b", b"app b", explicit=True,
        dependencies=[dependency("org.trait.lib")], kind="executable")
    commit_install(store, [lib, app_a, app_b])
    TRANSACTION.stage_remove(store, ["org.trait.app-a"])
...
def test_collision_low_space_and_transaction_refusals() -> None:
    store = TRANSACTION.create_store(capacity_bytes=1_000_000)
    first = package("org.trait.one", "1.0.0", "lib/shared", b"one",
                    explicit=True)
    commit_install(store, [first])
    second = package("org.trait.two", "1.0.0", "lib/shared", b"two",
                     explicit=True)
    ...
    cycle_a = package(
        "org.trait.a", "1.0.0", "lib/a", b"a", explicit=True,
        dependencies=[dependency("org.trait.b")])
    cycle_b = package(
        "org.trait.b", "1.0.0", "lib/b", b"b", explicit=False,
        dependencies=[dependency("org.trait.a")])
...
def test_verify_and_repair_preserve_user_data() -> None:
    user_data = {"org.trait.app/document.txt": b"do not replace"}
    store = TRANSACTION.create_store(
        capacity_bytes=1_000_000, user_data=user_data)
    payload = b"known authenticated application"
    app = package(
        "org.trait.app", "1.0.0", "bin/app", payload, explicit=True,
        kind="executable")
...
def test_recovery_falls_back_from_incomplete_new_generation() -> None:
    store = TRANSACTION.create_store(capacity_bytes=1_000_000)
    version_one = package(
        "org.trait.app", "1.0.0", "bin/app", b"one", explicit=True,
        kind="executable")
    commit_install(store, [version_one])
    version_two = package(
        "org.trait.app", "2.0.0", "bin/app", b"two", explicit=True,
        kind="executable")
    TRANSACTION.stage_install(store, [version_two])
    assert TRANSACTION.commit(store, interrupt="after-authority") \
        == "interrupted-after-authority"
...
def main() -> int:
    ...
    print(
        "Trait OS transaction host tests passed: canonical state, staging, atomic recovery, "
