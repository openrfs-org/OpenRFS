...
"""Capture a real Trait OS FAT32 create/sync/reboot/read interaction.
...
PROOF = b"Trait OS: Boot Ledger installed proof passed"
...
def open_terminal(qmp):
    """Open Terminal through ordinary Trait OS keyboard focus."""
    qmp.hmp("sendkey tab")
    time.sleep(0.10)
    qmp.hmp("sendkey ret")
...
def storage_arguments(system, data):
    return [
        ...
        "read-only=on",
        "-device", "nvme,serial=trait-system-fat32,drive=system-raw,"
        ...
        "read-only=off",
        "-device", "nvme,serial=trait-data-fat32,drive=data-raw,"
        "logical_block_size=512,physical_block_size=512,max_ioqpairs=1,"
        "msix_qsize=1",
    ]
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="trait-fat32-video-") as temp:
        temp = Path(temp)
        ...
        try:
            ...
            prompt_count = serial.read_bytes().count(b"trait> ")
            open_terminal(qmp)
            wait_count(serial, b"trait> ", prompt_count + 1, 15.0)
                ...
                if action_index < len(actions) and elapsed >= actions[action_index][0]:
                    ...
                    if text == "reboot":
                        ...
                        prompt_count = second_serial.read_bytes().count(
                            b"trait> ")
                        open_terminal(qmp)
                        wait_count(second_serial, b"trait> ",
                            prompt_count + 1, 15.0)
                        prompt_count = second_serial.read_bytes().count(
                            b"trait> ")
