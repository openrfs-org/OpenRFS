...
)
{
    ...
    put_text(application_record, 64U, "org.opengat.app");
    ...
    put_text(library_record, 64U, "org.opengat.lib");
...
)
{
    ...
    CHECK(package_builder_build(repository, NULL, plan, packages, 2U, policy,
        ...
        state_text_is(&workspace->packages[0].identifier, "org.opengat.app") &&
        workspace->packages[0].explicit_root &&
        workspace->packages[0].dependency_start == 0U &&
        workspace->packages[0].dependency_count == 1U &&
        state_text_is(&workspace->packages[1].identifier, "org.opengat.lib") &&
    ...
    CHECK(package_manager_plan_install(repository, &encoded_view,
        (const uint8_t *)"org.opengat.lib", 15U, policy, trust,
    ...
    CHECK(package_manager_plan_remove(&promoted_view,
        (const uint8_t *)"org.opengat.app", 15U, &removal_plan) ==
            PACKAGE_MANAGER_STATUS_OK && removal_plan.count == 1U &&
        text_is(&removal_plan.items[0].identifier, "org.opengat.app"));
    CHECK(package_builder_build(NULL, &promoted_view, &removal_plan, NULL, 0U,
        ...
        state_text_is(&workspace->packages[0].identifier, "org.opengat.lib") &&
        workspace->packages[0].explicit_root &&
    ...
    if (remove_all) {
        ...
        CHECK(package_builder_build(repository, installed, plan, NULL, 0U,
            ...
            state_text_is(&workspace->packages[0].identifier,
                "org.opengat.app") && workspace->packages[0].explicit_root &&
            state_text_is(&workspace->packages[1].identifier,
                "org.opengat.lib") && workspace->packages[1].explicit_root);
    }
    CHECK(package_generation_size(&workspace->spec, &encoded_bytes) ==
        PACKAGE_STATE_STATUS_OK);
    encoded = malloc(encoded_bytes);
...
)
{
    ...
    CHECK(package_manager_plan_install(&repository, installed,
        (const uint8_t *)"org.opengat.app", 15U, policy, trust, &plan) ==
            PACKAGE_MANAGER_STATUS_OK &&
        plan.operation == PACKAGE_MANAGER_PLAN_UPDATE && plan.count == 2U &&
        text_is(&plan.items[0].identifier, "org.opengat.newlib") &&
        text_is(&plan.items[1].identifier, "org.opengat.app"));
    packages[0] = (struct package_builder_package_bytes){
        library->bytes, library->count
    };
    packages[1] = (struct package_builder_package_bytes){
...
int main(int argc, char **argv)
{
    ...
    CHECK(repository.repository_version == FIXTURE_REPOSITORY_VERSION &&
        ...
        find_entry(&repository, "org.opengat.app", &application_entry) &&
        find_entry(&repository, "org.opengat.lib", &library_entry));
    ...
    CHECK(package_manager_plan_install(&repository, NULL,
        ...
        text_is(&plan.target, "org.opengat.app") &&
        text_is(&plan.root, "org.opengat.app") && plan.count == 2U &&
        text_is(&plan.items[0].identifier, "org.opengat.lib") &&
        text_is(&plan.items[1].identifier, "org.opengat.app"));
    CHECK(package_manager_plan_dependency_binding(&repository, &plan, 1U, 0U,
        ...
        text_is(&application_binding.requested, "org.opengat.lib") &&
        text_is(&application_binding.constraint, ">=1.0.0,<2.0.0") &&
        text_is(&application_binding.provider, "org.opengat.lib") &&
    ...
    CHECK(package_manager_package_open(application_bytes.bytes,
        ...
        text_is(&application_package.identifier, "org.opengat.app") &&
        ...
        text_is(&application_dependency.identifier, "org.opengat.lib") &&
    ...
    CHECK(package_manager_package_open(library_bytes.bytes, library_bytes.count,
        &library_entry, &policy, &trust, &library_package) ==
            PACKAGE_MANAGER_STATUS_OK &&
        text_is(&library_package.identifier, "org.opengat.lib") &&
    ...
    CHECK(package_manager_plan_remove(&installed,
        (const uint8_t *)"org.opengat.app", 15U, &plan) ==
            PACKAGE_MANAGER_STATUS_OK &&
        text_is(&plan.target, "org.opengat.app") && plan.count == 2U &&
        text_is(&plan.root, "org.opengat.app") &&
        text_is(&plan.items[0].identifier, "org.opengat.app") &&
        text_is(&plan.items[1].identifier, "org.opengat.lib"));
    CHECK(test_existing_builder(NULL, &installed, &plan, NULL, NULL, true) == 0);
    CHECK(package_manager_plan_remove(&installed,
        (const uint8_t *)"org.opengat.lib", 15U, &plan) ==
    ...
    CHECK(package_manager_plan_install(&repository, &installed,
        ...
        text_is(&plan.root, "org.opengat.lib") && plan.count == 0U);
    ...
    CHECK(expect_plan_status(argv[6], "org.opengat.a",
        PACKAGE_MANAGER_STATUS_CYCLE, &context) == 0);
    CHECK(expect_plan_status(argv[7], "org.opengat.conflict-app",
        PACKAGE_MANAGER_STATUS_CONFLICT, &context) == 0);
    CHECK(expect_plan_status(argv[8], "org.opengat.ambiguous-app",
        PACKAGE_MANAGER_STATUS_AMBIGUOUS_PROVIDER, &context) == 0);
    CHECK(expect_plan_status(argv[9], "org.opengat.unsatisfied",
        PACKAGE_MANAGER_STATUS_DEPENDENCY, &context) == 0);
    CHECK(expect_plan_status(argv[10], "org.opengat.backtrack",
        PACKAGE_MANAGER_STATUS_OK, &context) == 0);
    CHECK(expect_plan_status(argv[11], "org.opengat.chain00",
    ...
    (void)puts("OpenGAT bounded guest package-manager and generation-builder "
        "tests passed");
    return 0;
}
