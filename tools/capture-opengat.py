...
"""Capture the current OpenGAT desktop applications from a live QEMU guest.
...
_PROOF_PATH = Path(__file__).with_name("capture-opengat-proof.py")
_PROOF_SPEC = importlib.util.spec_from_file_location("capture_opengat_proof", _PROOF_PATH)
...
def main():
    ...
    durable_data = output / "opengat-data.raw"
    shutil.copyfile(Path(args.data).resolve(), durable_data)
    serial = output / "opengat-serial.log"
    if serial.exists():
        serial.unlink()
    ...
    with tempfile.TemporaryDirectory(prefix="opengat-desktop-capture-"):
        ...
        try:
            ...
            capture(qmp, output, "opengat-console")
            ...
            capture(qmp, output, "opengat-files")
            press(qmp, "esc")
            capture(qmp, output, "opengat-desktop")

            applications = (
                ("opengat-terminal", 1),
                ("opengat-task-manager", 1),
                ("opengat-packages", 1),
                ("opengat-settings", 1),
            )
