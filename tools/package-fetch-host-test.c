...
static const uint8_t body[] = "hello from the OpenGAT HTTPS peer\n";
...
static enum opengat_https_status forced_https_status;
...
static void reset_fixture(void)
{
    ...
    uploaded_bytes = 0U;
    forced_https_status = OPENGAT_HTTPS_OK;
...
long opengat_package_upload_write(opengat_handle_t upload, const void *bytes,
    size_t byte_count)
{
...
long opengat_package_upload_seal(opengat_handle_t upload,
    ...
    struct opengat_package_upload_report *report)
{
    ...
    if (memcmp(expected_sha256, report->actual_sha256,
        ...
        return -(long)OPENGAT_EACCES;
    }
    report->result_flags = OPENGAT_PACKAGE_UPLOAD_SEALED |
...
{
    if (volume != OPENGAT_VOLUME_DATA || strcmp(path, "FETCH.NEW") != 0 ||
        flags != (OPENGAT_OPEN_WRITE | OPENGAT_OPEN_CREATE |
            OPENGAT_OPEN_TRUNCATE) || handle_live) {
        return -(long)OPENGAT_EINVAL;
    }
...
long opengat_file_write(opengat_handle_t handle, const void *bytes, size_t count)
{
...
long opengat_handle_close(opengat_handle_t handle)
{
    ...
    handle_live = false;
    return fail_close ? -(long)OPENGAT_EIO : 0;
}
...
{
    if (volume != OPENGAT_VOLUME_DATA || strcmp(source, "FETCH.NEW") != 0 ||
    ...
    struct opengat_https_response *response)
{
    ...
    if (forced_https_status != OPENGAT_HTTPS_OK) {
...
static struct opengat_package_fetch_request request(bool exact)
{
    static const br_x509_trust_anchor anchor = {0};
    const struct opengat_package_fetch_request result = {
        "repo.opengat.test", 443U, 0U, "/index.sri", &anchor, 1U,
...
static struct opengat_package_fetch_upload_request upload_request(void)
{
    static const br_x509_trust_anchor anchor = {0};
    const struct opengat_package_fetch_upload_request result = {
        "repo.opengat.test", 443U, 0U, "/package.spk", &anchor, 1U,
...
{
    struct opengat_package_fetch_request input;
    struct opengat_package_fetch_report report;
    uint8_t wrong_digest[32] = {0U};

    reset_fixture();
    input = request(true);
    CHECK(opengat_package_fetch_stage(&input, &report) ==
            OPENGAT_PACKAGE_FETCH_OK && report.published && report.durable &&
    ...
    input = request(false);
    CHECK(opengat_package_fetch_stage(&input, &report) ==
            OPENGAT_PACKAGE_FETCH_OK && report.durable && staged_present, 2);

    reset_fixture();
    input = request(true);
    input.expected_sha256 = wrong_digest;
    CHECK(opengat_package_fetch_stage(&input, &report) ==
            OPENGAT_PACKAGE_FETCH_DIGEST && !temporary_present &&
        !staged_present && !handle_live && sync_calls == 1U, 3);

    reset_fixture();
    input = request(true);
    partial_https_body = true;
    forced_https_status = OPENGAT_HTTPS_BODY_TRUNCATED;
    CHECK(opengat_package_fetch_stage(&input, &report) ==
            OPENGAT_PACKAGE_FETCH_HTTPS &&
        report.https_status == OPENGAT_HTTPS_BODY_TRUNCATED &&
    ...
    CHECK(opengat_package_fetch_stage(&input, &report) ==
        ...
        report.https_status == OPENGAT_HTTPS_BODY_WRITE &&
        report.storage_error == -(long)OPENGAT_EIO && !temporary_present &&
        !handle_live, 5);
    ...
    fail_close = true;
    CHECK(opengat_package_fetch_stage(&input, &report) ==
            OPENGAT_PACKAGE_FETCH_CLOSE && !temporary_present && !handle_live, 6);

    reset_fixture();
    input = request(true);
    fail_sync_call = 1U;
    CHECK(opengat_package_fetch_stage(&input, &report) ==
            OPENGAT_PACKAGE_FETCH_SYNC && !temporary_present && !staged_present &&
        sync_calls == 2U, 7);
    ...
    fail_replace = true;
    CHECK(opengat_package_fetch_stage(&input, &report) ==
            OPENGAT_PACKAGE_FETCH_PUBLISH && !temporary_present &&
    ...
    fail_sync_call = 2U;
    CHECK(opengat_package_fetch_stage(&input, &report) ==
            OPENGAT_PACKAGE_FETCH_SYNC && report.published && !report.durable &&
    ...
    CHECK(opengat_package_fetch_stage(&input, &report) ==
            OPENGAT_PACKAGE_FETCH_ARGUMENT && !temporary_present &&
        !staged_present && !handle_live, 10);

    struct opengat_package_fetch_upload_request upload = upload_request();

    reset_fixture();
    CHECK(opengat_package_fetch_upload(&upload, &report) ==
            OPENGAT_PACKAGE_FETCH_OK && report.durable && !report.published &&
        report.upload == 19U && upload_live &&
        report.bytes_received == sizeof(body) - 1U &&
        memcmp(report.sha256, body_sha256, sizeof(body_sha256)) == 0 &&
        report.upload_flags == (OPENGAT_PACKAGE_UPLOAD_SEALED |
            OPENGAT_PACKAGE_UPLOAD_DURABLE), 11);
    CHECK(opengat_package_upload_close(report.upload) == 0 && !upload_live, 12);

    reset_fixture();
    upload = upload_request();
    upload.expected_sha256 = wrong_digest;
    CHECK(opengat_package_fetch_upload(&upload, &report) ==
            OPENGAT_PACKAGE_FETCH_DIGEST && !upload_live &&
        report.upload == OPENGAT_HANDLE_INVALID, 13);
    ...
    forced_https_status = OPENGAT_HTTPS_BODY_TRUNCATED;
    CHECK(opengat_package_fetch_upload(&upload, &report) ==
            OPENGAT_PACKAGE_FETCH_HTTPS && !upload_live &&
        report.upload == OPENGAT_HANDLE_INVALID, 14);

    reset_fixture();
    upload = upload_request();
    fail_upload_write = true;
    CHECK(opengat_package_fetch_upload(&upload, &report) ==
            OPENGAT_PACKAGE_FETCH_WRITE && !upload_live &&
        report.storage_error == -(long)OPENGAT_EIO, 15);

    reset_fixture();
    upload = upload_request();
    fail_upload_open = true;
    CHECK(opengat_package_fetch_upload(&upload, &report) ==
            OPENGAT_PACKAGE_FETCH_UPLOAD_OPEN && !upload_live, 16);

    reset_fixture();
    upload = upload_request();
    fail_upload_seal = true;
    CHECK(opengat_package_fetch_upload(&upload, &report) ==
            OPENGAT_PACKAGE_FETCH_UPLOAD_SEAL && !upload_live &&
        report.storage_error == -(long)OPENGAT_EIO, 17);

    reset_fixture();
    upload = upload_request();
    upload.expected_bytes = 0U;
    CHECK(opengat_package_fetch_upload(&upload, &report) ==
            OPENGAT_PACKAGE_FETCH_ARGUMENT && !upload_live, 18);
    ...
    CHECK(opengat_package_fetch_upload(&upload, &report) ==
            OPENGAT_PACKAGE_FETCH_HTTPS && upload_live && report.upload == 19U &&
        report.cleanup_error == -(long)OPENGAT_EIO, 19);
    fail_upload_close = false;
    CHECK(opengat_package_upload_close(report.upload) == 0 && !upload_live, 20);
    return 0;
}
...
int main(void)
{
    ...
    (void)puts("OpenGAT durable streaming package-fetch tests passed: "
