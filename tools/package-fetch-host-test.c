...
static const uint8_t body[] = "hello from the Trait OS HTTPS peer\n";
...
static enum trait_https_status forced_https_status;
...
static void reset_fixture(void)
{
    ...
    uploaded_bytes = 0U;
    forced_https_status = TRAIT_HTTPS_OK;
...
long trait_package_upload_write(trait_handle_t upload, const void *bytes,
    size_t byte_count)
{
...
long trait_package_upload_seal(trait_handle_t upload,
    ...
    struct trait_package_upload_report *report)
{
    ...
    if (memcmp(expected_sha256, report->actual_sha256,
        ...
        return -(long)TRAIT_EACCES;
    }
    report->result_flags = TRAIT_PACKAGE_UPLOAD_SEALED |
...
{
    if (volume != TRAIT_VOLUME_DATA || strcmp(path, "FETCH.NEW") != 0 ||
        flags != (TRAIT_OPEN_WRITE | TRAIT_OPEN_CREATE |
            TRAIT_OPEN_TRUNCATE) || handle_live) {
        return -(long)TRAIT_EINVAL;
    }
...
long trait_file_write(trait_handle_t handle, const void *bytes, size_t count)
{
...
long trait_handle_close(trait_handle_t handle)
{
    ...
    handle_live = false;
    return fail_close ? -(long)TRAIT_EIO : 0;
}
...
{
    if (volume != TRAIT_VOLUME_DATA || strcmp(source, "FETCH.NEW") != 0 ||
    ...
    struct trait_https_response *response)
{
    ...
    if (forced_https_status != TRAIT_HTTPS_OK) {
...
static struct trait_package_fetch_request request(bool exact)
{
    static const br_x509_trust_anchor anchor = {0};
    const struct trait_package_fetch_request result = {
        "repo.trait.test", 443U, 0U, "/index.sri", &anchor, 1U,
...
static struct trait_package_fetch_upload_request upload_request(void)
{
    static const br_x509_trust_anchor anchor = {0};
    const struct trait_package_fetch_upload_request result = {
        "repo.trait.test", 443U, 0U, "/package.spk", &anchor, 1U,
...
{
    struct trait_package_fetch_request input;
    struct trait_package_fetch_report report;
    uint8_t wrong_digest[32] = {0U};

    reset_fixture();
    input = request(true);
    CHECK(trait_package_fetch_stage(&input, &report) ==
            TRAIT_PACKAGE_FETCH_OK && report.published && report.durable &&
    ...
    input = request(false);
    CHECK(trait_package_fetch_stage(&input, &report) ==
            TRAIT_PACKAGE_FETCH_OK && report.durable && staged_present, 2);

    reset_fixture();
    input = request(true);
    input.expected_sha256 = wrong_digest;
    CHECK(trait_package_fetch_stage(&input, &report) ==
            TRAIT_PACKAGE_FETCH_DIGEST && !temporary_present &&
        !staged_present && !handle_live && sync_calls == 1U, 3);

    reset_fixture();
    input = request(true);
    partial_https_body = true;
    forced_https_status = TRAIT_HTTPS_BODY_TRUNCATED;
    CHECK(trait_package_fetch_stage(&input, &report) ==
            TRAIT_PACKAGE_FETCH_HTTPS &&
        report.https_status == TRAIT_HTTPS_BODY_TRUNCATED &&
    ...
    fail_write_call = 2U;
    CHECK(trait_package_fetch_stage(&input, &report) ==
            TRAIT_PACKAGE_FETCH_WRITE &&
        report.https_status == TRAIT_HTTPS_BODY_WRITE &&
        report.storage_error == -(long)TRAIT_EIO && !temporary_present &&
        !handle_live, 5);
    ...
    fail_close = true;
    CHECK(trait_package_fetch_stage(&input, &report) ==
            TRAIT_PACKAGE_FETCH_CLOSE && !temporary_present && !handle_live, 6);

    reset_fixture();
    input = request(true);
    fail_sync_call = 1U;
    CHECK(trait_package_fetch_stage(&input, &report) ==
            TRAIT_PACKAGE_FETCH_SYNC && !temporary_present && !staged_present &&
        sync_calls == 2U, 7);
    ...
    fail_replace = true;
    CHECK(trait_package_fetch_stage(&input, &report) ==
            TRAIT_PACKAGE_FETCH_PUBLISH && !temporary_present &&
    ...
    fail_sync_call = 2U;
    CHECK(trait_package_fetch_stage(&input, &report) ==
            TRAIT_PACKAGE_FETCH_SYNC && report.published && !report.durable &&
    ...
    CHECK(trait_package_fetch_stage(&input, &report) ==
            TRAIT_PACKAGE_FETCH_ARGUMENT && !temporary_present &&
        !staged_present && !handle_live, 10);

    struct trait_package_fetch_upload_request upload = upload_request();

    reset_fixture();
    CHECK(trait_package_fetch_upload(&upload, &report) ==
            TRAIT_PACKAGE_FETCH_OK && report.durable && !report.published &&
        report.upload == 19U && upload_live &&
        report.bytes_received == sizeof(body) - 1U &&
        memcmp(report.sha256, body_sha256, sizeof(body_sha256)) == 0 &&
        report.upload_flags == (TRAIT_PACKAGE_UPLOAD_SEALED |
            TRAIT_PACKAGE_UPLOAD_DURABLE), 11);
    CHECK(trait_package_upload_close(report.upload) == 0 && !upload_live, 12);

    reset_fixture();
    upload = upload_request();
    upload.expected_sha256 = wrong_digest;
    CHECK(trait_package_fetch_upload(&upload, &report) ==
            TRAIT_PACKAGE_FETCH_DIGEST && !upload_live &&
        report.upload == TRAIT_HANDLE_INVALID, 13);
    ...
    forced_https_status = TRAIT_HTTPS_BODY_TRUNCATED;
    CHECK(trait_package_fetch_upload(&upload, &report) ==
            TRAIT_PACKAGE_FETCH_HTTPS && !upload_live &&
        report.upload == TRAIT_HANDLE_INVALID, 14);

    reset_fixture();
    upload = upload_request();
    fail_upload_write = true;
    CHECK(trait_package_fetch_upload(&upload, &report) ==
            TRAIT_PACKAGE_FETCH_WRITE && !upload_live &&
        report.storage_error == -(long)TRAIT_EIO, 15);

    reset_fixture();
    upload = upload_request();
    fail_upload_open = true;
    CHECK(trait_package_fetch_upload(&upload, &report) ==
            TRAIT_PACKAGE_FETCH_UPLOAD_OPEN && !upload_live, 16);

    reset_fixture();
    upload = upload_request();
    fail_upload_seal = true;
    CHECK(trait_package_fetch_upload(&upload, &report) ==
            TRAIT_PACKAGE_FETCH_UPLOAD_SEAL && !upload_live &&
        report.storage_error == -(long)TRAIT_EIO, 17);

    reset_fixture();
    upload = upload_request();
    upload.expected_bytes = 0U;
    CHECK(trait_package_fetch_upload(&upload, &report) ==
            TRAIT_PACKAGE_FETCH_ARGUMENT && !upload_live, 18);
    ...
    CHECK(trait_package_fetch_upload(&upload, &report) ==
            TRAIT_PACKAGE_FETCH_HTTPS && upload_live && report.upload == 19U &&
        report.cleanup_error == -(long)TRAIT_EIO, 19);
    fail_upload_close = false;
    CHECK(trait_package_upload_close(report.upload) == 0 && !upload_live, 20);
    return 0;
}
...
int main(void)
{
    ...
    (void)puts("Trait OS durable streaming package-fetch tests passed: "
