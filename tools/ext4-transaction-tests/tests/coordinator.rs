...
// Complete byte equality remains mandatory, but a refusal regression must not
// format two 128 MiB disks into a gigabyte panic (or exhaust the CI runner).
macro_rules! assert_image_eq {
    ($actual:expr, $expected:expr) => {{
        assert_image_eq!($actual, $expected, "complete disk equality");
    }};
    ($actual:expr, $expected:expr, $($context:tt)+) => {{
        ...
        if actual != expected {
            ...
            panic!("disk mismatch: lengths {} / {}, first differing byte {:?}: {}",
                actual.len(), expected.len(), first, format_args!($($context)+));
        }
    }};
}
...
fn rollback_reload_failure_drops_allocator_view_and_sync_retries() {
    ...
    DEVICE.with_borrow(|device| {
        ...
        assert_image_eq!(device.bytes, before);
    });
...
fn linux_acl_entries_survive_user_xattr_mutation_and_acl_aware_chmod() {
    ...
    for case in 0..5 {
        for dirty in [false, true] {
            ...
            DEVICE.with_borrow(|device| { assert!(device.events.is_empty()); assert_image_eq!(device.bytes, hostile); });
        }
    }
}
...
fn inherited_acls_mask_new_modes_survive_rename_and_rollback_allocations() {
    ...
    for named in [0, 1, 20] {
        ...
        DEVICE.with_borrow(|device| { assert!(device.events.is_empty()); assert_image_eq!(device.bytes, baseline); });
        assert_eq!(ext4::free_bytes(&mounted).unwrap(), free);
...
fn external_xattr_updates_pack_hashes_and_release_storage_without_losing_inline_values() {
    ...
    DEVICE.with_borrow(|device| { assert!(device.events.is_empty()); assert_image_eq!(device.bytes, before); });
    assert_eq!(ext4::free_bytes(&mounted).unwrap(), free);
    ext4::set_stage_block_limit(&mut mounted, 64).unwrap();
...
fn external_xattr_checksums_shared_release_and_final_free() {
    ...
    for count in [1, 3] {
        ...
        DEVICE.with_borrow(|device| {
            ...
            assert_image_eq!(device.bytes, hostile);
        });
    }
    ...
    for (index, operation) in copy_operations.iter().enumerate() {
        for accept in [false, true] {
            ...
            DEVICE.with_borrow(|device| assert_image_eq!(device.bytes, copied_disk, "shared copy event {index}, accept={accept}"));
        }
...
fn allocation_bitmap_corruption_is_not_rechecksummed_into_a_transaction() {
    ...
    DEVICE.with_borrow(|device| {
        assert!(device.events.is_empty());
        assert_image_eq!(device.bytes, full);
    });
}
...
fn block_bitmap_census_refuses_unowned_allocations_fixed_frees_and_false_counters() {
    ...
    ext4::unmount(&mounted).unwrap();
    DEVICE.with_borrow(|device| { assert!(device.events.is_empty()); assert_image_eq!(device.bytes, pristine); });
...
fn checksummed_malformed_extent_trees_are_refused_before_traversal_or_mutation() {
    ...
    for case in 0..10 {
        ...
        DEVICE.with_borrow(|device| {
            ...
            assert_image_eq!(device.bytes, bytes);
        });
        if case >= 8 {
            ...
            DEVICE.with_borrow(|device| {
                assert!(device.events.is_empty());
                assert_image_eq!(device.bytes, empty);
            });
            debugfs(&image, &format!("set_inode_field <{number}> links_count 0"));
            ...
            DEVICE.with_borrow(|device| {
                ...
                assert_image_eq!(device.bytes, orphan);
            });
        }
    }
}
...
fn inode_checksums_follow_declared_extra_size_and_preserve_undeclared_bytes() {
    ...
    for extra in [0u16, 4, 16, 32] {
        ...
        if extra == 0 {
            ...
            DEVICE.with_borrow(|device| assert_image_eq!(device.bytes, before));
        }
    ...
    for target in [1, block_bitmap, inode_bitmap, inode_table, 8192, 8193] {
        ...
        for operation in 0..4 {
            ...
            DEVICE.with_borrow_mut(|device| {
                ...
                assert_image_eq!(device.bytes, baseline);
            });
            ext4::sync(&mut mounted).unwrap();
            assert_eq!(ext4::stat(&mounted, name), Ok(metadata));
            ext4::unmount(&mounted).unwrap();
            DEVICE.with_borrow(|device| assert_image_eq!(device.bytes, expected_clean));
        }
...
fn linear_directory_shrink_reclaims_previously_emptied_suffix_with_retries() {
    ...
    for failure in 0..operations.len() {
        for accepted in [false, true] {
            ...
            ext4::sync(&mut mounted).unwrap();
            ext4::unmount(&mounted).unwrap();
            DEVICE.with_borrow(|device| assert_image_eq!(device.bytes, final_bytes));
            drop(mounted);
            let recovered = mount_bytes(crash);
...
fn real_enospc_rolls_back_allocations_and_immutable_truncate_is_readonly() {
    ...
    let mounted = mount_fixture(&image);
    let available = ext4::free_bytes(&mounted).unwrap();
    assert!(available > 32 * 4096 && available < 64 * 4096);
    // A refused marker write retains the request. When fsync/volume sync
    // resumes it, ENOSPC after the first checkpoint must be reported, not
    // promoted from a short count to a full durability acknowledgement.
    let low_space = DEVICE.with_borrow(|device| device.bytes.clone());
    drop(mounted);
    for file_sync in [false, true] {
        let mut retry = mount_bytes(low_space.clone());
        let inode = ext4::stat(&retry, name).unwrap().inode;
        DEVICE.with_borrow_mut(|device| device.fail_event = Some(0));
        assert_eq!(ext4::transaction_probe(&mut retry, name, 0,
            &vec![0x52; 64 * 4096]), Err(Status::Io));
        DEVICE.with_borrow_mut(|device| device.fail_event = None);
        let result = if file_sync { ext4::fsync_inode(&mut retry, inode) }
            else { ext4::sync(&mut retry) };
        assert_eq!(result, Err(Status::Full));
        assert_eq!(ext4::stat(&retry, name).unwrap().size, 32 * 4096);
        let mut bytes = vec![0; 32 * 4096];
        read_exact(&retry, name, &mut bytes);
        assert!(bytes.iter().all(|byte| *byte == 0x52));
        // The failed suffix is now retired. Cleaning must not keep retrying
        // the impossible allocation, nor append unreported bytes later.
        ext4::sync(&mut retry).unwrap();
        ext4::unmount(&retry).unwrap();
        fsck(&path, if file_sync { "coordinator-fsync-full" }
            else { "coordinator-sync-full" });
    }
    let mut mounted = mount_bytes(low_space);
    ...
    for fail_at in [0, 1] {
        for accept in [false, true] {
            ...
            DEVICE.with_borrow(|device| {
                ...
                assert_image_eq!(device.bytes, initial, "sync published the failed create");
            });
...
fn repeated_mount_recovery_refusals_converge_to_the_same_clean_disk() {
    ...
    for accept in [false, true] {
        for fail_at in 0..expected_events.len() {
            ...
            ext4::unmount(&recovered).unwrap();
            DEVICE.with_borrow(|device| assert_image_eq!(device.bytes, expected_disk,
        ...
        for accept in [false, true] {
            for through_sync in [false, true] {
                ...
                DEVICE.with_borrow(|device| {
                    ...
                    assert_image_eq!(device.bytes, expected_disk);
                });
                ext4::unmount(&mounted).unwrap();
            }
        }
...
fn bounded_sparse_growth_partial_write_and_truncate_retry_contract() {
    ...
    DEVICE.with_borrow(|device| {
        assert_image_eq!(device.bytes, before_bounds.0, "bounded refusal changed the disk");
...
fn minimum_write_capacity_refusal_rolls_back_all_allocations() {
    ...
    ext4::sync(&mut mounted).unwrap();
    DEVICE.with_borrow(|device| assert_image_eq!(device.bytes, initial));
    ext4::set_stage_block_limit(&mut mounted, 64).unwrap();
