...
"""Build and audit OpenRFS's immutable platform package trust table."""
...
def emit_c(data: bytes) -> bytes:
    ...
    lines = [
        ...
        "#include <stdint.h>",
        "",
        f"const uint8_t openrfs_package_trust_asset[{len(data)}] = {{",
    ]
    ...
    lines.extend([
        "};",
        "const size_t openrfs_package_trust_asset_bytes =",
        "    sizeof(openrfs_package_trust_asset);",
        "",
    ])
...
def self_test() -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        if b"openrfs_package_trust_asset" not in emit_c(table):
...
def main() -> int:
    ...
    try:
        if arguments.command == "build":
            ...
            self_test()
            print("OpenRFS immutable package trust-table tests passed")
