...
"""Build and audit OpenGAT's immutable platform package trust table."""
...
def emit_c(data: bytes) -> bytes:
    ...
    lines = [
        ...
        "#include <stdint.h>",
        "",
        f"const uint8_t opengat_package_trust_asset[{len(data)}] = {{",
    ]
    ...
    lines.extend([
        "};",
        "const size_t opengat_package_trust_asset_bytes =",
        "    sizeof(opengat_package_trust_asset);",
        "",
    ])
...
def self_test() -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        if b"opengat_package_trust_asset" not in emit_c(table):
...
def main() -> int:
    ...
    try:
        if arguments.command == "build":
            ...
            self_test()
            print("OpenGAT immutable package trust-table tests passed")
