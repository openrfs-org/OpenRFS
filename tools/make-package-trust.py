...
"""Build and audit Trait OS's immutable platform package trust table."""
...
def emit_c(data: bytes) -> bytes:
    ...
    lines = [
        ...
        "#include <stdint.h>",
        "",
        f"const uint8_t trait_package_trust_asset[{len(data)}] = {{",
    ]
    ...
    lines.extend([
        "};",
        "const size_t trait_package_trust_asset_bytes =",
        "    sizeof(trait_package_trust_asset);",
        "",
    ])
...
def self_test() -> None:
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        if b"trait_package_trust_asset" not in emit_c(table):
...
def main() -> int:
    ...
    try:
        if arguments.command == "build":
            ...
            self_test()
            print("Trait OS immutable package trust-table tests passed")
