...
"""Capture three deterministic Trait OS frames from a live QEMU guest.
...
PROOF_LINE = b"Trait OS: Boot Ledger installed proof passed"
USERLAND_LINES = {
...
CAT_WAIT_LINE = b"RW USERLAND cat foreground launch yielded to Trait OS"
...
def storage_arguments(userspace, system, data):
    if userspace is not None:
        return [
            ...
            "-device",
            "nvme,serial=trait-userland,drive=userland-raw,logical_block_size=4096,physical_block_size=4096,max_ioqpairs=1,msix_qsize=1",
        ]
    return [
        "-blockdev",
        ...
        "-device",
        "nvme,serial=trait-system-fat32,drive=system-raw,logical_block_size=512,physical_block_size=512,max_ioqpairs=1,msix_qsize=1",
        "-blockdev",
        ...
        "-device",
        "nvme,serial=trait-data-fat32,drive=data-raw,logical_block_size=512,physical_block_size=512,max_ioqpairs=1,msix_qsize=1",
    ]
...
def main():
    ...
    try:
        ...
        wait_serial_after(serial, PROOF_LINE, b"trait> ")
        time.sleep(0.25)
        ...
        time.sleep(0.12)
        clean = capture(qmp, output, "trait-proof")
        qmp.hmp("sendkey esc")
        time.sleep(0.20)
        qmp.hmp("sendkey tab")
        time.sleep(0.20)
        focus = capture(qmp, output, "trait-proof-focus")

        # Enter opens the focused Trait OS terminal. The command and any
        # interactive input then travel through the guest's PS/2 path.
        qmp.hmp("sendkey ret")
        time.sleep(0.30)
        ...
        time.sleep(0.20)
        terminal = capture(qmp, output, "trait-proof-terminal")
        print(clean)
        print(focus)
        print(terminal)
    finally:
