#!/usr/bin/env python3
"""Cut Trait OS at every ext4 durability boundary and verify reboot recovery."""
...
def _build_iso(
    ...
    with tempfile.TemporaryDirectory(prefix="trait-ext4-cut-", dir=output.parent) as raw:
        root = Path(raw)
        boot = root / "boot"
        grub = boot / "grub"
        grub.mkdir(parents=True)
        shutil.copyfile(kernel, boot / "trait.elf")
        command_line = "trait.test=ext4-recovery"
        if cut is not None:
            command_line += f" trait.ext4-cut={cut}"
        (grub / "grub.cfg").write_text(
            "\n".join(
                (
                    ...
                    "set timeout=0",
                    "",
                    'menuentry "Trait OS ext4 durability test" {',
                    f"    multiboot2 /boot/trait.elf {command_line}",
                    "    boot",
                    "}",
                    "",
                )
...
def _run_qemu(
    ...
    command = (
        ...
        "-device",
        "nvme,serial=trait-ext4-powercut,drive=ext4-raw,logical_block_size=4096,physical_block_size=4096,max_ioqpairs=1,msix_qsize=1",
        "-cdrom",
        str(iso),
        "-display",
        "none",
...
def _verify_guest_result(image: Path, tools: dict[str, str], temporary: Path) -> None:
    ...
    prefix = b"Trait OS deterministic ext4 fixture\n"
    if (
        len(data) != 4097
...
def run(args: argparse.Namespace) -> dict[str, object]:
    ...
    for cut, boundary in enumerate(POWER_CUTS, start=1):
        ...
        if PASS_MARKER in transcript or "ST FAIL" in transcript or "Trait OS PANIC" in transcript:
        ...
        if (
            ...
            or "Trait OS PANIC" in transcript
        ):
            raise PowerCutError(
        ...
        with tempfile.TemporaryDirectory(prefix="trait-ext4-result-", dir=output) as raw:
