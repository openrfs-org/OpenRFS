#!/usr/bin/env python3
"""Canonical OpenGAT installed-state and transaction-recovery reference model.
...
PACKAGE_TOOL_PATH = Path(__file__).with_name("opengat-package.py")
PACKAGE_TOOL_SPEC = importlib.util.spec_from_file_location(
    "opengat_transaction_package_tool", PACKAGE_TOOL_PATH)
if PACKAGE_TOOL_SPEC is None or PACKAGE_TOOL_SPEC.loader is None:
    raise RuntimeError("could not load OpenGAT package tool")
...
AUTHORITY_MAGIC = b"OGTGN001"
AUTHORITY_BYTES = 128

JOURNAL_MAGIC = b"OGTTX001"
JOURNAL_BYTES = 512
...
def _reject_file_ancestors(paths: set[str], message: str) -> None:
    for path in paths:
        pieces = path.split("/")
        if any("/".join(pieces[:count]) in paths
...
def _normalize_package_metadata(value: Any, field: str) -> dict[str, Any]:
    ...
    if len(file_paths) != len(normalized_files):
        raise TransactionError(f"{field}.files contains a duplicate path")
    _reject_file_ancestors(
        file_paths, f"{field}.files contains an ancestor file path")
    return {
        "identifier": _identifier(value["identifier"], f"{field}.identifier"),
...
def _normalize_packages(value: Any) -> list[dict[str, Any]]:
    ...
    _reject_file_ancestors(
        set(owners), "an owned file cannot be another file's ancestor")
...
def parse_database(data: bytes) -> dict[str, Any]:
    ...
    _reject_file_ancestors(
        {file_value["path"] for package in packages
         for file_value in package["files"]},
        "an owned file cannot be another file's ancestor",
    )
...
def main() -> int:
    ...
    try:
        ...
        print(f"OpenGAT transaction state refused: {error}", file=sys.stderr)
        return 1
    return 0
