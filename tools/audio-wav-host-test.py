...
def self_test() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="opengat-audio-") as directory:
        root = Path(directory)
...
def main() -> int:
    ...
    if arguments.self_test:
        self_test()
        print("OpenGAT audio WAV host self-test passed")
        return 0
    ...
    try:
        ...
        print(f"OpenGAT audio WAV refused: {error}")
        return 1
    label = "AUDIO" if arguments.profile == "native-audio" else "SDL"
    print(f"OPENGAT {label} WAV PASS " + " ".join(
