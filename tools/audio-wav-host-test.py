...
def self_test() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="openrfs-audio-") as directory:
        root = Path(directory)
...
def main() -> int:
    ...
    if arguments.self_test:
        self_test()
        print("OpenRFS audio WAV host self-test passed")
        return 0
    ...
    try:
        ...
        print(f"OpenRFS audio WAV refused: {error}")
        return 1
    label = "AUDIO" if arguments.profile == "native-audio" else "SDL"
    print(f"OPENRFS {label} WAV PASS " + " ".join(
