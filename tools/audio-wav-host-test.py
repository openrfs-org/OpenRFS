...
def self_test() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="trait-audio-") as directory:
        root = Path(directory)
...
def main() -> int:
    ...
    if arguments.self_test:
        self_test()
        print("Trait OS audio WAV host self-test passed")
        return 0
    ...
    try:
        ...
        print(f"Trait OS audio WAV refused: {error}")
        return 1
    label = "AUDIO" if arguments.profile == "native-audio" else "SDL"
    print(f"TRAIT {label} WAV PASS " + " ".join(
