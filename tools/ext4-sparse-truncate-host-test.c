...
static void clear_fixture(void)
{
    ...
    ext4_mounts[OPENRFSFS_VOLUME_DATA].generation = 900U;
    ...
    disk_size = 0U;
    write_status = OPENRFS_EXT4_STATUS_OK;
    truncate_status = OPENRFS_EXT4_STATUS_OK;
    probe_status = OPENRFS_EXT4_STATUS_OK;
    stat_status = OPENRFS_EXT4_STATUS_OK;
    fsync_status = OPENRFS_EXT4_STATUS_OK;
    opens = 0U;
    closes = 0U;
    write_calls = 0U;
    truncate_calls = 0U;
...
int32_t openrfs_ext4_transaction_probe(uintptr_t mounted, const uint8_t *path,
    ...
    size_t *written)
{
    ...
    assert(source != NULL && written != NULL &&
        ext4_mounts[OPENRFSFS_VOLUME_DATA].session.writable);
    ++probe_calls;
    if (probe_status != OPENRFS_EXT4_STATUS_OK) return probe_status;
    ...
    size_t path_bytes, uint64_t size)
{
    ...
    ++truncate_calls;
    if (truncate_status != OPENRFS_EXT4_STATUS_OK) return truncate_status;
    disk_size = size;
    return OPENRFS_EXT4_STATUS_OK;
}
    ...
    const uint8_t *source, size_t length, size_t *written)
{
    assert(mounted == 1U && inode == 42U && source != NULL && written != NULL &&
        ext4_mounts[OPENRFSFS_VOLUME_DATA].session.writable);
    ++write_calls;
    if (write_status != OPENRFS_EXT4_STATUS_OK) return write_status;
    assert(offset <= (uint64_t)sizeof(disk_bytes) &&
        (uint64_t)length <= (uint64_t)sizeof(disk_bytes) - offset);
    const size_t start = (size_t)offset;
    memcpy(&disk_bytes[start], source, length);
    ...
    size_t path_bytes, struct openrfs_ext4_metadata *metadata)
{
    ...
    ++stat_calls;
    if (stat_status != OPENRFS_EXT4_STATUS_OK) return stat_status;
...
static openrfsfs_handle open_file(enum openrfsfs_access access)
{
    openrfsfs_handle handle = 0U;
    assert(allocate_handle(OPENRFSFS_VOLUME_DATA, "file", 42U, disk_size,
        access, false, 0U, &handle) == OPENRFSFS_STATUS_OK);
    return handle;
}
...
static void test_growth_shrink_shared_state_and_cursor(void)
{
    ...
    const openrfsfs_handle first = open_file(OPENRFSFS_ACCESS_READ_WRITE);
    const openrfsfs_handle second = open_file(OPENRFSFS_ACCESS_READ_WRITE);
    ...
    const uint64_t old_completion = ext4_backend_completion_count(OPENRFSFS_VOLUME_DATA);
    uint8_t before_growth[sizeof(disk_bytes)] = { 0 };
    memcpy(before_growth, disk_bytes, sizeof(disk_bytes));

    assert(ext4_backend_seek(first, (int64_t)position, OPENRFSFS_SEEK_START,
        &position) == OPENRFSFS_STATUS_OK);
    truncate_status = OPENRFS_EXT4_STATUS_IO;
    assert(ext4_backend_ftruncate(first, grown) == OPENRFSFS_STATUS_IO);
    ...
    assert(disk_size == 0U && memcmp(disk_bytes, before_growth,
        sizeof(disk_bytes)) == 0);
    assert(ext4_backend_completion_count(OPENRFSFS_VOLUME_DATA) == old_completion);
    assert(truncate_calls == old_truncates + 1U);
    truncate_status = OPENRFS_EXT4_STATUS_OK;
    assert(ext4_backend_ftruncate(first, grown) == OPENRFSFS_STATUS_OK);
    ...
    size_t written = 0U;
    assert(ext4_backend_write(first, &guarded[1], sizeof(source), &written) == OPENRFSFS_STATUS_OK);
    ...
    const uint64_t before_completion = ext4_backend_completion_count(OPENRFSFS_VOLUME_DATA);
    uint8_t before_bytes[sizeof(disk_bytes)] = { 0 };
    memcpy(before_bytes, disk_bytes, sizeof(disk_bytes));
    truncate_status = OPENRFS_EXT4_STATUS_IO;
    assert(ext4_backend_ftruncate(first, shrink) == OPENRFSFS_STATUS_IO);
    assert(ext4_handles[first_slot].size == before_size);
    assert(ext4_handles[second_slot].size == grown);
    assert(memcmp(disk_bytes, before_bytes, sizeof(disk_bytes)) == 0);
    assert(ext4_backend_completion_count(OPENRFSFS_VOLUME_DATA) == before_completion);
    truncate_status = OPENRFS_EXT4_STATUS_OK;
    assert(ext4_backend_ftruncate(second, shrink) == OPENRFSFS_STATUS_OK);
...
static void test_bounds_and_fsync_failure(void)
{
    ...
    const uint64_t old_completion = ext4_backend_completion_count(OPENRFSFS_VOLUME_DATA);
    const openrfsfs_handle handle = open_file(OPENRFSFS_ACCESS_READ_WRITE);
    size_t written = 99U;

    assert(ext4_backend_transaction_probe(OPENRFSFS_VOLUME_DATA, "file",
        OPENRFS_EXT4_MAX_MUTABLE_FILE_BYTES - 1U, source, sizeof(source), &written) ==
        OPENRFSFS_STATUS_RANGE);
    assert(written == 0U && probe_calls == old_probe);
    assert(ext4_backend_truncate_probe(OPENRFSFS_VOLUME_DATA, "file",
        OPENRFS_EXT4_MAX_MUTABLE_FILE_BYTES + 1U) == OPENRFSFS_STATUS_RANGE);
    assert(truncate_calls == old_truncate);

    fsync_status = OPENRFS_EXT4_STATUS_IO;
    assert(ext4_backend_fsync(handle) == OPENRFSFS_STATUS_IO);
    assert(fsync_calls == old_fsync + 1U);
    assert(ext4_backend_completion_count(OPENRFSFS_VOLUME_DATA) == old_completion);
    fsync_status = OPENRFS_EXT4_STATUS_OK;
    assert(ext4_backend_fsync(handle) == OPENRFSFS_STATUS_OK);
    assert(ext4_backend_completion_count(OPENRFSFS_VOLUME_DATA) == old_completion + 1U);
