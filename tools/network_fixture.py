...
"""Deterministic, offline Ethernet peer for Trait OS's QEMU socket backend."""
...
WELCOME = b"hello from the Trait OS network\n"

# Trait OS announces a port it is listening on, or one it deliberately is not,
...
class Fixture:
    ...
    def http_response(self, request: bytes) -> bytes:
        ...
        if self.mode == "http-redirect-loop":
            return b"HTTP/1.1 302 Found\r\nLocation: http://trait.test/loop\r\nContent-Length: 0\r\n\r\n"
        if self.mode == "http-redirect" and b"GET /start " in request:
            return b"HTTP/1.1 302 Found\r\nLocation: http://trait.test/welcome.txt\r\nContent-Length: 0\r\n\r\n"
        return (b"HTTP/1.1 200 OK\r\nContent-Length: " +
...
def self_test() -> int:
    sample = udp(GATEWAY_IP, GUEST_IP, 67, 68, b"trait")
