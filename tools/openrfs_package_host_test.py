#!/usr/bin/env python3
"""Host refusal and reproducibility tests for the OpenRFS package format."""
...
SPEC = importlib.util.spec_from_file_location(
    "openrfs_package", ROOT / "tools" / "openrfs-package.py")
...
def main() -> int:
    ...
    spec = {
        ...
        "arguments": ["PKGTEST.APP", "http://openrfs.test/welcome.txt"],
    }
    ...
    v3_spec = {
        ...
        "abi_min": 1,
        "abi_max": 2,
        "identifier": "org.openrfs.pkgtest",
        "name": "Package v3 Test",
        "version": "1.2.3-rc.1+hosttest",
        "publisher": "OpenRFS Project",
        "capabilities": ["data-read", "console"],
        "dependencies": [
            {"identifier": "org.openrfs.zlib", "constraint": ">=1.2.13,<2.0.0"},
            {"identifier": "org.openrfs.libc", "constraint": "^1.0.0"},
        ],
        "conflicts": [
            {"identifier": "org.openrfs.pkgtest-old", "constraint": "*"},
        ],
    }
    v3_files = (
    ...
    if crypto_available:
        ...
        assert report["identifier"] == "org.openrfs.pkgtest"
        ...
        assert [item["identifier"] for item in report["dependencies"]] == [
            "org.openrfs.libc", "org.openrfs.zlib"]
    ...
    print("OpenRFS package host tests passed: legacy/dynamic bytes, v3 canonical tables, bounds, digests, trust, Ed25519 verification/refusal")
    return 0
