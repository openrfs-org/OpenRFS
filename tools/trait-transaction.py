#!/usr/bin/env python3
"""Canonical Trait OS installed-state and transaction-recovery reference model.
...
PACKAGE_TOOL_PATH = Path(__file__).with_name("trait-package.py")
PACKAGE_TOOL_SPEC = importlib.util.spec_from_file_location(
    "trait_transaction_package_tool", PACKAGE_TOOL_PATH)
if PACKAGE_TOOL_SPEC is None or PACKAGE_TOOL_SPEC.loader is None:
    raise RuntimeError("could not load Trait OS package tool")
...
AUTHORITY_MAGIC = b"TRTGN001"
AUTHORITY_BYTES = 128

JOURNAL_MAGIC = b"TRTTX001"
JOURNAL_BYTES = 512
...
def main() -> int:
    ...
    try:
        ...
        print(f"Trait OS transaction state refused: {error}", file=sys.stderr)
        return 1
    return 0
