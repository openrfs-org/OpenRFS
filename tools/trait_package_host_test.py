#!/usr/bin/env python3
"""Host refusal and reproducibility tests for the Trait OS package format."""
...
SPEC = importlib.util.spec_from_file_location(
    "trait_package", ROOT / "tools" / "trait-package.py")
...
def main() -> int:
    ...
    spec = {
        ...
        "arguments": ["PKGTEST.APP", "http://trait.test/welcome.txt"],
    }
    ...
    assert hashlib.sha256(first).hexdigest().upper() == (
        "266C154572B6F0C2F6A026AD7C62BDE42E4C1A1B4EF46B84FAD6C11A64CE3DD1")
    ...
    assert report["arguments"] == [
        "PKGTEST.APP", "http://trait.test/welcome.txt"
    ]
    ...
    assert hashlib.sha256(resource_package).hexdigest().upper() == (
        "5E7AAC4114DBC11E00E28EDAB7E2A41C0F3CB3A7C0C5C2740D7B4922F85CCCD6")
    ...
    v3_spec = {
        ...
        "abi_min": 1,
        "abi_max": 2,
        "identifier": "org.trait.pkgtest",
        "name": "Package v3 Test",
        "version": "1.2.3-rc.1+hosttest",
        "publisher": "Trait OS Project",
        "capabilities": ["data-read", "console"],
        "dependencies": [
            {"identifier": "org.trait.zlib", "constraint": ">=1.2.13,<2.0.0"},
            {"identifier": "org.trait.libc", "constraint": "^1.0.0"},
        ],
        "conflicts": [
            {"identifier": "org.trait.pkgtest-old", "constraint": "*"},
        ],
    }
    v3_files = (
    ...
    if crypto_available:
        ...
        assert report["identifier"] == "org.trait.pkgtest"
    ...
    print("Trait OS package host tests passed: legacy/dynamic bytes, v3 canonical tables, bounds, digests, trust, Ed25519 verification/refusal")
    return 0
