...
class SuperblockTests(unittest.TestCase):
    def test_accepts_exact_trait_profile(self) -> None:
...
class FixtureScriptTests(unittest.TestCase):
    def test_crc32c_and_journal_profile_upgrade(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="trait-ext4-journal-test-") as raw:
    ...
    def test_format_invocation_and_debugfs_script_pin_reproducibility_inputs(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="trait-ext4-script-test-") as raw:
            root = Path(raw)
...
class E2fsprogsIntegrationTests(unittest.TestCase):
    def test_determinism_fixture_semantics_and_adversarial_refusals(self) -> None:
        with tempfile.TemporaryDirectory(prefix="trait-ext4-host-test-") as raw:
            root = Path(raw)
            ...
            rust_fixture = os.environ.get("TRAIT_EXT4_RUST_FIXTURE")
            if rust_fixture:
