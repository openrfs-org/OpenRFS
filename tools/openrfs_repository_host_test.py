...
SPEC = importlib.util.spec_from_file_location(
    "openrfs_repository", ROOT / "tools" / "openrfs-repository.py")
...
def main() -> int:
    ...
    if os.environ.get("OPENRFS_REQUIRE_ED25519") == "1" and not crypto_available:
    ...
    packages = [
        package("org.openrfs.desktop", "3.0.0", desktop_payload,
                dependencies=[
                    {"identifier": "virtual.renderer",
                     "constraint": ">=1.0.0,<2.0.0"},
                    {"identifier": "org.openrfs.libc", "constraint": "^1.0.0"},
                ]),
        package("org.openrfs.libc", "0.9.0", libc_old_payload),
        package("org.openrfs.render-soft", "2.0.0", renderer_payload,
                provides=[{"identifier": "virtual.renderer", "version": "1.5.0"}]),
        package("org.openrfs.libc", "1.0.0", libc_payload),
    ]
    ...
    if not crypto_available:
        ...
        print("OpenRFS repository host tests passed: Ed25519 unavailable and refused closed")
        return 0
    ...
    assert report["repository"] == "org.openrfs.main"
    ...
    assert [(item["identifier"], item["version"]) for item in report["packages"]] == [
        ("org.openrfs.desktop", "3.0.0"),
        ("org.openrfs.libc", "0.9.0"),
        ("org.openrfs.libc", "1.0.0"),
        ("org.openrfs.render-soft", "2.0.0"),
    ]
    ...
    plan, lock = REPOSITORY.resolve_repository(
        report, ["org.openrfs.desktop@^3.0.0"], architecture="x86_64",
    ...
    REPOSITORY.verify_download(
        next(item for item in report["packages"]
             if item["identifier"] == "org.openrfs.libc" and item["version"] == "1.0.0"),
        libc_payload)
    libc_entry = next(item for item in report["packages"]
                      if item["identifier"] == "org.openrfs.libc"
    ...
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            report, ["org.openrfs.desktop"], architecture="aarch64",
            abi_version=1),
        "architecture")
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            report, ["org.openrfs.desktop"], architecture="x86_64",
            abi_version=3),
    ...
    unsatisfied_report = parsed_with([
        package("org.openrfs.need-missing", "1.0.0", b"missing",
                dependencies=[{"identifier": "org.openrfs.absent",
                               "constraint": "*"}]),
    ])
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            unsatisfied_report, ["org.openrfs.need-missing"],
            architecture="x86_64", abi_version=1),
        "unsatisfied dependency")
    backtracking_report = parsed_with([
        package("org.openrfs.app", "1.0.0", b"app one",
                dependencies=[{"identifier": "org.openrfs.lib",
                               "constraint": "^1.0.0"}]),
        package("org.openrfs.app", "2.0.0", b"app two",
                dependencies=[{"identifier": "org.openrfs.lib",
                               "constraint": "^2.0.0"}]),
        package("org.openrfs.lib", "1.1.0", b"lib one"),
    ])
    backtracking_plan, _ = REPOSITORY.resolve_repository(
        backtracking_report, ["org.openrfs.app"], architecture="x86_64",
        abi_version=1)
    assert [(item["identifier"], item["version"])
            for item in backtracking_plan] == [
                ("org.openrfs.lib", "1.1.0"), ("org.openrfs.app", "1.0.0")]
    conflict_packages = copy.deepcopy(packages)
    next(item for item in conflict_packages
         if item["identifier"] == "org.openrfs.render-soft")["conflicts"] = [
             {"identifier": "org.openrfs.desktop", "constraint": "*"}]
    conflict_report = parsed_with(conflict_packages)
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            conflict_report, ["org.openrfs.desktop"],
            architecture="x86_64", abi_version=1),
        "conflicts with")
    ambiguous_packages = copy.deepcopy(packages)
    ambiguous_packages.append(
        package("org.openrfs.render-hw", "1.0.0", b"hardware renderer",
                provides=[{"identifier": "virtual.renderer", "version": "1.4.0"}]))
    ambiguous_report = parsed_with(ambiguous_packages)
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            ambiguous_report, ["org.openrfs.desktop"],
            architecture="x86_64", abi_version=1),
        "ambiguous providers")
    cycle_report = parsed_with([
        package("org.openrfs.a", "1.0.0", b"a",
                dependencies=[{"identifier": "org.openrfs.b", "constraint": "*"}]),
        package("org.openrfs.b", "1.0.0", b"b",
                dependencies=[{"identifier": "org.openrfs.a", "constraint": "*"}]),
    ])
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            cycle_report, ["org.openrfs.a"], architecture="x86_64", abi_version=1),
        "dependency cycle")
    chain_report = parsed_with([
        package("org.openrfs.a", "1.0.0", b"a",
                dependencies=[{"identifier": "org.openrfs.b", "constraint": "*"}]),
        package("org.openrfs.b", "1.0.0", b"b",
                dependencies=[{"identifier": "org.openrfs.c", "constraint": "*"}]),
        package("org.openrfs.c", "1.0.0", b"c"),
    ])
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            chain_report, ["org.openrfs.a"], architecture="x86_64",
            abi_version=1, max_packages=2),
        "graph exceeds its 2-package bound")

    print("OpenRFS repository host tests passed: canonical signature, freshness, digests, resolver refusals, exact lock")
    return 0
