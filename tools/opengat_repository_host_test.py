...
SPEC = importlib.util.spec_from_file_location(
    "opengat_repository", ROOT / "tools" / "opengat-repository.py")
...
def main() -> int:
    ...
    if os.environ.get("OPENGAT_REQUIRE_ED25519") == "1" and not crypto_available:
    ...
    packages = [
        package("org.opengat.desktop", "3.0.0", desktop_payload,
                dependencies=[
                    {"identifier": "virtual.renderer",
                     "constraint": ">=1.0.0,<2.0.0"},
                    {"identifier": "org.opengat.libc", "constraint": "^1.0.0"},
                ]),
        package("org.opengat.libc", "0.9.0", libc_old_payload),
        package("org.opengat.render-soft", "2.0.0", renderer_payload,
                provides=[{"identifier": "virtual.renderer", "version": "1.5.0"}]),
        package("org.opengat.libc", "1.0.0", libc_payload),
    ]
    ...
    if not crypto_available:
        ...
        print("OpenGAT repository host tests passed: Ed25519 unavailable and refused closed")
        return 0

    first = REPOSITORY.build_repository(repository_spec, private_seed)
    assert hashlib.sha256(first).hexdigest().upper() == (
        "B86AE4BE75A1E39E05B59F129989DAE1413DF17FF209A3E4EC6655F806ABF247")
    ...
    assert report["repository"] == "org.opengat.main"
    ...
    assert [(item["identifier"], item["version"]) for item in report["packages"]] == [
        ("org.opengat.desktop", "3.0.0"),
        ("org.opengat.libc", "0.9.0"),
        ("org.opengat.libc", "1.0.0"),
        ("org.opengat.render-soft", "2.0.0"),
    ]
    ...
    plan, lock = REPOSITORY.resolve_repository(
        report, ["org.opengat.desktop@^3.0.0"], architecture="x86_64",
    ...
    assert hashlib.sha256(lock).hexdigest().upper() == (
        "EB1E7E0A952A3D1320A45953C00CDEB78108B3D484CABADDB808E5F3E1B9AD58")
    ...
    REPOSITORY.verify_download(
        next(item for item in report["packages"]
             if item["identifier"] == "org.opengat.libc" and item["version"] == "1.0.0"),
        libc_payload)
    libc_entry = next(item for item in report["packages"]
                      if item["identifier"] == "org.opengat.libc"
    ...
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            report, ["org.opengat.desktop"], architecture="aarch64",
            abi_version=1),
        "architecture")
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            report, ["org.opengat.desktop"], architecture="x86_64",
            abi_version=3),
    ...
    unsatisfied_report = parsed_with([
        package("org.opengat.need-missing", "1.0.0", b"missing",
                dependencies=[{"identifier": "org.opengat.absent",
                               "constraint": "*"}]),
    ])
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            unsatisfied_report, ["org.opengat.need-missing"],
            architecture="x86_64", abi_version=1),
        "unsatisfied dependency")
    backtracking_report = parsed_with([
        package("org.opengat.app", "1.0.0", b"app one",
                dependencies=[{"identifier": "org.opengat.lib",
                               "constraint": "^1.0.0"}]),
        package("org.opengat.app", "2.0.0", b"app two",
                dependencies=[{"identifier": "org.opengat.lib",
                               "constraint": "^2.0.0"}]),
        package("org.opengat.lib", "1.1.0", b"lib one"),
    ])
    backtracking_plan, _ = REPOSITORY.resolve_repository(
        backtracking_report, ["org.opengat.app"], architecture="x86_64",
        abi_version=1)
    assert [(item["identifier"], item["version"])
            for item in backtracking_plan] == [
                ("org.opengat.lib", "1.1.0"), ("org.opengat.app", "1.0.0")]
    conflict_packages = copy.deepcopy(packages)
    next(item for item in conflict_packages
         if item["identifier"] == "org.opengat.render-soft")["conflicts"] = [
             {"identifier": "org.opengat.desktop", "constraint": "*"}]
    conflict_report = parsed_with(conflict_packages)
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            conflict_report, ["org.opengat.desktop"],
            architecture="x86_64", abi_version=1),
        "conflicts with")
    ambiguous_packages = copy.deepcopy(packages)
    ambiguous_packages.append(
        package("org.opengat.render-hw", "1.0.0", b"hardware renderer",
                provides=[{"identifier": "virtual.renderer", "version": "1.4.0"}]))
    ambiguous_report = parsed_with(ambiguous_packages)
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            ambiguous_report, ["org.opengat.desktop"],
            architecture="x86_64", abi_version=1),
        "ambiguous providers")
    cycle_report = parsed_with([
        package("org.opengat.a", "1.0.0", b"a",
                dependencies=[{"identifier": "org.opengat.b", "constraint": "*"}]),
        package("org.opengat.b", "1.0.0", b"b",
                dependencies=[{"identifier": "org.opengat.a", "constraint": "*"}]),
    ])
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            cycle_report, ["org.opengat.a"], architecture="x86_64", abi_version=1),
        "dependency cycle")
    chain_report = parsed_with([
        package("org.opengat.a", "1.0.0", b"a",
                dependencies=[{"identifier": "org.opengat.b", "constraint": "*"}]),
        package("org.opengat.b", "1.0.0", b"b",
                dependencies=[{"identifier": "org.opengat.c", "constraint": "*"}]),
        package("org.opengat.c", "1.0.0", b"c"),
    ])
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            chain_report, ["org.opengat.a"], architecture="x86_64",
            abi_version=1, max_packages=2),
        "graph exceeds its 2-package bound")

    print("OpenGAT repository host tests passed: canonical signature, freshness, digests, resolver refusals, exact lock")
    return 0
