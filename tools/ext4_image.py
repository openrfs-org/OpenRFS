#!/usr/bin/env python3
"""Build, inspect, verify, and deliberately damage OpenGAT ext4 fixtures.
...
VOLUME_LABEL = "OPENGATEXT4"
LAST_MOUNTED = "/opengat"
LARGE_SPARSE_BYTES = 5 * 1024 * 1024 * 1024 + 123
XATTR_NAME = "user.opengat"
...
def parse_superblock(image: bytes | bytearray) -> dict[str, object]:
    """Parse and strictly validate the OpenGAT ext4 superblock profile."""
    if len(image) < 2048:
    ...
    if fs_uuid != FILESYSTEM_UUID:
        _refuse(f"UUID {fs_uuid} does not match the pinned OpenGAT UUID")
...
def _upgrade_journal_superblock(image: Path, tools: dict[str, str]) -> None:
    """Upgrade mke2fs's unmounted clean journal to OpenGAT's JBD2 profile."""
...
def _write_debugfs_script(path: Path, payloads: dict[str, Path]) -> None:
    ...
    for index in range(256):
        lines.append(f"write {payloads['entry']} /indexed/entry-{index:04d}-opengat-fixture")
    lines.extend(
        [
...
""",
    ...
    payloads["readme"].write_bytes(b"OpenGAT deterministic ext4 fixture\n")
    payloads["app"].write_bytes(b"OPENGAT-APP\x00\x01\x02\x03\n")
...
def build_image(output: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="opengat-ext4-", dir=output.parent) as raw:
        temporary = Path(raw)
        candidate = temporary / "opengat-ext4.img"
...
def malform_image(kind: str, source: Path, output: Path) -> None:
    ...
    output.parent.mkdir(parents=True, exist_ok=True)
    with tempfile.NamedTemporaryFile(prefix="opengat-ext4-malform-", dir=output.parent, delete=False) as stream:
