...
"""Capture three deterministic OpenGAT frames from a live QEMU guest.
...
PROOF_LINE = b"OpenGAT: Boot Ledger installed proof passed"
TERMINAL_COMMAND = "echo opengat"
TERMINAL_RESULT = b"echo opengat\nopengat\nopengat$ "
PROMPT = b"opengat$ "
...
ACCOUNT_CREATED = b"OpenGAT user created. Run 'starty' to enter the desktop."
DESKTOP_STARTED = b"OpenGAT: authenticated desktop started"
CAPTURE_USERNAME = "opengat"
CAPTURE_PASSWORD = "opengatpass"
...
def storage_arguments(userspace, system, data):
    ...
    if system is not None and data is not None:
        arguments.extend([
            ...
            "-device",
            "nvme,serial=opengat-system-fat32,drive=system-raw,logical_block_size=512,physical_block_size=512,max_ioqpairs=1,msix_qsize=1",
            "-blockdev",
            ...
            "-device",
            "nvme,serial=opengat-data-fat32,drive=data-raw,logical_block_size=512,physical_block_size=512,max_ioqpairs=1,msix_qsize=1",
        ])
    if userspace is not None:
        arguments.extend([
            ...
            "-device",
            "nvme,serial=opengat-userland,drive=userland-raw,logical_block_size=4096,physical_block_size=4096,max_ioqpairs=1,msix_qsize=1",
        ])
    return arguments
...
def main():
    ...
    if args.data is not None:
        durable_data = output / "opengat-proof-data.raw"
    ...
    try:
        ...
        time.sleep(0.12)
        clean = capture(qmp, output, "opengat-proof")
        qmp.hmp("sendkey esc")
        time.sleep(0.20)
        qmp.hmp("sendkey tab")
        time.sleep(0.20)
        focus = capture(qmp, output, "opengat-proof-focus")

        # Enter opens the focused OpenGAT terminal. The deterministic command
        ...
        time.sleep(0.20)
        terminal = capture(qmp, output, "opengat-proof-terminal")
        print(clean)
        print(focus)
        print(terminal)
    finally:
    ...
    if durable_data is not None:
        ...
        login = [
            ...
            if item["path"] == "OPENGAT/LOGIN.DAT" and not item["directory"]
        ]
