...
static void build_new_database(uint8_t database[NEW_DATABASE_BYTES])
{
    ...
    put_text(application, 64U, "org.trait.app");
    ...
    put_text(library, 64U, "org.trait.lib");
...
)
{
    ...
    put_text(journal + 160U, 64U, "org.trait.app");
...
{
    /* Independently emitted by trait-transaction.py for this exact state. */
    static const uint8_t canonical_database_sha256[PACKAGE_STATE_SHA256_BYTES] = {
        0x7bU, 0xa0U, 0xefU, 0x78U, 0x1fU, 0x34U, 0x24U, 0x69U,
        0xabU, 0xf3U, 0x85U, 0xa8U, 0x31U, 0x0cU, 0xd7U, 0x6bU,
        0xdcU, 0x66U, 0xfeU, 0x80U, 0xb0U, 0xacU, 0xccU, 0xacU,
        0x08U, 0xc7U, 0x17U, 0x20U, 0x15U, 0x39U, 0xb0U, 0x9cU
    };
    ...
    CHECK(package_state_database_package(&database_view, 0U, &package_view) ==
        ...
        state_text_is(&package_view.identifier, "org.trait.app") &&
    ...
    CHECK(package_state_database_dependency(&database_view, 0U,
        ...
        state_text_is(&dependency_view.requested, "org.trait.lib") &&
        state_text_is(&dependency_view.constraint, "^1.0.0") &&
        state_text_is(&dependency_view.provider, "org.trait.lib") &&
    ...
    copy_bytes(changed, new_database, sizeof(changed));
    put_text(changed + NEW_EDGE_OFFSET + 120U, 64U, "org.trait.missing");
    finalize_database(changed, sizeof(changed));
    CHECK(package_state_database_parse(changed, sizeof(changed), &database_view) ==
        PACKAGE_STATE_STATUS_DEPENDENCY, 17);

    copy_bytes(changed, new_database, sizeof(changed));
    put_text(changed + NEW_EDGE_OFFSET + 120U, 64U, "org.trait.app");
    finalize_database(changed, sizeof(changed));
    CHECK(package_state_database_parse(changed, sizeof(changed), &database_view) ==
...
)
{
    ...
    packages[0].identifier = state_text("org.trait.lib");
    encoded[0] = 1U;
    CHECK(package_generation_encode(&spec, encoded, sizeof(encoded), &view) ==
            PACKAGE_STATE_STATUS_PACKAGE && all_zero(encoded, sizeof(encoded)) &&
        view.bytes == NULL, 52);
    packages[0].identifier = state_text("org.trait.app");
