...
{
    static const char phrase[] = "OpenRFS reproducible zlib stream\n";
...
{
    static const uint8_t source[] = "OpenRFS SDK zlib allocator";
    ...
    if (openrfs_zlib_stream_prepare(NULL) != -1 ||
        openrfs_zlib_stream_prepare(&encode) != 0 ||
    ...
    if (deflateEnd(&encode) != Z_OK || result != Z_STREAM_END ||
        ...
        openrfs_zlib_stream_prepare(&decode) != 0) {
        return 0;
    }
