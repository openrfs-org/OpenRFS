...
{
    static const char phrase[] = "OpenGAT reproducible zlib stream\n";
...
{
    static const uint8_t source[] = "OpenGAT SDK zlib allocator";
    ...
    if (opengat_zlib_stream_prepare(NULL) != -1 ||
        opengat_zlib_stream_prepare(&encode) != 0 ||
    ...
    if (deflateEnd(&encode) != Z_OK || result != Z_STREAM_END ||
        ...
        opengat_zlib_stream_prepare(&decode) != 0) {
        return 0;
    }
