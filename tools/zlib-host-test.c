...
{
    static const char phrase[] = "Trait OS reproducible zlib stream\n";
...
{
    static const uint8_t source[] = "Trait OS SDK zlib allocator";
    ...
    if (trait_zlib_stream_prepare(NULL) != -1 ||
        trait_zlib_stream_prepare(&encode) != 0 ||
    ...
    if (deflateEnd(&encode) != Z_OK || result != Z_STREAM_END ||
        ...
        trait_zlib_stream_prepare(&decode) != 0) {
        return 0;
    }
