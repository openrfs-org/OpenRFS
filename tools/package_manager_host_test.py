...
SPEC = importlib.util.spec_from_file_location(
    "trait_repository_for_guest_test", ROOT / "tools" / "trait-repository.py"
)
if SPEC is None or SPEC.loader is None:
    raise RuntimeError("could not load the Trait OS repository tool")
...
def package_spec(identifier: str, name: str, *,
    ...
    return {
        ...
        "name": name,
        "version": version,
        "publisher": "Trait OS Package Test",
...
def main() -> int:
    ...
    if not PACKAGE.ed25519_available():
        if os.environ.get("TRAIT_REQUIRE_ED25519") == "1":
            raise AssertionError("Python Ed25519 support is required")
        print("Trait OS guest package-manager tests skipped: Ed25519 unavailable")
        return 0
    ...
    dependency = [{
        "identifier": "org.trait.lib",
        "constraint": ">=1.0.0,<2.0.0",
    }]
    library = PACKAGE.build_package_v3(
        package_spec("org.trait.lib", "Proof Library"),
    ...
    application = PACKAGE.build_package_v3(
        package_spec("org.trait.app", "Proof Application", dependencies=dependency),
        ({"path": "bin/proof-app", "kind": "executable",
          "payload": b"\x7fELFproof-application"},),
        PUBLISHER_SEED,
    )
    main_packages = [
        repository_package("org.trait.app", "1.0.0", application,
                           publisher_key_id, dependencies=dependency),
        repository_package("org.trait.lib", "1.0.0", library,
    ...
    replacement_dependency = [{
        "identifier": "org.trait.newlib",
        "constraint": "^2.0.0",
    }]
    replacement_library = PACKAGE.build_package_v3(
        package_spec("org.trait.newlib", "Replacement Library", version="2.0.0"),
    ...
    replacement_application = PACKAGE.build_package_v3(
        package_spec("org.trait.app", "Proof Application", version="2.0.0",
    ...
    update_index = REPOSITORY.build_repository(repository_spec([
        repository_package("org.trait.app", "2.0.0", replacement_application,
                           publisher_key_id,
                           dependencies=replacement_dependency),
        repository_package("org.trait.newlib", "2.0.0", replacement_library,
                           publisher_key_id),
    ...
    cycle = REPOSITORY.build_repository(repository_spec([
        {**dummy("org.trait.a"), "dependencies": [
            {"identifier": "org.trait.b", "constraint": "*"}]},
        {**dummy("org.trait.b"), "dependencies": [
            {"identifier": "org.trait.a", "constraint": "*"}]},
    ]), ROOT_SEED)
    conflict = REPOSITORY.build_repository(repository_spec([
        {**dummy("org.trait.conflict-app"), "dependencies": [
            {"identifier": "org.trait.conflict-lib", "constraint": "*"}]},
        {**dummy("org.trait.conflict-lib"), "conflicts": [
            {"identifier": "org.trait.conflict-app", "constraint": "*"}]},
    ]), ROOT_SEED)
    ambiguous = REPOSITORY.build_repository(repository_spec([
        {**dummy("org.trait.ambiguous-app"), "dependencies": [
            {"identifier": "virtual.renderer", "constraint": "*"}]},
        {**dummy("org.trait.renderer-a"), "provides": [
            {"identifier": "virtual.renderer", "version": "1.0.0"}]},
        {**dummy("org.trait.renderer-b"), "provides": [
            {"identifier": "virtual.renderer", "version": "1.0.0"}]},
    ]), ROOT_SEED)
    unsatisfied = REPOSITORY.build_repository(repository_spec([
        {**dummy("org.trait.unsatisfied"), "dependencies": [
            {"identifier": "org.trait.missing", "constraint": "*"}]},
    ]), ROOT_SEED)
    backtrack = REPOSITORY.build_repository(repository_spec([
        repository_package("org.trait.backtrack", "1.0.0", b"old-app",
                           publisher_key_id, dependencies=[
                               {"identifier": "org.trait.old-lib",
                                "constraint": "^1.0.0"}]),
        repository_package("org.trait.backtrack", "2.0.0", b"new-app",
                           publisher_key_id, dependencies=[
                               {"identifier": "org.trait.new-lib",
                                "constraint": "^2.0.0"}]),
        dummy("org.trait.old-lib"),
    ]), ROOT_SEED)
    chain_packages = []
    for index in range(66):
        identifier = f"org.trait.chain{index:02d}"
        dependencies = [] if index == 65 else [{
            "identifier": f"org.trait.chain{index + 1:02d}",
            "constraint": "*",
        }]
    ...
    print(
        "Trait OS guest package-manager host tests passed: real signed bytes, "
