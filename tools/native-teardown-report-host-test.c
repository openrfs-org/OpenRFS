...
struct report_script {
    enum native_resource_close_result results[OPENRFS_HANDLE_PACKAGE_CONTROL + 1U];
    unsigned calls[OPENRFS_HANDLE_PACKAGE_CONTROL + 1U];
};
...
static void report_argument_and_reset_test(void)
{
    ...
    openrfs_handle_t handle = OPENRFS_HANDLE_INVALID;
    ...
    assert(native_handle_install(&table, OPENRFS_HANDLE_FILE,
...
static void mixed_resource_report_test(void)
{
    static const uint8_t types[] = {
        OPENRFS_HANDLE_FILE, OPENRFS_HANDLE_DIRECTORY, OPENRFS_HANDLE_WINDOW,
        OPENRFS_HANDLE_EVENT_QUEUE, OPENRFS_HANDLE_STREAM,
        OPENRFS_HANDLE_DATAGRAM, OPENRFS_HANDLE_TIMER, OPENRFS_HANDLE_THREAD,
        OPENRFS_HANDLE_AUDIO_OUTPUT, OPENRFS_HANDLE_PACKAGE_UPLOAD,
        OPENRFS_HANDLE_PACKAGE_CONTROL
    };
    struct native_handle_table table;
    struct native_handle_close_report report;
    struct report_script script;
    openrfs_handle_t handles[sizeof(types) / sizeof(types[0])];
    ...
    assert(script.calls[OPENRFS_HANDLE_FILE] == 1U &&
        ...
        script.calls[OPENRFS_HANDLE_AUDIO_OUTPUT] == 1U &&
        script.calls[OPENRFS_HANDLE_PACKAGE_UPLOAD] == 1U &&
        report_has_outcome(&report, OPENRFS_HANDLE_FILE,
            NATIVE_HANDLE_CLOSE_OUTCOME_DUPLICATE) &&
        report_has_outcome(&report, OPENRFS_HANDLE_DIRECTORY,
            NATIVE_HANDLE_CLOSE_OUTCOME_CONSUMED_ERROR) &&
        report_has_outcome(&report, OPENRFS_HANDLE_WINDOW,
            NATIVE_HANDLE_CLOSE_OUTCOME_RETAINED));
    assert(table.active_handles == 1U && table.active_objects == 1U);
    assert(script.calls[OPENRFS_HANDLE_DIRECTORY] == 1U &&
        script.calls[OPENRFS_HANDLE_PACKAGE_UPLOAD] == 1U);
    assert(report.types[OPENRFS_HANDLE_FILE].attempted_handles == 2U &&
        report.types[OPENRFS_HANDLE_FILE].callback_attempts == 1U &&
        report.types[OPENRFS_HANDLE_FILE].closed_resources == 1U &&
        report.types[OPENRFS_HANDLE_FILE].duplicate_references == 1U &&
        report.types[OPENRFS_HANDLE_FILE].retired_handles == 2U);
    assert(report.types[OPENRFS_HANDLE_DIRECTORY].attempted_handles == 1U &&
        report.types[OPENRFS_HANDLE_DIRECTORY].callback_attempts == 1U &&
        report.types[OPENRFS_HANDLE_DIRECTORY].consumed_error_resources ==
            1U && report.types[OPENRFS_HANDLE_DIRECTORY].retired_handles == 1U);
    assert(report.types[OPENRFS_HANDLE_WINDOW].attempted_handles == 1U &&
        report.types[OPENRFS_HANDLE_WINDOW].callback_attempts == 1U &&
        report.types[OPENRFS_HANDLE_WINDOW].retained_resources == 1U &&
        report.types[OPENRFS_HANDLE_WINDOW].retired_handles == 0U);
    for (size_t type = OPENRFS_HANDLE_EVENT_QUEUE;
         type <= OPENRFS_HANDLE_PACKAGE_CONTROL; ++type) {
        if (type == OPENRFS_HANDLE_PACKAGE_UPLOAD) {
            assert(report.types[type].consumed_error_resources == 1U);
        } else if (type != OPENRFS_HANDLE_WINDOW) {
    ...
    assert(report.status == NATIVE_HANDLE_OK && report.attempted_handles == 1U &&
        ...
        report_entry_at(&report, 0U)->type == OPENRFS_HANDLE_WINDOW &&
        report_entry_at(&report, 0U)->outcome ==
            NATIVE_HANDLE_CLOSE_OUTCOME_CLOSED);
    assert(script.calls[OPENRFS_HANDLE_WINDOW] == 2U &&
        script.calls[OPENRFS_HANDLE_DIRECTORY] == 1U &&
        script.calls[OPENRFS_HANDLE_PACKAGE_UPLOAD] == 1U);
}
...
static void duplicate_retry_progress_test(void)
{
    ...
    assert(second_report.attempted_handles == 1U &&
        ...
        script.calls[OPENRFS_HANDLE_FILE] == 2U);
...
static void ordinary_close_report_test(void)
{
    ...
    assert(report.status == NATIVE_HANDLE_STALE && report.invalid_entries == 1U &&
        ...
        !report.progress &&
        report.types[OPENRFS_HANDLE_FILE].attempted_handles == 0U);
    ...
    assert(native_handle_close_with_report(&table,
        (UINT64_C(1) << 32) | ((uint64_t)OPENRFS_HANDLE_FILE << 16) | 3U,
        scripted_close, &script, &report) == NATIVE_HANDLE_STALE);
...
static void malformed_entry_report_test(void)
{
    ...
    assert(report.types[OPENRFS_HANDLE_FILE].attempted_handles == 1U &&
        report.types[OPENRFS_HANDLE_FILE].invalid_entries == 1U &&
        report.types[OPENRFS_HANDLE_DIRECTORY].attempted_handles == 1U &&
        report.types[OPENRFS_HANDLE_DIRECTORY].stale_entries == 1U);
    for (size_t type = OPENRFS_HANDLE_WINDOW;
         type <= OPENRFS_HANDLE_PACKAGE_CONTROL; ++type) {
        assert(report.types[type].attempted_handles == 0U);
    }
    assert(script.calls[OPENRFS_HANDLE_FILE] == 0U &&
        script.calls[OPENRFS_HANDLE_DIRECTORY] == 0U);
...
static void bounded_capacity_report_test(void)
{
    ...
    for (size_t index = 0U; index < NATIVE_HANDLE_LIMIT; ++index) {
        install_resource(&table, OPENRFS_HANDLE_TIMER, &handle);
    }
    assert(native_handle_close_all_diagnostics(&table, scripted_close, &script,
        &report) == NATIVE_HANDLE_OK);
    assert(report.attempted_handles == NATIVE_HANDLE_LIMIT &&
        ...
        !report.retryable && script.calls[OPENRFS_HANDLE_TIMER] ==
            NATIVE_HANDLE_LIMIT);
    assert(report.types[OPENRFS_HANDLE_TIMER].attempted_handles ==
            NATIVE_HANDLE_LIMIT &&
        report.types[OPENRFS_HANDLE_TIMER].callback_attempts ==
            NATIVE_HANDLE_LIMIT &&
        report.types[OPENRFS_HANDLE_TIMER].closed_resources ==
            NATIVE_HANDLE_LIMIT &&
        report.types[OPENRFS_HANDLE_TIMER].retired_handles ==
            NATIVE_HANDLE_LIMIT);
    for (size_t index = 0U; index < NATIVE_HANDLE_CLOSE_REPORT_CAPACITY;
         ++index) {
        assert(report.entries[index].type == OPENRFS_HANDLE_TIMER &&
