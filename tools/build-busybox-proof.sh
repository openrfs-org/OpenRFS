...
build_only=${TRAIT_BUSYBOX_BUILD_ONLY:-0}
if [ "$build_only" != 0 ] && [ "$build_only" != 1 ]; then
    printf 'TRAIT_BUSYBOX_BUILD_ONLY must be 0 or 1\n' >&2
    exit 2
fi
...
env -i strace --argv0=busybox --quiet=all --string-limit=256 \
    --output="$trace_file" "$output_dir/busybox" echo TRAIT \
