#!/usr/bin/env python3
"""Host refusal and reproducibility tests for the OpenGAT package format."""
...
SPEC = importlib.util.spec_from_file_location(
    "opengat_package", ROOT / "tools" / "opengat-package.py")
...
def main() -> int:
    ...
    spec = {
        ...
        "arguments": ["PKGTEST.APP", "http://opengat.test/welcome.txt"],
    }
    ...
    assert hashlib.sha256(first).hexdigest().upper() == (
        "499EBFCE8FA954A8803ED3E2EC4405C74AB3D81B7AD4626CD80D8B8794506232")
    ...
    assert report["arguments"] == [
        "PKGTEST.APP", "http://opengat.test/welcome.txt"
    ]
    ...
    assert hashlib.sha256(resource_package).hexdigest().upper() == (
        "7E10CCD1969A0454C44D8BB92C0C335CDB4D361F724D28AE793966B51DE35BC3")
    ...
    if crypto_available:
        ...
        assert report["identifier"] == "org.opengat.pkgtest"
        ...
        assert [item["identifier"] for item in report["dependencies"]] == [
            "org.opengat.libc", "org.opengat.zlib"]
    ...
    print("OpenGAT package host tests passed: legacy/dynamic bytes, v3 canonical tables, bounds, digests, trust, Ed25519 verification/refusal")
    return 0
