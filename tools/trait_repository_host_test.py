...
SPEC = importlib.util.spec_from_file_location(
    "trait_repository", ROOT / "tools" / "trait-repository.py")
...
def main() -> int:
    ...
    if os.environ.get("TRAIT_REQUIRE_ED25519") == "1" and not crypto_available:
    ...
    packages = [
        package("org.trait.desktop", "3.0.0", desktop_payload,
                dependencies=[
                    {"identifier": "virtual.renderer",
                     "constraint": ">=1.0.0,<2.0.0"},
                    {"identifier": "org.trait.libc", "constraint": "^1.0.0"},
                ]),
        package("org.trait.libc", "0.9.0", libc_old_payload),
        package("org.trait.render-soft", "2.0.0", renderer_payload,
                provides=[{"identifier": "virtual.renderer", "version": "1.5.0"}]),
        package("org.trait.libc", "1.0.0", libc_payload),
    ]
    ...
    if not crypto_available:
        ...
        print("Trait OS repository host tests passed: Ed25519 unavailable and refused closed")
        return 0

    first = REPOSITORY.build_repository(repository_spec, private_seed)
    assert hashlib.sha256(first).hexdigest().upper() == (
        "31A1F2648B6D1293DA0279BA361CD6FC6E44EA8AE90332DE576BF6726423FB8F")
    ...
    assert report["repository"] == "org.trait.main"
    ...
    assert [(item["identifier"], item["version"]) for item in report["packages"]] == [
        ("org.trait.desktop", "3.0.0"),
        ("org.trait.libc", "0.9.0"),
        ("org.trait.libc", "1.0.0"),
        ("org.trait.render-soft", "2.0.0"),
    ]
    ...
    plan, lock = REPOSITORY.resolve_repository(
        report, ["org.trait.desktop@^3.0.0"], architecture="x86_64",
    ...
    assert hashlib.sha256(lock).hexdigest().upper() == (
        "E9F3E8EC52DA90B9F9C454E6D35AAE1C371F56265C4E736096381569449D901F")
    ...
    REPOSITORY.verify_download(
        next(item for item in report["packages"]
             if item["identifier"] == "org.trait.libc" and item["version"] == "1.0.0"),
        libc_payload)
    libc_entry = next(item for item in report["packages"]
                      if item["identifier"] == "org.trait.libc"
    ...
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            report, ["org.trait.desktop"], architecture="aarch64",
            abi_version=1),
        "architecture")
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            report, ["org.trait.desktop"], architecture="x86_64",
            abi_version=3),
    ...
    unsatisfied_report = parsed_with([
        package("org.trait.need-missing", "1.0.0", b"missing",
                dependencies=[{"identifier": "org.trait.absent",
                               "constraint": "*"}]),
    ])
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            unsatisfied_report, ["org.trait.need-missing"],
            architecture="x86_64", abi_version=1),
        "unsatisfied dependency")
    backtracking_report = parsed_with([
        package("org.trait.app", "1.0.0", b"app one",
                dependencies=[{"identifier": "org.trait.lib",
                               "constraint": "^1.0.0"}]),
        package("org.trait.app", "2.0.0", b"app two",
                dependencies=[{"identifier": "org.trait.lib",
                               "constraint": "^2.0.0"}]),
        package("org.trait.lib", "1.1.0", b"lib one"),
    ])
    backtracking_plan, _ = REPOSITORY.resolve_repository(
        backtracking_report, ["org.trait.app"], architecture="x86_64",
        abi_version=1)
    assert [(item["identifier"], item["version"])
            for item in backtracking_plan] == [
                ("org.trait.lib", "1.1.0"), ("org.trait.app", "1.0.0")]
    conflict_packages = copy.deepcopy(packages)
    next(item for item in conflict_packages
         if item["identifier"] == "org.trait.render-soft")["conflicts"] = [
             {"identifier": "org.trait.desktop", "constraint": "*"}]
    conflict_report = parsed_with(conflict_packages)
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            conflict_report, ["org.trait.desktop"],
            architecture="x86_64", abi_version=1),
        "conflicts with")
    ambiguous_packages = copy.deepcopy(packages)
    ambiguous_packages.append(
        package("org.trait.render-hw", "1.0.0", b"hardware renderer",
                provides=[{"identifier": "virtual.renderer", "version": "1.4.0"}]))
    ambiguous_report = parsed_with(ambiguous_packages)
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            ambiguous_report, ["org.trait.desktop"],
            architecture="x86_64", abi_version=1),
        "ambiguous providers")
    cycle_report = parsed_with([
        package("org.trait.a", "1.0.0", b"a",
                dependencies=[{"identifier": "org.trait.b", "constraint": "*"}]),
        package("org.trait.b", "1.0.0", b"b",
                dependencies=[{"identifier": "org.trait.a", "constraint": "*"}]),
    ])
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            cycle_report, ["org.trait.a"], architecture="x86_64", abi_version=1),
        "dependency cycle")
    chain_report = parsed_with([
        package("org.trait.a", "1.0.0", b"a",
                dependencies=[{"identifier": "org.trait.b", "constraint": "*"}]),
        package("org.trait.b", "1.0.0", b"b",
                dependencies=[{"identifier": "org.trait.c", "constraint": "*"}]),
        package("org.trait.c", "1.0.0", b"c"),
    ])
    expect_refusal(
        lambda: REPOSITORY.resolve_repository(
            chain_report, ["org.trait.a"], architecture="x86_64",
            abi_version=1, max_packages=2),
        "graph exceeds its 2-package bound")

    print("Trait OS repository host tests passed: canonical signature, freshness, digests, resolver refusals, exact lock")
    return 0
