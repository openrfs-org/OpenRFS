...
{
    const long result = trait_realtime_seconds();
...
clock_t clock(void) { return (clock_t)(trait_monotonic_ns() / 1000U); }
...
int nanosleep(const struct timespec *request, struct timespec *remaining)
{
    ...
    result = trait_sleep_until(now + interval);
...
{
    /* Trait OS has no timezone database yet; local civil time is explicitly UTC. */
