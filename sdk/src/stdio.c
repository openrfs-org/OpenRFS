...
static int mode_flags(const char *mode, uint32_t *open_flags)
{
    ...
    if (mode[0] == 'r') {
        ...
        *open_flags = OPENRFS_OPEN_READ |
            (update ? OPENRFS_OPEN_WRITE : 0U);
    } else if (mode[0] == 'w') {
...
FILE *fopen(const char *path, const char *mode)
{
    ...
    if (flags == 0 || openrfs_runtime_path(path, &parsed) != 0) {
        errno = EINVAL;
        return NULL;
    }
    handle = openrfs_file_open(parsed.volume, parsed.text, open_flags);
    if (handle < 0) {
        errno = (int)-handle;
        return NULL;
    }
    ...
    if (stream == NULL) {
        (void)openrfs_handle_close((openrfs_handle_t)handle);
        return NULL;
    }
    stream->handle = (openrfs_handle_t)handle;
    stream->flags = (unsigned int)flags;
    stream->pushed = -1;
    if ((flags & FILE_APPEND) != 0 &&
        openrfs_file_seek(stream->handle, 0, OPENRFS_SEEK_END) < 0) {
        (void)fclose(stream);
        return NULL;
    }
    return stream;
