...
static int flush_locked(FILE *stream)
{
    ...
    while (offset < stream->length) {
        ...
        if ((stream->flags & FILE_CONSOLE) != 0U) {
            ...
            result = trait_file_write(stream->handle,
...
int fflush(FILE *stream)
{
    ...
    trait_runtime_lock(&stream->lock);
    result = flush_locked(stream);
    trait_runtime_unlock(&stream->lock);
    return result;
}
...
static int mode_flags(const char *mode, uint32_t *open_flags)
{
    ...
    if (mode[0] == 'r') {
        ...
        *open_flags = TRAIT_OPEN_READ |
            (update ? TRAIT_OPEN_WRITE : 0U);
    } else if (mode[0] == 'w') {
        flags = FILE_WRITE | (update ? FILE_READ : 0);
        *open_flags = TRAIT_OPEN_WRITE | TRAIT_OPEN_CREATE |
            TRAIT_OPEN_TRUNCATE | (update ? TRAIT_OPEN_READ : 0U);
    } else if (mode[0] == 'a') {
        flags = FILE_WRITE | FILE_APPEND | (update ? FILE_READ : 0);
        *open_flags = TRAIT_OPEN_WRITE | TRAIT_OPEN_CREATE |
            (update ? TRAIT_OPEN_READ : 0U);
    }
    return flags;
}
...
FILE *fopen(const char *path, const char *mode)
{
    ...
    if (flags == 0 || trait_runtime_path(path, &parsed) != 0) {
        errno = EINVAL;
        return NULL;
    }
    handle = trait_file_open(parsed.volume, parsed.text, open_flags);
    if (handle < 0) {
        errno = (int)-handle;
        return NULL;
    }
    ...
    if (stream == NULL) {
        (void)trait_handle_close((trait_handle_t)handle);
        return NULL;
    }
    stream->handle = (trait_handle_t)handle;
    stream->flags = (unsigned int)flags;
    stream->pushed = -1;
    if ((flags & FILE_APPEND) != 0 &&
        trait_file_seek(stream->handle, 0, TRAIT_SEEK_END) < 0) {
        (void)fclose(stream);
        return NULL;
    }
    return stream;
...
FILE *freopen(const char *path, const char *mode, FILE *stream)
{
    ...
    (void)fflush(stream);
    (void)trait_handle_close(stream->handle);
    *stream = *replacement;
    free(replacement);
    return stream;
}
...
int fclose(FILE *stream)
{
    ...
    if (trait_handle_close(stream->handle) < 0) {
        result = EOF;
    }
    free(stream);
    return result;
...
static size_t read_locked(void *pointer, size_t bytes, FILE *stream)
{
    ...
    if ((stream->flags & FILE_CONSOLE) != 0U && completed < bytes) {
        const long result = trait_console_read(output + completed,
            bytes - completed);
    ...
    while (completed < bytes) {
        ...
        const long result = trait_file_read(stream->handle, stream->buffer,
...
static size_t write_locked(const void *pointer, size_t bytes, FILE *stream)
{
    ...
    if ((stream->flags & FILE_BUFFER_DIRTY) == 0U &&
        ...
        if ((stream->flags & FILE_CONSOLE) == 0U && unread != 0U) {
            seek_result = trait_file_seek(stream->handle, -(int64_t)unread,
                TRAIT_SEEK_CURRENT);
        }
        if (seek_result < 0) {
            stream->error = 1U;
...
int fseek(FILE *stream, long offset, int origin)
{
    ...
    stream->pushed = -1;
    stream->eof = 0U;
    result = trait_file_seek(stream->handle, offset, (uint32_t)origin);
    trait_runtime_unlock(&stream->lock);
    if (result < 0) {
        errno = (int)-result;
        return -1;
...
long ftell(FILE *stream)
{
    ...
    trait_runtime_lock(&stream->lock);
    result = trait_file_seek(stream->handle, 0, TRAIT_SEEK_CURRENT);
...
int remove(const char *path)
{
    ...
    result = trait_syscall2(TRAIT_SYS_PATH_UNLINK,
        (uint64_t)(uintptr_t)&request, 0U);
    return trait_result(result);
}
int rename(const char *source, const char *destination)
{
    struct trait_runtime_path from, to;
    struct trait_rename_request request;
    if (trait_runtime_path(source, &from) != 0 ||
        trait_runtime_path(destination, &to) != 0) return -1;
    request.size = sizeof(request); request.version = TRAIT_ABI_VERSION;
    request.source = (struct trait_path){(uint64_t)(uintptr_t)from.text,
        (uint32_t)from.length, from.volume, 0U};
    request.destination = (struct trait_path){(uint64_t)(uintptr_t)to.text,
        (uint32_t)to.length, to.volume, 0U};
    request.flags = 0U; request.reserved = 0U;
    return trait_result(trait_syscall1(TRAIT_SYS_PATH_RENAME,
