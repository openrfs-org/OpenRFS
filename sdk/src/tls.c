...
#define TRAIT_TLS_MAX_HOSTNAME 253U
#define TRAIT_TLS_MAX_LABEL 63U
#define TRAIT_TLS_MAX_TRUST_ANCHORS 16U
#define TRAIT_TLS_MAX_TRUST_BYTES 81920U
#define TRAIT_TLS_MAX_HANDSHAKE_STEPS 4096U
#define TRAIT_TLS_UNIX_EPOCH_DAYS UINT64_C(719528)
#define TRAIT_TLS_ENTROPY_BYTES 32U
#define TRAIT_TLS_CLOCK_MIN UINT64_C(1577836800)
#define TRAIT_TLS_CLOCK_MAX UINT64_C(4102444799)
#define TRAIT_HTTPS_REQUEST_BYTES 1536U
#define TRAIT_HTTPS_BODY_CHUNK_BYTES 4096U
...
struct trait_tls_client {
    ...
    br_x509_trust_anchor anchors[TRAIT_TLS_MAX_TRUST_ANCHORS];
    unsigned char *anchor_storage;
    size_t anchor_storage_length;
    char hostname[TRAIT_TLS_MAX_HOSTNAME + 1U];
    trait_handle_t stream;
...
static bool hostname_valid(const char *hostname)
{
    ...
    if (length == 0U || length > TRAIT_TLS_MAX_HOSTNAME) {
        return false;
    }
    for (size_t index = 0U; index < length; ++index) {
        ...
        if (value == '.') {
            if (label == 0U || label > TRAIT_TLS_MAX_LABEL ||
                hostname[index - 1U] == '-') {
                return false;
            }
            label = 0U;
    ...
    return label != 0U && label <= TRAIT_TLS_MAX_LABEL &&
        hostname[length - 1U] != '-';
}
    ...
    size_t count, size_t *total)
{
    ...
    for (size_t index = 0U; index < count; ++index) {
        ...
        if (!size_add(used, anchor->dn.len, &used) ||
            ...
            used > TRAIT_TLS_MAX_TRUST_BYTES) {
            return false;
        }
    }
    *total = used;
...
static bool copy_trust_anchors(struct trait_tls_client *client,
...
static void diagnostics_clear(struct trait_tls_diagnostics *diagnostics)
{
    if (diagnostics != NULL) {
...
static void diagnostics_capture(const struct trait_tls_client *client,
...
static long deadline_error(struct trait_tls_client *client)
{
    if (__atomic_load_n(&client->canceled, __ATOMIC_ACQUIRE)) {
        return -(long)TRAIT_ECANCELED;
    }
    if (client->deadline_ns <= trait_monotonic_ns()) {
        return -(long)TRAIT_ETIMEDOUT;
    }
    return 0;
}

static bool set_operation_deadline(struct trait_tls_client *client,
    uint64_t deadline_ns)
{
    client->deadline_ns = deadline_ns;
    client->transport_error = 0;
    if (deadline_ns <= trait_monotonic_ns()) {
        client->transport_error = -(long)TRAIT_ETIMEDOUT;
        client->status = TRAIT_TLS_IO;
        return false;
    }
    return true;
}
...
{
    struct trait_tls_client *client = context;
    long count = deadline_error(client);

    if (count == 0) {
        count = trait_stream_read(client->stream, buffer, length,
            client->deadline_ns);
    }
    if (count <= 0 || count > INT_MAX || (size_t)count > length) {
        if (count == -(long)TRAIT_EIO &&
            ...
            count = -(long)TRAIT_EPIPE;
        }
        client->transport_error = count == 0 ? -(long)TRAIT_EPIPE : count;
        return -1;
    }
    ...
    size_t length)
{
    ...
    if (count == 0) {
        count = trait_stream_write(client->stream, buffer, length,
            client->deadline_ns);
    }
    if (count <= 0 || count > INT_MAX || (size_t)count > length) {
        client->transport_error = count == 0 ? -(long)TRAIT_EPIPE : count;
...
static enum trait_tls_status drive_handshake(struct trait_tls_client *client)
{
    for (size_t step = 0U; step < TRAIT_TLS_MAX_HANDSHAKE_STEPS; ++step) {
        const unsigned state = br_ssl_engine_current_state(&client->ssl.eng);
        size_t length = 0U;

        if ((state & BR_SSL_CLOSED) != 0U) {
            return TRAIT_TLS_HANDSHAKE;
        }
...
static void release_client(struct trait_tls_client *client,
    uint64_t deadline_ns)
{
    if (client == NULL) {
        return;
    }
    if (client->stream != TRAIT_HANDLE_INVALID) {
        (void)trait_stream_shutdown(client->stream,
            TRAIT_SHUTDOWN_WRITE | TRAIT_SHUTDOWN_READ, deadline_ns);
        (void)trait_handle_close(client->stream);
        client->stream = TRAIT_HANDLE_INVALID;
    }
    if (client->anchor_storage != NULL) {
...
{
    struct trait_tls_client *client;
    struct trait_ipv4_endpoint endpoint;
    unsigned char entropy[TRAIT_TLS_ENTROPY_BYTES];
    ...
    *result = NULL;
    if (config == NULL || config->reserved != 0U || config->port == 0U ||
        ...
        config->trust_anchor_count > TRAIT_TLS_MAX_TRUST_ANCHORS ||
        config->deadline_ns <= trait_monotonic_ns()) {
        return TRAIT_TLS_ARGUMENT;
    }
    for (size_t index = 0U; index < config->trust_anchor_count; ++index) {
        if (!trust_anchor_valid(&config->trust_anchors[index])) {
            return TRAIT_TLS_TRUST;
        }
    }
    realtime = trait_realtime_seconds();
    if (realtime < 0 || (uint64_t)realtime < TRAIT_TLS_CLOCK_MIN ||
        (uint64_t)realtime > TRAIT_TLS_CLOCK_MAX ||
        (uint64_t)realtime / UINT64_C(86400) >
            UINT32_MAX - TRAIT_TLS_UNIX_EPOCH_DAYS) {
        return TRAIT_TLS_CLOCK;
    }
    client = calloc(1U, sizeof(*client));
    if (client == NULL) {
        return TRAIT_TLS_NO_MEMORY;
    }
    client->stream = TRAIT_HANDLE_INVALID;
    ...
    if (trait_random_strong(entropy, sizeof(entropy)) !=
            (long)sizeof(entropy)) {
        secure_zero(entropy, sizeof(entropy));
        release_client(client, config->deadline_ns);
        return TRAIT_TLS_ENTROPY;
    }
    resolved = trait_dns_resolve(client->hostname, config->deadline_ns);
    if (resolved <= 0 || (uint64_t)resolved > UINT32_MAX) {
        secure_zero(entropy, sizeof(entropy));
        release_client(client, config->deadline_ns);
        return TRAIT_TLS_DNS;
    }
    opened = trait_stream_open();
    if (opened < 0) {
        secure_zero(entropy, sizeof(entropy));
    ...
    client->stream = (trait_handle_t)opened;
    endpoint = (struct trait_ipv4_endpoint){
        (uint32_t)resolved, config->port, 0U};
    connected = trait_stream_connect(client->stream, &endpoint,
        config->deadline_ns);
    if (connected < 0) {
        secure_zero(entropy, sizeof(entropy));
...
enum trait_tls_status trait_tls_client_open(
    ...
    struct trait_tls_client **result)
{
    return trait_tls_client_open_diagnostic(config, NULL, result);
}
...
long trait_tls_client_cancel(struct trait_tls_client *client)
{
    ...
    if (client == NULL || client->stream == TRAIT_HANDLE_INVALID) {
        return -(long)TRAIT_EINVAL;
    }
    __atomic_store_n(&client->canceled, true, __ATOMIC_RELEASE);
    result = trait_network_cancel(client->stream);
    return result;
}

static enum trait_tls_status close_client(
    struct trait_tls_client *client, uint64_t deadline_ns,
    struct trait_tls_diagnostics *diagnostics)
{
    enum trait_tls_status status;
...
{
    return client == NULL ? TRAIT_TLS_ARGUMENT : client->status;
}
...
static enum trait_https_status transport_https_status(long error,
    enum trait_https_status fallback)
{
    if (error == -(long)TRAIT_ETIMEDOUT) {
        return TRAIT_HTTPS_TIMEOUT;
    }
    if (error == -(long)TRAIT_ECANCELED) {
        return TRAIT_HTTPS_CANCELED;
    }
    if (error == -(long)TRAIT_EIO) {
        return TRAIT_HTTPS_RESET;
    }
    if (error == -(long)TRAIT_EPIPE) {
        return TRAIT_HTTPS_TRUNCATED;
    }
    return fallback;
}

static enum trait_https_status handshake_https_status(
    enum trait_tls_status status, const struct trait_tls_diagnostics *details)
{
    if (status == TRAIT_TLS_ARGUMENT) {
        return TRAIT_HTTPS_ARGUMENT;
    }
    if (status == TRAIT_TLS_NO_MEMORY) {
        return TRAIT_HTTPS_NO_MEMORY;
    }
    if (status == TRAIT_TLS_TRUST) {
        return TRAIT_HTTPS_TRUST;
    }
    if (status == TRAIT_TLS_CLOCK) {
        return TRAIT_HTTPS_CLOCK;
    }
    if (status == TRAIT_TLS_ENTROPY) {
        return TRAIT_HTTPS_ENTROPY;
    }
    if (status == TRAIT_TLS_DNS) {
        return TRAIT_HTTPS_DNS;
    }
    if (details->bearssl_error == BR_ERR_X509_BAD_SERVER_NAME ||
        details->bearssl_error == BR_ERR_X509_DN_MISMATCH) {
        return TRAIT_HTTPS_HOSTNAME;
    }
    if (details->bearssl_error == BR_ERR_X509_EXPIRED ||
        ...
        return TRAIT_HTTPS_CERTIFICATE_TIME;
    }
    if (details->bearssl_error == BR_ERR_X509_NOT_TRUSTED ||
        ...
        return TRAIT_HTTPS_AUTHENTICATION;
    }
    if (status == TRAIT_TLS_TRANSPORT) {
        return transport_https_status(details->transport_error,
            TRAIT_HTTPS_TRANSPORT);
    }
    return TRAIT_HTTPS_HANDSHAKE;
}

static enum trait_https_status client_io_status(
    struct trait_tls_client *client, enum trait_https_status fallback)
{
    return transport_https_status(
        trait_tls_client_transport_error(client), fallback);
}

static void abort_client(struct trait_tls_client *client,
    uint64_t deadline_ns)
{
    if (client != NULL) {
        (void)trait_tls_client_cancel(client);
        release_client(client, deadline_ns);
    }
}
...
static bool path_valid(const char *path, size_t *length)
{
    ...
    used = strlen(path);
    if (used == 0U || used > TRAIT_HTTPS_MAX_PATH_BYTES || path[0] != '/') {
        return false;
    }
...
static bool make_http_request(const struct trait_https_stream_request *request,
...
static enum trait_https_status parse_http_headers(
    ...
    struct trait_https_response *response)
{
    ...
    for (size_t index = 0U; index < length; ++index) {
        ...
        if ((value < 0x20U && value != '\r' && value != '\n' &&
            ...
            return TRAIT_HTTPS_HTTP_HEADERS;
        }
    }
    ...
    if (!content_length_seen) {
        return TRAIT_HTTPS_CONTENT_LENGTH_REQUIRED;
    }
    if (response->content_length > body_capacity) {
        return TRAIT_HTTPS_CONTENT_TOO_LARGE;
    }
    return TRAIT_HTTPS_OK;
}

static void capture_response_diagnostics(struct trait_tls_client *client,
    struct trait_https_response *response)
{
    response->bearssl_error = trait_tls_client_bearssl_error(client);
    response->transport_error = trait_tls_client_transport_error(client);
}
...
enum trait_https_status trait_https_get_stream(
    ...
    struct trait_https_response *response)
{
    struct trait_tls_client_config tls_config;
    struct trait_tls_diagnostics diagnostics;
    struct trait_tls_client *client = NULL;
    unsigned char header[TRAIT_HTTPS_MAX_HEADER_BYTES];
    unsigned char body[TRAIT_HTTPS_BODY_CHUNK_BYTES];
    char wire_request[TRAIT_HTTPS_REQUEST_BYTES];
    size_t request_length;
    size_t sent = 0U;
    size_t header_length = 0U;
    unsigned delimiter = 0U;
    enum trait_tls_status tls_status;
    enum trait_https_status status;
    ...
    while (sent < request_length) {
        const long count = trait_tls_client_write(client,
            wire_request + sent, request_length - sent, request->deadline_ns);

        if (count <= 0) {
            status = client_io_status(client, TRAIT_HTTPS_IO);
    ...
    if (trait_tls_client_flush(client, request->deadline_ns) !=
            TRAIT_TLS_OK) {
        status = client_io_status(client, TRAIT_HTTPS_IO);
        capture_response_diagnostics(client, response);
        abort_client(client, request->deadline_ns);
    ...
    while (header_length < sizeof(header) && delimiter != 4U) {
        long count = trait_tls_client_read(client, &header[header_length], 1U,
            request->deadline_ns);

        if (count <= 0) {
            status = count == 0 ? TRAIT_HTTPS_TRUNCATED :
                client_io_status(client, TRAIT_HTTPS_IO);
            capture_response_diagnostics(client, response);
            abort_client(client, request->deadline_ns);
            return status;
        }
    ...
    while (response->body_length < response->content_length) {
        ...
        long count = trait_tls_client_read(client,
            body, requested, request->deadline_ns);

        if (count <= 0) {
            if (count == 0 || trait_tls_client_transport_error(client) ==
                    -(long)TRAIT_EPIPE) {
                status = TRAIT_HTTPS_BODY_TRUNCATED;
            } else {
        ...
        if (request->write_body(request->write_context, body,
            ...
            capture_response_diagnostics(client, response);
            abort_client(client, request->deadline_ns);
            return TRAIT_HTTPS_BODY_WRITE;
        }
        response->body_length += (size_t)count;
    }
    {
        unsigned char extra;
        const long count = trait_tls_client_read(client, &extra, 1U,
            request->deadline_ns);

        if (count > 0) {
            capture_response_diagnostics(client, response);
            abort_client(client, request->deadline_ns);
            return TRAIT_HTTPS_BODY_EXTRA;
        }
        if (count < 0) {
            status = client_io_status(client, TRAIT_HTTPS_TRUNCATED);
            capture_response_diagnostics(client, response);
            abort_client(client, request->deadline_ns);
            return status;
        }
    ...
    response->bearssl_error = diagnostics.bearssl_error;
    response->transport_error = diagnostics.transport_error;
    return tls_status == TRAIT_TLS_OK ? TRAIT_HTTPS_OK :
    ...
    struct trait_https_response *response)
{
    ...
    return trait_https_get_stream(&stream, response);
}
