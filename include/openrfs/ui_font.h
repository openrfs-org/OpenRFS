...
#ifndef OPENRFS_UI_FONT_H
#define OPENRFS_UI_FONT_H
...
int32_t openrfs_ui_font_self_test(void);
size_t openrfs_ui_font_size(void);
uint64_t openrfs_ui_font_fingerprint(void);
int32_t openrfs_ui_font_geometry(struct ui_font_metrics *metrics);
int32_t openrfs_ui_font_glyph(
    uint32_t code,
    uint8_t *out,
    size_t out_len
);
int32_t openrfs_ui_font_glyph_advance(uint32_t code, uint32_t *advance);
