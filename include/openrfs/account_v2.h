...
#define ACCOUNT_V2_FLAG_DATA_ENCRYPTED 1U
#define ACCOUNT_V2_FLAG_DATA_MIGRATING 2U
...
/* The migration-complete bit is authenticated by the verifier and key wrap.
 * Only a verified, fully migrated Data tree may set it. Whole-volume rollback
 * of an older record still needs an external freshness root. */
enum account_v2_status account_v2_seal_flags(
    const char *username, const uint8_t *password, size_t password_bytes,
    uint64_t generation, uint8_t flags, const uint8_t salt[16],
    const uint8_t nonce[ACCOUNT_V2_NONCE_BYTES],
    const uint8_t data_key[ACCOUNT_V2_KEY_BYTES],
    uint8_t record[ACCOUNT_V2_RECORD_BYTES]);
