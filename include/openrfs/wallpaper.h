...
#ifndef OPENRFS_WALLPAPER_H
#define OPENRFS_WALLPAPER_H
...
int32_t openrfs_wallpaper_self_test(void);
size_t openrfs_wallpaper_size(void);
int32_t openrfs_wallpaper_geometry(
    uint32_t *width,
    uint32_t *height,
    uint32_t *frames
);
int32_t openrfs_wallpaper_decode(
    uint32_t frame,
    uint32_t *out,
    size_t out_pixels,
    uint32_t out_width,
