...
#ifndef OPENRFS_NATIVE_TEARDOWN_DIAGNOSTICS_H
#define OPENRFS_NATIVE_TEARDOWN_DIAGNOSTICS_H
...
#define NATIVE_TEARDOWN_RESOURCE_CLASS_MASK(type) \
    ((uint32_t)(type) >= OPENRFS_HANDLE_FILE && \
        (uint32_t)(type) <= OPENRFS_HANDLE_PACKAGE_CONTROL ? \
        (UINT32_C(1) << ((uint32_t)(type) - OPENRFS_HANDLE_FILE)) : 0U)
