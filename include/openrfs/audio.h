...
#ifndef OPENRFS_AUDIO_H
#define OPENRFS_AUDIO_H
...
/*
 * The first OpenRFS driver that lets a device write into kernel memory on its
 ...
 * DMA. OpenRFS has no IOMMU, so a device that can write one ring can write
...
/* A page-aligned silent guard keeps a delayed polled stop from replaying PCM. */
#define AUDIO_PCM_GUARD_FRAMES 48128U
#define AUDIO_PCM_GUARD_BYTES (AUDIO_PCM_GUARD_FRAMES * AUDIO_PCM_FRAME_BYTES)
#define AUDIO_PCM_PERIOD_BYTES AUDIO_PCM_BYTES
#define AUDIO_PCM_BDL_ENTRIES 2U
#define AUDIO_PCM_DMA_BYTES (AUDIO_PCM_BYTES + AUDIO_PCM_GUARD_BYTES)
