...
#ifndef OPENRFS_LOGO_H
#define OPENRFS_LOGO_H
 ...
 *
 * This is the first thing in OpenRFS that is not C, and the choice is argued in
...
int32_t openrfs_logo_self_test(void);

/* How many bytes the built-in image occupies. */
size_t openrfs_logo_size(void);

/* Read the declared size without decoding. Both pointers must be non-null. */
int32_t openrfs_logo_geometry(uint32_t *width, uint32_t *height);
...
int32_t openrfs_logo_decode_alpha(uint8_t *out, size_t out_pixels);
