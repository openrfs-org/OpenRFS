...
#ifndef OPENGAT_FAT16_H
#define OPENGAT_FAT16_H
...
enum fat16_status opengat_fat16_parse_bpb(
    const uint8_t *block,
    size_t block_len,
    uint64_t namespace_blocks,
...
enum fat16_status opengat_fat16_make_query(
    const uint8_t *name,
    size_t name_len,
    struct fat16_root_query *out
);
enum fat16_status opengat_fat16_find_root(
    const uint8_t *block,
    size_t block_len,
...
enum fat16_status opengat_fat16_parse_fat(
    const uint8_t *block,
    size_t block_len,
    const struct fat16_geometry *geometry,
    struct fat16_fat_state *out
);
enum fat16_status opengat_fat16_validate_extent(
    const struct fat16_geometry *geometry,
    const struct fat16_root_entry *entry,
    const struct fat16_fat_state *fat,
    struct fat16_extent *out
);
enum fat16_status opengat_fat16_validate_payload(
    const uint8_t *data,
    size_t data_len,
