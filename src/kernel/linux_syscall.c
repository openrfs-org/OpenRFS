...
static const struct linux_utsname_record trait_uts_record = {
    .sysname = "Linux",
    .nodename = "trait",
    .release = "2.2.0-trait",
    .version = "Trait OS",
...
bool linux_syscall_uname_semantic_self_test(void)
{
    ...
    if (!uts_field_valid(trait_uts_record.sysname, "Linux", 5U) ||
        !uts_field_valid(trait_uts_record.nodename, "trait", 5U) ||
        !uts_field_valid(trait_uts_record.release, "2.2.0-trait", 11U) ||
        !uts_field_valid(trait_uts_record.version, "Trait OS", 8U) ||
        !uts_field_valid(trait_uts_record.machine, "x86_64", 6U) ||
        !uts_field_valid(trait_uts_record.domainname, "(none)", 6U) ||
        sizeof(trait_uts_record) != 390U ||
...
bool linux_syscall_uname_copyout_self_test(size_t *completed_tests)
{
    ...
    *completed_tests = 0U;
    if (!runtime.active ||
        ...
        copy_to_user_stack(0U, &trait_uts_record,
            sizeof(trait_uts_record)) ||
        copy_to_user_stack(UINT64_MAX - 1U, &trait_uts_record,
            sizeof(trait_uts_record)) ||
        copy_to_user_stack(UINT64_C(0x0000800000000000),
            &trait_uts_record, sizeof(trait_uts_record)) ||
...
)
{
    ...
    case 2U:
        ...
        if (!copy_to_user_stack(frame->rdi, &trait_uts_record,
                sizeof(trait_uts_record))) {
