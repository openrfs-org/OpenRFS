 ...
 * owns; everything else in the low word is Trait OS's to write.
 */
 ...
 * electricals Trait OS cannot program, so it is refused rather than read as the
...
/* Compose the two halves of a redirection entry from what Trait OS intends. */
 ...
 * IRR from software except the mask and unmask dance. Trait OS refuses that
...
enum ioapic_status ioapic_initialize(const struct acpi_topology *topology)
{
    ...
    for (size_t index = 0; index < topology->io_apic_count; ++index) {
        ...
        /* Firmware may leave any entry unmasked; none of them are Trait OS's. */
...
)
{
    ...
    /*
     * Read the entry back and compare only the fields Trait OS wrote. Delivery
...
enum ioapic_status ioapic_send_eoi(uint8_t vector)
{
     ...
     * entry that is level triggered in Trait OS's records but edge triggered in
