...
{
    return type >= TRAIT_HANDLE_FILE &&
        type <= TRAIT_HANDLE_PACKAGE_CONTROL;
}

static trait_handle_t encode_handle(
    size_t index,
    uint8_t type,
    uint32_t generation
)
...
)
{
    ...
    if (!table->initialized || handle == TRAIT_HANDLE_INVALID ||
...
)
{
    ...
    *duplicate = TRAIT_HANDLE_INVALID;
...
)
{
    ...
    for (size_t index = 0U; index < table->limit; ++index) {
        if (table->slots[index].active) {
            const trait_handle_t handle = encode_handle(index,
...
)
{
    ...
    if (type != TRAIT_HANDLE_FILE || resource == NULL || closed == NULL ||
...
bool native_handle_self_test(size_t *completed_tests)
{
    ...
    *completed_tests = 0U;
    if (!valid_type(TRAIT_HANDLE_PACKAGE_CONTROL) ||
        valid_type((uint8_t)(TRAIT_HANDLE_PACKAGE_CONTROL + 1U))) {
        return false;
    }
    ++*completed_tests;
    if (native_handle_table_initialize(&table, 2U) != NATIVE_HANDLE_OK ||
        native_handle_install(&table, TRAIT_HANDLE_FILE, &initial, &first) !=
            NATIVE_HANDLE_OK ||
        native_handle_resolve(&table, first, TRAIT_HANDLE_FILE, &resolved) !=
            NATIVE_HANDLE_OK || resolved->words[0] != initial.words[0]) {
        return false;
    }
    ++*completed_tests;
    if (native_handle_resolve(&table, first, TRAIT_HANDLE_TIMER, &resolved) !=
    ...
    if (native_handle_close_all(&table, test_close, &closed) !=
        ...
        native_handle_resolve(&table, duplicate, TRAIT_HANDLE_FILE,
