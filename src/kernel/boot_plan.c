...
/*
 * The installed OpenGAT Boot Ledger plan.
 *
...
static bool test_uses_opengat_proof_userland(enum kernel_test_scenario scenario)
{
    return scenario == KERNEL_TEST_OPENGAT_PROOF_USERLAND ||
        scenario == KERNEL_TEST_OPENGAT_PROOF_USERLAND_ABSENT ||
...
)
{
    ...
    if (framebuffer->present) {
        ...
        if (framebuffer->size == 0U) {
            ...
            if (framebuffer_end > OPENGAT_EARLY_PHYSICAL_LIMIT) {
...
)
{
    ...
    if (status != INTERRUPT_STATUS_OK) {
        if (status == INTERRUPT_STATUS_CPU_TABLE_FAILURE) {
            console_write("OpenGAT: CPU table detail: ");
    ...
    console_write("OpenGAT: kernel online\n");
    console_write("OpenGAT: descriptor tables verified\n");
    console_write("OpenGAT: interrupt foundation online\n");
    boot_stage_result_succeed(descriptor, result);
}
...
)
{
    ...
    } else if (opengat_logo_self_test() != 1) {
        failure = "logo decoder self-test failed";
    } else if (opengat_font_self_test() != 1) {
    ...
    console_write("OpenGAT: parser rejection tests passed\n");
    boot_stage_result_succeed(descriptor, result);
}
...
)
{
    ...
    console_write("OpenGAT: ACPI root verified\n");
    console_write("OpenGAT: ACPI MADT verified\n");
    console_write("OpenGAT: ACPI topology verified\n");
    console_write("OpenGAT: ACPI FADT verified\n");
    console_write("OpenGAT: ACPI configuration windows verified\n");
    ...
    console_write("OpenGAT: local APIC online\n");
    ...
    console_write("OpenGAT: I/O APIC online\n");
    boot_stage_result_succeed(descriptor, result);
}
...
)
{
    ...
    console_write("OpenGAT: font verified\n");
    boot_stage_result_succeed(descriptor, result);
    result->proof_counters[0] = opengat_ui_font_size();
    result->proof_counters[1] = opengat_ui_font_fingerprint();
    result->proof_counter_count = 2U;
}
...
)
{
    ...
    if (status == POINTER_STATUS_OK) {
        console_write("OpenGAT: PS/2 pointer available\n");
    } else {
        console_write("OpenGAT: PS/2 pointer unavailable: ");
...
)
{
    ...
    console_write("OpenGAT: layout validated\n");
    boot_stage_result_succeed(descriptor, result);
...
{
    console_write("OpenGAT: day one passed\n");
    console_write("OpenGAT: memory foundation passed\n");
...
)
{
    ...
    console_write("OpenGAT: PCI resource ownership negative controls 4/4 passed\n");
    console_write("OpenGAT: supervisor NX UC device-MMIO arena established\n");
    boot_stage_result_succeed(descriptor, result);
...
)
{
    ...
    console_write("OpenGAT: dynamic vector negative controls 4/4 passed\n");
    console_write("OpenGAT: dynamic interrupt vector foundation established\n");
    boot_stage_result_succeed(descriptor, result);
...
)
{
    ...
    console_write("OpenGAT: bounded DMA negative controls 2/2 passed\n");
    console_write("OpenGAT: contiguous DMA ownership foundation established\n");
    boot_stage_result_succeed(descriptor, result);
}
...
)
{
    ...
    console_write("OpenGAT: network controls ");
    ...
    if (status == NETWORK_STATUS_OK) {
        console_write("OpenGAT: virtio-net0 initialized\n");
    } else if (status == NETWORK_STATUS_LINK_DOWN) {
        console_write("OpenGAT: virtio-net0 initialized without carrier\n");
    } else {
        console_write("OpenGAT: virtio-net0 absent\n");
    }
    boot_stage_result_succeed(descriptor, result);
...
)
{
    ...
    if (status == DEVICE_SUBSTRATE_STATUS_ABSENT) {
        console_write("OpenGAT: device-substrate fixture absent\n");
        boot_stage_result_skip(descriptor, result);
        return;
    }
    ...
    console_putc('\n');
    console_write("OpenGAT: device substrate teardown complete\n");
    boot_stage_result_succeed(descriptor, result);
...
)
{
    ...
    console_write("OpenGAT: xHCI foundation robustness controls ");
    console_write_u64(completed);
    console_putc('/');
    console_write_u64(XHCI_FOUNDATION_ROBUSTNESS_TESTS);
    console_write(" passed\n");
    console_write(
        "OpenGAT: bounded xHCI host-controller foundation established\n");
    boot_stage_result_succeed(descriptor, result);
}
...
)
{
    ...
    if (status == XHCI_STATUS_ABSENT) {
        console_write("OpenGAT: xHCI fixture absent\n");
        boot_stage_result_skip(descriptor, result);
        return;
    }
    ...
    console_write("OpenGAT: xHCI controller ready\n");
    console_write("OpenGAT: USB device descriptor DMA completed: ");
    console_write_u64(proof.descriptor_bytes);
    console_write(" bytes\n");
    console_write("OpenGAT: xHCI MSI-X descriptor completion count ");
    console_write_u64(proof.msix_completion_count);
    console_putc('\n');
    console_write(
        "OpenGAT: xHCI DMA ownership CPU-CONTROLLER-CPU complete\n");
    console_write("OpenGAT: xHCI teardown complete\n");
    boot_stage_result_succeed(descriptor, result);
...
)
{
    ...
    console_write("OpenGAT: NVMe foundation robustness controls ");
    console_write_u64(completed);
    console_putc('/');
    console_write_u64(NVME_FOUNDATION_ROBUSTNESS_TESTS);
    console_write(" passed\n");
    console_write(
        "OpenGAT: bounded NVMe block-controller foundation established\n");
    boot_stage_result_succeed(descriptor, result);
}
...
)
{
    ...
    if (context->test_scenario == KERNEL_TEST_NONE ||
        ...
        test_uses_opengat_proof_userland(context->test_scenario) ||
        test_uses_fat32_volumes(context->test_scenario)) {
        console_write("OpenGAT: NVMe fixture absent\n");
        boot_stage_result_skip(descriptor, result);
        return;
    }
    ...
    if (status != NVME_STATUS_OK) {
        console_write("OpenGAT: NVMe read proof violated invariant: ");
    ...
    console_write("OpenGAT: NVMe controller ready\n");
    console_write("OpenGAT: NVMe namespace ready\n");
    console_write("OpenGAT: NVMe block read completed: ");
    console_write_u64(proof.block_bytes);
    console_write(" bytes\n");
    console_write("OpenGAT: NVMe MSI-X read completion count ");
    console_write_u64(proof.msix_completion_count);
    console_putc('\n');
    console_write(
        "OpenGAT: NVMe DMA ownership CPU-CONTROLLER-CPU complete\n");
    console_write("OpenGAT: NVMe teardown complete\n");
    boot_stage_result_succeed(descriptor, result);
...
)
{
    ...
    console_write("OpenGAT: FAT16 foundation robustness controls ");
    console_write_u64(completed);
    console_putc('/');
    console_write_u64(FILESYSTEM_INTEGRATION_CONTROLS);
    console_write(" passed\n");
    console_write(
        "OpenGAT: bounded read-only FAT16 foundation established\n");
    boot_stage_result_succeed(descriptor, result);
}
...
)
{
    ...
    if (context->test_scenario == KERNEL_TEST_NONE ||
        ...
        test_uses_opengat_proof_userland(context->test_scenario) ||
        test_uses_fat32_volumes(context->test_scenario)) {
        console_write("OpenGAT: FAT16 fixture absent\n");
        boot_stage_result_skip(descriptor, result);
        return;
    }
    ...
    if (status != FILESYSTEM_STATUS_OK) {
        console_write("OpenGAT: FAT16 file proof violated invariant: ");
    ...
    console_write("OpenGAT: FAT16 volume ready\n");
    console_write("OpenGAT: FAT16 file OPENGAT.BIN read: ");
    console_write_u64(proof.file_bytes);
    console_write(" bytes\n");
    console_write("OpenGAT: FAT16 MSI-X completion count ");
    console_write_u64(proof.msix_completion_count);
    console_putc('\n');
    console_write(
        "OpenGAT: FAT16 DMA ownership CPU-CONTROLLER-CPU complete\n");
    console_write("OpenGAT: FAT16 teardown complete\n");
    console_write("ST FAT16 file OPENGAT.BIN bytes ");
    console_write_u64(proof.file_bytes);
    console_write(" reads ");
...
)
{
    ...
    console_write("OpenGAT: process address-space foundation controls ");
    console_write_u64(completed);
    console_putc('/');
...
)
{
    ...
    console_write("OpenGAT: ELF64 parser robustness controls ");
    console_write_u64(completed);
    console_putc('/');
...
)
{
    ...
    if (context->test_scenario != KERNEL_TEST_PROCESS) {
        console_write("OpenGAT: process fixture absent\n");
        boot_stage_result_skip(descriptor, result);
        return;
    }
    ...
    if (status != PROCESS_STATUS_OK) {
        console_write("OpenGAT: process proof violated invariant: ");
        console_write(process_status_string(status));
        console_putc('\n');
        stage_failed(context, result, process_status_string(status));
        return;
    }
    console_write("ST PROCESS ELF64 OPENGAT.BIN bytes ");
    console_write_u64(proof.file_bytes);
...
)
{
    ...
    console_write("OpenGAT: Linux SYSCALL CPU foundation controls ");
    console_write_u64(completed);
    console_putc('/');
...
)
{
    ...
    console_write("OpenGAT: BusyBox image and Linux stack controls ");
    console_write_u64(completed);
    console_putc('/');
...
)
{
    ...
    if (!linux_uname_image_uts_foundation_self_test(&completed) ||
        ...
        console_write("OpenGAT: BusyBox uname foundation stopped after ");
        console_write_u64(completed);
    ...
    if (!linux_cat_image_stdin_foundation_self_test(&cat_completed) ||
        ...
        console_write("OpenGAT: BusyBox cat foundation stopped after ");
    ...
    console_write("OpenGAT: BusyBox uname image and UTS controls ");
    console_write_u64(completed);
    console_putc('/');
    console_write_u64(LINUX_UNAME_ABI_IMAGE_UTS_FOUNDATION_CONTROLS);
    console_write(" passed\n");
    console_write("OpenGAT: BusyBox cat image and stdin controls ");
    console_write_u64(cat_completed);
    console_putc('/');
...
)
{
    ...
    if (context->test_scenario != KERNEL_TEST_LINUX_ABI) {
        console_write("OpenGAT: Linux ABI fixture absent\n");
        boot_stage_result_skip(descriptor, result);
        return;
    }
    ...
    if (status != LINUX_ABI_STATUS_OK) {
        console_write("OpenGAT: Linux ABI proof violated invariant: ");
...
)
{
    ...
    if (context->test_scenario != KERNEL_TEST_LINUX_ABI_UNAME) {
        console_write("OpenGAT: Linux uname ABI fixture absent\n");
        boot_stage_result_skip(descriptor, result);
        return;
    }
    ...
    if (status != LINUX_UNAME_ABI_STATUS_OK) {
        console_write("OpenGAT: Linux uname ABI proof violated invariant: ");
...
)
{
    ...
    console_write("OpenGAT: exception probes passed\n");
    console_write("OpenGAT: PIC spurious paths passed\n");
    console_write("OpenGAT: PIT delivered eight interrupts\n");
    console_write("OpenGAT: I/O APIC delivered eight interrupts\n");
    console_write("OpenGAT: legacy 8259 retired\n");
    console_write("OpenGAT: timer survives legacy retirement\n");
    console_write(
        "OpenGAT: I/O APIC delivered eight level-triggered interrupts\n"
    );
    console_write("OpenGAT: level-triggered routing established\n");
    console_write("OpenGAT: local APIC timer delivered eight interrupts\n");
    console_write("OpenGAT: TSC reference established\n");
    console_write("OpenGAT: PM timer independent reference established\n");
    console_write("OpenGAT: PIT retired\n");
    console_write("OpenGAT: clocks survive PIT retirement\n");
    console_write("OpenGAT: deadline timers online\n");
    console_write("OpenGAT: monotonic time established\n");
    console_write("OpenGAT: virtual memory established\n");
    console_write("OpenGAT: kernel heap established\n");
    console_write("OpenGAT: PCI enumeration established\n");
    console_write("OpenGAT: device foundations established\n");
    console_write("OpenGAT: kernel threads passed\n");
    console_write("OpenGAT: preemption passed\n");
    if (framebuffer_is_active()) {
        console_write("OpenGAT: framebuffer passed\n");
        console_write("OpenGAT: logo passed\n");
        console_write("OpenGAT: screen console passed\n");
        console_write("OpenGAT: shell passed\n");
    }
    console_write("OpenGAT: keyboard passed\n");
    console_write("OpenGAT: never triple fault milestone passed\n");
    boot_stage_result_succeed(descriptor, result);
}

static bool scenario_starts_desktop(enum kernel_test_scenario scenario)
{
    return scenario == KERNEL_TEST_OPENGAT_PROOF ||
        scenario == KERNEL_TEST_OPENGAT_PROOF_USERLAND_INTERACTIVE ||
        scenario == KERNEL_TEST_OPENGAT_PROOF_USERLAND_INTERACTIVE_ABSENT ||
...
enum ui_status boot_plan_start_desktop(void)
{
    ...
    /*
     * The interactive desktop is deferred on an ordinary boot.  It may only
     * consume the framebuffer, input, font and layout foundations after the
     * installed ledger has published receipts for all of them.
     */
    ...
    if (status != UI_STATUS_OK && status != UI_STATUS_ALREADY_INITIALIZED) {
        return status;
    }
    return ui_activate();
}
...
)
{
    ...
    if (!scenario_starts_desktop(context->test_scenario)) {
        console_write("OpenGAT: command line ready; desktop waits for starty\n");
        boot_stage_result_skip(descriptor, result);
        return;
    }
    status = ui_construct(pointer_is_present());
    if (status != UI_STATUS_OK) {
        console_write("OpenGAT: desktop construction failed: ");
    ...
    console_write("OpenGAT: desktop constructed\n");
    boot_stage_result_succeed(descriptor, result);
}
...
)
{
    ...
    if (status != UI_STATUS_OK) {
        console_write("OpenGAT: desktop activation failed: ");
        console_write(ui_status_string(status));
        console_putc('\n');
        stage_failed(context, result, ui_status_string(status));
        return;
    ...
    console_write("OpenGAT: desktop activated\n");
    boot_stage_result_succeed(descriptor, result);
}

static void execute_opengat_installed_proof(
    struct boot_context *context,
    const struct boot_stage_descriptor *descriptor,
    struct boot_stage_result *result
)
{
    ...
    console_write("OpenGAT: installed proof passed\n");
    boot_stage_result_succeed(descriptor, result);
...
)
{
    ...
    console_write("OpenGAT: multiprocess foundation controls ");
    console_write_u64(completed);
    console_putc('/');
...
)
{
    ...
    if (status != MULTIPROCESS_STATUS_OK) {
        console_write("OpenGAT: multiprocess proof violated invariant: ");
...
)
{
    ...
    console_write("OpenGAT: PCI driver matrix controls ");
    console_write_u64(completed);
    console_putc('/');
...
)
{
    ...
    if (context->test_scenario != KERNEL_TEST_DRIVER_MATRIX &&
        ...
        console_write("OpenGAT: PCI driver matrix devices absent\n");
        boot_stage_result_skip(descriptor, result);
        return;
    }
    ...
    if (status != DRIVER_STATUS_OK) {
        console_write("OpenGAT: PCI driver matrix violated invariant: ");
...
)
{
    ...
    console_write("OpenGAT: HD Audio foundation controls ");
    console_write_u64(completed);
    console_putc('/');
...
)
{
    ...
    if (context->test_scenario != KERNEL_TEST_AUDIO) {
        console_write("OpenGAT: HD Audio controller absent\n");
        boot_stage_result_skip(descriptor, result);
        return;
    }
    ...
    if (status != AUDIO_STATUS_OK) {
        console_write("OpenGAT: HD Audio proof violated invariant: ");
...
)
{
    ...
    console_write("OpenGAT: NVIDIA driver foundation controls ");
    console_write_u64(completed);
    console_putc('/');
...
)
{
    ...
    if (context->test_scenario != KERNEL_TEST_NVIDIA) {
        console_write("OpenGAT: NVIDIA functions absent\n");
        boot_stage_result_skip(descriptor, result);
        return;
    }
    ...
    if (status != NVIDIA_STATUS_OK) {
        console_write("OpenGAT: NVIDIA probe violated invariant: ");
...
static const struct boot_stage_descriptor installed_descriptors[] = {
    ...
    OPTIONAL_STAGE(BOOT_STAGE_UI_FONT, "OpenGAT UI font",
    ...
    OPTIONAL_STAGE(BOOT_STAGE_UI_LAYOUT, "OpenGAT layout",
    ...
    OPTIONAL_STAGE(BOOT_STAGE_OPENGAT_INSTALLED_PROOF,
        "OpenGAT installed proof", BOOT_PHASE_PROOFS,
        BOOT_IRREVERSIBLE_NONE, execute_opengat_installed_proof)
};
...
)
{
    ...
    case BOOT_STAGE_DESKTOP_ACTIVATION:
        ...
        descriptor->provided_capability_count = 1U;
        break;
    case BOOT_STAGE_OPENGAT_INSTALLED_PROOF:
        descriptor->required_capabilities[0] =
            BOOT_CAPABILITY_DESKTOP_SHELL_ACTIVATED;
        descriptor->required_capabilities[1] =
            BOOT_CAPABILITY_FRAMEBUFFER_WC_INDEPENDENTLY_PROVED;
