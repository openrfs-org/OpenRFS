...
)
{
    ...
    return trait_fat16_parse_bpb(block, length, namespace_blocks,
...
)
{
    ...
    return trait_fat16_find_root(block, FAT16_BLOCK_BYTES, geometry,
...
)
{
    ...
    return trait_fat16_parse_fat(block, FAT16_BLOCK_BYTES, geometry,
        &output) == expected;
}
...
static bool control_rust_parser(void)
{
    ...
    if (trait_fat16_parse_bpb(block, sizeof(block), FAT16_TOTAL_SECTORS,
    ...
    if (trait_fat16_make_query(query_bytes, sizeof(query_bytes), &query) !=
            FAT16_STATUS_OK) {
        return false;
    }
    query_bytes[0] = 'X';
    ...
    if (trait_fat16_find_root(block, sizeof(block), &geometry, &query,
            FAT16_FILE_BYTES, &entry) != FAT16_STATUS_OK) {
        return false;
    }
    fat16_test_make_fat(block);
    if (trait_fat16_parse_fat(block, sizeof(block), &geometry, &fat) !=
            FAT16_STATUS_OK) {
        return false;
    }
    fat16_test_put_u16(block, FAT16_FILE_CLUSTER * 2U,
    ...
    fat16_test_make_fat(block);
    if (trait_fat16_validate_extent(&geometry, &entry, &fat, &extent) !=
    ...
    if (trait_fat16_make_query(fat16_test_name, sizeof(fat16_test_name),
    ...
    enum fat16_status status = trait_fat16_validate_extent(
    ...
    if (trait_fat16_parse_bpb(block, 1U, FAT16_TOTAL_SECTORS,
    ...
    if (trait_fat16_validate_payload(payload_bytes, sizeof(payload_bytes),
        ...
        trait_fat16_validate_payload(payload_bytes,
            sizeof(payload_bytes) - 1U, &payload) !=
            FAT16_STATUS_PAYLOAD_LENGTH) {
        return false;
    }
    payload_bytes[64] ^= 1U;
    return trait_fat16_validate_payload(payload_bytes,
...
)
{
    ...
    if (trait_fat16_make_query(canonical_name, sizeof(canonical_name),
            &query) != FAT16_STATUS_OK) {
    ...
    if (result != FILESYSTEM_STATUS_OK ||
        trait_fat16_parse_bpb(block, block_length,
    ...
    if (result != FILESYSTEM_STATUS_OK ||
        trait_fat16_parse_fat(block, block_length, &volume.geometry,
            &fat) != FAT16_STATUS_OK) {
        result = result == FILESYSTEM_STATUS_OK ?
            FILESYSTEM_STATUS_PARSER_FAILURE : result;
        goto cleanup;
    ...
    if (result != FILESYSTEM_STATUS_OK ||
        trait_fat16_find_root(block, block_length, &volume.geometry,
            &query, sizeof(content.bytes), &entry) != FAT16_STATUS_OK ||
        trait_fat16_validate_extent(&volume.geometry, &entry, &fat,
    ...
    if (result != FILESYSTEM_STATUS_OK || block_length < entry.file_size ||
        trait_fat16_validate_payload(block, entry.file_size,
...
)
{
    ...
    result = read_block(&private_read_runtime.session,
        ...
        &block, &block_length);
    if (result != FILESYSTEM_STATUS_OK ||
        trait_fat16_parse_fat(block, block_length, &geometry, &fat) !=
            FAT16_STATUS_OK) {
        result = result == FILESYSTEM_STATUS_OK ?
            FILESYSTEM_STATUS_PARSER_FAILURE : result;
        goto fail;
    ...
    result = read_block(&private_read_runtime.session,
        ...
        &block, &block_length);
    if (result != FILESYSTEM_STATUS_OK ||
        trait_fat16_find_root(block, block_length, &geometry, &query,
            (uint32_t)destination_bytes, &entry) != FAT16_STATUS_OK ||
        trait_fat16_validate_extent(&geometry, &entry, &fat, &extent) !=
            FAT16_STATUS_OK ||
        !equal_bytes(entry.canonical_name, canonical_name,
            sizeof(canonical_name))) {
        result = result == FILESYSTEM_STATUS_OK ?
...
)
{
    switch (profile) {
    case LINUX_READ_PROFILE_UNAME:
        return trait_linux_uname_fat16_make_query(query);
    case LINUX_READ_PROFILE_CAT:
        return trait_linux_cat_fat16_make_query(query);
    case LINUX_READ_PROFILE_ECHO:
    default:
        return trait_linux_fat16_make_query(query);
    }
}
...
)
{
    switch (profile) {
    case LINUX_READ_PROFILE_UNAME:
        return trait_linux_uname_fat16_find_root(block, block_length,
            geometry, query, destination_bytes, entry);
    case LINUX_READ_PROFILE_CAT:
        return trait_linux_cat_fat16_find_root(block, block_length,
            geometry, query, destination_bytes, entry);
    case LINUX_READ_PROFILE_ECHO:
    default:
        return trait_linux_fat16_find_root(block, block_length, geometry,
...
)
{
    switch (profile) {
    case LINUX_READ_PROFILE_UNAME:
        return trait_linux_uname_fat16_build_chain(fat, fat_length,
            geometry, entry, chain);
    case LINUX_READ_PROFILE_CAT:
        return trait_linux_cat_fat16_build_chain(fat, fat_length,
            geometry, entry, chain);
    case LINUX_READ_PROFILE_ECHO:
    default:
        return trait_linux_fat16_build_chain(fat, fat_length, geometry,
            entry, chain);
    }
}
...
)
{
    switch (profile) {
    case LINUX_READ_PROFILE_UNAME:
        return trait_linux_uname_fat16_validate_payload(destination,
            destination_bytes, payload);
    case LINUX_READ_PROFILE_CAT:
        return trait_linux_cat_fat16_validate_payload(destination,
            destination_bytes, payload);
    case LINUX_READ_PROFILE_ECHO:
    default:
        return trait_linux_fat16_validate_payload(destination,
            destination_bytes, payload);
    }
}
...
)
{
    ...
    if (failure_boundary == 0U &&
        ...
        uint64_t before = traitfs_completion_count(TRAITFS_VOLUME_SYSTEM);
        uint64_t after;
        enum traitfs_status open_status = traitfs_open(TRAITFS_VOLUME_SYSTEM,
            fat32_path, TRAITFS_ACCESS_READ, &handle);

        if (open_status != TRAITFS_STATUS_OK) {
            return open_status == TRAITFS_STATUS_NOT_FOUND ?
                FILESYSTEM_STATUS_ABSENT : FILESYSTEM_STATUS_NVME_FAILURE;
        }
        open_status = traitfs_read(handle, destination, destination_bytes,
            &read_bytes);
        if (traitfs_close(handle) != TRAITFS_STATUS_OK &&
            open_status == TRAITFS_STATUS_OK) {
            open_status = TRAITFS_STATUS_STALE_HANDLE;
        }
        if (open_status != TRAITFS_STATUS_OK || read_bytes != destination_bytes ||
        ...
        after = traitfs_completion_count(TRAITFS_VOLUME_SYSTEM);
    ...
    if (trait_fat16_parse_bpb(block, block_length,
            private_read_runtime.session.namespace_blocks,
            private_read_runtime.session.logical_block_bytes,
            &geometry) != FAT16_STATUS_OK) {
