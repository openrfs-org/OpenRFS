...
#define EXT4_MAX_HANDLES OPENGATFS_MAX_HANDLES
...
struct ext4_mount_state {
    ...
    struct opengat_ext4_identity identity;
...
static struct ext4_mount_state ext4_mounts[OPENGATFS_VOLUME_COUNT];
static struct ext4_handle_state ext4_handles[EXT4_MAX_HANDLES];
static enum opengatfs_status ext4_last_mount_status[OPENGATFS_VOLUME_COUNT];
static struct opengat_ext4_mount_diagnostic
    ext4_mount_diagnostics[OPENGATFS_VOLUME_COUNT];
...
static enum opengat_ext4_test_storage_kind ext4_test_storage_failure_kind =
    OPENGAT_EXT4_TEST_STORAGE_KIND_COUNT;

extern int32_t opengat_ext4_mount(uintptr_t context, uint64_t media_bytes,
    struct opengat_ext4_identity *identity, uintptr_t *mounted_out);
extern int32_t opengat_ext4_prepare_unmount(uintptr_t mounted);
extern int32_t opengat_ext4_sync(uintptr_t mounted);
extern int32_t opengat_ext4_free_bytes(uintptr_t mounted, uint64_t *free_bytes);
extern int32_t opengat_ext4_unmount(uintptr_t mounted);
extern int32_t opengat_ext4_stat(uintptr_t mounted, const uint8_t *path,
    size_t path_length, struct opengat_ext4_metadata *metadata);
extern int32_t opengat_ext4_pread(uintptr_t mounted, const uint8_t *path,
    size_t path_length, uint64_t offset, uint8_t *destination,
    size_t capacity, size_t *read_out);
extern int32_t opengat_ext4_transaction_probe(uintptr_t mounted,
    const uint8_t *path, size_t path_length, uint64_t offset,
    const uint8_t *source, size_t source_length, size_t *written_out);
extern int32_t opengat_ext4_truncate_probe(uintptr_t mounted,
    const uint8_t *path, size_t path_length, uint64_t size);
extern int32_t opengat_ext4_create_file_probe(uintptr_t mounted,
    const uint8_t *path, size_t path_length, uint16_t mode);
extern int32_t opengat_ext4_unlink_file_probe(uintptr_t mounted,
    const uint8_t *path, size_t path_length);
extern int32_t opengat_ext4_link_file_probe(uintptr_t mounted,
    const uint8_t *source, size_t source_length, const uint8_t *destination,
    size_t destination_length);
extern int32_t opengat_ext4_create_directory_probe(uintptr_t mounted,
    const uint8_t *path, size_t path_length);
extern int32_t opengat_ext4_remove_directory_probe(uintptr_t mounted,
    const uint8_t *path, size_t path_length);
extern int32_t opengat_ext4_rename_probe(uintptr_t mounted,
    const uint8_t *source, size_t source_length, const uint8_t *destination,
    size_t destination_length);
extern int32_t opengat_ext4_directory_entry(uintptr_t mounted,
    const uint8_t *path, size_t path_length, uint64_t index,
    struct opengat_ext4_directory_entry *entry, bool *present);

_Static_assert(sizeof(struct opengat_ext4_metadata) == 40U,
    "ext4 metadata C/Rust ABI drift");
_Static_assert(offsetof(struct opengat_ext4_metadata, file_type) == 28U,
    "ext4 metadata C/Rust ABI offset drift");
_Static_assert(sizeof(struct opengat_ext4_directory_entry) == 304U,
    "ext4 directory C/Rust ABI drift");
_Static_assert(sizeof(struct opengat_ext4_identity) == 48U,
    "ext4 identity C/Rust ABI drift");
_Static_assert(offsetof(struct opengat_ext4_identity, recovered_transactions) ==
        32U,
    "ext4 identity C/Rust ABI offset drift");
_Static_assert(offsetof(struct opengat_ext4_identity, recovery_performed) == 44U,
...
static size_t path_length(const char *path)
{
    ...
    while (length < OPENGATFS_MAX_PATH && path[length] != '\0') {
        ++length;
    }
    return length;
}
...
{
    static const char prefix[] = "opengat.ext4-cut=";
...
bool ext4_backend_test_fail_storage_once(uint32_t operation_ordinal)
{
    ...
    ext4_test_storage_failure_kind = OPENGAT_EXT4_TEST_STORAGE_KIND_COUNT;
    return true;
}
...
static bool fail_test_storage_operation(
    enum opengat_ext4_test_storage_kind kind)
{
...
{
    switch (boundary) {
    case OPENGAT_EXT4_FLUSH_FILESYSTEM_STATE: return "filesystem-state";
    case OPENGAT_EXT4_FLUSH_ORDERED_DATA: return "ordered-data";
    case OPENGAT_EXT4_FLUSH_JOURNAL_PAYLOAD: return "journal-payload";
    case OPENGAT_EXT4_FLUSH_COMMIT: return "commit";
    case OPENGAT_EXT4_FLUSH_CHECKPOINT: return "checkpoint";
    case OPENGAT_EXT4_FLUSH_JOURNAL_STATE: return "journal-state";
    default: return NULL;
    }
}
...
static enum opengatfs_status map_status(int32_t status)
{
    switch (status) {
    case OPENGAT_EXT4_STATUS_OK:
        return OPENGATFS_STATUS_OK;
    case OPENGAT_EXT4_STATUS_NULL_ARGUMENT:
        return OPENGATFS_STATUS_INVALID_ARGUMENT;
    case OPENGAT_EXT4_STATUS_VOLUME:
        return OPENGATFS_STATUS_NOT_MOUNTED;
    case OPENGAT_EXT4_STATUS_IO:
        return OPENGATFS_STATUS_IO;
    case OPENGAT_EXT4_STATUS_INVALID:
        return OPENGATFS_STATUS_CORRUPT;
    case OPENGAT_EXT4_STATUS_NOT_FOUND:
        return OPENGATFS_STATUS_NOT_FOUND;
    case OPENGAT_EXT4_STATUS_NOT_DIRECTORY:
        return OPENGATFS_STATUS_NOT_DIRECTORY;
    case OPENGAT_EXT4_STATUS_IS_DIRECTORY:
        return OPENGATFS_STATUS_IS_DIRECTORY;
    case OPENGAT_EXT4_STATUS_RANGE:
        return OPENGATFS_STATUS_RANGE;
    case OPENGAT_EXT4_STATUS_SPECIAL:
        return OPENGATFS_STATUS_ACCESS;
    case OPENGAT_EXT4_STATUS_EXISTS:
        return OPENGATFS_STATUS_EXISTS;
    case OPENGAT_EXT4_STATUS_NOT_EMPTY:
...
)
{
    ...
    if (fail_test_storage_operation(OPENGAT_EXT4_TEST_STORAGE_WRITE)) {
        return -1;
    }
    while (remaining != 0U) {
        const uint64_t lba = position / session->logical_block_bytes;
...
int32_t opengat_ext4_block_flush(uintptr_t context, uint32_t boundary)
{
    ...
    if (fail_test_storage_operation(OPENGAT_EXT4_TEST_STORAGE_FLUSH)) {
        return -1;
    }
...
static enum opengatfs_status checked_stat(
    struct ext4_mount_state *mount,
    const char *path,
...
)
{
    ...
    if (metadata == NULL || length == 0U || length >= OPENGATFS_MAX_PATH) {
        return OPENGATFS_STATUS_INVALID_ARGUMENT;
    }
    ...
    status = map_status(opengat_ext4_stat(mount->rust_mount,
...
static void fill_stat(
    const struct opengat_ext4_metadata *source,
    struct opengatfs_stat *destination
)
{
    ...
    destination->directory = source->file_type == OPENGAT_EXT4_FILE_DIRECTORY;
...
static enum opengatfs_status allocate_handle(enum opengatfs_volume volume,
    const char *path, uint64_t inode, uint64_t size,
    enum opengatfs_access access, bool directory, opengatfs_handle *handle)
{
    const size_t length = path_length(path);
    size_t slot = EXT4_MAX_HANDLES;

    if (handle == NULL || length == 0U || length >= OPENGATFS_MAX_PATH) {
        return OPENGATFS_STATUS_INVALID_ARGUMENT;
    }
...
static void update_open_sizes(enum opengatfs_volume volume, uint64_t inode,
    uint64_t size)
{
    for (size_t index = 0U; index < EXT4_MAX_HANDLES; ++index) {
        if (ext4_handles[index].active &&
...
static bool inode_is_open(enum opengatfs_volume volume, uint64_t inode)
{
    for (size_t index = 0U; index < EXT4_MAX_HANDLES; ++index) {
        if (ext4_handles[index].active &&
            ext4_handles[index].volume == volume &&
...
void ext4_backend_initialize(void)
{
    ...
    for (enum opengatfs_volume volume = OPENGATFS_VOLUME_SYSTEM;
        ...
        ext4_last_mount_status[volume] = OPENGATFS_STATUS_NOT_MOUNTED;
        ext4_mount_diagnostics[volume].begin_status = OPENGATFS_STATUS_NOT_MOUNTED;
        ext4_mount_diagnostics[volume].rust_status = OPENGAT_EXT4_STATUS_COUNT;
        ext4_mount_diagnostics[volume].close_status = OPENGATFS_STATUS_NOT_MOUNTED;
...
enum opengatfs_status ext4_backend_mount(enum opengatfs_volume volume)
{
    ...
    if (mount->active) {
        ext4_last_mount_status[volume] = OPENGATFS_STATUS_ALREADY_MOUNTED;
        return OPENGATFS_STATUS_ALREADY_MOUNTED;
    }
    zero_bytes(mount, sizeof(*mount));
    mount->controller_index = volume == OPENGATFS_VOLUME_SYSTEM ?
    ...
    rust_status = opengat_ext4_mount((uintptr_t)mount, mount->media_bytes,
    ...
    if (status != OPENGATFS_STATUS_OK || close_status != OPENGATFS_STATUS_OK) {
        const enum opengatfs_status result = status != OPENGATFS_STATUS_OK ?
            status : close_status;

        if (mount->rust_mount != 0U) {
            (void)opengat_ext4_unmount(mount->rust_mount);
        }
        zero_bytes(mount, sizeof(*mount));
    ...
    mount->active = true;
    ext4_last_mount_status[volume] = OPENGATFS_STATUS_OK;
    return OPENGATFS_STATUS_OK;
}
...
enum opengatfs_status ext4_backend_unmount(enum opengatfs_volume volume)
{
    ...
    rust_status = opengat_ext4_prepare_unmount(mount->rust_mount);
    close_status = end_operation(mount, NULL);
    status = map_status(rust_status);
    if (status != OPENGATFS_STATUS_OK || close_status != OPENGATFS_STATUS_OK) {
        return status != OPENGATFS_STATUS_OK ? status : close_status;
    }
    if (opengat_ext4_unmount(mount->rust_mount) != OPENGAT_EXT4_STATUS_OK) {
        return OPENGATFS_STATUS_CORRUPT;
    }
    zero_bytes(mount, sizeof(*mount));
    return OPENGATFS_STATUS_OK;
...
enum opengatfs_status ext4_backend_sync(enum opengatfs_volume volume)
{
    ...
    status = map_status(opengat_ext4_sync(mount->rust_mount));
    close_status = end_operation(mount, NULL);
    return status != OPENGATFS_STATUS_OK ? status : close_status;
}
...
{
    struct opengatfs_drive_info drive = {0};
    struct ext4_mount_state *mount;
    ...
    if (mount->active &&
        opengat_ext4_free_bytes(mount->rust_mount, &free_bytes) ==
            OPENGAT_EXT4_STATUS_OK) {
    ...
    const char *path, enum opengatfs_access access, opengatfs_handle *handle)
{
    ...
    *handle = 0U;
    ...
    if (metadata.file_type == OPENGAT_EXT4_FILE_DIRECTORY) {
        return OPENGATFS_STATUS_IS_DIRECTORY;
    }
    ...
    size_t *read_bytes)
{
    ...
    *read_bytes = 0U;
    ...
    status = map_status(opengat_ext4_pread(mount->rust_mount,
    ...
    uint8_t *destination, size_t capacity, size_t *read_bytes)
{
    ...
    status = ext4_backend_pread(handle, destination, capacity, state->offset,
        read_bytes);
    if (status == OPENGATFS_STATUS_OK && read_bytes != NULL) {
    ...
    size_t source_bytes, size_t *written_bytes)
{
    ...
    if (!valid_volume(volume) || length == 0U ||
        length >= OPENGATFS_MAX_PATH || source == NULL || source_bytes == 0U ||
    ...
    *written_bytes = 0U;
    ...
    status = map_status(opengat_ext4_transaction_probe(mount->rust_mount,
    ...
    const char *path, uint64_t size)
{
    ...
    if (!valid_volume(volume) || length == 0U || length >= OPENGATFS_MAX_PATH) {
        return OPENGATFS_STATUS_INVALID_ARGUMENT;
    }
    mount = &ext4_mounts[volume];
    status = begin_operation(mount, true);
    ...
    status = map_status(opengat_ext4_truncate_probe(mount->rust_mount,
    ...
    const char *path, uint16_t mode)
{
    ...
    status = map_status(opengat_ext4_create_file_probe(mount->rust_mount,
    ...
    const char *path)
{
    ...
    status = map_status(opengat_ext4_unlink_file_probe(mount->rust_mount,
    ...
    const char *source, const char *destination)
{
    ...
    if (!valid_volume(volume) || source_length == 0U ||
        source_length >= OPENGATFS_MAX_PATH || destination_length == 0U ||
        destination_length >= OPENGATFS_MAX_PATH) {
        return OPENGATFS_STATUS_INVALID_ARGUMENT;
    }
    mount = &ext4_mounts[volume];
    status = begin_operation(mount, true);
    ...
    status = map_status(opengat_ext4_link_file_probe(mount->rust_mount,
    ...
    enum opengatfs_volume volume, const char *path)
{
    ...
    status = map_status(opengat_ext4_create_directory_probe(mount->rust_mount,
        (const uint8_t *)path, length));
    close_status = end_operation(mount, NULL);
    return status != OPENGATFS_STATUS_OK ? status : close_status;
}
    ...
    enum opengatfs_volume volume, const char *path)
{
    ...
    status = map_status(opengat_ext4_remove_directory_probe(mount->rust_mount,
        (const uint8_t *)path, length));
    close_status = end_operation(mount, NULL);
    return status != OPENGATFS_STATUS_OK ? status : close_status;
}
    ...
    const char *source, const char *destination)
{
    ...
    status = map_status(opengat_ext4_rename_probe(mount->rust_mount,
        (const uint8_t *)source, source_length,
        (const uint8_t *)destination, destination_length));
    close_status = end_operation(mount, NULL);
    return status != OPENGATFS_STATUS_OK ? status : close_status;
    ...
    const uint8_t *source, size_t source_bytes, size_t *written_bytes)
{
    ...
    *written_bytes = 0U;
    ...
    if (source_bytes > EXT4_TRANSACTION_PROBE_MAX_BYTES ||
        state->offset > OPENGATFS_MAX_FILE_BYTES ||
        source_bytes > OPENGATFS_MAX_FILE_BYTES - state->offset) {
        return OPENGATFS_STATUS_RANGE;
    }
    mount = &ext4_mounts[state->volume];
    status = begin_operation(mount, true);
    ...
    enum opengatfs_seek_origin origin, uint64_t *position)
{
    ...
    *position = 0U;
    ...
    base = origin == OPENGATFS_SEEK_START ? 0U :
        (origin == OPENGATFS_SEEK_CURRENT ? state->offset :
            (origin == OPENGATFS_SEEK_END ? state->size : UINT64_MAX));
    if (base == UINT64_MAX) {
        return OPENGATFS_STATUS_INVALID_ARGUMENT;
    }
    if (offset < 0) {
...
static void fill_entry(const struct opengat_ext4_directory_entry *source,
    struct opengatfs_list_entry *destination)
{
    zero_bytes(destination, sizeof(*destination));
    ...
    destination->directory =
        source->metadata.file_type == OPENGAT_EXT4_FILE_DIRECTORY;
}

static enum opengatfs_status indexed_entry(struct ext4_handle_state *state,
    uint64_t index, struct opengatfs_list_entry *entry, bool *present)
{
    struct opengat_ext4_directory_entry raw;
    struct ext4_mount_state *mount = &ext4_mounts[state->volume];
    enum opengatfs_status status = begin_operation(mount, false);
    enum opengatfs_status close_status;
    ...
    status = map_status(opengat_ext4_directory_entry(mount->rust_mount,
        (const uint8_t *)state->path, path_length(state->path), index, &raw,
        present));
    close_status = end_operation(mount, NULL);
    if (status == OPENGATFS_STATUS_OK && *present) {
        if (raw.name_length == 0U || raw.name_length >=
                OPENGATFS_MAX_COMPONENT_BYTES) {
            status = OPENGATFS_STATUS_NAME;
        } else {
    ...
    const char *path, opengatfs_handle *handle)
{
    ...
    *handle = 0U;
    ...
    return allocate_handle(volume, path, metadata.inode, metadata.size,
        OPENGATFS_ACCESS_READ, true, handle);
}
    ...
    size_t *entry_count)
{
    ...
    *entry_count = 0U;
    ...
    while (status == OPENGATFS_STATUS_OK && count < capacity) {
        bool present = false;
    ...
    if (status == OPENGATFS_STATUS_OK && count == capacity) {
        struct opengatfs_list_entry ignored;
        bool present = false;
    ...
    if (handle != 0U) {
        enum opengatfs_status close_status = ext4_backend_directory_close(handle);
