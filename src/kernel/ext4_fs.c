...
#define EXT4_MAX_HANDLES TRAITFS_MAX_HANDLES
...
struct ext4_mount_state {
    ...
    struct trait_ext4_identity identity;
...
static struct ext4_mount_state ext4_mounts[TRAITFS_VOLUME_COUNT];
static struct ext4_handle_state ext4_handles[EXT4_MAX_HANDLES];
static enum traitfs_status ext4_last_mount_status[TRAITFS_VOLUME_COUNT];
static struct trait_ext4_mount_diagnostic
    ext4_mount_diagnostics[TRAITFS_VOLUME_COUNT];
...
static enum trait_ext4_test_storage_kind ext4_test_storage_failure_kind =
    TRAIT_EXT4_TEST_STORAGE_KIND_COUNT;

extern int32_t trait_ext4_mount(uintptr_t context, uint64_t media_bytes,
    struct trait_ext4_identity *identity, uintptr_t *mounted_out);
extern int32_t trait_ext4_prepare_unmount(uintptr_t mounted);
extern int32_t trait_ext4_sync(uintptr_t mounted);
extern int32_t trait_ext4_free_bytes(uintptr_t mounted, uint64_t *free_bytes);
extern int32_t trait_ext4_unmount(uintptr_t mounted);
extern int32_t trait_ext4_stat(uintptr_t mounted, const uint8_t *path,
    size_t path_length, struct trait_ext4_metadata *metadata);
extern int32_t trait_ext4_pread(uintptr_t mounted, const uint8_t *path,
    size_t path_length, uint64_t offset, uint8_t *destination,
    size_t capacity, size_t *read_out);
extern int32_t trait_ext4_transaction_probe(uintptr_t mounted,
    const uint8_t *path, size_t path_length, uint64_t offset,
    const uint8_t *source, size_t source_length, size_t *written_out);
extern int32_t trait_ext4_truncate_probe(uintptr_t mounted,
    const uint8_t *path, size_t path_length, uint64_t size);
extern int32_t trait_ext4_create_file_probe(uintptr_t mounted,
    const uint8_t *path, size_t path_length, uint16_t mode);
extern int32_t trait_ext4_unlink_file_probe(uintptr_t mounted,
    const uint8_t *path, size_t path_length);
extern int32_t trait_ext4_link_file_probe(uintptr_t mounted,
    const uint8_t *source, size_t source_length, const uint8_t *destination,
    size_t destination_length);
extern int32_t trait_ext4_create_directory_probe(uintptr_t mounted,
    const uint8_t *path, size_t path_length);
extern int32_t trait_ext4_remove_directory_probe(uintptr_t mounted,
    const uint8_t *path, size_t path_length);
extern int32_t trait_ext4_rename_probe(uintptr_t mounted,
    const uint8_t *source, size_t source_length, const uint8_t *destination,
    size_t destination_length);
extern int32_t trait_ext4_directory_entry(uintptr_t mounted,
    const uint8_t *path, size_t path_length, uint64_t index,
    struct trait_ext4_directory_entry *entry, bool *present);

_Static_assert(sizeof(struct trait_ext4_metadata) == 40U,
    "ext4 metadata C/Rust ABI drift");
_Static_assert(offsetof(struct trait_ext4_metadata, file_type) == 28U,
    "ext4 metadata C/Rust ABI offset drift");
_Static_assert(sizeof(struct trait_ext4_directory_entry) == 304U,
    "ext4 directory C/Rust ABI drift");
_Static_assert(sizeof(struct trait_ext4_identity) == 48U,
    "ext4 identity C/Rust ABI drift");
_Static_assert(offsetof(struct trait_ext4_identity, recovered_transactions) ==
        32U,
    "ext4 identity C/Rust ABI offset drift");
_Static_assert(offsetof(struct trait_ext4_identity, recovery_performed) == 44U,
...
static size_t path_length(const char *path)
{
    ...
    while (length < TRAITFS_MAX_PATH && path[length] != '\0') {
        ++length;
    }
    return length;
}
...
{
    static const char prefix[] = "trait.ext4-cut=";
...
bool ext4_backend_test_fail_storage_once(uint32_t operation_ordinal)
{
    ...
    ext4_test_storage_failure_kind = TRAIT_EXT4_TEST_STORAGE_KIND_COUNT;
    return true;
}
...
static bool fail_test_storage_operation(
    enum trait_ext4_test_storage_kind kind)
{
...
{
    switch (boundary) {
    case TRAIT_EXT4_FLUSH_FILESYSTEM_STATE: return "filesystem-state";
    case TRAIT_EXT4_FLUSH_ORDERED_DATA: return "ordered-data";
    case TRAIT_EXT4_FLUSH_JOURNAL_PAYLOAD: return "journal-payload";
    case TRAIT_EXT4_FLUSH_COMMIT: return "commit";
    case TRAIT_EXT4_FLUSH_CHECKPOINT: return "checkpoint";
    case TRAIT_EXT4_FLUSH_JOURNAL_STATE: return "journal-state";
    default: return NULL;
    }
}
...
static enum traitfs_status map_status(int32_t status)
{
    switch (status) {
    case TRAIT_EXT4_STATUS_OK:
        return TRAITFS_STATUS_OK;
    case TRAIT_EXT4_STATUS_NULL_ARGUMENT:
        return TRAITFS_STATUS_INVALID_ARGUMENT;
    case TRAIT_EXT4_STATUS_VOLUME:
        return TRAITFS_STATUS_NOT_MOUNTED;
    case TRAIT_EXT4_STATUS_IO:
        return TRAITFS_STATUS_IO;
    case TRAIT_EXT4_STATUS_INVALID:
        return TRAITFS_STATUS_CORRUPT;
    case TRAIT_EXT4_STATUS_NOT_FOUND:
        return TRAITFS_STATUS_NOT_FOUND;
    case TRAIT_EXT4_STATUS_NOT_DIRECTORY:
        return TRAITFS_STATUS_NOT_DIRECTORY;
    case TRAIT_EXT4_STATUS_IS_DIRECTORY:
        return TRAITFS_STATUS_IS_DIRECTORY;
    case TRAIT_EXT4_STATUS_RANGE:
        return TRAITFS_STATUS_RANGE;
    case TRAIT_EXT4_STATUS_SPECIAL:
        return TRAITFS_STATUS_ACCESS;
    case TRAIT_EXT4_STATUS_EXISTS:
        return TRAITFS_STATUS_EXISTS;
    case TRAIT_EXT4_STATUS_NOT_EMPTY:
...
)
{
    ...
    if (fail_test_storage_operation(TRAIT_EXT4_TEST_STORAGE_WRITE)) {
        return -1;
    }
    while (remaining != 0U) {
        const uint64_t lba = position / session->logical_block_bytes;
...
int32_t trait_ext4_block_flush(uintptr_t context, uint32_t boundary)
{
    ...
    if (fail_test_storage_operation(TRAIT_EXT4_TEST_STORAGE_FLUSH)) {
        return -1;
    }
...
static enum traitfs_status checked_stat(
    struct ext4_mount_state *mount,
    const char *path,
...
)
{
    ...
    if (metadata == NULL || length == 0U || length >= TRAITFS_MAX_PATH) {
        return TRAITFS_STATUS_INVALID_ARGUMENT;
    }
    ...
    status = map_status(trait_ext4_stat(mount->rust_mount,
...
static void fill_stat(
    const struct trait_ext4_metadata *source,
    struct traitfs_stat *destination
)
{
    ...
    destination->directory = source->file_type == TRAIT_EXT4_FILE_DIRECTORY;
...
static enum traitfs_status allocate_handle(enum traitfs_volume volume,
    const char *path, uint64_t inode, uint64_t size,
    enum traitfs_access access, bool directory, traitfs_handle *handle)
{
    const size_t length = path_length(path);
    size_t slot = EXT4_MAX_HANDLES;

    if (handle == NULL || length == 0U || length >= TRAITFS_MAX_PATH) {
        return TRAITFS_STATUS_INVALID_ARGUMENT;
    }
...
static void update_open_sizes(enum traitfs_volume volume, uint64_t inode,
    uint64_t size)
{
    for (size_t index = 0U; index < EXT4_MAX_HANDLES; ++index) {
        if (ext4_handles[index].active &&
...
static bool inode_is_open(enum traitfs_volume volume, uint64_t inode)
{
    for (size_t index = 0U; index < EXT4_MAX_HANDLES; ++index) {
        if (ext4_handles[index].active &&
            ext4_handles[index].volume == volume &&
...
void ext4_backend_initialize(void)
{
    ...
    for (enum traitfs_volume volume = TRAITFS_VOLUME_SYSTEM;
        ...
        ext4_last_mount_status[volume] = TRAITFS_STATUS_NOT_MOUNTED;
        ext4_mount_diagnostics[volume].begin_status = TRAITFS_STATUS_NOT_MOUNTED;
        ext4_mount_diagnostics[volume].rust_status = TRAIT_EXT4_STATUS_COUNT;
        ext4_mount_diagnostics[volume].close_status = TRAITFS_STATUS_NOT_MOUNTED;
...
enum traitfs_status ext4_backend_mount(enum traitfs_volume volume)
{
    ...
    if (mount->active) {
        ext4_last_mount_status[volume] = TRAITFS_STATUS_ALREADY_MOUNTED;
        return TRAITFS_STATUS_ALREADY_MOUNTED;
    }
    zero_bytes(mount, sizeof(*mount));
    mount->controller_index = volume == TRAITFS_VOLUME_SYSTEM ?
    ...
    rust_status = trait_ext4_mount((uintptr_t)mount, mount->media_bytes,
    ...
    if (status != TRAITFS_STATUS_OK || close_status != TRAITFS_STATUS_OK) {
        const enum traitfs_status result = status != TRAITFS_STATUS_OK ?
            status : close_status;

        if (mount->rust_mount != 0U) {
            (void)trait_ext4_unmount(mount->rust_mount);
        }
        zero_bytes(mount, sizeof(*mount));
    ...
    mount->active = true;
    ext4_last_mount_status[volume] = TRAITFS_STATUS_OK;
    return TRAITFS_STATUS_OK;
}
...
enum traitfs_status ext4_backend_unmount(enum traitfs_volume volume)
{
    ...
    rust_status = trait_ext4_prepare_unmount(mount->rust_mount);
    close_status = end_operation(mount, NULL);
    status = map_status(rust_status);
    if (status != TRAITFS_STATUS_OK || close_status != TRAITFS_STATUS_OK) {
        return status != TRAITFS_STATUS_OK ? status : close_status;
    }
    if (trait_ext4_unmount(mount->rust_mount) != TRAIT_EXT4_STATUS_OK) {
        return TRAITFS_STATUS_CORRUPT;
    }
    zero_bytes(mount, sizeof(*mount));
    return TRAITFS_STATUS_OK;
...
enum traitfs_status ext4_backend_sync(enum traitfs_volume volume)
{
    ...
    status = map_status(trait_ext4_sync(mount->rust_mount));
    close_status = end_operation(mount, NULL);
    return status != TRAITFS_STATUS_OK ? status : close_status;
}
...
{
    struct traitfs_drive_info drive = {0};
    struct ext4_mount_state *mount;
    ...
    if (mount->active &&
        trait_ext4_free_bytes(mount->rust_mount, &free_bytes) ==
            TRAIT_EXT4_STATUS_OK) {
    ...
    const char *path, enum traitfs_access access, traitfs_handle *handle)
{
    ...
    *handle = 0U;
    ...
    if (metadata.file_type == TRAIT_EXT4_FILE_DIRECTORY) {
        return TRAITFS_STATUS_IS_DIRECTORY;
    }
    ...
    size_t *read_bytes)
{
    ...
    *read_bytes = 0U;
    ...
    status = map_status(trait_ext4_pread(mount->rust_mount,
    ...
    uint8_t *destination, size_t capacity, size_t *read_bytes)
{
    ...
    status = ext4_backend_pread(handle, destination, capacity, state->offset,
        read_bytes);
    if (status == TRAITFS_STATUS_OK && read_bytes != NULL) {
    ...
    size_t source_bytes, size_t *written_bytes)
{
    ...
    if (!valid_volume(volume) || length == 0U ||
        length >= TRAITFS_MAX_PATH || source == NULL || source_bytes == 0U ||
    ...
    *written_bytes = 0U;
    ...
    status = map_status(trait_ext4_transaction_probe(mount->rust_mount,
    ...
    const char *path, uint64_t size)
{
    ...
    if (!valid_volume(volume) || length == 0U || length >= TRAITFS_MAX_PATH) {
        return TRAITFS_STATUS_INVALID_ARGUMENT;
    }
    mount = &ext4_mounts[volume];
    status = begin_operation(mount, true);
    ...
    status = map_status(trait_ext4_truncate_probe(mount->rust_mount,
    ...
    const char *path, uint16_t mode)
{
    ...
    status = map_status(trait_ext4_create_file_probe(mount->rust_mount,
    ...
    const char *path)
{
    ...
    status = map_status(trait_ext4_unlink_file_probe(mount->rust_mount,
    ...
    const char *source, const char *destination)
{
    ...
    if (!valid_volume(volume) || source_length == 0U ||
        source_length >= TRAITFS_MAX_PATH || destination_length == 0U ||
        destination_length >= TRAITFS_MAX_PATH) {
        return TRAITFS_STATUS_INVALID_ARGUMENT;
    }
    mount = &ext4_mounts[volume];
    status = begin_operation(mount, true);
    ...
    status = map_status(trait_ext4_link_file_probe(mount->rust_mount,
    ...
    enum traitfs_volume volume, const char *path)
{
    ...
    status = map_status(trait_ext4_create_directory_probe(mount->rust_mount,
        (const uint8_t *)path, length));
    close_status = end_operation(mount, NULL);
    return status != TRAITFS_STATUS_OK ? status : close_status;
}
    ...
    enum traitfs_volume volume, const char *path)
{
    ...
    status = map_status(trait_ext4_remove_directory_probe(mount->rust_mount,
        (const uint8_t *)path, length));
    close_status = end_operation(mount, NULL);
    return status != TRAITFS_STATUS_OK ? status : close_status;
}
    ...
    const char *source, const char *destination)
{
    ...
    status = map_status(trait_ext4_rename_probe(mount->rust_mount,
        (const uint8_t *)source, source_length,
        (const uint8_t *)destination, destination_length));
    close_status = end_operation(mount, NULL);
    return status != TRAITFS_STATUS_OK ? status : close_status;
    ...
    const uint8_t *source, size_t source_bytes, size_t *written_bytes)
{
    ...
    *written_bytes = 0U;
    ...
    if (source_bytes > EXT4_TRANSACTION_PROBE_MAX_BYTES ||
        state->offset > TRAITFS_MAX_FILE_BYTES ||
        source_bytes > TRAITFS_MAX_FILE_BYTES - state->offset) {
        return TRAITFS_STATUS_RANGE;
    }
    mount = &ext4_mounts[state->volume];
    status = begin_operation(mount, true);
    ...
    enum traitfs_seek_origin origin, uint64_t *position)
{
    ...
    *position = 0U;
    ...
    base = origin == TRAITFS_SEEK_START ? 0U :
        (origin == TRAITFS_SEEK_CURRENT ? state->offset :
            (origin == TRAITFS_SEEK_END ? state->size : UINT64_MAX));
    if (base == UINT64_MAX) {
        return TRAITFS_STATUS_INVALID_ARGUMENT;
    }
    if (offset < 0) {
...
static void fill_entry(const struct trait_ext4_directory_entry *source,
    struct traitfs_list_entry *destination)
{
    zero_bytes(destination, sizeof(*destination));
    ...
    destination->directory =
        source->metadata.file_type == TRAIT_EXT4_FILE_DIRECTORY;
}

static enum traitfs_status indexed_entry(struct ext4_handle_state *state,
    uint64_t index, struct traitfs_list_entry *entry, bool *present)
{
    struct trait_ext4_directory_entry raw;
    struct ext4_mount_state *mount = &ext4_mounts[state->volume];
    enum traitfs_status status = begin_operation(mount, false);
    enum traitfs_status close_status;
    ...
    status = map_status(trait_ext4_directory_entry(mount->rust_mount,
        (const uint8_t *)state->path, path_length(state->path), index, &raw,
        present));
    close_status = end_operation(mount, NULL);
    if (status == TRAITFS_STATUS_OK && *present) {
        if (raw.name_length == 0U || raw.name_length >=
                TRAITFS_MAX_COMPONENT_BYTES) {
            status = TRAITFS_STATUS_NAME;
        } else {
    ...
    const char *path, traitfs_handle *handle)
{
    ...
    *handle = 0U;
    ...
    return allocate_handle(volume, path, metadata.inode, metadata.size,
        TRAITFS_ACCESS_READ, true, handle);
}
    ...
    size_t *entry_count)
{
    ...
    *entry_count = 0U;
    ...
    while (status == TRAITFS_STATUS_OK && count < capacity) {
        bool present = false;
    ...
    if (status == TRAITFS_STATUS_OK && count == capacity) {
        struct traitfs_list_entry ignored;
        bool present = false;
    ...
    if (handle != 0U) {
        enum traitfs_status close_status = ext4_backend_directory_close(handle);
