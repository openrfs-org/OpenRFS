...
{
    enum traitfs_status filesystem_status = traitfs_mount(TRAITFS_VOLUME_DATA);

    if (filesystem_status != TRAITFS_STATUS_OK &&
        filesystem_status != TRAITFS_STATUS_ALREADY_MOUNTED) {
        console_write("Trait OS: package recovery unavailable: ");
        console_write(traitfs_status_string(filesystem_status));
        console_putc('\n');
        return;
    }
    ...
    if (status == PACKAGE_SERVICE_STATUS_ABSENT) {
        console_write("Trait OS: package transaction state absent\n");
        return;
    }
    if (status != PACKAGE_SERVICE_STATUS_OK) {
        console_write("Trait OS: package recovery refused: ");
        ...
        console_write(traitfs_status_string(report.filesystem_status));
        console_putc('\n');
        console_panic("unsafe package transaction state");
    }
    console_write("Trait OS: package generation ");
...
static void initialize_package_uploads(void)
{
    ...
    if (status != PACKAGE_UPLOAD_STATUS_OK) {
        console_write("Trait OS: package upload service unavailable: ");
...
{
    console_write("Trait OS: Boot Ledger refusal: ");
...
_Noreturn void kernel_main(uint32_t magic, uintptr_t boot_information)
{
    ...
    console_write("Trait OS: Boot Ledger installed proof passed\n");
    if (!traitfs_self_test(&filesystem_tests)) {
        console_panic("FAT32 store self-test failed");
    }
    console_write("Trait OS: FAT32 store controls ");
    ...
    console_write("Trait OS: native userspace controls ");
    console_write_u64(native_process_tests);
    console_write(" passed\n");
    if (ui_is_active() && ui_flush() != UI_STATUS_OK) {
        console_write("Trait OS: ledger status redraw failed\n");
    }
    ...
    if (installed_context.test_scenario == KERNEL_TEST_TRAIT_PROOF) {
        kernel_test_complete_trait_proof();
    }
    ...
    if (installed_context.test_scenario == KERNEL_TEST_TRAIT_PROOF_USERLAND) {
        kernel_test_complete_trait_proof_userland();
    }

    if (installed_context.test_scenario ==
            KERNEL_TEST_TRAIT_PROOF_USERLAND_ABSENT) {
        kernel_test_complete_trait_proof_userland_absent();
    }

    if (installed_context.test_scenario ==
            KERNEL_TEST_TRAIT_PROOF_USERLAND_INTERACTIVE) {
        kernel_test_complete_trait_proof_userland_interactive();
    }

    if (installed_context.test_scenario ==
            KERNEL_TEST_TRAIT_PROOF_USERLAND_INTERACTIVE_ABSENT) {
        kernel_test_complete_trait_proof_userland_interactive_absent();
    }
    ...
    if (installed_context.test_scenario == KERNEL_TEST_NATIVE_TRAIT) {
        kernel_test_complete_native_trait();
    }
