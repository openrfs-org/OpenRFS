...
/* OpenGAT early-boot policy bounds firmware-controlled work. */
 ...
 * OpenGAT requires is inside that span, so a table of exactly that size is still
...
/*
 * Two allocation structures, which is one more than any machine OpenGAT is
...
static const char test_oem_id[6] = {'O', 'P', 'N', 'G', 'A', 'T'};
static const char test_oem_table_id[8] = {'O', 'P', 'E', 'N', 'G', 'A', 'T', ' '};
 ...
 * Every table OpenGAT reads comes through here, so a referenced table is bounds
 ...
 *
 * This is the first optional table OpenGAT reads. A machine with no PCI Express
...
bool acpi_tables_self_test(void)
{
    ...
    root = test_root(&fixture, ACPI_ROOT_XSDT);
    root.physical_address =
        OPENGAT_EARLY_PHYSICAL_LIMIT - ACPI_SDT_HEADER_SIZE + 1U;
