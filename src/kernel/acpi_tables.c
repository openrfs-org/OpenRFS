...
/* Trait OS early-boot policy bounds firmware-controlled work. */
 ...
 * Trait OS requires is inside that span, so a table of exactly that size is still
...
/*
 * Two allocation structures, which is one more than any machine Trait OS is
...
static const char test_oem_id[6] = {'T', 'R', 'A', 'I', 'T', ' '};
static const char test_oem_table_id[8] = {'T', 'R', 'A', 'I', 'T', 'O', 'S', ' '};
 ...
 * Every table Trait OS reads comes through here, so a referenced table is bounds
 ...
 *
 * This is the first optional table Trait OS reads. A machine with no PCI Express
...
bool acpi_tables_self_test(void)
{
    ...
    root = test_root(&fixture, ACPI_ROOT_XSDT);
    root.physical_address =
        TRAIT_EARLY_PHYSICAL_LIMIT - ACPI_SDT_HEADER_SIZE + 1U;
