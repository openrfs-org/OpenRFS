...
static char filesystem_cwd[TRAITFS_MAX_PATH + 1U] = ".";
...
static void command_help(void)
{
    ...
    console_write("  fetch     Trait OS identity and live system summary\n");
...
static void command_linux(const char *arguments)
{
    ...
    console_serial_write("RW USERLAND command accepted through Trait OS shell linux ");
...
static void filesystem_error(const char *command, enum traitfs_status status)
{
...
{
    char combined[TRAITFS_MAX_PATH + 1U];
    size_t used = 0U;
    size_t index = 0U;
    ...
    if (filesystem_cwd[0] != '.' || filesystem_cwd[1] != '\0') {
        while (filesystem_cwd[used] != '\0' && used < TRAITFS_MAX_PATH) {
            combined[used] = filesystem_cwd[used];
            ++used;
        }
        if (argument[0] != '\0' && used < TRAITFS_MAX_PATH) {
            combined[used++] = '/';
        }
    }
    while (argument[index] != '\0' && used < TRAITFS_MAX_PATH) {
    ...
    while (index < used) {
        ...
        if (end - index == 1U && combined[index] == '.') {
            ...
            if (output_used != 0U) {
                if (output_used >= TRAITFS_MAX_PATH) {
                    return false;
                }
                output[output_used++] = '/';
            }
...
static void print_drive(const char *name, struct traitfs_drive_info drive)
{
    console_write(name);
    console_write("  fat32  ");
    if (!drive.present) {
...
{
    print_drive("system", traitfs_drive(TRAITFS_VOLUME_SYSTEM));
    print_drive("data  ", traitfs_drive(TRAITFS_VOLUME_DATA));
}

static void command_mount(const char *arguments)
{
    enum traitfs_volume first = TRAITFS_VOLUME_SYSTEM;
    enum traitfs_volume last = TRAITFS_VOLUME_DATA;

    if (argument_equals(arguments, "system")) {
        last = TRAITFS_VOLUME_SYSTEM;
    } else if (argument_equals(arguments, "data")) {
        first = TRAITFS_VOLUME_DATA;
    } else if (arguments[0] != '\0') {
        console_write("mount: use 'mount system' or 'mount data'\n");
        return;
    }
    for (enum traitfs_volume volume = first; volume <= last;
         volume = (enum traitfs_volume)(volume + 1)) {
        struct traitfs_drive_info drive = traitfs_drive(volume);
        enum traitfs_status status;

        if (drive.mounted) {
            continue;
        }
        status = traitfs_mount(volume);
        if (status != TRAITFS_STATUS_OK) {
            filesystem_error(volume == TRAITFS_VOLUME_SYSTEM ?
...
static void command_cd(const char *arguments)
{
    ...
    status = traitfs_stat_path(TRAITFS_VOLUME_DATA, path, &stat);
    if (status != TRAITFS_STATUS_OK) {
        filesystem_error("cd", status);
        return;
    }
    if (!stat.directory) {
        filesystem_error("cd", TRAITFS_STATUS_NOT_DIRECTORY);
        return;
    }
    size_t index = 0U;
    do {
...
static void command_ls(const char *arguments)
{
    ...
    status = traitfs_list(TRAITFS_VOLUME_DATA, path, entries,
...
static void command_mkdir(const char *arguments)
{
    ...
    status = traitfs_mkdir(TRAITFS_VOLUME_DATA, path);
    if (status != TRAITFS_STATUS_OK) {
...
static void command_touch(const char *arguments)
{
    ...
    if (status == TRAITFS_STATUS_OK) {
        if (stat.directory) {
            filesystem_error("touch", TRAITFS_STATUS_IS_DIRECTORY);
        }
        return;
    }
    ...
    status = traitfs_create(TRAITFS_VOLUME_DATA, path);
    if (status != TRAITFS_STATUS_OK) {
        filesystem_error("touch", status);
    }
}
...
static void command_read(const char *arguments)
{
    ...
    status = traitfs_open(TRAITFS_VOLUME_DATA, path, TRAITFS_ACCESS_READ, &handle);
    if (status != TRAITFS_STATUS_OK) {
    ...
    for (;;) {
        ...
        status = traitfs_read(handle, buffer, sizeof(buffer), &read_bytes);
        if (read_bytes != 0U) {
            console_write_n((const char *)buffer, read_bytes);
        }
        if (status != TRAITFS_STATUS_OK || read_bytes == 0U) {
            break;
        }
    }
    if (traitfs_close(handle) != TRAITFS_STATUS_OK && status == TRAITFS_STATUS_OK) {
        status = TRAITFS_STATUS_STALE_HANDLE;
    }
    if (status != TRAITFS_STATUS_OK) {
        filesystem_error("read", status);
    }
...
{
    char argument_path[TRAITFS_MAX_PATH + 1U];
    char path[TRAITFS_MAX_PATH + 1U];
    const char *text;
    ...
    if (status == TRAITFS_STATUS_OK && !append) {
        status = traitfs_truncate(TRAITFS_VOLUME_DATA, path, 0U);
    }
    if (status == TRAITFS_STATUS_OK) {
        status = traitfs_open(TRAITFS_VOLUME_DATA, path,
            TRAITFS_ACCESS_WRITE, &handle);
        opened = status == TRAITFS_STATUS_OK;
    }
    if (status == TRAITFS_STATUS_OK && append) {
        uint64_t position;

        status = traitfs_seek(handle, 0, TRAITFS_SEEK_END, &position);
    }
    if (status == TRAITFS_STATUS_OK) {
        status = traitfs_write(handle, content, content_bytes, &written);
    }
    if (opened && traitfs_close(handle) != TRAITFS_STATUS_OK &&
        status == TRAITFS_STATUS_OK) {
        status = TRAITFS_STATUS_STALE_HANDLE;
    }
    if (status != TRAITFS_STATUS_OK || written != content_bytes) {
        filesystem_error(append ? "append" : "write",
            status != TRAITFS_STATUS_OK ? status : TRAITFS_STATUS_WRITEBACK);
    }
}
...
{
    char argument_path[TRAITFS_MAX_PATH + 1U];
    char argument_offset[TRAITFS_MAX_PATH + 1U];
    char path[TRAITFS_MAX_PATH + 1U];
    ...
    opened = status == TRAITFS_STATUS_OK;
    if (status == TRAITFS_STATUS_OK) {
        status = traitfs_seek(handle, (int64_t)offset,
    ...
    if (status != TRAITFS_STATUS_OK || position != offset ||
        written != content_bytes) {
        filesystem_error("writeat", status != TRAITFS_STATUS_OK ? status :
...
static void command_truncate(const char *arguments)
{
    ...
    status = traitfs_truncate(TRAITFS_VOLUME_DATA, path, size);
    if (status != TRAITFS_STATUS_OK) {
...
{
    char first[TRAITFS_MAX_PATH + 1U];
    char source[TRAITFS_MAX_PATH + 1U];
    char destination[TRAITFS_MAX_PATH + 1U];
    const char *second;
    enum traitfs_status status;
...
static void command_rm(const char *arguments)
{
    ...
    status = traitfs_stat_path(TRAITFS_VOLUME_DATA, path, &stat);
    if (status == TRAITFS_STATUS_OK) {
        status = stat.directory ? traitfs_rmdir(TRAITFS_VOLUME_DATA, path) :
            traitfs_unlink(TRAITFS_VOLUME_DATA, path);
    }
    if (status != TRAITFS_STATUS_OK) {
...
{
    enum traitfs_status status = traitfs_unmount(TRAITFS_VOLUME_DATA);

    if (status != TRAITFS_STATUS_OK && status != TRAITFS_STATUS_NOT_MOUNTED) {
        filesystem_error("reboot", status);
        return;
    }
    status = traitfs_unmount(TRAITFS_VOLUME_SYSTEM);
    if (status != TRAITFS_STATUS_OK && status != TRAITFS_STATUS_NOT_MOUNTED) {
        filesystem_error("reboot", status);
        (void)traitfs_mount(TRAITFS_VOLUME_DATA);
        return;
    }
    ...
    (void)traitfs_mount(TRAITFS_VOLUME_SYSTEM);
    (void)traitfs_mount(TRAITFS_VOLUME_DATA);
}
...
static void command_version(void)
{
    ...
    console_write("Trait OS 2.2.0 dev, a proof-driven x86_64 operating system.\n");
...
static void print_fetch_drive(struct traitfs_drive_info drive)
{
...
static void command_fetch(void)
{
    const struct screen_state screen = screen_get_state();
    const struct heap_state heap = heap_get_state();
    const struct traitfs_drive_info system = traitfs_drive(TRAITFS_VOLUME_SYSTEM);
    ...
    console_write("\n");
    console_write("  Trait OS\n");
    console_write("  kernel      Trait OS 2.2.0 dev / x86_64\n");
...
{
    char address_text[TRAITFS_MAX_PATH + 1U];
    char mask_text[TRAITFS_MAX_PATH + 1U];
    char gateway_text[TRAITFS_MAX_PATH + 1U];
    char dns_text[TRAITFS_MAX_PATH + 1U];
    const char *remainder;
    uint32_t address;
    uint32_t mask;
    uint32_t gateway;
...
{
    char hostname[TRAITFS_MAX_PATH + 1U];
    const char *remainder;
    uint32_t address;
    enum network_status status;
...
static void write_prompt_restored(void)
{
    ...
    if (linux_prompt_evidence_pending) {
        console_serial_write("\nRW USERLAND Trait OS prompt restored\n");
    ...
    for (;;) {
        ...
        if (ui_operational) {
            ...
            if (status != UI_STATUS_OK) {
                ...
                console_write("Trait OS: runtime disabled: ");
                console_write(ui_status_string(status));
                console_putc('\n');
            }
        }
        if (ui_operational) {
            char manifest[TRAITFS_MAX_PATH + 1U];
