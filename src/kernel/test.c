...
#include <trait/linux_userland.h>
...
_Static_assert(
    PAGING_TEST_HUGE_ADDRESS % PAGING_HUGE_PAGE_SIZE == 0U &&
        PAGING_TEST_HUGE_ADDRESS < TRAIT_EARLY_PHYSICAL_LIMIT,
...
)
{
    ...
    if (token_equals(value, length, "trait-proof")) {
        return KERNEL_TEST_TRAIT_PROOF;
    }
    ...
    if (token_equals(value, length, "trait-proof-userland")) {
        return KERNEL_TEST_TRAIT_PROOF_USERLAND;
    }

    if (token_equals(value, length, "trait-proof-userland-absent")) {
        return KERNEL_TEST_TRAIT_PROOF_USERLAND_ABSENT;
    }

    if (token_equals(value, length, "trait-proof-userland-interactive")) {
        return KERNEL_TEST_TRAIT_PROOF_USERLAND_INTERACTIVE;
    }

    if (token_equals(
            value, length, "trait-proof-userland-interactive-absent")) {
        return KERNEL_TEST_TRAIT_PROOF_USERLAND_INTERACTIVE_ABSENT;
    }
    ...
    if (token_equals(value, length, "native-trait")) {
        return KERNEL_TEST_NATIVE_TRAIT;
    }
...
static uint8_t scenario_exit_value(enum kernel_test_scenario scenario)
{
    ...
    case KERNEL_TEST_LINUX_ABI_UNAME:
        return UINT8_C(0x37);
    case KERNEL_TEST_TRAIT_PROOF_USERLAND:
        return UINT8_C(0x38);
    case KERNEL_TEST_TRAIT_PROOF_USERLAND_ABSENT:
        return UINT8_C(0x39);
    case KERNEL_TEST_TRAIT_PROOF_USERLAND_INTERACTIVE:
        return UINT8_C(0x3A);
    case KERNEL_TEST_TRAIT_PROOF_USERLAND_INTERACTIVE_ABSENT:
        return UINT8_C(0x3B);
    ...
    case KERNEL_TEST_NATIVE_TRAIT: return UINT8_C(0x87);
    default:
...
{
    static const char prefix[] = "trait.test=";
...
static void ioapic_level_scenario(void)
{
     ...
     * Trait OS's records called it level triggered would deliver every interrupt
 ...
 * increment that assertion was the only thing standing behind Trait OS's W^X
...
static void paging_scenario(const struct paging_device_windows *device_windows)
{
    ...
    if (paging_map(text & ~(TRAIT_PAGE_SIZE - 1U), 0U, TRAIT_PAGE_SIZE,
            PAGING_WRITE) != PAGING_STATUS_ALREADY_MAPPED ||
        paging_unmap(PAGING_PROBE_ADDRESS, TRAIT_PAGE_SIZE) !=
            PAGING_STATUS_NOT_MAPPED ||
        paging_protect(PAGING_PROBE_ADDRESS, TRAIT_PAGE_SIZE, PAGING_READ) !=
            PAGING_STATUS_NOT_MAPPED) {
     ...
     * the whole 2 MiB.
     */
    if (paging_protect(PAGING_TEST_HUGE_ADDRESS, TRAIT_PAGE_SIZE,
            PAGING_READ) != PAGING_STATUS_HUGE_PAGE_PRESENT ||
        paging_unmap(PAGING_TEST_HUGE_ADDRESS, TRAIT_PAGE_SIZE) !=
    ...
    if (paging_map(PAGING_PROBE_ADDRESS, frame, TRAIT_PAGE_SIZE,
            PAGING_WRITE) != PAGING_STATUS_OK ||
        paging_map(PAGING_PROBE_ADDRESS, frame, TRAIT_PAGE_SIZE,
            PAGING_WRITE) != PAGING_STATUS_ALREADY_MAPPED) {
    ...
    *probe = PAGING_TEST_PATTERN;
    ...
    if (paging_protect(PAGING_PROBE_ADDRESS, TRAIT_PAGE_SIZE, PAGING_READ) !=
        PAGING_STATUS_OK) {
     ...
     */
    if (paging_unmap(PAGING_PROBE_ADDRESS, TRAIT_PAGE_SIZE) !=
        PAGING_STATUS_OK) {
    ...
    for (size_t cycle = 0; cycle < PAGING_TEST_CYCLES; ++cycle) {
        ...
        if (frame_allocate(&cycle_frame) != FRAME_STATUS_OK ||
            paging_map(PAGING_PROBE_ADDRESS, cycle_frame, TRAIT_PAGE_SIZE,
                PAGING_WRITE) != PAGING_STATUS_OK ||
            paging_unmap(PAGING_PROBE_ADDRESS, TRAIT_PAGE_SIZE) !=
                PAGING_STATUS_OK ||
...
)
{
    ...
    /*
     * A bus past what Trait OS mapped is refused rather than folded back into the
...
static void shell_scenario(void)
{
    ...
    after = shell_get_state();
    console_write("Trait OS: shell scenario ran ");
...
static void keyboard_scenario(void)
{
    ...
    console_write("Trait OS: keyboard scenario queued ");
...
static void screen_scenario(void)
{
    ...
    console_write("Trait OS: screen scenario drew ");
...
)
{
    ...
    case KERNEL_TEST_NATIVE_TRAIT:
    case KERNEL_TEST_EXT4_RECOVERY:
        /* Deferred until Trait OS and the Boot Ledger are published. */
        return;
    ...
    case KERNEL_TEST_INVALID:
        kernel_test_fail("invalid or duplicate trait.test argument");
...
_Noreturn void kernel_test_complete_ext4_recovery(void)
{
    static const uint8_t expected[] =
        "Trait OS deterministic ext4 fixture\n";
    ...
    struct trait_ext4_mount_diagnostic mount_diagnostic = {0};
    struct trait_ext4_recovery_report clean_remount = {0};
    struct trait_ext4_recovery_report recovery = {0};
    const struct traitfs_drive_info drive = traitfs_drive(TRAITFS_VOLUME_SYSTEM);
    struct traitfs_stat stat = {0};
    traitfs_handle handle = 0U;
    ...
    const uint64_t before = traitfs_completion_count(TRAITFS_VOLUME_SYSTEM);
    ...
    if (!drive.present || !drive.mounted || drive.read_only || !drive.healthy) {
        if (!ext4_backend_mount_diagnostic(TRAITFS_VOLUME_SYSTEM,
                &mount_diagnostic)) {
    ...
    if (!ext4_backend_recovery_report(TRAITFS_VOLUME_SYSTEM, &recovery)) {
    ...
    if (traitfs_stat_path(TRAITFS_VOLUME_SYSTEM, "system/README.TXT", &stat) !=
            TRAITFS_STATUS_OK || stat.directory || stat.read_only ||
        (stat.size != sizeof(expected) - 1U && stat.size != UINT64_C(4097)) ||
        traitfs_open(TRAITFS_VOLUME_SYSTEM, "system/README.TXT",
            TRAITFS_ACCESS_READ, &handle) != TRAITFS_STATUS_OK ||
        traitfs_pread(handle, bytes, sizeof(expected) - 1U, 0U, &read_bytes) !=
            TRAITFS_STATUS_OK || read_bytes != sizeof(expected) - 1U) {
    ...
    if (!contents_match ||
        (transaction_already_visible &&
            (traitfs_pread(handle, &appended, sizeof(appended), UINT64_C(4096),
                &read_bytes) != TRAITFS_STATUS_OK || read_bytes != 1U ||
             appended != transaction_byte)) ||
        traitfs_close(handle) != TRAITFS_STATUS_OK ||
        traitfs_close(handle) != TRAITFS_STATUS_STALE_HANDLE ||
        traitfs_completion_count(TRAITFS_VOLUME_SYSTEM) <= before) {
        kernel_test_fail("ext4 recovery read leaked or changed data");
    }
    if (traitfs_open(TRAITFS_VOLUME_SYSTEM, "system/README.TXT",
            TRAITFS_ACCESS_WRITE, &handle) != TRAITFS_STATUS_OK ||
        traitfs_pread(handle, &appended, sizeof(appended), 0U, &read_bytes) !=
            TRAITFS_STATUS_ACCESS ||
        traitfs_close(handle) != TRAITFS_STATUS_OK) {
        kernel_test_fail("ext4 writable handle access enforcement failed");
    }
    if (traitfs_sync(TRAITFS_VOLUME_SYSTEM) != TRAITFS_STATUS_OK) {
        kernel_test_fail("clean ext4 sync failed");
    }
    if (!power_cut && !transaction_already_visible) {
        if (!ext4_backend_test_fail_storage_once(3U) ||
            ext4_backend_transaction_probe(TRAITFS_VOLUME_SYSTEM,
                "system/README.TXT", UINT64_C(4096), &transaction_byte,
                sizeof(transaction_byte), &written_bytes) != TRAITFS_STATUS_IO ||
            !ext4_backend_test_storage_failure_observed(
                TRAIT_EXT4_TEST_STORAGE_WRITE) ||
            !ext4_backend_test_fail_storage_once(3U) ||
            traitfs_sync(TRAITFS_VOLUME_SYSTEM) != TRAITFS_STATUS_IO ||
            !ext4_backend_test_storage_failure_observed(
    ...
    if (!transaction_already_visible &&
        (ext4_backend_transaction_probe(TRAITFS_VOLUME_SYSTEM,
            "system/README.TXT", UINT64_C(4096), &transaction_byte,
            sizeof(transaction_byte), &written_bytes) != TRAITFS_STATUS_OK ||
    ...
    if (traitfs_sync(TRAITFS_VOLUME_SYSTEM) != TRAITFS_STATUS_OK ||
        traitfs_stat_path(TRAITFS_VOLUME_SYSTEM, "system/README.TXT", &stat) !=
            TRAITFS_STATUS_OK || stat.size != UINT64_C(4097) ||
        traitfs_open(TRAITFS_VOLUME_SYSTEM, "system/README.TXT",
            TRAITFS_ACCESS_READ, &handle) != TRAITFS_STATUS_OK ||
        traitfs_pread(handle, &appended, sizeof(appended), UINT64_C(4096),
            &read_bytes) != TRAITFS_STATUS_OK || read_bytes != 1U ||
    ...
    if (!power_cut && !transaction_already_visible) {
        if (traitfs_truncate(TRAITFS_VOLUME_SYSTEM,
                "system/README.TXT", sizeof(expected) - 1U) !=
                TRAITFS_STATUS_OK ||
            traitfs_sync(TRAITFS_VOLUME_SYSTEM) != TRAITFS_STATUS_OK ||
            traitfs_stat_path(TRAITFS_VOLUME_SYSTEM, "system/README.TXT", &stat) !=
                TRAITFS_STATUS_OK || stat.size != sizeof(expected) - 1U ||
            traitfs_open(TRAITFS_VOLUME_SYSTEM, "system/README.TXT",
                TRAITFS_ACCESS_READ, &handle) != TRAITFS_STATUS_OK ||
            traitfs_pread(handle, &appended, sizeof(appended), UINT64_C(4096),
                &read_bytes) != TRAITFS_STATUS_OK || read_bytes != 0U ||
            traitfs_close(handle) != TRAITFS_STATUS_OK) {
        ...
        if (traitfs_open(TRAITFS_VOLUME_SYSTEM, "system/README.TXT",
                TRAITFS_ACCESS_WRITE, &handle) != TRAITFS_STATUS_OK ||
            traitfs_seek(handle, INT64_C(4096), TRAITFS_SEEK_START, NULL) !=
                TRAITFS_STATUS_INVALID_ARGUMENT ||
            traitfs_seek(handle, INT64_C(4096), TRAITFS_SEEK_START,
                &stat.size) != TRAITFS_STATUS_OK || stat.size != UINT64_C(4096) ||
            traitfs_write(handle, &transaction_byte, sizeof(transaction_byte),
                &written_bytes) != TRAITFS_STATUS_OK ||
            written_bytes != sizeof(transaction_byte) ||
            traitfs_close(handle) != TRAITFS_STATUS_OK ||
            traitfs_sync(TRAITFS_VOLUME_SYSTEM) != TRAITFS_STATUS_OK ||
            traitfs_stat_path(TRAITFS_VOLUME_SYSTEM, "system/README.TXT", &stat) !=
        ...
        if (traitfs_create_mode(TRAITFS_VOLUME_SYSTEM,
                "data/user/JRNLPROBE.TMP", UINT16_C(0555)) !=
                TRAITFS_STATUS_OK ||
            traitfs_stat_path(TRAITFS_VOLUME_SYSTEM, "data/user/JRNLPROBE.TMP",
                &stat) != TRAITFS_STATUS_OK || stat.directory || stat.size != 0U ||
            stat.read_only || (stat.mode & UINT16_C(0777)) != UINT16_C(0555) ||
            traitfs_open(TRAITFS_VOLUME_SYSTEM, "data/user/JRNLPROBE.TMP",
                TRAITFS_ACCESS_READ_WRITE, &handle) != TRAITFS_STATUS_OK ||
            traitfs_write(handle, &transaction_byte, sizeof(transaction_byte),
                &written_bytes) != TRAITFS_STATUS_OK || written_bytes != 1U ||
            traitfs_seek(handle, 0, TRAITFS_SEEK_START, &stat.size) !=
                TRAITFS_STATUS_OK || stat.size != 0U ||
            traitfs_read(handle, &appended, sizeof(appended), &read_bytes) !=
                TRAITFS_STATUS_OK || read_bytes != 1U ||
            appended != transaction_byte ||
            traitfs_unlink(TRAITFS_VOLUME_SYSTEM,
                "data/user/JRNLPROBE.TMP") != TRAITFS_STATUS_BUSY ||
            traitfs_rename(TRAITFS_VOLUME_SYSTEM,
                "data/user/JRNLPROBE.TMP", "data/user/JRNLPROBE.BUSY") !=
                    TRAITFS_STATUS_BUSY ||
            traitfs_close(handle) != TRAITFS_STATUS_OK ||
            traitfs_sync(TRAITFS_VOLUME_SYSTEM) != TRAITFS_STATUS_OK ||
            traitfs_link(TRAITFS_VOLUME_SYSTEM,
                "data/user/JRNLPROBE.TMP", "data/user/JRNLPROBE.LNK") !=
                    TRAITFS_STATUS_OK ||
            traitfs_sync(TRAITFS_VOLUME_SYSTEM) != TRAITFS_STATUS_OK ||
            traitfs_unlink(TRAITFS_VOLUME_SYSTEM,
                "data/user/JRNLPROBE.TMP") != TRAITFS_STATUS_OK ||
            traitfs_sync(TRAITFS_VOLUME_SYSTEM) != TRAITFS_STATUS_OK ||
            traitfs_stat_path(TRAITFS_VOLUME_SYSTEM, "data/user/JRNLPROBE.TMP",
                &stat) != TRAITFS_STATUS_NOT_FOUND ||
            traitfs_stat_path(TRAITFS_VOLUME_SYSTEM, "data/user/JRNLPROBE.LNK",
                &stat) != TRAITFS_STATUS_OK || stat.directory || stat.size != 1U ||
            traitfs_rename(TRAITFS_VOLUME_SYSTEM,
            ...
            traitfs_stat_path(TRAITFS_VOLUME_SYSTEM, "data/user/JRNLPROBE.LNK",
                &stat) != TRAITFS_STATUS_NOT_FOUND ||
            traitfs_stat_path(TRAITFS_VOLUME_SYSTEM, "data/user/JRNLPROBE.REN",
                &stat) != TRAITFS_STATUS_OK || stat.directory || stat.size != 1U ||
            traitfs_unlink(TRAITFS_VOLUME_SYSTEM,
                "data/user/JRNLPROBE.REN") != TRAITFS_STATUS_OK ||
            traitfs_sync(TRAITFS_VOLUME_SYSTEM) != TRAITFS_STATUS_OK ||
            traitfs_stat_path(TRAITFS_VOLUME_SYSTEM, "data/user/JRNLPROBE.REN",
                &stat) != TRAITFS_STATUS_NOT_FOUND ||
            traitfs_mkdir(TRAITFS_VOLUME_SYSTEM,
                "data/user/JRNLPROBE.DIR") != TRAITFS_STATUS_OK ||
            traitfs_sync(TRAITFS_VOLUME_SYSTEM) != TRAITFS_STATUS_OK ||
            traitfs_stat_path(TRAITFS_VOLUME_SYSTEM, "data/user/JRNLPROBE.DIR",
                &stat) != TRAITFS_STATUS_OK || !stat.directory ||
            traitfs_rename(TRAITFS_VOLUME_SYSTEM,
            ...
            traitfs_stat_path(TRAITFS_VOLUME_SYSTEM, "data/user/JRNLPROBE.DIR",
                &stat) != TRAITFS_STATUS_NOT_FOUND ||
            traitfs_stat_path(TRAITFS_VOLUME_SYSTEM, "data/user/JRNLPROBE.RDR",
                &stat) != TRAITFS_STATUS_OK || !stat.directory ||
            traitfs_create(TRAITFS_VOLUME_SYSTEM,
                "data/user/JRNLPROBE.RDR/CHILD.TMP") != TRAITFS_STATUS_OK ||
            traitfs_sync(TRAITFS_VOLUME_SYSTEM) != TRAITFS_STATUS_OK ||
            traitfs_rmdir(TRAITFS_VOLUME_SYSTEM,
                "data/user/JRNLPROBE.RDR") != TRAITFS_STATUS_NOT_EMPTY ||
            traitfs_stat_path(TRAITFS_VOLUME_SYSTEM,
                "data/user/JRNLPROBE.RDR/CHILD.TMP", &stat) !=
                    TRAITFS_STATUS_OK || stat.directory ||
            traitfs_unlink(TRAITFS_VOLUME_SYSTEM,
                "data/user/JRNLPROBE.RDR/CHILD.TMP") != TRAITFS_STATUS_OK ||
            traitfs_sync(TRAITFS_VOLUME_SYSTEM) != TRAITFS_STATUS_OK ||
            traitfs_rmdir(TRAITFS_VOLUME_SYSTEM,
                "data/user/JRNLPROBE.RDR") != TRAITFS_STATUS_OK ||
            traitfs_sync(TRAITFS_VOLUME_SYSTEM) != TRAITFS_STATUS_OK ||
            traitfs_stat_path(TRAITFS_VOLUME_SYSTEM, "data/user/JRNLPROBE.RDR",
                &stat) != TRAITFS_STATUS_NOT_FOUND) {
    ...
    if (traitfs_unmount(TRAITFS_VOLUME_SYSTEM) != TRAITFS_STATUS_OK ||
        traitfs_drive(TRAITFS_VOLUME_SYSTEM).mounted ||
    ...
    if (traitfs_mount(TRAITFS_VOLUME_SYSTEM) != TRAITFS_STATUS_OK ||
        !ext4_backend_recovery_report(TRAITFS_VOLUME_SYSTEM, &clean_remount) ||
        ...
        traitfs_pread(handle, &appended, sizeof(appended), UINT64_C(4096),
            &read_bytes) != TRAITFS_STATUS_OK || read_bytes != 1U ||
        appended != transaction_byte || traitfs_close(handle) != TRAITFS_STATUS_OK ||
        traitfs_unmount(TRAITFS_VOLUME_SYSTEM) != TRAITFS_STATUS_OK ||
        traitfs_drive(TRAITFS_VOLUME_SYSTEM).mounted ||
        !nvme_filesystem_session_resources_released() ||
        heap_verify() != HEAP_STATUS_OK ||
        paging_verify() != PAGING_STATUS_OK) {
...
_Noreturn void kernel_test_complete_native(void)
{
    ...
    if (launch_status != NATIVE_PROCESS_OK) {
        console_write("Trait OS: native launch refusal: ");
    ...
    if (!result.exited || result.faulted ||
        ...
        console_write("Trait OS: native result exit ");
    ...
    console_write("TRAIT PERF context-switch transitions=");
    ...
    console_write_u64(result.context_cycles_with_fpu /
        result.context_transition_samples);
    console_putc('\n');
    if (traitfs_stat_path(TRAITFS_VOLUME_DATA, "NATIVET/FOUND.TXT", &output) !=
            TRAITFS_STATUS_OK || output.directory || output.size != sizeof(bytes) ||
        traitfs_open(TRAITFS_VOLUME_DATA, "NATIVET/FOUND.TXT", TRAITFS_ACCESS_READ,
            &file) != TRAITFS_STATUS_OK ||
        traitfs_read(file, bytes, sizeof(bytes), &read_bytes) != TRAITFS_STATUS_OK ||
    ...
    if (traitfs_close(file) != TRAITFS_STATUS_OK || !content_matches) {
        kernel_test_fail("native Ring 3 file result is wrong");
    }
    console_write("Trait OS: native general loader, SDK, TLS, threads and FPU passed\n");
    kernel_test_pass();
}
...
_Noreturn void kernel_test_complete_native_lua(void)
{
    ...
    if (traitfs_stat_path(TRAITFS_VOLUME_DATA, "LUA/RESULT.TXT", &output) !=
            TRAITFS_STATUS_OK || output.directory || output.size != sizeof(bytes) ||
        traitfs_open(TRAITFS_VOLUME_DATA, "LUA/RESULT.TXT", TRAITFS_ACCESS_READ,
            &file) != TRAITFS_STATUS_OK ||
        traitfs_read(file, bytes, sizeof(bytes), &read_bytes) != TRAITFS_STATUS_OK ||
        read_bytes != sizeof(bytes)) {
    ...
    if (traitfs_close(file) != TRAITFS_STATUS_OK || !content_matches ||
        traitfs_sync(TRAITFS_VOLUME_DATA) != TRAITFS_STATUS_OK) {
        kernel_test_fail("Lua result file is wrong or could not be synchronized");
    }
    console_write("Trait OS: upstream Lua used stdin, Data, math and stdout\n");
    kernel_test_pass();
}
...
_Noreturn void kernel_test_complete_native_sqlite(void)
{
    static const uint8_t expected[] =
        "rows=3\nsum=66\nintegrity=ok\n";
    struct native_process_result result;
    struct traitfs_stat database;
    struct traitfs_stat journal;
    struct traitfs_stat output;
    traitfs_handle file;
    uint8_t bytes[sizeof(expected) - 1U];
    size_t read_bytes = 0U;
    bool content_matches = true;
    const enum traitfs_status before = traitfs_stat_path(TRAITFS_VOLUME_DATA,
    ...
    if (before != TRAITFS_STATUS_OK && before != TRAITFS_STATUS_NOT_FOUND) {
    ...
    if (traitfs_stat_path(TRAITFS_VOLUME_DATA, "SQLITE/PORT.JRN", &journal) !=
            TRAITFS_STATUS_NOT_FOUND) {
        kernel_test_fail("SQLite left a rollback journal after clean close");
    }
    if (before == TRAITFS_STATUS_NOT_FOUND) {
        if (traitfs_stat_path(TRAITFS_VOLUME_DATA, "SQLITE/PORT.DB", &database) !=
                TRAITFS_STATUS_OK || database.directory || database.size == 0U ||
            traitfs_unmount(TRAITFS_VOLUME_DATA) != TRAITFS_STATUS_OK) {
            kernel_test_fail("SQLite first phase did not synchronize its database");
        }
        console_write("Trait OS: upstream SQLite synchronized reboot phase\n");
        cpu_out8(UINT16_C(0x0064), UINT8_C(0xFE));
        kernel_test_fail("platform reset did not restart SQLite scenario");
    }
    if (traitfs_stat_path(TRAITFS_VOLUME_DATA, "SQLITE/RESULT.TXT", &output) !=
            TRAITFS_STATUS_OK || output.directory || output.size != sizeof(bytes) ||
        traitfs_open(TRAITFS_VOLUME_DATA, "SQLITE/RESULT.TXT", TRAITFS_ACCESS_READ,
            &file) != TRAITFS_STATUS_OK ||
        traitfs_read(file, bytes, sizeof(bytes), &read_bytes) != TRAITFS_STATUS_OK ||
        read_bytes != sizeof(bytes)) {
    ...
    console_write("Trait OS: upstream SQLite retained and verified three rows after reboot\n");
    kernel_test_pass();
}

_Noreturn void kernel_test_complete_native_network(void)
{
    static const uint8_t expected[] = "hello from the Trait OS network\n";
    struct native_process_result result = { 0 };
    struct traitfs_stat output;
    ...
    if (launch_status != NATIVE_PROCESS_OK ||
        ...
        !native_process_resources_released()) {
        console_write("Trait OS: native network launch ");
        console_write(native_process_status_string(launch_status));
    ...
    if (traitfs_stat_path(TRAITFS_VOLUME_DATA, "NETAPP/HTTP.TXT", &output) !=
            TRAITFS_STATUS_OK || output.directory || output.size != sizeof(bytes) ||
        traitfs_open(TRAITFS_VOLUME_DATA, "NETAPP/HTTP.TXT", TRAITFS_ACCESS_READ,
            &file) != TRAITFS_STATUS_OK ||
        traitfs_read(file, bytes, sizeof(bytes), &read_bytes) != TRAITFS_STATUS_OK ||
        read_bytes != sizeof(bytes)) {
    ...
    if (traitfs_close(file) != TRAITFS_STATUS_OK || !matches) {
        kernel_test_fail("native HTTP body framing or contents are wrong");
    }
    console_write("Trait OS: native DNS, TCP, UDP, timeout, reset and cancellation passed\n");
...
_Noreturn void kernel_test_complete_native_rust(void)
{
    ...
    if (traitfs_stat_path(TRAITFS_VOLUME_DATA, "RUSTAPP/RUST.TXT", &output) !=
            TRAITFS_STATUS_OK || output.directory || output.size != sizeof(bytes) ||
        traitfs_open(TRAITFS_VOLUME_DATA, "RUSTAPP/RUST.TXT", TRAITFS_ACCESS_READ,
            &file) != TRAITFS_STATUS_OK ||
        traitfs_read(file, bytes, sizeof(bytes), &read_bytes) != TRAITFS_STATUS_OK ||
        read_bytes != sizeof(bytes)) {
    ...
    console_write("Trait OS: no_std Rust application used native ABI v1 services\n");
    kernel_test_pass();
}
...
_Noreturn void kernel_test_complete_native_crash(void)
{
    ...
    if (native_process_launch("CRASH.MAN", &crash) != NATIVE_PROCESS_OK ||
        !crash.exited || !crash.faulted || crash.exit_status != -TRAIT_EFAULT ||
    ...
    console_write("Trait OS: native crash contained; mappings handles threads windows FS x87 SSE reclaimed\n");
    kernel_test_pass();
}
...
_Noreturn void kernel_test_complete_native_admission_refusal(void)
{
    ...
    case KERNEL_TEST_NATIVE_ELF_REFUSAL:
        manifest = "BADELF.MAN";
        diagnostic = "Trait OS: native malformed ELF refused; resource census unchanged\n";
        break;
    case KERNEL_TEST_NATIVE_DIGEST_REFUSAL:
        manifest = "BADDGST.MAN";
        diagnostic = "Trait OS: native manifest digest mismatch refused; resource census unchanged\n";
        break;
    case KERNEL_TEST_NATIVE_ABI_REFUSAL:
        manifest = "BADABI.MAN";
        diagnostic = "Trait OS: native unsupported ABI version refused; resource census unchanged\n";
        break;
    default:
...
_Noreturn void kernel_test_complete_native_relaunch(void)
{
    ...
    console_write("Trait OS: native relaunch advanced generation; both resource censuses clean\n");
    kernel_test_pass();
}
...
_Noreturn void kernel_test_complete_native_audio(void)
{
    ...
    console_write(
        "Trait OS: native audio ABI capability, mixing, cancellation and teardown passed\n");
    kernel_test_pass();
}
...
_Noreturn void kernel_test_complete_native_sdl(void)
{
    ...
    if (traitfs_stat_path(TRAITFS_VOLUME_DATA, state_path, &state) !=
            TRAITFS_STATUS_OK || state.directory || state.size != sizeof(bytes) ||
        traitfs_open(TRAITFS_VOLUME_DATA, state_path, TRAITFS_ACCESS_READ, &file) !=
            TRAITFS_STATUS_OK ||
        traitfs_read(file, bytes, sizeof(bytes), &read_bytes) != TRAITFS_STATUS_OK ||
        read_bytes != sizeof(bytes) || traitfs_close(file) != TRAITFS_STATUS_OK ||
        bytes[0] != 2U || bytes[1] != 0U || bytes[2] != 0U || bytes[3] != 0U) {
        kernel_test_fail("SDL preference state did not survive process relaunch");
    }
    console_write(
        "Trait OS: SDL 2 window, input, partial damage, PCM and persistence passed\n");
    kernel_test_pass();
}
...
{
    static const uint8_t expected[] =
        "hello from the Trait OS HTTPS peer\n";
    struct native_process_result proof = { 0 };
    struct traitfs_stat output;
    struct network_state network;
    traitfs_handle file;
    ...
    if (traitfs_stat_path(TRAITFS_VOLUME_DATA, "HTTPSAPP/HTTPS.TXT", &output) !=
            TRAITFS_STATUS_OK || output.directory || output.size != sizeof(bytes) ||
        traitfs_open(TRAITFS_VOLUME_DATA, "HTTPSAPP/HTTPS.TXT", TRAITFS_ACCESS_READ,
            &file) != TRAITFS_STATUS_OK ||
        traitfs_read(file, bytes, sizeof(bytes), &read_bytes) != TRAITFS_STATUS_OK ||
        read_bytes != sizeof(bytes)) {
    ...
    console_write("Trait OS: HTTPS strong hardware entropy passed\n");
    console_write(
        "Trait OS: HTTPS TLS 1.2 hostname time trust framing close and teardown passed\n");
    console_write("ST NETWORK production path bounded and recoverable\n");
    kernel_test_pass();
}

static bool native_trait_authority_is_canonical(
    uint8_t *database,
...
_Noreturn void kernel_test_complete_native_trait(void)
{
    ...
    struct native_process_result proof = { 0 };
    struct package_service_report service;
    const struct traitfs_drive_info data = traitfs_drive(TRAITFS_VOLUME_DATA);
    struct traitfs_stat authority;
    struct traitfs_stat output;
    traitfs_handle file;
    uint8_t bytes[sizeof(expected) - 1U];
    size_t read_bytes = 0U;
    ...
    bool matches = true;
    struct network_state network;
    enum traitfs_status authority_status;
    enum native_process_status launch_status;

    if (active_scenario != KERNEL_TEST_NATIVE_TRAIT) {
        kernel_test_fail("native trait completion used outside its scenario");
    }
    if (random_get_state().capability != RANDOM_CAPABILITY_INITIALIZED) {
        kernel_test_fail("native trait did not retain strong hardware entropy");
    }
    if (!data.present || !data.mounted || data.read_only || !data.healthy ||
        ...
        kernel_test_fail("native trait writable ext4 volume is unavailable");
    }
    authority_status = traitfs_stat_path(TRAITFS_VOLUME_DATA,
        PACKAGE_SERVICE_AUTHORITY_PATH, &authority);
    if (authority_status == TRAITFS_STATUS_NOT_FOUND) {
        launch_status = native_process_launch("TRAIT.MAN", &proof);
        if (launch_status != NATIVE_PROCESS_OK ||
            !proof.exited || proof.faulted || proof.exit_status != 0 ||
            !proof.resources_released || proof.peak_handles < 3U ||
            proof.syscall_count < 20U || proof.thread_switches == 0U ||
            !native_process_resources_released()) {
            console_write("ST TRAIT DIAGNOSTIC launch ");
            ...
            console_putc('\n');
            kernel_test_fail("native trait client did not leave a clean census");
        }
        if (!native_trait_authority_is_canonical(database, sizeof(database),
                &service) || service.generation != 1U) {
            kernel_test_fail("native trait installed authority is not canonical");
        }
        if (traitfs_sync(TRAITFS_VOLUME_DATA) != TRAITFS_STATUS_OK ||
            traitfs_unmount(TRAITFS_VOLUME_DATA) != TRAITFS_STATUS_OK ||
            !nvme_filesystem_session_resources_released() ||
            !native_process_resources_released()) {
            kernel_test_fail("native trait ext4 reboot barrier leaked resources");
        }
        console_write(
            "Trait OS: signed HTTPS package install synchronized reboot phase\n");
        cpu_out8(UINT16_C(0x0064), UINT8_C(0xFE));
        kernel_test_fail("platform reset did not restart QEMU");
    }
    if (authority_status != TRAITFS_STATUS_OK || authority.directory ||
        authority.size == 0U) {
        kernel_test_fail("native trait reboot authority is unavailable");
    }
    network = network_get_state();
    if (network.udp_sockets != 0U || network.tcp_connections != 0U ||
        network.timers != 0U) {
        kernel_test_fail("native trait network resources survived teardown");
    }
    if (!native_trait_authority_is_canonical(database, sizeof(database),
            &service)) {
        kernel_test_fail("native trait reboot authority is not canonical");
    }
    if (service.generation == 1U) {
        if (native_process_launch("TRAIT.MAN", &proof) != NATIVE_PROCESS_OK ||
            !proof.exited || proof.faulted || proof.exit_status != 0 ||
            !proof.resources_released || proof.peak_handles < 3U ||
            proof.syscall_count < 20U || proof.thread_switches == 0U ||
            !native_process_resources_released() ||
            !native_trait_authority_is_canonical(database, sizeof(database),
                &service) || service.generation != 2U ||
            traitfs_sync(TRAITFS_VOLUME_DATA) != TRAITFS_STATUS_OK ||
            traitfs_unmount(TRAITFS_VOLUME_DATA) != TRAITFS_STATUS_OK ||
            !nvme_filesystem_session_resources_released()) {
            kernel_test_fail("native trait ext4 update did not commit cleanly");
        }
        console_write(
            "Trait OS: signed HTTPS package update synchronized reboot phase\n");
        cpu_out8(UINT16_C(0x0064), UINT8_C(0xFE));
        kernel_test_fail("platform reset did not restart QEMU");
    }
    if (service.generation != 2U ||
        native_process_launch("TRAIT.MAN", &proof) != NATIVE_PROCESS_OK ||
        !proof.exited || proof.faulted || proof.exit_status != 21 ||
        !proof.resources_released || !native_process_resources_released() ||
        !native_trait_authority_is_canonical(database, sizeof(database),
            &service) || service.generation != 2U) {
        kernel_test_fail("native trait signed rollback was not refused cleanly");
    }
    if (traitfs_truncate(TRAITFS_VOLUME_DATA, damaged_manifest, UINT64_C(1)) !=
            TRAITFS_STATUS_OK ||
        traitfs_sync(TRAITFS_VOLUME_DATA) != TRAITFS_STATUS_OK ||
        ...
        kernel_test_fail("native trait damaged generation was not quarantined");
    }
    console_write(
        "Trait OS: damaged package generation quarantined before repair passed\n");
    if (native_process_launch("TRAITREP.MAN", &proof) != NATIVE_PROCESS_OK ||
        !proof.exited || proof.faulted || proof.exit_status != 0 ||
        !proof.resources_released || proof.peak_handles < 3U ||
        proof.syscall_count < 20U || proof.thread_switches == 0U ||
        !native_process_resources_released() ||
        ...
        proof.thread_switches == 0U || !native_process_resources_released() ||
        traitfs_stat_path(TRAITFS_VOLUME_DATA, state_path, &output) !=
            TRAITFS_STATUS_OK || output.directory ||
        output.size != sizeof(bytes) ||
        traitfs_open(TRAITFS_VOLUME_DATA, state_path,
            TRAITFS_ACCESS_READ, &file) != TRAITFS_STATUS_OK ||
        traitfs_read(file, bytes, sizeof(bytes), &read_bytes) !=
            TRAITFS_STATUS_OK || read_bytes != sizeof(bytes)) {
        kernel_test_fail("native trait authenticated repair did not launch");
    }
    for (size_t index = 0U; index < sizeof(bytes); ++index) {
        matches = matches && bytes[index] == expected[index];
    }
    if (traitfs_close(file) != TRAITFS_STATUS_OK || !matches ||
        ...
        traitfs_unmount(TRAITFS_VOLUME_DATA) != TRAITFS_STATUS_OK ||
        !nvme_filesystem_session_resources_released()) {
        kernel_test_fail("native trait upstream SDL launch did not cleanly sync");
    }
    console_write(
        "Trait OS: damaged SDL package repaired authenticated and launched from writable ext4 passed\n");
    console_write("ST NETWORK production path bounded and recoverable\n");
    kernel_test_pass();
}
...
static uint32_t trait_proof_pixel(uint32_t x, uint32_t y)
{
    uint32_t pixel = 0U;
    ...
    if (surface == NULL ||
        ...
        kernel_test_fail("Trait OS cached-surface pixel read failed");
    }
    return pixel;
}

static void trait_proof_process_ui(const char *failure)
{
...
static void trait_proof_inject_pointer(
    uint8_t flags,
    int32_t delta_x,
    int32_t delta_y,
    const char *failure
)
{
    ...
    trait_proof_process_ui(failure);
}

static void trait_proof_move_pointer(
    uint32_t target_x,
    uint32_t target_y,
    const char *failure
)
{
    for (uint32_t packets = 0U; packets < 64U; ++packets) {
        ...
        trait_proof_inject_pointer(0U, delta_x, delta_y, failure);
    }
    kernel_test_fail("Trait OS cursor did not reach its UI target");
}

_Noreturn void kernel_test_complete_trait_proof(void)
{
    const struct boot_ledger *ledger = boot_ledger_installed();
    ...
    if (active_scenario != KERNEL_TEST_TRAIT_PROOF) {
        kernel_test_fail("Trait OS completion used outside its scenario");
    }
    if (ledger == NULL || !ledger->validated || !ledger->executed ||
            ledger->status != BOOT_LEDGER_STATUS_OK || ledger->degraded ||
            !boot_ledger_fingerprint_valid(ledger)) {
        kernel_test_fail("Trait OS installed ledger is invalid");
    }
    ...
    if (font == NULL || layout == NULL || construction == NULL ||
        ...
        kernel_test_fail("Trait OS required desktop receipt is invalid");
    }
    if (!boot_ledger_has_capability(ledger,
        ...
        kernel_test_fail("Trait OS installed desktop capability is missing");
    }
    if (!ui->active || !ui->pointer_present || !ui->ledger_pass ||
        ...
        kernel_test_fail("Trait OS installed desktop state is incomplete");
    }
    if (trait_proof_pixel(512U, 250U) == 0U ||
            trait_proof_pixel(512U, 767U) == 0U) {
        kernel_test_fail("Trait OS wallpaper or panel is not integrated");
    }

    trait_proof_move_pointer(200U, 160U,
        "Trait OS cursor did not move over the new desktop");
    if (ui_get_state()->renders.cursor_moves <= initial_renders.cursor_moves) {
        kernel_test_fail("Trait OS cursor movement was not recorded");
    }

    if (ui_handle_keyboard(&keyboard) != UI_STATUS_OK) {
        kernel_test_fail("Trait OS focused window did not close");
    }
    trait_proof_process_ui("Trait OS close redraw failed");
    keyboard.scancode = 0x0FU;
    if (ui_handle_keyboard(&keyboard) != UI_STATUS_OK) {
        kernel_test_fail("Trait OS keyboard focus-next failed");
    }
    trait_proof_process_ui("Trait OS focus-next redraw failed");
    if (ui_get_state()->focus != UI_ELEMENT_DOCK_TERMINAL) {
        kernel_test_fail("Trait OS keyboard focus-next chose wrong app");
    }
    keyboard.shift = true;
    if (ui_handle_keyboard(&keyboard) != UI_STATUS_OK) {
        kernel_test_fail("Trait OS keyboard focus-previous failed");
    }
    trait_proof_process_ui("Trait OS focus-previous redraw failed");
    if (ui_get_state()->focus != UI_ELEMENT_DOCK_FILES) {
        kernel_test_fail("Trait OS keyboard focus-previous chose wrong app");
    }
    keyboard.scancode = 0x1CU;
    keyboard.shift = false;
    if (ui_handle_keyboard(&keyboard) != UI_STATUS_OK) {
        kernel_test_fail("Trait OS keyboard activation failed");
    }
    trait_proof_process_ui("Trait OS application redraw failed");
    if (ui_get_state()->active_panel != UI_PANEL_FILES) {
        kernel_test_fail("Trait OS Files window did not open");
    }
    console_serial_write("ST TRAIT DE keyboard and pointer passed\n");

    if (!boot_plan_pointer_absence_self_test()) {
        kernel_test_fail("Trait OS pointer-absence synthetic plan failed");
    }
    ...
    if (proof.width != 1024U || proof.height != 768U ||
        ...
        kernel_test_fail("Trait OS final desktop proof is inconsistent");
    }

    console_write("ST TRAIT_PROOF geometry ");
...
_Noreturn void kernel_test_complete_linux_abi(void)
{
    ...
    if (proof.file_bytes != LINUX_ABI_IMAGE_BYTES ||
        ...
        proof.stdout_bytes != 6U || proof.syscall_count != 9U ||
...
static bool focus_trait_proof_terminal(void)
{
...
static bool installed_trait_proof_ready(void)
{
    const struct boot_ledger *ledger = boot_ledger_installed();
...
_Noreturn void kernel_test_complete_trait_proof_userland(void)
{
    ...
    if (active_scenario != KERNEL_TEST_TRAIT_PROOF_USERLAND ||
        !installed_trait_proof_ready() || !shell_is_active()) {
        kernel_test_fail("Trait OS userspace prerequisites are incomplete");
    }
    cpu_interrupt_enable();
    console_write("\n");
    console_write("trait> ");
    if (!feed_shell_line("linux unsupported") ||
        ...
        !feed_shell_line("linux echo") ||
        !feed_shell_line("linux uname")) {
        kernel_test_fail("Trait OS shell input injection was refused");
    }
    ...
    if (shell_get_state().commands != before.commands + 6U ||
        ...
        kernel_test_fail("Trait OS userspace relaunch contract failed");
    }
    console_write("\nST TRAIT_PROOF_USERLAND shell production echo 2 uname 2 ");
    console_write("invalid-profile recovered CPL3 SYSCALL stdout exact exit 0 ");
    console_write("teardown clean prompt restored\n");
    kernel_test_pass();
}

_Noreturn void kernel_test_complete_trait_proof_userland_absent(void)
{
    const struct shell_state before = shell_get_state();
    const uint32_t echo_before =
        linux_userland_completed(LINUX_USERLAND_PROFILE_ECHO);

    if (active_scenario != KERNEL_TEST_TRAIT_PROOF_USERLAND_ABSENT ||
        !installed_trait_proof_ready() || !shell_is_active()) {
    ...
    console_write("\nST TRAIT_PROOF_USERLAND_ABSENT concise refusal prompt usable ");
    console_write("teardown clean\n");
    kernel_test_pass();
}

_Noreturn void kernel_test_complete_trait_proof_userland_interactive(void)
{
    const struct shell_state before = shell_get_state();
    ...
    if (active_scenario != KERNEL_TEST_TRAIT_PROOF_USERLAND_INTERACTIVE ||
        !installed_trait_proof_ready() || !shell_is_active() ||
    ...
    console_write("\nST TRAIT_PROOF_USERLAND_INTERACTIVE cat 2 keyboard IRQ ");
...
_Noreturn void kernel_test_complete_trait_proof_userland_interactive_absent(
    void
)
{
    const struct shell_state before = shell_get_state();
    ...
    if (active_scenario !=
            KERNEL_TEST_TRAIT_PROOF_USERLAND_INTERACTIVE_ABSENT ||
        !installed_trait_proof_ready() || !shell_is_active() ||
        !keyboard_is_initialized()) {
    ...
    console_write("\nST TRAIT_PROOF_USERLAND_INTERACTIVE_ABSENT cat missing ");
...
)
{
    ...
    if (path == NULL || buffer == NULL || file_bytes == NULL ||
        traitfs_stat_path(TRAITFS_VOLUME_DATA, path, &stat) != TRAITFS_STATUS_OK ||
        stat.directory || stat.size > capacity ||
        traitfs_open(TRAITFS_VOLUME_DATA, path, TRAITFS_ACCESS_READ, &handle) !=
            TRAITFS_STATUS_OK) {
        return false;
    }
    status = traitfs_read(handle, buffer, capacity, &read_bytes);
    if (traitfs_close(handle) != TRAITFS_STATUS_OK) {
        return false;
    }
    *file_bytes = read_bytes;
    return status == TRAITFS_STATUS_OK && read_bytes == stat.size;
}
...
static void fat32_feed(const char *line)
{
    if (!feed_shell_line(line)) {
        kernel_test_fail("Trait OS refused a FAT32 command line");
    }
}

static void fat32_require_base(bool data_required)
{
    struct traitfs_drive_info system = traitfs_drive(TRAITFS_VOLUME_SYSTEM);
    struct traitfs_drive_info data = traitfs_drive(TRAITFS_VOLUME_DATA);

    if (!installed_trait_proof_ready() || !shell_is_active() ||
...
static void fat32_nested_scenario(void)
{
    ...
    if (!fat32_file_equals("projects/cuts/notes.txt", expected,
            sizeof(expected) - 1U) ||
        traitfs_stat_path(TRAITFS_VOLUME_DATA, "projects/cuts", &stat) !=
            TRAITFS_STATUS_OK || !stat.directory) {
...
static void fat32_growth_scenario(void)
{
    ...
    if (traitfs_stat_path(TRAITFS_VOLUME_DATA, "growth.bin", &stat) !=
            TRAITFS_STATUS_OK || stat.size != 568U || stat.cluster_count != 2U ||
...
static void fat32_truncate_scenario(void)
{
    ...
    if (traitfs_stat_path(TRAITFS_VOLUME_DATA, "trim.bin", &stat) !=
            TRAITFS_STATUS_OK || stat.size != 100U || stat.cluster_count != 1U) {
...
static void fat32_rename_scenario(void)
{
    ...
    if (traitfs_rename(TRAITFS_VOLUME_DATA, "a", "a/child/a") !=
            TRAITFS_STATUS_PATH) {
    ...
    if (traitfs_rename(TRAITFS_VOLUME_DATA, "conflict.txt",
            "archive/moved.txt") != TRAITFS_STATUS_EXISTS) {
        kernel_test_fail("FAT32 rename conflict was not rejected");
    }
    fat32_feed("ls archive");
    if (traitfs_stat_path(TRAITFS_VOLUME_DATA, "a/note.txt", &stat) !=
            TRAITFS_STATUS_NOT_FOUND ||
...
{
    struct traitfs_stat first;
    struct traitfs_stat second;

    fat32_require_base(true);
    fat32_feed("write first.bin \"one\"");
    if (traitfs_stat_path(TRAITFS_VOLUME_DATA, "first.bin", &first) !=
            TRAITFS_STATUS_OK) {
    ...
    if (traitfs_stat_path(TRAITFS_VOLUME_DATA, "second.bin", &second) !=
            TRAITFS_STATUS_OK || first.first_cluster != second.first_cluster ||
        traitfs_rmdir(TRAITFS_VOLUME_DATA, "kept") != TRAITFS_STATUS_NOT_EMPTY) {
...
static void fat32_full_scenario(void)
{
    ...
    fat32_require_base(true);
    if (traitfs_drive(TRAITFS_VOLUME_DATA).free_bytes != 0U) {
        kernel_test_fail("full FAT32 fixture retained free clusters");
    }
    fat32_feed("write recovery.txt \"blocked\"");
    if (traitfs_stat_path(TRAITFS_VOLUME_DATA, "recovery.txt", &stat) !=
            TRAITFS_STATUS_OK || stat.size != 0U) {
    ...
    fat32_feed("sync");
    if (traitfs_stat_path(TRAITFS_VOLUME_DATA, "recovery.txt", &stat) !=
            TRAITFS_STATUS_OK || stat.size != 10U) {
...
static void fat32_persistence_scenario(void)
{
    ...
    fat32_require_base(true);
    status = traitfs_stat_path(TRAITFS_VOLUME_DATA, "projects/notes.txt", &stat);
...
static void fat32_immutable_scenario(void)
{
    ...
    fat32_require_base(true);
    fat32_feed("linux echo");
    if (traitfs_create(TRAITFS_VOLUME_SYSTEM, "ATTACK.TXT") !=
            TRAITFS_STATUS_READ_ONLY ||
        traitfs_open(TRAITFS_VOLUME_SYSTEM, "BUSYBOX", TRAITFS_ACCESS_WRITE,
            &handle) != TRAITFS_STATUS_READ_ONLY ||
...
{
    traitfs_handle handles[TRAITFS_MAX_HANDLES];
    traitfs_handle extra;
    size_t bytes = 0U;
    uint8_t byte = 0U;

    fat32_require_base(true);
    fat32_feed("write handle.txt \"generation\"");
    if (traitfs_open(TRAITFS_VOLUME_DATA, "handle.txt", TRAITFS_ACCESS_READ,
            &handles[0]) != TRAITFS_STATUS_OK ||
        traitfs_write(handles[0], &byte, 1U, &bytes) != TRAITFS_STATUS_ACCESS ||
        traitfs_read(handles[0], NULL, 1U, &bytes) !=
            TRAITFS_STATUS_INVALID_ARGUMENT ||
        traitfs_unlink(TRAITFS_VOLUME_DATA, "handle.txt") != TRAITFS_STATUS_BUSY ||
        traitfs_close(handles[0]) != TRAITFS_STATUS_OK ||
        traitfs_close(handles[0]) != TRAITFS_STATUS_STALE_HANDLE ||
        traitfs_read(handles[0], &byte, 1U, &bytes) !=
            TRAITFS_STATUS_STALE_HANDLE) {
        kernel_test_fail("FAT32 handle generation controls changed");
    }
    for (size_t index = 0U; index < TRAITFS_MAX_HANDLES; ++index) {
        if (traitfs_open(TRAITFS_VOLUME_DATA, "handle.txt", TRAITFS_ACCESS_READ,
                &handles[index]) != TRAITFS_STATUS_OK) {
            kernel_test_fail("FAT32 handle table filled early");
        }
    }
    if (traitfs_open(TRAITFS_VOLUME_DATA, "handle.txt", TRAITFS_ACCESS_READ,
            &extra) != TRAITFS_STATUS_NO_HANDLES) {
        kernel_test_fail("FAT32 handle table exceeded its fixed bound");
    }
    for (size_t index = 0U; index < TRAITFS_MAX_HANDLES; ++index) {
        if (traitfs_close(handles[index]) != TRAITFS_STATUS_OK) {
...
static const uint8_t network_welcome[] =
    "hello from the Trait OS network\n";
...
static void network_syscall_http_download_scenario(void)
{
    ...
    static const char url[] = "http://trait.test/welcome.txt";
...
static const uint8_t network_listen_request[] = "TRAIT LISTEN\n";
...
static void network_download(const char *destination)
{
    ...
    if (network_http_download(NETWORK_TEST_OWNER,
            "http://trait.test/welcome.txt", destination, false,
...
_Noreturn void kernel_test_complete_network(void)
{
    ...
    case KERNEL_TEST_NETWORK_DNS_CNAME: {
        ...
        network_require_dhcp();
        if (network_resolve("trait.test", &address,
                NETWORK_DEFAULT_OPERATION_TIMEOUT_NS) != NETWORK_STATUS_OK ||
    ...
    case KERNEL_TEST_NETWORK_DNS_MALFORMED: {
        ...
        network_require_dhcp();
        status = network_resolve("trait.test", &address,
            UINT64_C(1000000000));
        if (status != NETWORK_STATUS_TIMEOUT &&
    ...
    case KERNEL_TEST_NETWORK_HTTP_CHUNKED:
        network_http_download_scenario(
            "http://trait.test/welcome.txt", "HTTPCHNK.TXT", true, 0U);
        break;
    case KERNEL_TEST_NETWORK_HTTP_REDIRECT:
        network_http_download_scenario(
            "http://trait.test/start", "HTTPREDR.TXT", false, 1U);
        break;
    case KERNEL_TEST_NETWORK_HTTP_MALFORMED: {
        struct network_http_result result;
        struct traitfs_stat stat;
        ...
        network_require_dhcp();
        if (network_http_download(NETWORK_TEST_OWNER,
                "http://trait.test/welcome.txt", "BADHTTP.TXT", false,
                UINT64_C(5000000000), &result) == NETWORK_STATUS_OK ||
            traitfs_stat_path(TRAITFS_VOLUME_DATA, "BADHTTP.TXT", &stat) !=
                TRAITFS_STATUS_NOT_FOUND) {
    ...
    case KERNEL_TEST_NETWORK_HTTP_NESTED:
        fat32_require_base(true);
        if (traitfs_mkdir(TRAITFS_VOLUME_DATA, "DOWNLDS") != TRAITFS_STATUS_OK &&
            traitfs_stat_path(TRAITFS_VOLUME_DATA, "DOWNLDS",
                &(struct traitfs_stat){0}) != TRAITFS_STATUS_OK) {
    ...
    case KERNEL_TEST_NETWORK_HTTP_DISK_FULL: {
        ...
        status = network_http_download(NETWORK_TEST_OWNER,
            "http://trait.test/welcome.txt", "full.txt", false,
    ...
    case KERNEL_TEST_NETWORK_SYSTEM_IMMUTABLE: {
        ...
        fat32_require_base(true);
        network_http_download_scenario(
            "http://trait.test/welcome.txt", "IMMUTABL.TXT",
            false, 0U);
        if (traitfs_open(TRAITFS_VOLUME_SYSTEM, "BUSYBOX", TRAITFS_ACCESS_WRITE,
                &handle) != TRAITFS_STATUS_READ_ONLY) {
    ...
    case KERNEL_TEST_NETWORK_MISSING_LINUX_CAT:
        ...
        break;
    case KERNEL_TEST_NETWORK_FILES: {
        struct traitfs_list_entry entries[4];
        size_t count = 0U;

        fat32_require_base(true);
        network_require_dhcp();
        if (traitfs_list(TRAITFS_VOLUME_DATA, ".", entries, 4U, &count) !=
                TRAITFS_STATUS_OK || ui_flush() != UI_STATUS_OK) {
    ...
    case KERNEL_TEST_NETWORK_PERSISTENCE: {
        ...
        fat32_require_base(true);
        status = traitfs_stat_path(TRAITFS_VOLUME_DATA, "network.txt", &stat);
        if (status == TRAITFS_STATUS_NOT_FOUND) {
            network_download("network.txt");
            if (traitfs_unmount(TRAITFS_VOLUME_DATA) != TRAITFS_STATUS_OK) {
...
const char *kernel_test_scenario_name(enum kernel_test_scenario scenario)
{
    ...
    case KERNEL_TEST_TRAIT_PROOF_USERLAND:
        return "trait-proof-userland";
    case KERNEL_TEST_TRAIT_PROOF_USERLAND_ABSENT:
        return "trait-proof-userland-absent";
    case KERNEL_TEST_TRAIT_PROOF_USERLAND_INTERACTIVE:
        return "trait-proof-userland-interactive";
    case KERNEL_TEST_TRAIT_PROOF_USERLAND_INTERACTIVE_ABSENT:
        return "trait-proof-userland-interactive-absent";
    case KERNEL_TEST_FAT32_SYSTEM:
        return "fat32-system";
    case KERNEL_TEST_FAT32_DATA:
        return "fat32-data";
