...
extern uint32_t trait_nvbios_self_test(void);
extern uint32_t trait_nvbios_controls(void);
extern size_t trait_nvbios_reference(uint8_t *out, size_t capacity);
extern int trait_nvbios_parse(
    const uint8_t *input,
    size_t input_len,
...
)
{
    ...
    status = trait_nvbios_parse(vbios_window, sizeof(vbios_window), &image);
 ...
 * register contract Trait OS's ICH9 driver already proves: a version, and a
 ...
 * having agreed to it. Nothing in Trait OS enabled them on this function and
 * nothing outside Trait OS may have: this driver's whole job is to say so out
...
)
{
     ...
     * claim, no mapping, no command-register change. Trait OS has no IOMMU, and
...
static bool reference_vbios_agrees(void)
{
    ...
    if (trait_nvbios_reference(written, sizeof(written)) !=
            sizeof(written)) {
        return false;
    }
...
bool nvidia_foundation_self_test(size_t *completed_tests)
{
    ...
    *completed_tests = 0U;
    ...
    if (trait_nvbios_controls() != NVIDIA_VBIOS_ROBUSTNESS_CONTROLS ||
        trait_nvbios_self_test() != NVIDIA_VBIOS_ROBUSTNESS_CONTROLS) {
        return false;
    }
    ++completed;
    ...
    zero_bytes(&image, sizeof(image));
    if (trait_nvbios_parse(reference, reference_length, &image) != 0 ||
    ...
    zero_bytes(&image, sizeof(image));
    if (trait_nvbios_parse(reference, 16U, &image) == 0 ||
    ...
    /* So is a null one. */
    zero_bytes(&image, sizeof(image));
    if (trait_nvbios_parse(NULL, reference_length, &image) == 0) {
        return false;
    }
    ++completed;
