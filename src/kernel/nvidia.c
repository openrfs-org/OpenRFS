...
extern uint32_t opengat_nvbios_self_test(void);
extern uint32_t opengat_nvbios_controls(void);
extern size_t opengat_nvbios_reference(uint8_t *out, size_t capacity);
extern int opengat_nvbios_parse(
    const uint8_t *input,
    size_t input_len,
...
)
{
    ...
    status = opengat_nvbios_parse(vbios_window, sizeof(vbios_window), &image);
 ...
 * register contract OpenGAT's ICH9 driver already proves: a version, and a
 ...
 * having agreed to it. Nothing in OpenGAT enabled them on this function and
 * nothing outside OpenGAT may have: this driver's whole job is to say so out
...
)
{
     ...
     * claim, no mapping, no command-register change. OpenGAT has no IOMMU, and
...
static bool reference_vbios_agrees(void)
{
    ...
    if (opengat_nvbios_reference(written, sizeof(written)) !=
            sizeof(written)) {
        return false;
    }
...
bool nvidia_foundation_self_test(size_t *completed_tests)
{
    ...
    *completed_tests = 0U;
    ...
    if (opengat_nvbios_controls() != NVIDIA_VBIOS_ROBUSTNESS_CONTROLS ||
        opengat_nvbios_self_test() != NVIDIA_VBIOS_ROBUSTNESS_CONTROLS) {
        return false;
    }
    ++completed;
    ...
    zero_bytes(&image, sizeof(image));
    if (opengat_nvbios_parse(reference, reference_length, &image) != 0 ||
    ...
    zero_bytes(&image, sizeof(image));
    if (opengat_nvbios_parse(reference, 16U, &image) == 0 ||
    ...
    /* So is a null one. */
    zero_bytes(&image, sizeof(image));
    if (opengat_nvbios_parse(NULL, reference_length, &image) == 0) {
        return false;
    }
    ++completed;
