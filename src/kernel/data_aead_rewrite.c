...
static enum data_aead_status migrate_plain_shadow_impl(
    const uint8_t key[DATA_AEAD_KEY_BYTES], const char *canonical_path,
    uint64_t old_plaintext_bytes,
    const uint8_t supplied_stable_id[DATA_AEAD_ID_BYTES],
    const uint8_t supplied_revision_id[DATA_AEAD_ID_BYTES],
    uint64_t generation, const struct data_aead_rewrite_io *io,
    ...
    uint64_t *new_physical_bytes)
{
    ...
    if (key == NULL || canonical_path == NULL || io == NULL ||
            ...
            workspace == NULL ||
            workspace_bytes < DATA_AEAD_REWRITE_WORKSPACE_BYTES ||
            new_physical_bytes == NULL || generation == 0U ||
            (supplied_stable_id == NULL) !=
                (supplied_revision_id == NULL)) {
        if (workspace != NULL) crypto_wipe(workspace,
            workspace_bytes < DATA_AEAD_REWRITE_WORKSPACE_BYTES ?
...
enum data_aead_status data_aead_migrate_plain_shadow(
    ...
    uint64_t *new_physical_bytes)
{
    return migrate_plain_shadow_impl(key, canonical_path,
        old_plaintext_bytes, NULL, NULL, 1U, io, workspace,
        workspace_bytes, new_physical_bytes);
}
...
{
    return migrate_plain_shadow_impl(key, canonical_path,
        old_plaintext_bytes, stable_id, revision_id, generation, io,
