...
/* ACPI 6.6 tables 5.22 and 5.26 define the flag fields Trait OS accepts. */
 ...
 * ISA bus, whose sixteen legacy IRQs Trait OS already owns through the 8259 pair.
 */
 ...
 * type Trait OS does not model yet.
 */
...
static bool apic_addresses_are_bounded(void)
{
    ...
    write_u64(entry + 4U, TRAIT_EARLY_PHYSICAL_LIMIT);
    fixture_seal(&fixture);
    madt = fixture_madt(&fixture);
...
static bool recorded_table_is_revalidated(void)
{
    ...
    madt = fixture_madt(&fixture);
    madt.physical_address = TRAIT_EARLY_PHYSICAL_LIMIT - madt.length + 1U;
