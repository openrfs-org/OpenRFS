...
/* ACPI 6.6 tables 5.22 and 5.26 define the flag fields OpenGAT accepts. */
 ...
 * ISA bus, whose sixteen legacy IRQs OpenGAT already owns through the 8259 pair.
 */
 ...
 * type OpenGAT does not model yet.
 */
...
static bool apic_addresses_are_bounded(void)
{
    ...
    write_u64(entry + 4U, OPENGAT_EARLY_PHYSICAL_LIMIT);
    fixture_seal(&fixture);
    madt = fixture_madt(&fixture);
...
static bool recorded_table_is_revalidated(void)
{
    ...
    madt = fixture_madt(&fixture);
    madt.physical_address = OPENGAT_EARLY_PHYSICAL_LIMIT - madt.length + 1U;
