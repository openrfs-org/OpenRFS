...
_Static_assert(TRAIT_PAGE_SIZE == NVME_BLOCK_BYTES,
    "the bounded fixture uses Trait OS's one 4 KiB DMA page");
...
)
{
    ...
    if ((base & (TRAIT_PAGE_SIZE - 1U)) != 0U || depth < 2U ||
...
static enum nvme_status prepare_dma(struct nvme_runtime *controller)
{
    ...
    controller->read.data_offset =
        (uint64_t)NVME_READ_DATA_PAGE * TRAIT_PAGE_SIZE;
...
)
{
    ...
    if (allocation == NULL || !allocation->active || length == 0U ||
        ...
        (address & (TRAIT_PAGE_SIZE - 1U)) != 0U ||
        length > TRAIT_PAGE_SIZE ||
        (address & ~(TRAIT_PAGE_SIZE - 1U)) !=
            ((address + length - 1U) & ~(TRAIT_PAGE_SIZE - 1U))) {
...
bool nvme_foundation_self_test(size_t *completed_tests)
{
    ...
    *completed_tests = 0U;
    ...
    if (!test_record(validate_queue_geometry(UINT64_C(0x2000), 16U,
        ...
        validate_queue_geometry(UINT64_C(0x2000), TRAIT_PAGE_SIZE,
    ...
    struct dma_allocation synthetic_dma = {
        ...
        .byte_length = TRAIT_PAGE_SIZE * 2U,
        .owner = DMA_OWNER_CPU,
        .active = true
    };
    if (!test_record(validate_prp(&synthetic_dma, TRAIT_PAGE_SIZE,
            TRAIT_PAGE_SIZE) == NVME_STATUS_OK &&
        validate_prp(&synthetic_dma, TRAIT_PAGE_SIZE - 4U, 8U) ==
            NVME_STATUS_PRP_INVALID &&
        validate_prp(&synthetic_dma, TRAIT_PAGE_SIZE * 2U,
            TRAIT_PAGE_SIZE) == NVME_STATUS_PRP_INVALID, &completed)) {
        return false;
    }
...
const char *nvme_status_string(enum nvme_status status)
{
    ...
    case NVME_STATUS_UNSUPPORTED_PAGE_SIZE:
        return "NVMe controller cannot use Trait OS's page size";
