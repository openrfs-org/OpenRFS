...
)
{
    ...
    if (record == NULL || record->handle != allocation ||
        ...
        record->frames.page_count > UINT64_MAX / OPENGAT_PAGE_SIZE ||
        allocation->byte_length !=
            (uint64_t)record->frames.page_count * OPENGAT_PAGE_SIZE ||
...
)
{
    ...
    allocation->byte_length =
        (uint64_t)allocation->frames.page_count * OPENGAT_PAGE_SIZE;
...
bool dma_self_test(void)
{
    struct dma_request request = {
        .page_count = 2U,
        .alignment = OPENGAT_PAGE_SIZE * 2U,
    ...
    request.alignment = OPENGAT_PAGE_SIZE + 1U;
    if (dma_allocate(&request, &allocation) !=
            DMA_STATUS_FRAME_ALLOCATION_FAILURE) {
        return false;
    }

    request.alignment = OPENGAT_EARLY_PHYSICAL_LIMIT * 2U;
    if (dma_allocate(&request, &allocation) !=
            DMA_STATUS_FRAME_ALLOCATION_FAILURE) {
        return false;
    }
    ...
    request.alignment = OPENGAT_PAGE_SIZE * 2U;
