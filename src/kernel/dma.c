...
)
{
    ...
    if (record == NULL || record->handle != allocation ||
        ...
        record->frames.page_count > UINT64_MAX / TRAIT_PAGE_SIZE ||
        allocation->byte_length !=
            (uint64_t)record->frames.page_count * TRAIT_PAGE_SIZE ||
...
)
{
    ...
    allocation->byte_length =
        (uint64_t)allocation->frames.page_count * TRAIT_PAGE_SIZE;
...
bool dma_self_test(void)
{
    ...
    request.alignment = TRAIT_PAGE_SIZE + 1U;
    if (dma_allocate(&request, &allocation) !=
            DMA_STATUS_FRAME_ALLOCATION_FAILURE) {
        return false;
    }

    request.alignment = TRAIT_EARLY_PHYSICAL_LIMIT * 2U;
    if (dma_allocate(&request, &allocation) !=
            DMA_STATUS_FRAME_ALLOCATION_FAILURE) {
        return false;
    }
    ...
    request.alignment = TRAIT_PAGE_SIZE * 2U;
