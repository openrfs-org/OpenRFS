 ...
 *
 * Every subsystem in OpenRFS carries a self-test that runs on synthetic data,
...
void prove_level_route(void)
{
    ...
    console_write("OpenRFS: I/O APIC level route id ");
    ...
    console_write("OpenRFS: I/O APIC level deliveries ");
...
void prove_apic_timer(void)
{
    ...
    console_write("OpenRFS: local APIC timer calibrated at ");
...
void prove_tsc(void)
{
    ...
    tsc = tsc_get_state();
    console_write("OpenRFS: TSC calibrated at ");
...
void prove_pm_timer(void)
{
    ...
    console_write("OpenRFS: PM timer counted ");
...
void prove_clocks_without_pit(void)
{
    ...
    console_write("OpenRFS: clocks agree: PM ");
...
void prove_monotonic_time(void)
{
    ...
    console_write("OpenRFS: monotonic clock on ");
     ...
     */
    console_write("OpenRFS: deadline table of ");
...
void install_page_tables(const struct paging_device_windows *device_windows)
{
    ...
    console_write("OpenRFS: paging root ");
    ...
    console_write("OpenRFS: paging leaves ");
    ...
    if (status != PAGING_STATUS_OK) {
        ...
        console_write("OpenRFS: installed device-window proof failed: ");
    ...
    console_write("OpenRFS: kernel page tables installed\n");
    console_write("OpenRFS: no writable executable mapping\n");
}
...
)
{
    ...
    console_write("OpenRFS: IA32_PAT before ");
    ...
    console_write("OpenRFS: framebuffer memory type ");
    ...
    console_putc('\n');
    console_write("OpenRFS: write-combining established\n");
}
...
void prove_paging_lifecycle(void)
{
    ...
    *probe = UINT8_C(0xA5);
    ...
    status = paging_unmap(PAGING_PROBE_ADDRESS, OPENRFS_PAGE_SIZE);
...
void bring_up_heap(void)
{
    ...
    heap = heap_get_state();
    console_write("OpenRFS: heap window ");
...
void prove_heap_lifecycle(void)
{
    ...
    heap = heap_get_state();
    console_write("OpenRFS: heap committed ");
    ...
    console_write("OpenRFS: kernel heap online\n");
    console_write("OpenRFS: heap coalesced to one free block\n");
}
...
void bring_up_pci(const struct acpi_mcfg *mcfg, bool present)
{
    ...
    pci = pci_get_state();
    console_write("OpenRFS: PCI mechanism 1 online, ");
    ...
    console_putc('\n');
    console_write("OpenRFS: PCI buses ");
    ...
    console_write("OpenRFS: PCI configuration space enumerated\n");
     ...
     */
    console_write("OpenRFS: PCI mechanisms agree on ");
...
void prove_threads(void)
{
    ...
    console_write("OpenRFS: threads online, ");
    ...
    console_write("OpenRFS: thread rotation ");
    ...
    console_write("OpenRFS: threads switched ");
    ...
    console_write("OpenRFS: kernel threads established\n");
}
...
void prove_framebuffer(const struct boot_framebuffer *framebuffer)
{
    ...
    /*
     * A loader that set no graphics mode is not a failure. OpenRFS has run on
    ...
    if (status == FRAMEBUFFER_STATUS_ABSENT) {
        console_write("OpenRFS: no framebuffer, serial console only\n");
        return;
    }
    ...
    console_write("OpenRFS: framebuffer ");
    ...
    console_write("OpenRFS: framebuffer verified ");
    ...
    console_write("OpenRFS: framebuffer established\n");
}
...
void prove_surface(void)
{
    ...
    console_write(" bytes\n");
    console_write("OpenRFS: surface cycles full present ");
    ...
    console_putc('\n');
    console_write("OpenRFS: surface split cycles full draw ");
    ...
    console_putc('\n');
    console_write("OpenRFS: surface sparse two-corner cycles total ");
    ...
    console_putc('\n');
    console_write("OpenRFS: surface copied ");
    ...
    console_write("OpenRFS: cached surface established\n");
}
...
void draw_logo(void)
{
    ...
    int32_t status = openrfs_logo_geometry(&width, &height);

    if (status != LOGO_STATUS_OK) {
        console_panic(logo_status_string(status));
    }

    console_write("OpenRFS: logo ");
    console_write_u64(width);
    console_putc('x');
    console_write_u64(height);
    console_write(" from ");
    console_write_u64(openrfs_logo_size());
     ...
     */
    if (openrfs_logo_decode(decoded, (size_t)((uint64_t)width * height - 1U),
    ...
    if (openrfs_logo_decode(NULL, (size_t)((uint64_t)width * height),
            screen.red_position, screen.green_position, screen.blue_position,
            background) != LOGO_STATUS_NULL_ARGUMENT) {
        console_panic("the logo decoder accepted a null buffer");
    }

    status = openrfs_logo_decode(decoded, (size_t)((uint64_t)width * height),
        screen.red_position, screen.green_position, screen.blue_position,
        background);
    ...
    console_write("OpenRFS: logo verified ");
    ...
    console_write("OpenRFS: logo established\n");
}
...
void prove_preemption(void)
{
    ...
    console_write("OpenRFS: preempted ");
    ...
    console_write("OpenRFS: unyielding threads ran");
    ...
    console_write("OpenRFS: preemption established\n");
}
...
void prove_frame_lifecycle(void)
{
    ...
    if (first_frame == second_frame ||
        (first_frame & (OPENRFS_PAGE_SIZE - 1U)) != 0U ||
        (second_frame & (OPENRFS_PAGE_SIZE - 1U)) != 0U) {
        console_panic("frame allocator returned an invalid address");
    }

    console_write("OpenRFS: frame probe: ");
...
{
    static const char sample[] = "OpenRFS";
    ...
    console_write("OpenRFS: screen console ");
    ...
    console_write(", font ");
    console_write_u64(openrfs_font_size());
    console_write(" bytes\n");
    ...
    after = screen_get_state();
    console_write("OpenRFS: screen console drew ");
    console_write_u64(after.characters);
    console_write(" characters and scrolled ");
    console_write_u64(after.scrolls);
    console_write(" times\n");
    console_write("OpenRFS: screen console established\n");
}

/*
 * The keyboard, proved without a person at the machine.
 *
 * Every other device OpenRFS brings up either announces itself or can be asked a
...
void prove_keyboard(void)
{
    ...
    console_write("OpenRFS: keyboard 8042 online, IRQ 1 routed, ");
    ...
    console_write("OpenRFS: keyboard decoded \"");
    for (size_t index = 0; index < characters; ++index) {
        console_putc(seen[index]);
    }
    console_write("\" from injected scancodes\n");
    console_write("OpenRFS: keyboard established\n");
}
...
void prove_shell(void)
{
    ...
    static const char prompt[] = "openrfs$ ";
     ...
     */
    console_putc('\n');
    console_write("OpenRFS: shell ran \"");
    console_write(echoed);
    console_write("\" from ");
    console_write_u64(sizeof(typed));
    console_write(" injected scancodes\n");
    console_write("OpenRFS: shell output verified on screen\n");
    console_write("OpenRFS: shell established\n");
}
