...
#define FRAME_COUNT ((size_t)(TRAIT_EARLY_PHYSICAL_LIMIT / TRAIT_PAGE_SIZE))
...
{
    const uint64_t page_mask = TRAIT_PAGE_SIZE - 1U;
    uint64_t end;
    ...
    if (length == 0U || base >= TRAIT_EARLY_PHYSICAL_LIMIT) {
        *first_frame = 0;
        *past_last_frame = 0;
        return FRAME_STATUS_OK;
    }

    clipped_end = end < TRAIT_EARLY_PHYSICAL_LIMIT
        ? end
        : TRAIT_EARLY_PHYSICAL_LIMIT;
    ...
    *first_frame = (size_t)(aligned_base / TRAIT_PAGE_SIZE);
    *past_last_frame = (size_t)((clipped_end & ~page_mask) / TRAIT_PAGE_SIZE);
    return FRAME_STATUS_OK;
}
...
)
{
    ...
    if (base >= TRAIT_EARLY_PHYSICAL_LIMIT) {
    ...
    clipped_end = end < TRAIT_EARLY_PHYSICAL_LIMIT
        ? end
        : TRAIT_EARLY_PHYSICAL_LIMIT;
    *first_frame = (size_t)(base / TRAIT_PAGE_SIZE);
    *past_last_frame = (size_t)(clipped_end / TRAIT_PAGE_SIZE);

    if ((clipped_end & (TRAIT_PAGE_SIZE - 1U)) != 0U) {
        ++*past_last_frame;
    }
...
static void recompute_stats(void)
{
    ...
    for (size_t frame = 0; frame < FRAME_COUNT; ++frame) {
        ...
        stats.highest_allocatable_address =
            ((uint64_t)frame + 1U) * TRAIT_PAGE_SIZE;
...
)
{
    ...
    status = reserve_internal(0U, TRAIT_LOW_MEMORY_RESERVATION);
    ...
    next_search_index = (size_t)(TRAIT_LOW_MEMORY_RESERVATION / TRAIT_PAGE_SIZE);
...
enum frame_status frame_allocate(uintptr_t *physical_address)
{
    ...
    for (size_t step = 0; step < FRAME_COUNT; ++step) {
        ...
        if (bitmap_get(eligible_bitmap, frame) && !bitmap_get(used_bitmap, frame)) {
            ...
            *physical_address = (uintptr_t)((uint64_t)frame * TRAIT_PAGE_SIZE);
            return FRAME_STATUS_OK;
        }
    }
...
enum frame_status frame_release(uintptr_t physical_address)
{
    ...
    if (((uint64_t)physical_address & (TRAIT_PAGE_SIZE - 1U)) != 0U) {
        return FRAME_STATUS_UNALIGNED_ADDRESS;
    }

    if ((uint64_t)physical_address >= TRAIT_EARLY_PHYSICAL_LIMIT) {
        return FRAME_STATUS_RANGE_OUTSIDE_LIMIT;
    }

    frame = (size_t)((uint64_t)physical_address / TRAIT_PAGE_SIZE);
...
)
{
    ...
    if (!power_of_two(request->alignment) ||
        request->alignment < TRAIT_PAGE_SIZE ||
        request->alignment % TRAIT_PAGE_SIZE != 0U) {
        return FRAME_STATUS_BAD_ALIGNMENT;
    }

    if (request->alignment > TRAIT_EARLY_PHYSICAL_LIMIT ||
    ...
    if (request->page_count > UINT64_MAX / TRAIT_PAGE_SIZE) {
        return FRAME_STATUS_RANGE_OVERFLOW;
    }

    length = (uint64_t)request->page_count * TRAIT_PAGE_SIZE;
    ...
    if (bound_end > TRAIT_EARLY_PHYSICAL_LIMIT) {
        bound_end = TRAIT_EARLY_PHYSICAL_LIMIT;
    }
    maximum_past_frame = (size_t)(bound_end / TRAIT_PAGE_SIZE);
    ...
    for (size_t first = 0U;
         ...
         ++first) {
        const uint64_t base = (uint64_t)first * TRAIT_PAGE_SIZE;
        bool available = true;
...
)
{
    ...
    first = (size_t)((uint64_t)record->physical_base / TRAIT_PAGE_SIZE);
...
)
{
    ...
    if (base_address >= TRAIT_EARLY_PHYSICAL_LIMIT) {
        return false;
    }
    if (end > TRAIT_EARLY_PHYSICAL_LIMIT) {
        end = TRAIT_EARLY_PHYSICAL_LIMIT;
    }

    first_frame = (size_t)(base_address / TRAIT_PAGE_SIZE);
    past_last_frame = (size_t)(end / TRAIT_PAGE_SIZE);
    if ((end & (TRAIT_PAGE_SIZE - 1U)) != 0U) {
        ++past_last_frame;
    }
    for (size_t frame = first_frame; frame < past_last_frame; ++frame) {
        if (bitmap_get(eligible_bitmap, frame)) {
