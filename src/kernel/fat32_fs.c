...
/*
 * OpenGAT's bounded FAT32 file store. Raw metadata is accepted only after the
...
#define OPENGATFS_SECTOR_BYTES 512U
#define OPENGATFS_DIRECTORY_ENTRIES (OPENGATFS_SECTOR_BYTES / FAT32_DIRECTORY_ENTRY_BYTES)
#define OPENGATFS_ATTR_READ_ONLY UINT8_C(0x01)
#define OPENGATFS_ATTR_VOLUME UINT8_C(0x08)
#define OPENGATFS_ATTR_DIRECTORY UINT8_C(0x10)
#define OPENGATFS_ATTR_LONG_NAME UINT8_C(0x0F)
#define OPENGATFS_MAX_CHAIN_CLUSTERS (OPENGATFS_MAX_FILE_BYTES / OPENGATFS_SECTOR_BYTES)
#define OPENGATFS_SYSTEM_CONTROLLER 0U
#define OPENGATFS_DATA_CONTROLLER 1U
#define OPENGATFS_FSINFO_FREE_OFFSET 488U
#define OPENGATFS_FSINFO_NEXT_OFFSET 492U
#define OPENGATFS_FSINFO_TRAIL_OFFSET 508U
#define OPENGATFS_VOLUME_SECTORS UINT64_C(131072)
#define OPENGATFS_RESERVED_SECTORS 32U
#define OPENGATFS_FAT_SECTORS UINT64_C(1009)
#define OPENGATFS_ROOT_CLUSTER 2U
#define OPENGATFS_MAX_MEDIA_CLUSTER 129023U
#define OPENGATFS_VALIDATION_BITMAP_BYTES 16128U
#define OPENGATFS_MAX_VALIDATION_DIRECTORIES 256U

struct opengatfs_mount_state {
...
struct opengatfs_cache_entry {
    uint8_t data[OPENGATFS_SECTOR_BYTES];
    uint64_t mount_generation;
    uint64_t sector;
    uint64_t stamp;
    enum opengatfs_volume volume;
...
struct opengatfs_operation {
    struct nvme_volume_session nvme;
    struct opengatfs_mount_state *mount;
    enum opengatfs_volume volume;
    bool active;
    bool writable;
};

struct opengatfs_validation_directory {
    uint32_t cluster;
    uint32_t parent;
    uint32_t depth;
};

struct opengatfs_location {
...
struct opengatfs_parent {
    uint32_t cluster;
    struct fat32_name name;
};

static struct opengatfs_mount_state mounts[OPENGATFS_MAX_MOUNTS];
static struct opengatfs_handle_state handles[OPENGATFS_MAX_HANDLES];
static struct opengatfs_cache_entry cache[OPENGATFS_CACHE_ENTRIES];
static uint8_t validation_seen[OPENGATFS_VALIDATION_BITMAP_BYTES];
static struct opengatfs_validation_directory
    validation_directories[OPENGATFS_MAX_VALIDATION_DIRECTORIES];
...
_Static_assert(OPENGATFS_MAX_FILE_BYTES % OPENGATFS_SECTOR_BYTES == 0U,
    "file bound must contain whole clusters");
_Static_assert(OPENGATFS_MAX_HANDLES <= UINT8_MAX,
...
static enum opengatfs_status nvme_result(enum nvme_status status)
{
    ...
    if (status == NVME_STATUS_ABSENT ||
        ...
        return OPENGATFS_STATUS_ABSENT;
    }
...
{
    console_write("OpenGAT: FAT32 NVMe ");
...
static void invalidate_cache(enum opengatfs_volume volume)
{
    for (size_t index = 0U; index < OPENGATFS_CACHE_ENTRIES; ++index) {
...
static enum opengatfs_status direct_read(
...
static enum opengatfs_status direct_write(
    struct nvme_volume_session *session,
    uint64_t sector,
    const uint8_t *data
)
...
static enum opengatfs_status flush_cache_entry(
    struct opengatfs_operation *operation,
    struct opengatfs_cache_entry *entry
)
{
...
static enum opengatfs_status flush_cache(struct opengatfs_operation *operation)
{
    for (size_t index = 0U; index < OPENGATFS_CACHE_ENTRIES; ++index) {
        enum opengatfs_status status = flush_cache_entry(operation, &cache[index]);
...
static enum opengatfs_status cache_sector(
    struct opengatfs_operation *operation,
    uint64_t sector,
    struct opengatfs_cache_entry **result
)
{
    struct opengatfs_cache_entry *selected = NULL;
    ...
    *result = NULL;
    for (size_t index = 0U; index < OPENGATFS_CACHE_ENTRIES; ++index) {
        struct opengatfs_cache_entry *entry = &cache[index];
    ...
    if (selected->valid && selected->dirty) {
        enum opengatfs_status status = flush_cache_entry(operation, selected);
...
static enum opengatfs_status flush_one(
    struct opengatfs_operation *operation,
    struct opengatfs_cache_entry *entry
)
{
...
static enum opengatfs_status begin_operation(
    enum opengatfs_volume volume,
    bool writable,
    struct opengatfs_operation *operation
)
...
static enum opengatfs_status end_operation(
    struct opengatfs_operation *operation,
    bool commit
)
{
    enum opengatfs_status result = OPENGATFS_STATUS_OK;
    ...
    if (result == OPENGATFS_STATUS_OK && close_status != NVME_STATUS_OK) {
...
static enum opengatfs_status fat_entry(
    struct opengatfs_operation *operation,
    uint32_t cluster,
    uint32_t *value
)
{
    ...
    sector = operation->mount->geometry.first_fat_sector +
        byte_offset / OPENGATFS_SECTOR_BYTES;
    offset = (uint32_t)(byte_offset % OPENGATFS_SECTOR_BYTES);
...
static enum opengatfs_status set_fat_copy(
    struct opengatfs_operation *operation,
    uint32_t copy,
    uint32_t cluster,
    uint32_t value
...
static enum opengatfs_status set_fat(
    struct opengatfs_operation *operation,
    uint32_t cluster,
    uint32_t value
)
...
static enum opengatfs_status cluster_sector(
    const struct opengatfs_mount_state *mount,
    uint32_t cluster,
    uint64_t *sector
)
{
...
static enum opengatfs_status next_cluster(
    struct opengatfs_operation *operation,
    uint32_t cluster,
    uint32_t *next,
    bool *end
)
{
    uint32_t raw;
    uint32_t checked = 0U;
    enum opengatfs_status status = fat_entry(operation, cluster, &raw);

    if (next == NULL || end == NULL || status != OPENGATFS_STATUS_OK) {
        return status != OPENGATFS_STATUS_OK ? status : OPENGATFS_STATUS_INVALID_ARGUMENT;
    }
    ...
    if (opengat_fat32_classify_cluster(raw, &operation->mount->geometry,
...
static enum opengatfs_status allocate_cluster(
    struct opengatfs_operation *operation,
    uint32_t *allocated
)
{
    ...
    for (attempts = 0U;
        ...
        uint32_t value;
        enum opengatfs_status status = fat_entry(operation, cluster, &value);
...
static enum opengatfs_status release_chain(
    struct opengatfs_operation *operation,
    uint32_t first_cluster
)
{
    ...
    for (uint32_t steps = 0U; steps < OPENGATFS_MAX_CHAIN_CLUSTERS; ++steps) {
        uint32_t next;
        bool end;
        enum opengatfs_status status = next_cluster(operation, cluster, &next, &end);
...
static enum opengatfs_status path_validation(const char *path, size_t *length)
{
    uint32_t components = 0U;
    size_t found = bounded_length(path, OPENGATFS_MAX_PATH);
    enum fat32_status status;

    if (length == NULL || found == 0U || found > OPENGATFS_MAX_PATH) {
        return found == 0U ? OPENGATFS_STATUS_PATH : OPENGATFS_STATUS_INVALID_ARGUMENT;
    }
    status = opengat_fat32_validate_path((const uint8_t *)path, found,
        &components);
    if (status == FAT32_STATUS_NAME_MALFORMED ||
        status == FAT32_STATUS_COMPONENT_TOO_LONG) {
        return OPENGATFS_STATUS_NAME;
    }
...
static enum opengatfs_status scan_directory(
    struct opengatfs_operation *operation,
    uint32_t directory_cluster,
    const struct fat32_name *query,
    struct opengatfs_location *location,
    struct opengatfs_free_slot *free_slot
)
{
    ...
    for (uint32_t steps = 0U; steps < OPENGATFS_MAX_CHAIN_CLUSTERS; ++steps) {
        ...
        uint32_t next;
        bool end;
        enum opengatfs_status status = cluster_sector(operation->mount, cluster,
            &first_sector);
        ...
        for (uint32_t sector_index = 0U;
             ...
             ++sector_index) {
            struct opengatfs_cache_entry *sector;
            ...
            for (uint32_t offset = 0U; offset < OPENGATFS_SECTOR_BYTES;
                ...
                if (parsed.kind != FAT32_ENTRY_ORDINARY ||
                    (parsed.attributes & OPENGATFS_ATTR_VOLUME) != 0U) {
                    continue;
                }
                if (free_slot != NULL &&
                    ...
                    if (free_slot->live_entries > OPENGATFS_MAX_LIST_ENTRIES) {
                        return OPENGATFS_STATUS_CORRUPT;
                    }
                }
...
{
    uint32_t stack[OPENGATFS_MAX_DEPTH + 1U];
    size_t length;
    size_t start = 0U;
    uint32_t depth = 0U;
    enum opengatfs_status status = path_validation(path, &length);

    if (status != OPENGATFS_STATUS_OK || operation == NULL || location == NULL) {
        return status != OPENGATFS_STATUS_OK ? status : OPENGATFS_STATUS_INVALID_ARGUMENT;
    }
    ...
    while (start < length) {
        ...
        last = end == length;
        if (opengat_fat32_parse_component((const uint8_t *)&path[start],
                end - start, &name) != FAT32_STATUS_OK) {
            return OPENGATFS_STATUS_NAME;
        }
        if (name.kind == FAT32_NAME_CURRENT) {
            if (last) {
                location->entry.attributes = OPENGATFS_ATTR_DIRECTORY;
            ...
            if ((location->entry.attributes & OPENGATFS_ATTR_DIRECTORY) == 0U) {
...
static enum opengatfs_status resolve_parent(
    struct opengatfs_operation *operation,
    const char *path,
    struct opengatfs_parent *parent
)
{
    char parent_path[OPENGATFS_MAX_PATH + 1U];
    size_t length;
    size_t split;
    struct opengatfs_location location;
    enum opengatfs_status status = path_validation(path, &length);

    if (status != OPENGATFS_STATUS_OK || operation == NULL || parent == NULL) {
        return status != OPENGATFS_STATUS_OK ? status : OPENGATFS_STATUS_INVALID_ARGUMENT;
    }
    split = length;
    ...
    if (opengat_fat32_parse_component((const uint8_t *)&path[split],
...
static enum opengatfs_status write_directory_entry(
    struct opengatfs_operation *operation,
...
)
{
    ...
    if (offset > OPENGATFS_SECTOR_BYTES - FAT32_DIRECTORY_ENTRY_BYTES) {
        return OPENGATFS_STATUS_RANGE;
    }
    status = cache_sector(operation, sector_number, &sector);
    if (status != OPENGATFS_STATUS_OK) {
...
static enum opengatfs_status update_location(
    struct opengatfs_operation *operation,
    const struct opengatfs_location *location,
    uint32_t first_cluster,
    uint32_t size
)
{
...
static enum opengatfs_status mark_deleted(
    struct opengatfs_operation *operation,
    const struct opengatfs_location *location
)
{
...
static enum opengatfs_status find_or_grow_slot(
    struct opengatfs_operation *operation,
    uint32_t directory_cluster,
    const struct fat32_name *name,
    struct opengatfs_free_slot *slot
)
{
    struct opengatfs_location duplicate;
    enum opengatfs_status status = scan_directory(operation, directory_cluster,
        name, &duplicate, slot);
    ...
    if (status != OPENGATFS_STATUS_OK) {
        return status == OPENGATFS_STATUS_FULL ? OPENGATFS_STATUS_DIRECTORY_FULL : status;
    }
...
static enum opengatfs_status chain_cluster_at(
    struct opengatfs_operation *operation,
    uint32_t first_cluster,
    uint32_t ordinal,
    uint32_t *result
)
{
    ...
    if (result == NULL || first_cluster < 2U ||
        ordinal >= OPENGATFS_MAX_CHAIN_CLUSTERS) {
        return OPENGATFS_STATUS_RANGE;
    }
    for (uint32_t index = 0U; index < ordinal; ++index) {
        ...
        if (status != OPENGATFS_STATUS_OK || end) {
            return status != OPENGATFS_STATUS_OK ? status : OPENGATFS_STATUS_CORRUPT;
        }
        cluster = next;
    }
    *result = cluster;
...
static enum opengatfs_status zero_chain_bytes(
    struct opengatfs_operation *operation,
    uint32_t first_cluster,
    uint32_t start,
    uint32_t end
...
{
    while (start < end) {
        uint32_t ordinal = start / OPENGATFS_SECTOR_BYTES;
        uint32_t within = start % OPENGATFS_SECTOR_BYTES;
        uint32_t cluster;
        uint64_t sector_number;
        struct opengatfs_cache_entry *sector;
        uint32_t chunk = OPENGATFS_SECTOR_BYTES - within;
        enum opengatfs_status status;
...
static enum opengatfs_status count_chain(
    struct opengatfs_operation *operation,
    uint32_t first_cluster,
    uint32_t *count,
    uint32_t *last
...
static enum opengatfs_status ensure_chain(
    struct opengatfs_operation *operation,
...
)
{
    ...
    if (required > OPENGATFS_MAX_CHAIN_CLUSTERS || result_first == NULL) {
        return OPENGATFS_STATUS_RANGE;
    }
...
static enum opengatfs_status validate_volume_identity(
    enum opengatfs_volume volume,
...
)
{
    ...
    static const uint8_t data_label[11] =
        {'O', 'P', 'E', 'N', 'G', 'A', 'T', 'D', 'A', 'T', 'A'};
    const uint8_t *label = volume == OPENGATFS_VOLUME_SYSTEM ?
        system_label : data_label;
    uint32_t identifier = volume == OPENGATFS_VOLUME_SYSTEM ?
        FAT32_SYSTEM_VOLUME_ID : FAT32_DATA_VOLUME_ID;

    return geometry->bytes_per_sector == OPENGATFS_SECTOR_BYTES &&
        geometry->sectors_per_cluster == 1U &&
        geometry->reserved_sectors == OPENGATFS_RESERVED_SECTORS &&
        geometry->fat_copies == 2U &&
        geometry->total_sectors == OPENGATFS_VOLUME_SECTORS &&
        geometry->fat_sectors == OPENGATFS_FAT_SECTORS &&
        geometry->root_cluster == OPENGATFS_ROOT_CLUSTER &&
        geometry->volume_id == identifier &&
        equal_bytes(geometry->volume_label, label, sizeof(system_label)) ?
        OPENGATFS_STATUS_OK : OPENGATFS_STATUS_CORRUPT;
}

static enum opengatfs_status validate_fats(
    struct nvme_volume_session *session,
    const struct fat32_geometry *geometry,
    uint64_t *free_clusters,
    uint32_t *next_free
...
{
    uint8_t first[OPENGATFS_SECTOR_BYTES];
    uint8_t second[OPENGATFS_SECTOR_BYTES];
    ...
    for (uint64_t ordinal = 0U; ordinal < geometry->fat_sectors; ++ordinal) {
        enum opengatfs_status status = direct_read(session,
        ...
        if (ordinal == 0U && opengat_fat32_validate_fat_pair(first,
                sizeof(first), second, sizeof(second), geometry) !=
                FAT32_STATUS_OK) {
            return OPENGATFS_STATUS_CORRUPT;
        }
        for (size_t offset = 0U; offset < sizeof(first); offset += 4U) {
            uint64_t cluster64 = ordinal * (OPENGATFS_SECTOR_BYTES / 4U) +
                offset / 4U;
            uint32_t value;
...
static enum opengatfs_status validation_claim_cluster(uint32_t cluster)
{
    if (cluster < 2U || cluster > OPENGATFS_MAX_MEDIA_CLUSTER ||
...
static enum opengatfs_status validate_file_chain(
    struct opengatfs_operation *operation,
    uint32_t first_cluster,
    uint32_t file_size
)
{
    uint32_t expected = (uint32_t)(((uint64_t)file_size +
        OPENGATFS_SECTOR_BYTES - 1U) / OPENGATFS_SECTOR_BYTES);
    uint32_t cluster = first_cluster;

    if (file_size > OPENGATFS_MAX_FILE_BYTES ||
        (expected == 0U) != (first_cluster == 0U) ||
        expected > OPENGATFS_MAX_CHAIN_CLUSTERS) {
        return OPENGATFS_STATUS_CORRUPT;
    }
    for (uint32_t count = 0U; count < expected; ++count) {
        uint32_t next;
        bool end;
        enum opengatfs_status status = validation_claim_cluster(cluster);

        if (status != OPENGATFS_STATUS_OK) {
            return status;
        }
        status = next_cluster(operation, cluster, &next, &end);
        if (status != OPENGATFS_STATUS_OK || end != (count + 1U == expected)) {
            return OPENGATFS_STATUS_CORRUPT;
        }
        cluster = next;
    }
...
static enum opengatfs_status validation_enqueue_directory(
    uint32_t cluster,
    uint32_t parent,
    uint32_t depth,
    size_t *tail
...
{
    if (tail == NULL || depth > OPENGATFS_MAX_DEPTH ||
        *tail >= OPENGATFS_MAX_VALIDATION_DIRECTORIES) {
        return OPENGATFS_STATUS_CORRUPT;
    }
...
static enum opengatfs_status validate_directory_tree(
    struct opengatfs_operation *operation,
    const struct opengatfs_validation_directory *directory,
    size_t *tail
)
{
    ...
    static const uint8_t dotdot[11] =
        {'.', '.', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '};
    uint8_t directory_bytes[OPENGATFS_SECTOR_BYTES];
    uint8_t names[OPENGATFS_MAX_LIST_ENTRIES][FAT32_SHORT_NAME_BYTES];
    ...
    for (uint32_t steps = 0U;
         steps < OPENGATFS_MAX_CHAIN_CLUSTERS; ++steps) {
        uint64_t sector_number;
        struct opengatfs_cache_entry *sector;
        uint32_t next;
        bool chain_end;
        ...
        for (uint32_t offset = 0U; offset < OPENGATFS_SECTOR_BYTES;
            ...
            if (opengat_fat32_parse_directory_entry(&directory_bytes[offset],
            ...
            if (entry.kind != FAT32_ENTRY_ORDINARY ||
                (entry.attributes & OPENGATFS_ATTR_VOLUME) != 0U) {
                continue;
            }
            if (name_equal(entry.short_name, dot)) {
                if (directory->depth == 0U || dot_seen ||
                    (entry.attributes & OPENGATFS_ATTR_DIRECTORY) == 0U ||
            ...
            if (name_count >= OPENGATFS_MAX_LIST_ENTRIES) {
                return OPENGATFS_STATUS_CORRUPT;
            }
            ...
            if ((entry.attributes & OPENGATFS_ATTR_DIRECTORY) != 0U) {
        ...
        if (status != OPENGATFS_STATUS_OK || (directory_end && !chain_end)) {
            return OPENGATFS_STATUS_CORRUPT;
        }
        if (chain_end) {
...
static enum opengatfs_status validate_live_tree(
    struct nvme_volume_session *session,
    struct opengatfs_mount_state *mount,
    enum opengatfs_volume volume
)
{
    struct opengatfs_operation operation = {0};
    size_t head = 0U;
    size_t tail = 0U;
    enum opengatfs_status status;

    if (session == NULL || mount == NULL ||
        mount->geometry.maximum_cluster != OPENGATFS_MAX_MEDIA_CLUSTER) {
        return OPENGATFS_STATUS_CORRUPT;
    }
    ...
    while (status == OPENGATFS_STATUS_OK && head < tail) {
    ...
    for (uint32_t cluster = 2U;
        ...
        if (status == OPENGATFS_STATUS_OK &&
            ...
            status = OPENGATFS_STATUS_CORRUPT;
        }
    }
    invalidate_cache(volume);
    return status;
...
{
    uint8_t boot[OPENGATFS_SECTOR_BYTES];
    uint8_t backup[OPENGATFS_SECTOR_BYTES];
    uint8_t info[OPENGATFS_SECTOR_BYTES];
    uint8_t backup_info[OPENGATFS_SECTOR_BYTES];
    struct nvme_volume_session session;
    struct opengatfs_mount_state candidate;
    ...
    controller = volume == OPENGATFS_VOLUME_SYSTEM ?
        OPENGATFS_SYSTEM_CONTROLLER : OPENGATFS_DATA_CONTROLLER;
    ...
    candidate.read_only = volume == OPENGATFS_VOLUME_SYSTEM;
    status = direct_read(&session, 0U, boot);
    if (status == OPENGATFS_STATUS_OK &&
        opengat_fat32_parse_bpb(boot, sizeof(boot), session.namespace_blocks,
    ...
    if (status == OPENGATFS_STATUS_OK &&
        candidate.geometry.bytes_per_sector != OPENGATFS_SECTOR_BYTES) {
        status = OPENGATFS_STATUS_CORRUPT;
    }
    if (status == OPENGATFS_STATUS_OK) {
    ...
    if (status == OPENGATFS_STATUS_OK) {
        ...
        status = direct_read(&session, candidate.geometry.fsinfo_sector,
            info);
        if (status == OPENGATFS_STATUS_OK &&
            opengat_fat32_parse_fsinfo(info, sizeof(info),
    ...
    if (close_status != NVME_STATUS_OK && status == OPENGATFS_STATUS_OK) {
        validation_stage = "controller close";
        status = OPENGATFS_STATUS_IO;
    }
    invalidate_cache(volume);
    if (status != OPENGATFS_STATUS_OK) {
        console_write("OpenGAT: FAT32 ");
        console_write(volume == OPENGATFS_VOLUME_SYSTEM ? "system" : "data");
...
static enum opengatfs_status write_fsinfo(struct opengatfs_operation *operation)
{
    struct opengatfs_cache_entry *primary;
    struct opengatfs_cache_entry *backup;
    uint32_t free_hint;
    enum opengatfs_status status;
    ...
    write_u32(&primary->data[OPENGATFS_FSINFO_FREE_OFFSET], free_hint);
    write_u32(&primary->data[OPENGATFS_FSINFO_NEXT_OFFSET],
        operation->mount->next_free);
    write_u32(&primary->data[OPENGATFS_FSINFO_TRAIL_OFFSET], UINT32_C(0xAA550000));
...
enum opengatfs_status fat32_backend_sync(enum opengatfs_volume volume)
{
    ...
    close_status = end_operation(&operation, status == OPENGATFS_STATUS_OK);
    return status != OPENGATFS_STATUS_OK ? status : close_status;
}
...
void fat32_backend_initialize(void)
{
    ...
    cache_stamp = 0U;
    (void)fat32_backend_mount(OPENGATFS_VOLUME_SYSTEM);
    (void)fat32_backend_mount(OPENGATFS_VOLUME_DATA);
}

struct opengatfs_drive_info fat32_backend_drive(enum opengatfs_volume volume)
{
    struct opengatfs_drive_info result = {0};
    struct opengatfs_mount_state *mount;
...
static enum opengatfs_status handle_state(
    opengatfs_handle handle,
    struct opengatfs_handle_state **state
)
{
    ...
    if (state == NULL || encoded_index == 0U ||
        encoded_index > OPENGATFS_MAX_HANDLES || generation == 0U) {
...
)
{
    ...
    enum opengatfs_status status;
    enum opengatfs_status close_status;
    size_t slot = OPENGATFS_MAX_HANDLES;

    if (handle == NULL || !valid_volume(volume) ||
        (access != OPENGATFS_ACCESS_READ && access != OPENGATFS_ACCESS_WRITE &&
            access != OPENGATFS_ACCESS_READ_WRITE)) {
        return OPENGATFS_STATUS_INVALID_ARGUMENT;
    }
    *handle = 0U;
    if ((access & OPENGATFS_ACCESS_WRITE) != 0U && mounts[volume].read_only) {
        return OPENGATFS_STATUS_READ_ONLY;
    }
    for (size_t index = 0U; index < OPENGATFS_MAX_HANDLES; ++index) {
    ...
    if (close_status != OPENGATFS_STATUS_OK) {
        return close_status;
    }
    if ((location.entry.attributes & OPENGATFS_ATTR_DIRECTORY) != 0U) {
...
{
    struct opengatfs_handle_state *state;
    enum opengatfs_status status = handle_state(handle, &state);
...
)
{
    ...
    *position = 0U;
    ...
    if (origin == OPENGATFS_SEEK_START) {
        base = 0U;
    } else if (origin == OPENGATFS_SEEK_CURRENT) {
        base = state->offset;
    } else if (origin == OPENGATFS_SEEK_END) {
        base = state->size;
    } else {
        return OPENGATFS_STATUS_INVALID_ARGUMENT;
    }
    ...
    if (target > OPENGATFS_MAX_FILE_BYTES) {
        return OPENGATFS_STATUS_RANGE;
    }
...
)
{
    ...
    *read_bytes = 0U;
    ...
    if ((state->access & OPENGATFS_ACCESS_READ) == 0U) {
    ...
    while (completed < available) {
        ...
        uint32_t ordinal = file_offset / OPENGATFS_SECTOR_BYTES;
        uint32_t within = file_offset % OPENGATFS_SECTOR_BYTES;
        uint32_t cluster;
        uint64_t sector_number;
        struct opengatfs_cache_entry *sector;
        size_t chunk = OPENGATFS_SECTOR_BYTES - within;
...
)
{
    ...
    if (offset > OPENGATFS_MAX_FILE_BYTES) {
        return OPENGATFS_STATUS_RANGE;
    }
    saved = state->offset;
...
static enum opengatfs_status handle_location(
    struct opengatfs_operation *operation,
    const struct opengatfs_handle_state *state,
    struct opengatfs_location *location
)
{
    struct opengatfs_cache_entry *sector;
    ...
    parsed = opengat_fat32_parse_directory_entry(
...
)
{
    ...
    *written_bytes = 0U;
    ...
    if ((state->access & OPENGATFS_ACCESS_WRITE) == 0U) {
        return OPENGATFS_STATUS_ACCESS;
    }
    if (!add_u64(state->offset, source_bytes, &end_offset) ||
        end_offset > OPENGATFS_MAX_FILE_BYTES) {
        return OPENGATFS_STATUS_RANGE;
    }
    if (source_bytes == 0U) {
        return OPENGATFS_STATUS_OK;
    ...
    required = ((uint32_t)end_offset + OPENGATFS_SECTOR_BYTES - 1U) /
        OPENGATFS_SECTOR_BYTES;
    first = state->first_cluster;
    if (status == OPENGATFS_STATUS_OK) {
        status = ensure_chain(&operation, first, required, &first);
    }
    while (status == OPENGATFS_STATUS_OK && completed < source_bytes) {
        uint32_t file_offset = state->offset + (uint32_t)completed;
        uint32_t ordinal = file_offset / OPENGATFS_SECTOR_BYTES;
        uint32_t within = file_offset % OPENGATFS_SECTOR_BYTES;
        uint32_t cluster;
...
)
{
    ...
    if (status == OPENGATFS_STATUS_OK) {
        ...
        stat->directory =
            (location.entry.attributes & OPENGATFS_ATTR_DIRECTORY) != 0U;
        stat->read_only = operation.mount->read_only ||
            (location.entry.attributes & OPENGATFS_ATTR_READ_ONLY) != 0U;
    }
    close_status = end_operation(&operation, false);
    return status != OPENGATFS_STATUS_OK ? status : close_status;
}
...
{
    struct opengatfs_operation operation;
    struct opengatfs_location directory;
    uint32_t cluster;
    size_t count = 0U;
    enum opengatfs_status status;
    enum opengatfs_status close_status;
    ...
    *entry_count = 0U;
    ...
    if (status == OPENGATFS_STATUS_OK &&
        (directory.entry.attributes & OPENGATFS_ATTR_DIRECTORY) == 0U) {
    ...
    for (uint32_t steps = 0U;
         status == OPENGATFS_STATUS_OK && steps < OPENGATFS_MAX_CHAIN_CLUSTERS;
         ++steps) {
        uint64_t first_sector;
        uint32_t next;
        bool end;
        ...
        for (uint32_t sector_index = 0U;
            ...
            for (uint32_t offset = 0U; offset < OPENGATFS_SECTOR_BYTES;
                ...
                struct fat32_directory_entry parsed;
                enum fat32_status parse_status =
                    opengat_fat32_parse_directory_entry(&sector->data[offset],
                ...
                if (count >= capacity || count >= OPENGATFS_MAX_LIST_ENTRIES) {
                    status = OPENGATFS_STATUS_RANGE;
                    goto listing_complete;
                }
                ...
                entries[count].directory =
                    (parsed.attributes & OPENGATFS_ATTR_DIRECTORY) != 0U;
                ++count;
            }
        }
        if (status != OPENGATFS_STATUS_OK) {
...
listing_complete:
    *entry_count = count;
    close_status = end_operation(&operation, false);
    if (status == OPENGATFS_STATUS_OK && close_status != OPENGATFS_STATUS_OK) {
        status = close_status;
    }
    return status;
}
...
)
{
    ...
    struct opengatfs_parent parent;
    struct opengatfs_free_slot slot;
    enum opengatfs_status status = begin_operation(volume, true, &operation);
    enum opengatfs_status close_status;
...
)
{
    ...
    if (status == OPENGATFS_STATUS_OK) {
        ...
        encode_directory_entry(&sector->data[0], dot, OPENGATFS_ATTR_DIRECTORY,
            cluster, 0U);
        encode_directory_entry(&sector->data[FAT32_DIRECTORY_ENTRY_BYTES],
            dotdot, OPENGATFS_ATTR_DIRECTORY, parent.cluster, 0U);
        status = flush_one(&operation, sector);
    }
    if (status == OPENGATFS_STATUS_OK) {
        status = write_directory_entry(&operation, slot.sector, slot.offset,
            parent.name.canonical, OPENGATFS_ATTR_DIRECTORY, cluster, 0U);
    }
    if (status != OPENGATFS_STATUS_OK && cluster != 0U) {
...
)
{
    ...
    if (status == OPENGATFS_STATUS_OK && location_open(volume, &location)) {
...
static enum opengatfs_status directory_empty(
    struct opengatfs_operation *operation,
    uint32_t directory_cluster,
    bool *empty
)
    ...
    for (uint32_t steps = 0U; steps < OPENGATFS_MAX_CHAIN_CLUSTERS; ++steps) {
        ...
        for (uint32_t sector_index = 0U;
            ...
            for (uint32_t offset = 0U; offset < OPENGATFS_SECTOR_BYTES;
                ...
                if (opengat_fat32_parse_directory_entry(&sector->data[offset],
                        FAT32_DIRECTORY_ENTRY_BYTES, &parsed) !=
                        FAT32_STATUS_OK) {
                    return OPENGATFS_STATUS_CORRUPT;
                }
...
)
{
    ...
    status = resolve_path(&operation, path, &location);
    if (status == OPENGATFS_STATUS_OK && location.root) {
        status = OPENGATFS_STATUS_ACCESS;
    }
    if (status == OPENGATFS_STATUS_OK &&
        (location.entry.attributes & OPENGATFS_ATTR_DIRECTORY) == 0U) {
        status = OPENGATFS_STATUS_NOT_DIRECTORY;
    ...
    if (status == OPENGATFS_STATUS_OK && !empty) {
        status = OPENGATFS_STATUS_NOT_EMPTY;
    }
    if (status == OPENGATFS_STATUS_OK) {
        status = mark_deleted(&operation, &location);
    }
...
)
{
    ...
    if (size > OPENGATFS_MAX_FILE_BYTES) {
        return OPENGATFS_STATUS_RANGE;
    }
    ...
    first = status == OPENGATFS_STATUS_OK ? location.entry.first_cluster : 0U;
    old_size = status == OPENGATFS_STATUS_OK ? location.entry.size : 0U;
    required = (checked_size + OPENGATFS_SECTOR_BYTES - 1U) /
        OPENGATFS_SECTOR_BYTES;
    if (status == OPENGATFS_STATUS_OK) {
        status = count_chain(&operation, first, &old_count, &old_last);
        (void)old_last;
    }
    if (status == OPENGATFS_STATUS_OK && required > old_count) {
        status = ensure_chain(&operation, first, required, &first);
        if (status == OPENGATFS_STATUS_OK) {
    ...
    } else if (status == OPENGATFS_STATUS_OK && required == 0U) {
        status = update_location(&operation, &location, 0U, 0U);
        if (status == OPENGATFS_STATUS_OK) {
            status = release_chain(&operation, first);
        }
    } else if (status == OPENGATFS_STATUS_OK && required < old_count) {
        uint32_t kept;
        uint32_t tail;
        bool end;
        ...
        if (status == OPENGATFS_STATUS_OK) {
            status = zero_chain_bytes(&operation, first, checked_size,
                required * OPENGATFS_SECTOR_BYTES);
        }
        if (status == OPENGATFS_STATUS_OK) {
            status = update_location(&operation, &location, first,
                checked_size);
    ...
    } else if (status == OPENGATFS_STATUS_OK) {
...
static enum opengatfs_status destination_inside_directory(
    struct opengatfs_operation *operation,
    uint32_t directory_cluster,
...
)
{
    ...
    *inside = false;
    ...
    for (uint32_t depth = 0U; depth <= OPENGATFS_MAX_DEPTH; ++depth) {
        struct opengatfs_location parent_entry;
        enum opengatfs_status status;
        ...
        status = scan_directory(operation, current, &parent_name,
            &parent_entry, NULL);
        if (status != OPENGATFS_STATUS_OK ||
            (parent_entry.entry.attributes & OPENGATFS_ATTR_DIRECTORY) == 0U ||
...
)
{
    static const uint8_t dotdot[11] =
        {'.', '.', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '};
    struct opengatfs_operation operation;
    struct opengatfs_location source_location;
    struct opengatfs_parent destination_parent;
    struct opengatfs_free_slot destination_slot;
    bool inside = false;
    enum opengatfs_status status = begin_operation(volume, true, &operation);
    enum opengatfs_status close_status;
    ...
    if (status == OPENGATFS_STATUS_OK && source_location.root) {
        status = OPENGATFS_STATUS_ACCESS;
    }
    if (status == OPENGATFS_STATUS_OK &&
    ...
    if (status == OPENGATFS_STATUS_OK &&
        (source_location.entry.attributes & OPENGATFS_ATTR_DIRECTORY) != 0U) {
        status = destination_inside_directory(&operation,
            source_location.entry.first_cluster,
            destination_parent.cluster, &inside);
        if (status == OPENGATFS_STATUS_OK && inside) {
            status = OPENGATFS_STATUS_PATH;
        }
    }
    if (status == OPENGATFS_STATUS_OK) {
...
bool fat32_backend_self_test(size_t *completed_tests)
{
    ...
    uint32_t components = 0U;
    struct fat32_name name;
    struct opengatfs_cache_entry model[OPENGATFS_CACHE_ENTRIES];

    if (completed_tests == NULL) {
        return false;
    }
    *completed_tests = 0U;
    if (opengat_fat32_validate_path((const uint8_t *)"projects/notes.txt",
            18U, &components) != FAT32_STATUS_OK || components != 2U) {
        return false;
    }
    ++completed;
    if (opengat_fat32_validate_path((const uint8_t *)"../escape", 9U,
            &components) != FAT32_STATUS_ABOVE_ROOT) {
        return false;
    }
    ++completed;
    if (opengat_fat32_parse_component((const uint8_t *)"notes.txt", 9U,
    ...
    size_t oldest = 0U;
    for (size_t index = 1U; index < OPENGATFS_CACHE_ENTRIES; ++index) {
    ...
    ++completed;
    opengatfs_handle encoded = (UINT64_C(7) << 8U) | UINT64_C(3);
...
{
    switch (status) {
    case OPENGATFS_STATUS_OK: return "ok";
    case OPENGATFS_STATUS_ABSENT: return "volume is absent";
    case OPENGATFS_STATUS_CORRUPT: return "FAT32 metadata is inconsistent";
    case OPENGATFS_STATUS_IO: return "storage I/O failed";
    case OPENGATFS_STATUS_INVALID_ARGUMENT: return "invalid filesystem argument";
    case OPENGATFS_STATUS_NOT_MOUNTED: return "volume is not mounted";
    case OPENGATFS_STATUS_ALREADY_MOUNTED: return "volume is already mounted";
    case OPENGATFS_STATUS_READ_ONLY: return "volume is read-only";
    case OPENGATFS_STATUS_NOT_FOUND: return "path was not found";
    case OPENGATFS_STATUS_EXISTS: return "destination already exists";
    case OPENGATFS_STATUS_NOT_DIRECTORY: return "path component is not a directory";
    case OPENGATFS_STATUS_IS_DIRECTORY: return "operation needs a regular file";
    case OPENGATFS_STATUS_NOT_EMPTY: return "directory is not empty";
    case OPENGATFS_STATUS_BUSY: return "file or volume is busy";
    case OPENGATFS_STATUS_NO_HANDLES: return "file-handle table is full";
    case OPENGATFS_STATUS_STALE_HANDLE: return "file handle is stale or closed";
    case OPENGATFS_STATUS_ACCESS: return "file handle access mode refused the operation";
    case OPENGATFS_STATUS_RANGE: return "filesystem offset or capacity is out of range";
    case OPENGATFS_STATUS_FULL: return "volume has no free cluster";
    case OPENGATFS_STATUS_DIRECTORY_FULL: return "directory has no bounded free slot";
    case OPENGATFS_STATUS_NAME: return "filename is outside the 8.3 ASCII subset";
    case OPENGATFS_STATUS_PATH: return "path is malformed or escapes the mount";
    case OPENGATFS_STATUS_WRITEBACK: return "dirty data could not be written";
    case OPENGATFS_STATUS_RESET: return "device generation changed";
