...
/*
 * Trait OS's bounded FAT32 file store. Raw metadata is accepted only after the
...
#define TRAITFS_SECTOR_BYTES 512U
#define TRAITFS_DIRECTORY_ENTRIES (TRAITFS_SECTOR_BYTES / FAT32_DIRECTORY_ENTRY_BYTES)
#define TRAITFS_ATTR_READ_ONLY UINT8_C(0x01)
#define TRAITFS_ATTR_VOLUME UINT8_C(0x08)
#define TRAITFS_ATTR_DIRECTORY UINT8_C(0x10)
#define TRAITFS_ATTR_LONG_NAME UINT8_C(0x0F)
#define TRAITFS_MAX_CHAIN_CLUSTERS (TRAITFS_MAX_FILE_BYTES / TRAITFS_SECTOR_BYTES)
#define TRAITFS_SYSTEM_CONTROLLER 0U
#define TRAITFS_DATA_CONTROLLER 1U
#define TRAITFS_FSINFO_FREE_OFFSET 488U
#define TRAITFS_FSINFO_NEXT_OFFSET 492U
#define TRAITFS_FSINFO_TRAIL_OFFSET 508U
#define TRAITFS_VOLUME_SECTORS UINT64_C(131072)
#define TRAITFS_RESERVED_SECTORS 32U
#define TRAITFS_FAT_SECTORS UINT64_C(1009)
#define TRAITFS_ROOT_CLUSTER 2U
#define TRAITFS_MAX_MEDIA_CLUSTER 129023U
#define TRAITFS_VALIDATION_BITMAP_BYTES 16128U
#define TRAITFS_MAX_VALIDATION_DIRECTORIES 256U

struct traitfs_mount_state {
...
struct traitfs_cache_entry {
    uint8_t data[TRAITFS_SECTOR_BYTES];
    uint64_t mount_generation;
    uint64_t sector;
    uint64_t stamp;
    enum traitfs_volume volume;
...
struct traitfs_operation {
    struct nvme_volume_session nvme;
    struct traitfs_mount_state *mount;
    enum traitfs_volume volume;
    bool active;
    bool writable;
};

struct traitfs_validation_directory {
    uint32_t cluster;
    uint32_t parent;
    uint32_t depth;
};

struct traitfs_location {
...
struct traitfs_parent {
    uint32_t cluster;
    struct fat32_name name;
};

static struct traitfs_mount_state mounts[TRAITFS_MAX_MOUNTS];
static struct traitfs_handle_state handles[TRAITFS_MAX_HANDLES];
static struct traitfs_cache_entry cache[TRAITFS_CACHE_ENTRIES];
static uint8_t validation_seen[TRAITFS_VALIDATION_BITMAP_BYTES];
static struct traitfs_validation_directory
    validation_directories[TRAITFS_MAX_VALIDATION_DIRECTORIES];
...
_Static_assert(TRAITFS_MAX_FILE_BYTES % TRAITFS_SECTOR_BYTES == 0U,
    "file bound must contain whole clusters");
_Static_assert(TRAITFS_MAX_HANDLES <= UINT8_MAX,
...
static enum traitfs_status nvme_result(enum nvme_status status)
{
...
{
    console_write("Trait OS: FAT32 NVMe ");
...
static void invalidate_cache(enum traitfs_volume volume)
{
    for (size_t index = 0U; index < TRAITFS_CACHE_ENTRIES; ++index) {
...
static enum traitfs_status direct_read(
...
static enum traitfs_status direct_write(
    struct nvme_volume_session *session,
    uint64_t sector,
    const uint8_t *data
)
...
static enum traitfs_status flush_cache_entry(
    struct traitfs_operation *operation,
    struct traitfs_cache_entry *entry
)
{
...
static enum traitfs_status flush_cache(struct traitfs_operation *operation)
{
    for (size_t index = 0U; index < TRAITFS_CACHE_ENTRIES; ++index) {
        enum traitfs_status status = flush_cache_entry(operation, &cache[index]);
...
static enum traitfs_status cache_sector(
    struct traitfs_operation *operation,
    uint64_t sector,
    struct traitfs_cache_entry **result
)
{
    struct traitfs_cache_entry *selected = NULL;
    ...
    *result = NULL;
    for (size_t index = 0U; index < TRAITFS_CACHE_ENTRIES; ++index) {
        struct traitfs_cache_entry *entry = &cache[index];
    ...
    if (selected->valid && selected->dirty) {
        enum traitfs_status status = flush_cache_entry(operation, selected);
...
static enum traitfs_status flush_one(
    struct traitfs_operation *operation,
    struct traitfs_cache_entry *entry
)
{
...
static enum traitfs_status begin_operation(
    enum traitfs_volume volume,
    bool writable,
    struct traitfs_operation *operation
)
...
static enum traitfs_status end_operation(
    struct traitfs_operation *operation,
    bool commit
)
{
    enum traitfs_status result = TRAITFS_STATUS_OK;
    ...
    if (result == TRAITFS_STATUS_OK && close_status != NVME_STATUS_OK) {
...
static enum traitfs_status fat_entry(
    struct traitfs_operation *operation,
    uint32_t cluster,
    uint32_t *value
)
{
    ...
    sector = operation->mount->geometry.first_fat_sector +
        byte_offset / TRAITFS_SECTOR_BYTES;
    offset = (uint32_t)(byte_offset % TRAITFS_SECTOR_BYTES);
...
static enum traitfs_status set_fat_copy(
    struct traitfs_operation *operation,
    uint32_t copy,
    uint32_t cluster,
    uint32_t value
...
static enum traitfs_status set_fat(
    struct traitfs_operation *operation,
    uint32_t cluster,
    uint32_t value
)
...
static enum traitfs_status cluster_sector(
    const struct traitfs_mount_state *mount,
    uint32_t cluster,
    uint64_t *sector
)
{
...
static enum traitfs_status next_cluster(
    struct traitfs_operation *operation,
    uint32_t cluster,
    uint32_t *next,
    bool *end
)
{
    uint32_t raw;
    uint32_t checked = 0U;
    enum traitfs_status status = fat_entry(operation, cluster, &raw);

    if (next == NULL || end == NULL || status != TRAITFS_STATUS_OK) {
        return status != TRAITFS_STATUS_OK ? status : TRAITFS_STATUS_INVALID_ARGUMENT;
    }
    ...
    if (trait_fat32_classify_cluster(raw, &operation->mount->geometry,
...
static enum traitfs_status allocate_cluster(
    struct traitfs_operation *operation,
    uint32_t *allocated
)
{
    ...
    for (attempts = 0U;
        ...
        uint32_t value;
        enum traitfs_status status = fat_entry(operation, cluster, &value);
...
static enum traitfs_status release_chain(
    struct traitfs_operation *operation,
    uint32_t first_cluster
)
{
    ...
    for (uint32_t steps = 0U; steps < TRAITFS_MAX_CHAIN_CLUSTERS; ++steps) {
        uint32_t next;
        bool end;
        enum traitfs_status status = next_cluster(operation, cluster, &next, &end);
...
static enum traitfs_status path_validation(const char *path, size_t *length)
{
    uint32_t components = 0U;
    size_t found = bounded_length(path, TRAITFS_MAX_PATH);
    enum fat32_status status;

    if (length == NULL || found == 0U || found > TRAITFS_MAX_PATH) {
        return found == 0U ? TRAITFS_STATUS_PATH : TRAITFS_STATUS_INVALID_ARGUMENT;
    }
    status = trait_fat32_validate_path((const uint8_t *)path, found,
        &components);
    if (status == FAT32_STATUS_NAME_MALFORMED ||
        status == FAT32_STATUS_COMPONENT_TOO_LONG) {
        return TRAITFS_STATUS_NAME;
    }
...
static enum traitfs_status scan_directory(
    struct traitfs_operation *operation,
    uint32_t directory_cluster,
    const struct fat32_name *query,
    struct traitfs_location *location,
    struct traitfs_free_slot *free_slot
)
{
    ...
    for (uint32_t steps = 0U; steps < TRAITFS_MAX_CHAIN_CLUSTERS; ++steps) {
        ...
        uint32_t next;
        bool end;
        enum traitfs_status status = cluster_sector(operation->mount, cluster,
            &first_sector);
        ...
        for (uint32_t sector_index = 0U;
             ...
             ++sector_index) {
            struct traitfs_cache_entry *sector;
            ...
            for (uint32_t offset = 0U; offset < TRAITFS_SECTOR_BYTES;
                ...
                if (parsed.kind != FAT32_ENTRY_ORDINARY ||
                    (parsed.attributes & TRAITFS_ATTR_VOLUME) != 0U) {
                    continue;
                }
                if (free_slot != NULL &&
                    ...
                    if (free_slot->live_entries > TRAITFS_MAX_LIST_ENTRIES) {
                        return TRAITFS_STATUS_CORRUPT;
                    }
                }
...
{
    uint32_t stack[TRAITFS_MAX_DEPTH + 1U];
    size_t length;
    size_t start = 0U;
    uint32_t depth = 0U;
    enum traitfs_status status = path_validation(path, &length);

    if (status != TRAITFS_STATUS_OK || operation == NULL || location == NULL) {
        return status != TRAITFS_STATUS_OK ? status : TRAITFS_STATUS_INVALID_ARGUMENT;
    }
    ...
    while (start < length) {
        ...
        last = end == length;
        if (trait_fat32_parse_component((const uint8_t *)&path[start],
                end - start, &name) != FAT32_STATUS_OK) {
            return TRAITFS_STATUS_NAME;
        }
        if (name.kind == FAT32_NAME_CURRENT) {
            if (last) {
                location->entry.attributes = TRAITFS_ATTR_DIRECTORY;
            ...
            if ((location->entry.attributes & TRAITFS_ATTR_DIRECTORY) == 0U) {
                return TRAITFS_STATUS_NOT_DIRECTORY;
            }
            if (depth >= TRAITFS_MAX_DEPTH) {
                return TRAITFS_STATUS_PATH;
            }
            ++depth;
...
static enum traitfs_status resolve_parent(
    struct traitfs_operation *operation,
    const char *path,
    struct traitfs_parent *parent
)
{
    char parent_path[TRAITFS_MAX_PATH + 1U];
    size_t length;
    size_t split;
    struct traitfs_location location;
    enum traitfs_status status = path_validation(path, &length);

    if (status != TRAITFS_STATUS_OK || operation == NULL || parent == NULL) {
        return status != TRAITFS_STATUS_OK ? status : TRAITFS_STATUS_INVALID_ARGUMENT;
    }
    split = length;
    ...
    if (trait_fat32_parse_component((const uint8_t *)&path[split],
...
static enum traitfs_status write_directory_entry(
    struct traitfs_operation *operation,
...
)
{
    ...
    if (offset > TRAITFS_SECTOR_BYTES - FAT32_DIRECTORY_ENTRY_BYTES) {
        return TRAITFS_STATUS_RANGE;
    }
    status = cache_sector(operation, sector_number, &sector);
    if (status != TRAITFS_STATUS_OK) {
...
static enum traitfs_status update_location(
    struct traitfs_operation *operation,
    const struct traitfs_location *location,
    uint32_t first_cluster,
    uint32_t size
)
{
...
static enum traitfs_status mark_deleted(
    struct traitfs_operation *operation,
    const struct traitfs_location *location
)
{
...
static enum traitfs_status find_or_grow_slot(
    struct traitfs_operation *operation,
    uint32_t directory_cluster,
    const struct fat32_name *name,
    struct traitfs_free_slot *slot
)
{
    struct traitfs_location duplicate;
    enum traitfs_status status = scan_directory(operation, directory_cluster,
        name, &duplicate, slot);
    ...
    if (status != TRAITFS_STATUS_OK) {
        return status == TRAITFS_STATUS_FULL ? TRAITFS_STATUS_DIRECTORY_FULL : status;
    }
...
static enum traitfs_status chain_cluster_at(
    struct traitfs_operation *operation,
    uint32_t first_cluster,
    uint32_t ordinal,
    uint32_t *result
)
{
    ...
    if (result == NULL || first_cluster < 2U ||
        ordinal >= TRAITFS_MAX_CHAIN_CLUSTERS) {
        return TRAITFS_STATUS_RANGE;
    }
    for (uint32_t index = 0U; index < ordinal; ++index) {
        ...
        if (status != TRAITFS_STATUS_OK || end) {
            return status != TRAITFS_STATUS_OK ? status : TRAITFS_STATUS_CORRUPT;
        }
        cluster = next;
    }
    *result = cluster;
...
static enum traitfs_status zero_chain_bytes(
    struct traitfs_operation *operation,
    uint32_t first_cluster,
    uint32_t start,
    uint32_t end
...
{
    while (start < end) {
        uint32_t ordinal = start / TRAITFS_SECTOR_BYTES;
        uint32_t within = start % TRAITFS_SECTOR_BYTES;
        uint32_t cluster;
        uint64_t sector_number;
        struct traitfs_cache_entry *sector;
        uint32_t chunk = TRAITFS_SECTOR_BYTES - within;
        enum traitfs_status status;
...
static enum traitfs_status count_chain(
    struct traitfs_operation *operation,
    uint32_t first_cluster,
    uint32_t *count,
    uint32_t *last
...
static enum traitfs_status ensure_chain(
    struct traitfs_operation *operation,
...
)
{
    ...
    if (required > TRAITFS_MAX_CHAIN_CLUSTERS || result_first == NULL) {
        return TRAITFS_STATUS_RANGE;
    }
...
static enum traitfs_status validate_volume_identity(
    enum traitfs_volume volume,
...
)
{
    ...
    static const uint8_t data_label[11] =
        {'T', 'R', 'A', 'I', 'T', 'D', 'A', 'T', 'A', ' ', ' '};
    const uint8_t *label = volume == TRAITFS_VOLUME_SYSTEM ?
        system_label : data_label;
    uint32_t identifier = volume == TRAITFS_VOLUME_SYSTEM ?
        FAT32_SYSTEM_VOLUME_ID : FAT32_DATA_VOLUME_ID;

    return geometry->bytes_per_sector == TRAITFS_SECTOR_BYTES &&
        geometry->sectors_per_cluster == 1U &&
        geometry->reserved_sectors == TRAITFS_RESERVED_SECTORS &&
        geometry->fat_copies == 2U &&
        geometry->total_sectors == TRAITFS_VOLUME_SECTORS &&
        geometry->fat_sectors == TRAITFS_FAT_SECTORS &&
        geometry->root_cluster == TRAITFS_ROOT_CLUSTER &&
        geometry->volume_id == identifier &&
        equal_bytes(geometry->volume_label, label, sizeof(system_label)) ?
        TRAITFS_STATUS_OK : TRAITFS_STATUS_CORRUPT;
}

static enum traitfs_status validate_fats(
    struct nvme_volume_session *session,
    const struct fat32_geometry *geometry,
    uint64_t *free_clusters,
    uint32_t *next_free
...
{
    uint8_t first[TRAITFS_SECTOR_BYTES];
    uint8_t second[TRAITFS_SECTOR_BYTES];
    ...
    for (uint64_t ordinal = 0U; ordinal < geometry->fat_sectors; ++ordinal) {
        enum traitfs_status status = direct_read(session,
        ...
        if (ordinal == 0U && trait_fat32_validate_fat_pair(first,
                sizeof(first), second, sizeof(second), geometry) !=
                FAT32_STATUS_OK) {
            return TRAITFS_STATUS_CORRUPT;
        }
        for (size_t offset = 0U; offset < sizeof(first); offset += 4U) {
            uint64_t cluster64 = ordinal * (TRAITFS_SECTOR_BYTES / 4U) +
                offset / 4U;
            uint32_t value;
...
static enum traitfs_status validation_claim_cluster(uint32_t cluster)
{
    if (cluster < 2U || cluster > TRAITFS_MAX_MEDIA_CLUSTER ||
...
static enum traitfs_status validate_file_chain(
    struct traitfs_operation *operation,
    uint32_t first_cluster,
    uint32_t file_size
)
{
    uint32_t expected = (uint32_t)(((uint64_t)file_size +
        TRAITFS_SECTOR_BYTES - 1U) / TRAITFS_SECTOR_BYTES);
    uint32_t cluster = first_cluster;

    if (file_size > TRAITFS_MAX_FILE_BYTES ||
        (expected == 0U) != (first_cluster == 0U) ||
        expected > TRAITFS_MAX_CHAIN_CLUSTERS) {
        return TRAITFS_STATUS_CORRUPT;
    }
    for (uint32_t count = 0U; count < expected; ++count) {
        uint32_t next;
        bool end;
        enum traitfs_status status = validation_claim_cluster(cluster);

        if (status != TRAITFS_STATUS_OK) {
            return status;
        }
        status = next_cluster(operation, cluster, &next, &end);
        if (status != TRAITFS_STATUS_OK || end != (count + 1U == expected)) {
            return TRAITFS_STATUS_CORRUPT;
        }
        cluster = next;
    }
...
static enum traitfs_status validation_enqueue_directory(
    uint32_t cluster,
    uint32_t parent,
    uint32_t depth,
    size_t *tail
...
{
    if (tail == NULL || depth > TRAITFS_MAX_DEPTH ||
        *tail >= TRAITFS_MAX_VALIDATION_DIRECTORIES) {
        return TRAITFS_STATUS_CORRUPT;
    }
...
static enum traitfs_status validate_directory_tree(
    struct traitfs_operation *operation,
    const struct traitfs_validation_directory *directory,
    size_t *tail
)
{
    ...
    static const uint8_t dotdot[11] =
        {'.', '.', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '};
    uint8_t directory_bytes[TRAITFS_SECTOR_BYTES];
    uint8_t names[TRAITFS_MAX_LIST_ENTRIES][FAT32_SHORT_NAME_BYTES];
    ...
    for (uint32_t steps = 0U;
         steps < TRAITFS_MAX_CHAIN_CLUSTERS; ++steps) {
        uint64_t sector_number;
        struct traitfs_cache_entry *sector;
        uint32_t next;
        bool chain_end;
        ...
        for (uint32_t offset = 0U; offset < TRAITFS_SECTOR_BYTES;
            ...
            if (trait_fat32_parse_directory_entry(&directory_bytes[offset],
            ...
            if (entry.kind != FAT32_ENTRY_ORDINARY ||
                (entry.attributes & TRAITFS_ATTR_VOLUME) != 0U) {
                continue;
            }
            if (name_equal(entry.short_name, dot)) {
                if (directory->depth == 0U || dot_seen ||
                    (entry.attributes & TRAITFS_ATTR_DIRECTORY) == 0U ||
            ...
            if (name_count >= TRAITFS_MAX_LIST_ENTRIES) {
                return TRAITFS_STATUS_CORRUPT;
            }
            ...
            if ((entry.attributes & TRAITFS_ATTR_DIRECTORY) != 0U) {
        ...
        if (status != TRAITFS_STATUS_OK || (directory_end && !chain_end)) {
            return TRAITFS_STATUS_CORRUPT;
        }
        if (chain_end) {
...
static enum traitfs_status validate_live_tree(
    struct nvme_volume_session *session,
    struct traitfs_mount_state *mount,
    enum traitfs_volume volume
)
{
    struct traitfs_operation operation = {0};
    size_t head = 0U;
    size_t tail = 0U;
    enum traitfs_status status;

    if (session == NULL || mount == NULL ||
        mount->geometry.maximum_cluster != TRAITFS_MAX_MEDIA_CLUSTER) {
        return TRAITFS_STATUS_CORRUPT;
    }
...
{
    uint8_t boot[TRAITFS_SECTOR_BYTES];
    uint8_t backup[TRAITFS_SECTOR_BYTES];
    uint8_t info[TRAITFS_SECTOR_BYTES];
    uint8_t backup_info[TRAITFS_SECTOR_BYTES];
    struct nvme_volume_session session;
    struct traitfs_mount_state candidate;
    ...
    controller = volume == TRAITFS_VOLUME_SYSTEM ?
        TRAITFS_SYSTEM_CONTROLLER : TRAITFS_DATA_CONTROLLER;
    ...
    candidate.read_only = volume == TRAITFS_VOLUME_SYSTEM;
    status = direct_read(&session, 0U, boot);
    if (status == TRAITFS_STATUS_OK &&
        trait_fat32_parse_bpb(boot, sizeof(boot), session.namespace_blocks,
    ...
    if (status == TRAITFS_STATUS_OK &&
        candidate.geometry.bytes_per_sector != TRAITFS_SECTOR_BYTES) {
        status = TRAITFS_STATUS_CORRUPT;
    }
    if (status == TRAITFS_STATUS_OK) {
    ...
    if (status == TRAITFS_STATUS_OK) {
        ...
        status = direct_read(&session, candidate.geometry.fsinfo_sector,
            info);
        if (status == TRAITFS_STATUS_OK &&
            trait_fat32_parse_fsinfo(info, sizeof(info),
    ...
    if (close_status != NVME_STATUS_OK && status == TRAITFS_STATUS_OK) {
        validation_stage = "controller close";
        status = TRAITFS_STATUS_IO;
    }
    invalidate_cache(volume);
    if (status != TRAITFS_STATUS_OK) {
        console_write("Trait OS: FAT32 ");
        console_write(volume == TRAITFS_VOLUME_SYSTEM ? "system" : "data");
        console_write(" mount refused at ");
        console_write(validation_stage);
        console_write(": ");
        console_write(traitfs_status_string(status));
        console_write("\n");
...
static enum traitfs_status write_fsinfo(struct traitfs_operation *operation)
{
    struct traitfs_cache_entry *primary;
    struct traitfs_cache_entry *backup;
    uint32_t free_hint;
    enum traitfs_status status;
    ...
    write_u32(&primary->data[TRAITFS_FSINFO_FREE_OFFSET], free_hint);
    write_u32(&primary->data[TRAITFS_FSINFO_NEXT_OFFSET],
        operation->mount->next_free);
    write_u32(&primary->data[TRAITFS_FSINFO_TRAIL_OFFSET], UINT32_C(0xAA550000));
...
enum traitfs_status fat32_backend_sync(enum traitfs_volume volume)
{
    ...
    close_status = end_operation(&operation, status == TRAITFS_STATUS_OK);
    return status != TRAITFS_STATUS_OK ? status : close_status;
}
...
void fat32_backend_initialize(void)
{
    ...
    cache_stamp = 0U;
    (void)fat32_backend_mount(TRAITFS_VOLUME_SYSTEM);
    (void)fat32_backend_mount(TRAITFS_VOLUME_DATA);
}

struct traitfs_drive_info fat32_backend_drive(enum traitfs_volume volume)
{
    struct traitfs_drive_info result = {0};
    struct traitfs_mount_state *mount;
...
static enum traitfs_status handle_state(
    traitfs_handle handle,
    struct traitfs_handle_state **state
)
{
    ...
    if (state == NULL || encoded_index == 0U ||
        encoded_index > TRAITFS_MAX_HANDLES || generation == 0U) {
...
)
{
    ...
    enum traitfs_status status;
    enum traitfs_status close_status;
    size_t slot = TRAITFS_MAX_HANDLES;

    if (handle == NULL || !valid_volume(volume) ||
        (access != TRAITFS_ACCESS_READ && access != TRAITFS_ACCESS_WRITE &&
            access != TRAITFS_ACCESS_READ_WRITE)) {
        return TRAITFS_STATUS_INVALID_ARGUMENT;
    }
    *handle = 0U;
    if ((access & TRAITFS_ACCESS_WRITE) != 0U && mounts[volume].read_only) {
        return TRAITFS_STATUS_READ_ONLY;
    }
    for (size_t index = 0U; index < TRAITFS_MAX_HANDLES; ++index) {
    ...
    if (close_status != TRAITFS_STATUS_OK) {
        return close_status;
    }
    if ((location.entry.attributes & TRAITFS_ATTR_DIRECTORY) != 0U) {
...
{
    struct traitfs_handle_state *state;
    enum traitfs_status status = handle_state(handle, &state);
...
)
{
    ...
    *position = 0U;
    ...
    if (origin == TRAITFS_SEEK_START) {
        base = 0U;
    } else if (origin == TRAITFS_SEEK_CURRENT) {
        base = state->offset;
    } else if (origin == TRAITFS_SEEK_END) {
        base = state->size;
    } else {
        return TRAITFS_STATUS_INVALID_ARGUMENT;
    }
    ...
    if (target > TRAITFS_MAX_FILE_BYTES) {
        return TRAITFS_STATUS_RANGE;
    }
...
)
{
    ...
    *read_bytes = 0U;
    ...
    if ((state->access & TRAITFS_ACCESS_READ) == 0U) {
    ...
    while (completed < available) {
        ...
        uint32_t ordinal = file_offset / TRAITFS_SECTOR_BYTES;
        uint32_t within = file_offset % TRAITFS_SECTOR_BYTES;
        uint32_t cluster;
        uint64_t sector_number;
        struct traitfs_cache_entry *sector;
        size_t chunk = TRAITFS_SECTOR_BYTES - within;
...
)
{
    ...
    if (offset > TRAITFS_MAX_FILE_BYTES) {
        return TRAITFS_STATUS_RANGE;
    }
    saved = state->offset;
...
static enum traitfs_status handle_location(
    struct traitfs_operation *operation,
    const struct traitfs_handle_state *state,
    struct traitfs_location *location
)
{
    struct traitfs_cache_entry *sector;
    ...
    parsed = trait_fat32_parse_directory_entry(
...
)
{
    ...
    *written_bytes = 0U;
    ...
    if ((state->access & TRAITFS_ACCESS_WRITE) == 0U) {
        return TRAITFS_STATUS_ACCESS;
    }
    if (!add_u64(state->offset, source_bytes, &end_offset) ||
        end_offset > TRAITFS_MAX_FILE_BYTES) {
        return TRAITFS_STATUS_RANGE;
    }
    if (source_bytes == 0U) {
        return TRAITFS_STATUS_OK;
    ...
    required = ((uint32_t)end_offset + TRAITFS_SECTOR_BYTES - 1U) /
        TRAITFS_SECTOR_BYTES;
    first = state->first_cluster;
    if (status == TRAITFS_STATUS_OK) {
        status = ensure_chain(&operation, first, required, &first);
    }
    while (status == TRAITFS_STATUS_OK && completed < source_bytes) {
        uint32_t file_offset = state->offset + (uint32_t)completed;
        uint32_t ordinal = file_offset / TRAITFS_SECTOR_BYTES;
        uint32_t within = file_offset % TRAITFS_SECTOR_BYTES;
        uint32_t cluster;
...
)
{
    ...
    if (status == TRAITFS_STATUS_OK) {
        ...
        stat->directory =
            (location.entry.attributes & TRAITFS_ATTR_DIRECTORY) != 0U;
        stat->read_only = operation.mount->read_only ||
            (location.entry.attributes & TRAITFS_ATTR_READ_ONLY) != 0U;
    }
    close_status = end_operation(&operation, false);
    return status != TRAITFS_STATUS_OK ? status : close_status;
}
...
{
    struct traitfs_operation operation;
    struct traitfs_location directory;
    uint32_t cluster;
    size_t count = 0U;
    enum traitfs_status status;
    enum traitfs_status close_status;
    ...
    *entry_count = 0U;
    ...
    if (status == TRAITFS_STATUS_OK &&
        (directory.entry.attributes & TRAITFS_ATTR_DIRECTORY) == 0U) {
    ...
    for (uint32_t steps = 0U;
         status == TRAITFS_STATUS_OK && steps < TRAITFS_MAX_CHAIN_CLUSTERS;
         ++steps) {
        uint64_t first_sector;
        uint32_t next;
        bool end;
        ...
        for (uint32_t sector_index = 0U;
            ...
            for (uint32_t offset = 0U; offset < TRAITFS_SECTOR_BYTES;
                ...
                struct fat32_directory_entry parsed;
                enum fat32_status parse_status =
                    trait_fat32_parse_directory_entry(&sector->data[offset],
                ...
                if (count >= capacity || count >= TRAITFS_MAX_LIST_ENTRIES) {
                    status = TRAITFS_STATUS_RANGE;
                    goto listing_complete;
                }
                ...
                entries[count].directory =
                    (parsed.attributes & TRAITFS_ATTR_DIRECTORY) != 0U;
                ++count;
            }
        }
        if (status != TRAITFS_STATUS_OK) {
...
listing_complete:
    *entry_count = count;
    close_status = end_operation(&operation, false);
    if (status == TRAITFS_STATUS_OK && close_status != TRAITFS_STATUS_OK) {
        status = close_status;
    }
    return status;
}
...
)
{
    ...
    struct traitfs_parent parent;
    struct traitfs_free_slot slot;
    enum traitfs_status status = begin_operation(volume, true, &operation);
    enum traitfs_status close_status;
...
)
{
    ...
    if (status == TRAITFS_STATUS_OK) {
        ...
        encode_directory_entry(&sector->data[0], dot, TRAITFS_ATTR_DIRECTORY,
            cluster, 0U);
        encode_directory_entry(&sector->data[FAT32_DIRECTORY_ENTRY_BYTES],
            dotdot, TRAITFS_ATTR_DIRECTORY, parent.cluster, 0U);
        status = flush_one(&operation, sector);
    }
    if (status == TRAITFS_STATUS_OK) {
        status = write_directory_entry(&operation, slot.sector, slot.offset,
            parent.name.canonical, TRAITFS_ATTR_DIRECTORY, cluster, 0U);
    }
    if (status != TRAITFS_STATUS_OK && cluster != 0U) {
...
)
{
    ...
    if (status == TRAITFS_STATUS_OK && location_open(volume, &location)) {
...
static enum traitfs_status directory_empty(
    struct traitfs_operation *operation,
    uint32_t directory_cluster,
    bool *empty
)
    ...
    for (uint32_t steps = 0U; steps < TRAITFS_MAX_CHAIN_CLUSTERS; ++steps) {
        ...
        for (uint32_t sector_index = 0U;
            ...
            for (uint32_t offset = 0U; offset < TRAITFS_SECTOR_BYTES;
                ...
                if (trait_fat32_parse_directory_entry(&sector->data[offset],
                        FAT32_DIRECTORY_ENTRY_BYTES, &parsed) !=
                        FAT32_STATUS_OK) {
                    return TRAITFS_STATUS_CORRUPT;
                }
...
)
{
    ...
    status = resolve_path(&operation, path, &location);
    if (status == TRAITFS_STATUS_OK && location.root) {
        status = TRAITFS_STATUS_ACCESS;
    }
    if (status == TRAITFS_STATUS_OK &&
        (location.entry.attributes & TRAITFS_ATTR_DIRECTORY) == 0U) {
        status = TRAITFS_STATUS_NOT_DIRECTORY;
    ...
    if (status == TRAITFS_STATUS_OK && !empty) {
        status = TRAITFS_STATUS_NOT_EMPTY;
    }
    if (status == TRAITFS_STATUS_OK) {
        status = mark_deleted(&operation, &location);
    }
...
)
{
    ...
    if (size > TRAITFS_MAX_FILE_BYTES) {
        return TRAITFS_STATUS_RANGE;
    }
    ...
    first = status == TRAITFS_STATUS_OK ? location.entry.first_cluster : 0U;
    old_size = status == TRAITFS_STATUS_OK ? location.entry.size : 0U;
    required = (checked_size + TRAITFS_SECTOR_BYTES - 1U) /
        TRAITFS_SECTOR_BYTES;
    if (status == TRAITFS_STATUS_OK) {
        status = count_chain(&operation, first, &old_count, &old_last);
        (void)old_last;
    }
    if (status == TRAITFS_STATUS_OK && required > old_count) {
        status = ensure_chain(&operation, first, required, &first);
        if (status == TRAITFS_STATUS_OK) {
    ...
    } else if (status == TRAITFS_STATUS_OK && required == 0U) {
        status = update_location(&operation, &location, 0U, 0U);
        if (status == TRAITFS_STATUS_OK) {
            status = release_chain(&operation, first);
        }
    } else if (status == TRAITFS_STATUS_OK && required < old_count) {
        uint32_t kept;
        uint32_t tail;
        bool end;
    ...
    } else if (status == TRAITFS_STATUS_OK) {
...
static enum traitfs_status destination_inside_directory(
    struct traitfs_operation *operation,
    uint32_t directory_cluster,
...
)
{
    ...
    *inside = false;
    ...
    for (uint32_t depth = 0U; depth <= TRAITFS_MAX_DEPTH; ++depth) {
        struct traitfs_location parent_entry;
        enum traitfs_status status;
        ...
        status = scan_directory(operation, current, &parent_name,
            &parent_entry, NULL);
        if (status != TRAITFS_STATUS_OK ||
            (parent_entry.entry.attributes & TRAITFS_ATTR_DIRECTORY) == 0U ||
...
)
{
    static const uint8_t dotdot[11] =
        {'.', '.', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' '};
    struct traitfs_operation operation;
    struct traitfs_location source_location;
    struct traitfs_parent destination_parent;
    struct traitfs_free_slot destination_slot;
    bool inside = false;
    enum traitfs_status status = begin_operation(volume, true, &operation);
    enum traitfs_status close_status;
    ...
    if (status == TRAITFS_STATUS_OK && source_location.root) {
        status = TRAITFS_STATUS_ACCESS;
    }
    if (status == TRAITFS_STATUS_OK &&
    ...
    if (status == TRAITFS_STATUS_OK &&
        (source_location.entry.attributes & TRAITFS_ATTR_DIRECTORY) != 0U) {
        status = destination_inside_directory(&operation,
            source_location.entry.first_cluster,
            destination_parent.cluster, &inside);
        if (status == TRAITFS_STATUS_OK && inside) {
            status = TRAITFS_STATUS_PATH;
        }
    }
    if (status == TRAITFS_STATUS_OK) {
...
bool fat32_backend_self_test(size_t *completed_tests)
{
    ...
    uint32_t components = 0U;
    struct fat32_name name;
    struct traitfs_cache_entry model[TRAITFS_CACHE_ENTRIES];

    if (completed_tests == NULL) {
        return false;
    }
    *completed_tests = 0U;
    if (trait_fat32_validate_path((const uint8_t *)"projects/notes.txt",
            18U, &components) != FAT32_STATUS_OK || components != 2U) {
        return false;
    }
    ++completed;
    if (trait_fat32_validate_path((const uint8_t *)"../escape", 9U,
            &components) != FAT32_STATUS_ABOVE_ROOT) {
        return false;
    }
    ++completed;
    if (trait_fat32_parse_component((const uint8_t *)"notes.txt", 9U,
    ...
    size_t oldest = 0U;
    for (size_t index = 1U; index < TRAITFS_CACHE_ENTRIES; ++index) {
    ...
    ++completed;
    traitfs_handle encoded = (UINT64_C(7) << 8U) | UINT64_C(3);
...
{
    switch (status) {
    case TRAITFS_STATUS_OK: return "ok";
    case TRAITFS_STATUS_ABSENT: return "volume is absent";
    case TRAITFS_STATUS_CORRUPT: return "FAT32 metadata is inconsistent";
    case TRAITFS_STATUS_IO: return "storage I/O failed";
    case TRAITFS_STATUS_INVALID_ARGUMENT: return "invalid filesystem argument";
    case TRAITFS_STATUS_NOT_MOUNTED: return "volume is not mounted";
    case TRAITFS_STATUS_ALREADY_MOUNTED: return "volume is already mounted";
    case TRAITFS_STATUS_READ_ONLY: return "volume is read-only";
    case TRAITFS_STATUS_NOT_FOUND: return "path was not found";
    case TRAITFS_STATUS_EXISTS: return "destination already exists";
    case TRAITFS_STATUS_NOT_DIRECTORY: return "path component is not a directory";
    case TRAITFS_STATUS_IS_DIRECTORY: return "operation needs a regular file";
    case TRAITFS_STATUS_NOT_EMPTY: return "directory is not empty";
    case TRAITFS_STATUS_BUSY: return "file or volume is busy";
    case TRAITFS_STATUS_NO_HANDLES: return "file-handle table is full";
    case TRAITFS_STATUS_STALE_HANDLE: return "file handle is stale or closed";
    case TRAITFS_STATUS_ACCESS: return "file handle access mode refused the operation";
    case TRAITFS_STATUS_RANGE: return "filesystem offset or capacity is out of range";
    case TRAITFS_STATUS_FULL: return "volume has no free cluster";
    case TRAITFS_STATUS_DIRECTORY_FULL: return "directory has no bounded free slot";
    case TRAITFS_STATUS_NAME: return "filename is outside the 8.3 ASCII subset";
    case TRAITFS_STATUS_PATH: return "path is malformed or escapes the mount";
    case TRAITFS_STATUS_WRITEBACK: return "dirty data could not be written";
    case TRAITFS_STATUS_RESET: return "device generation changed";
