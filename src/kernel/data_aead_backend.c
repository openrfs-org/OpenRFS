...
{
    if (reader == NULL) return DATA_AEAD_ARGUMENT;
    enum data_aead_status result = DATA_AEAD_OK;
    if (reader->active)
        for (unsigned at = 0U; at < DATA_AEAD_SEGMENTS_MAX; ++at)
            if (reader->segment_open[at] &&
                    reader->backend->close(reader->segments[at]) !=
                        OPENRFSFS_STATUS_OK)
                result = DATA_AEAD_IO;
    zero_bytes(reader, sizeof(*reader));
    return result;
}
    ...
    struct data_aead_backend_reader *reader)
{
    ...
    enum data_aead_status result = data_aead_backend_load_manifest(backend,
        ...
        &reader->manifest, &slot);
    if (result != DATA_AEAD_OK) return result;
    reader->backend = backend;
    reader->active = true;
    for (unsigned at = 0U; at < reader->manifest.segment_count; ++at) {
        char path[DATA_AEAD_PATH_MAX + 1U];
        result = data_aead_segment_storage_path(key,
            reader->manifest.segments[at].revision_id, path);
        if (result != DATA_AEAD_OK) break;
        struct openrfsfs_stat stat;
        const enum openrfsfs_status found = backend->lstat_path != NULL ?
            backend->lstat_path(volume, path, &stat) :
            backend->stat_path(volume, path, &stat);
        if (found != OPENRFSFS_STATUS_OK || stat.directory ||
                is_symlink(&stat) ||
                backend->open(volume, path, OPENRFSFS_ACCESS_READ,
                    &reader->segments[at]) != OPENRFSFS_STATUS_OK) {
            result = DATA_AEAD_IO;
            zero_bytes(path, sizeof(path));
            break;
        }
        reader->segment_open[at] = true;
        if (backend->fstat != NULL &&
                (backend->fstat(reader->segments[at], &stat) !=
                    OPENRFSFS_STATUS_OK || stat.directory)) {
            result = DATA_AEAD_IO;
            zero_bytes(path, sizeof(path));
            break;
        }
        reader->physical_bytes[at] = stat.size;
        struct held_segment_reader held = {
            backend, reader->segments[at]
        };
        result = data_aead_manifest_verify_segment(key,
            &reader->manifest, at, stat.size, held_segment_read,
            &held, workspace, workspace_bytes);
        zero_bytes(path, sizeof(path));
        if (result != DATA_AEAD_OK) break;
    }
    if (result != DATA_AEAD_OK)
        (void)data_aead_backend_reader_close(reader);
    zero_bytes(workspace, DATA_AEAD_REWRITE_WORKSPACE_BYTES);
    return result;
}
    ...
    size_t workspace_bytes, size_t *read_bytes)
{
    ...
    if (reader == NULL || !reader->active || key == NULL ||
            destination == NULL || read_bytes == NULL ||
            workspace == NULL ||
            workspace_bytes < DATA_AEAD_REWRITE_WORKSPACE_BYTES) {
        if (destination != NULL) zero_bytes(destination, capacity);
        return DATA_AEAD_ARGUMENT;
    }
    if (offset >= reader->manifest.plaintext_bytes || capacity == 0U)
        return DATA_AEAD_OK;
    const uint64_t available = reader->manifest.plaintext_bytes - offset;
    ...
    while (done < total) {
        ...
        if (index >= reader->manifest.segment_count ||
                !reader->segment_open[index]) {
            result = DATA_AEAD_FORMAT;
            break;
        }
        const struct data_aead_segment *segment =
            &reader->manifest.segments[index];
        const uint64_t local = position % DATA_AEAD_SEGMENT_BYTES;
        if (local >= segment->plaintext_bytes) {
            result = DATA_AEAD_FORMAT;
            break;
        }
        size_t amount = total - done;
        if (segment->plaintext_bytes - local < amount)
            amount = (size_t)(segment->plaintext_bytes - local);
        struct held_segment_reader held = {
            reader->backend, reader->segments[index]
        };
        char binding[DATA_AEAD_PATH_MAX + 1U];
        result = data_aead_segment_binding(reader->manifest.stable_id,
            index, binding);
        if (result == DATA_AEAD_OK) {
            size_t got = 0U;
            result = data_aead_read_range(key, binding,
                reader->physical_bytes[index], local,
                destination + done, amount, held_segment_read, &held,
                workspace, workspace_bytes, &got);
    ...
    else zero_bytes(destination, capacity);
    zero_bytes(workspace, DATA_AEAD_REWRITE_WORKSPACE_BYTES);
    return result;
}
...
static enum data_aead_status migration_compare_source(
    const struct vfs_backend_ops *backend, enum openrfsfs_volume volume,
    const uint8_t key[DATA_AEAD_KEY_BYTES], const char *source_path,
    const struct data_aead_manifest *manifest, uint8_t *workspace,
    size_t workspace_bytes)
{
    struct openrfsfs_stat source_stat;
    const enum openrfsfs_status found = backend->lstat_path != NULL ?
        backend->lstat_path(volume, source_path, &source_stat) :
        backend->stat_path(volume, source_path, &source_stat);
    if (found == OPENRFSFS_STATUS_NOT_FOUND) return DATA_AEAD_OK;
    if (found != OPENRFSFS_STATUS_OK || source_stat.directory ||
            is_symlink(&source_stat)) return DATA_AEAD_IO;
    if (source_stat.size != manifest->plaintext_bytes)
        return DATA_AEAD_CONFLICT;
    openrfsfs_handle source = 0U;
    if (backend->open(volume, source_path, OPENRFSFS_ACCESS_READ,
            &source) != OPENRFSFS_STATUS_OK) return DATA_AEAD_IO;
    enum data_aead_status result = DATA_AEAD_OK;
    if (backend->fstat != NULL &&
            (backend->fstat(source, &source_stat) !=
                OPENRFSFS_STATUS_OK || source_stat.directory ||
             source_stat.size != manifest->plaintext_bytes))
        result = DATA_AEAD_CONFLICT;
    uint8_t plain[DATA_AEAD_CHUNK_BYTES];
    uint64_t global_offset = 0U;
    for (unsigned index = 0U; result == DATA_AEAD_OK &&
            index < manifest->segment_count; ++index) {
        const struct data_aead_segment *segment = &manifest->segments[index];
        char path[DATA_AEAD_PATH_MAX + 1U];
        char binding[DATA_AEAD_PATH_MAX + 1U];
        openrfsfs_handle sealed = 0U;
        bool opened = false;
        ...
        if (result == DATA_AEAD_OK)
            result = data_aead_segment_binding(manifest->stable_id,
                index, binding);
        struct openrfsfs_stat sealed_stat;
        const enum openrfsfs_status located = result == DATA_AEAD_OK ?
            (backend->lstat_path != NULL ?
                backend->lstat_path(volume, path, &sealed_stat) :
                backend->stat_path(volume, path, &sealed_stat)) :
            OPENRFSFS_STATUS_INVALID_ARGUMENT;
        if (result == DATA_AEAD_OK &&
                (located != OPENRFSFS_STATUS_OK || sealed_stat.directory ||
                 is_symlink(&sealed_stat) ||
                 backend->open(volume, path, OPENRFSFS_ACCESS_READ,
                    &sealed) != OPENRFSFS_STATUS_OK))
            result = DATA_AEAD_IO;
        else if (result == DATA_AEAD_OK)
            opened = true;
        struct held_segment_reader reader = {backend, sealed};
        uint8_t header[DATA_AEAD_HEADER_BYTES];
        if (result == DATA_AEAD_OK)
            result = held_segment_read(&reader, 0U, header,
                sizeof(header)) ? data_aead_manifest_check_segment(key,
                    manifest, index, header, sealed_stat.size) :
                DATA_AEAD_IO;
        zero_bytes(header, sizeof(header));
        for (uint64_t local = 0U; result == DATA_AEAD_OK &&
                local < segment->plaintext_bytes;) {
            size_t amount = DATA_AEAD_CHUNK_BYTES;
            if (segment->plaintext_bytes - local < amount)
                amount = (size_t)(segment->plaintext_bytes - local);
            size_t got = 0U;
            result = data_aead_read_range(key, binding, sealed_stat.size,
                local, plain, amount, held_segment_read, &reader,
                workspace, workspace_bytes, &got);
            if (result == DATA_AEAD_OK && got != amount)
                result = DATA_AEAD_IO;
            if (result == DATA_AEAD_OK &&
                    (backend->pread(source, workspace, amount,
                        global_offset, &got) != OPENRFSFS_STATUS_OK ||
                     got != amount))
                result = DATA_AEAD_IO;
            if (result == DATA_AEAD_OK) {
                uint8_t difference = 0U;
                for (size_t at = 0U; at < amount; ++at)
                    difference |= plain[at] ^ workspace[at];
                if (difference != 0U) result = DATA_AEAD_CONFLICT;
            }
            zero_bytes(plain, sizeof(plain));
            zero_bytes(workspace, DATA_AEAD_REWRITE_WORKSPACE_BYTES);
            local += amount;
            global_offset += amount;
        }
        if (opened && backend->close(sealed) != OPENRFSFS_STATUS_OK)
            result = DATA_AEAD_IO;
        zero_bytes(path, sizeof(path));
        zero_bytes(binding, sizeof(binding));
    }
    zero_bytes(plain, sizeof(plain));
    if (backend->close(source) != OPENRFSFS_STATUS_OK)
        result = DATA_AEAD_IO;
    zero_bytes(workspace, DATA_AEAD_REWRITE_WORKSPACE_BYTES);
    return result;
}

static enum data_aead_status migrate_plain_impl(
    const struct vfs_backend_ops *backend, enum openrfsfs_volume volume,
    const uint8_t key[DATA_AEAD_KEY_BYTES], const char *source_path,
    const char *binding_path, bool remove_source,
    uint8_t *workspace, size_t workspace_bytes,
    struct data_aead_manifest *manifest)
{
    if (manifest != NULL) zero_bytes(manifest, sizeof(*manifest));
    if (!required_ops(backend) || volume != OPENRFSFS_VOLUME_DATA ||
            key == NULL || source_path == NULL || binding_path == NULL ||
            workspace == NULL ||
            workspace_bytes < DATA_AEAD_REWRITE_WORKSPACE_BYTES ||
            manifest == NULL) return DATA_AEAD_ARGUMENT;
    uint8_t expected_id[DATA_AEAD_ID_BYTES] = {0};
    uint8_t unused_id[DATA_AEAD_ID_BYTES] = {0};
    enum data_aead_status result = data_aead_migration_ids(key,
        source_path, 0U, expected_id, unused_id);
    zero_bytes(unused_id, sizeof(unused_id));
    if (result != DATA_AEAD_OK) return result;
    unsigned slot = 0U;
    result = data_aead_backend_load_manifest(backend,
        volume, key, binding_path, workspace, workspace_bytes,
        manifest, &slot);
    if (result == DATA_AEAD_OK) {
        if (!same_id(manifest->stable_id, expected_id))
            result = DATA_AEAD_CONFLICT;
        if (result == DATA_AEAD_OK)
            result = migration_compare_source(backend, volume, key,
                source_path, manifest, workspace, workspace_bytes);
        if (remove_source && result == DATA_AEAD_OK)
            result = migration_remove_source(backend, source_path);
        if (result != DATA_AEAD_OK) zero_bytes(manifest, sizeof(*manifest));
        zero_bytes(expected_id, sizeof(expected_id));
        return result;
    }
    if (result != DATA_AEAD_NOT_FOUND) {
        zero_bytes(expected_id, sizeof(expected_id));
        return result;
    }
    ...
    for (size_t at = 0U; at < DATA_AEAD_ID_BYTES; ++at)
        candidate.stable_id[at] = expected_id[at];
    ...
    for (unsigned at = 0U; at < candidate.segment_count; ++at) {
        ...
        result = data_aead_migration_ids(key, source_path, at,
    ...
    }
    {
        ...
        result = data_aead_backend_publish_manifest(backend, volume, key,
            binding_path, &candidate, workspace, workspace_bytes,
            &published, &slot);
        if (result == DATA_AEAD_OK) {
        ...
        if (result == DATA_AEAD_OK)
            result = migration_compare_source(backend, volume, key,
                source_path, &published, workspace, workspace_bytes);
        if (result == DATA_AEAD_OK && remove_source)
            result = migration_remove_source(backend, source_path);
...
enum data_aead_status data_aead_backend_migrate_plain(
    ...
    struct data_aead_manifest *manifest)
{
    return migrate_plain_impl(backend, volume, key, canonical_path,
        canonical_path, true, workspace, workspace_bytes, manifest);
}
...
enum data_aead_status data_aead_backend_migrate_plain_as(
    ...
    struct data_aead_manifest *manifest)
{
    return migrate_plain_impl(backend, volume, key, source_path,
        binding_path, false, workspace, workspace_bytes, manifest);
}
...
static void patch_part(uint64_t base, uint64_t segment_bytes,
    uint64_t patch_offset, const uint8_t *patch, size_t patch_bytes,
    uint64_t *local_offset, const uint8_t **part, size_t *part_bytes)
{
    *local_offset = 0U;
    *part = NULL;
    *part_bytes = 0U;
    if (patch_bytes == 0U) return;
    const uint64_t start = patch_offset > base ? patch_offset : base;
    const uint64_t patch_end = patch_offset + patch_bytes;
    const uint64_t segment_end = base + segment_bytes;
    const uint64_t end = patch_end < segment_end ? patch_end :
        segment_end;
    if (end <= start) return;
    *local_offset = start - base;
    *part = patch + (size_t)(start - patch_offset);
    *part_bytes = (size_t)(end - start);
}

static enum data_aead_status rewrite_revision_id(
    const uint8_t key[DATA_AEAD_KEY_BYTES],
    const uint8_t stable_id[DATA_AEAD_ID_BYTES],
    const uint8_t old_revision[DATA_AEAD_ID_BYTES],
    uint64_t generation, unsigned index, uint64_t old_bytes,
    uint64_t new_bytes, uint64_t patch_offset,
    const uint8_t *patch, size_t patch_bytes,
    uint8_t revision_id[DATA_AEAD_ID_BYTES])
{
    static const uint8_t label[] = "OpenRFS/v1/data/rewrite-revision";
    uint8_t numbers[48] = {0};
    encode_u64(numbers, generation);
    encode_u64(numbers + 8U, index);
    encode_u64(numbers + 16U, old_bytes);
    encode_u64(numbers + 24U, new_bytes);
    encode_u64(numbers + 32U, patch_offset);
    encode_u64(numbers + 40U, patch_bytes);
    crypto_blake2b_ctx hash;
    crypto_blake2b_keyed_init(&hash, DATA_AEAD_ID_BYTES, key,
        DATA_AEAD_KEY_BYTES);
    crypto_blake2b_update(&hash, label, sizeof(label) - 1U);
    ...
    crypto_blake2b_update(&hash, numbers, sizeof(numbers));
    if (patch_bytes != 0U)
        crypto_blake2b_update(&hash, patch, patch_bytes);
    crypto_blake2b_final(&hash, revision_id);
    crypto_wipe(&hash, sizeof(hash));
    zero_bytes(numbers, sizeof(numbers));
    uint8_t any = 0U;
...
static enum data_aead_status rewrite_candidate(
    const uint8_t key[DATA_AEAD_KEY_BYTES],
    const struct data_aead_manifest *active,
    uint64_t new_plaintext_bytes, uint64_t patch_offset,
    const uint8_t *patch, size_t patch_bytes,
    struct data_aead_manifest *candidate)
{
    if (active->generation == UINT64_MAX ||
            new_plaintext_bytes >
                DATA_AEAD_SEGMENT_BYTES * DATA_AEAD_SEGMENTS_MAX ||
    ...
    *candidate = *active;
    candidate->generation++;
    candidate->plaintext_bytes = new_plaintext_bytes;
    candidate->segment_count = (unsigned)((new_plaintext_bytes +
        DATA_AEAD_SEGMENT_BYTES - 1U) / DATA_AEAD_SEGMENT_BYTES);
    for (unsigned index = 0U; index < candidate->segment_count; ++index) {
        const uint64_t base = (uint64_t)index * DATA_AEAD_SEGMENT_BYTES;
        const uint64_t remaining = new_plaintext_bytes - base;
        const uint64_t new_bytes = remaining < DATA_AEAD_SEGMENT_BYTES ?
            remaining : DATA_AEAD_SEGMENT_BYTES;
        const bool replacing = index < active->segment_count;
        const struct data_aead_segment *previous = replacing ?
            &active->segments[index] : NULL;
        const uint64_t old_bytes = replacing ?
            previous->plaintext_bytes : 0U;
        uint64_t local_offset = 0U;
        const uint8_t *part = NULL;
        size_t part_bytes = 0U;
        patch_part(base, new_bytes, patch_offset, patch, patch_bytes,
            &local_offset, &part, &part_bytes);
        if (replacing && old_bytes == new_bytes && part_bytes == 0U)
            continue;
        struct data_aead_segment *next = &candidate->segments[index];
        uint8_t old_revision[DATA_AEAD_ID_BYTES] = {0};
        if (replacing)
            for (size_t at = 0U; at < DATA_AEAD_ID_BYTES; ++at)
                old_revision[at] = previous->revision_id[at];
        const enum data_aead_status result = rewrite_revision_id(key,
            active->stable_id, old_revision, candidate->generation,
            index, old_bytes, new_bytes, local_offset, part,
            part_bytes, next->revision_id);
        zero_bytes(old_revision, sizeof(old_revision));
        if (result != DATA_AEAD_OK) return result;
        next->plaintext_bytes = new_bytes;
        next->generation = replacing ? previous->generation + 1U : 1U;
        if (next->generation == 0U) return DATA_AEAD_RANGE;
    }
    for (unsigned at = candidate->segment_count;
            at < DATA_AEAD_SEGMENTS_MAX; ++at)
        zero_bytes(&candidate->segments[at],
            sizeof(candidate->segments[at]));
    return DATA_AEAD_OK;
}
...
{
    if (published != NULL) zero_bytes(published, sizeof(*published));
    if (published == NULL || key == NULL ||
            (patch_bytes != 0U && patch == NULL) ||
            patch_bytes > DATA_AEAD_CHUNK_BYTES ||
            expected_generation == 0U || workspace == NULL ||
            workspace_bytes < DATA_AEAD_REWRITE_WORKSPACE_BYTES)
        return DATA_AEAD_ARGUMENT;
    struct data_aead_manifest active = {0};
    ...
    if (expected_generation != UINT64_MAX &&
        ...
        if (result == DATA_AEAD_OK &&
            ...
            result = DATA_AEAD_CONFLICT;
        if (result == DATA_AEAD_OK)
            result = rewrite_candidate(key, &previous,
                new_plaintext_bytes, patch_offset, patch, patch_bytes,
                &predicted);
        if (result == DATA_AEAD_OK && !same_manifest(&active, &predicted))
            result = DATA_AEAD_CONFLICT;
        if (result == DATA_AEAD_OK) *published = active;
    ...
    if (patch_bytes == 0U &&
            new_plaintext_bytes == active.plaintext_bytes) {
        *published = active;
        result = DATA_AEAD_OK;
        goto done;
    }
    struct data_aead_manifest candidate = {0};
    result = rewrite_candidate(key, &active, new_plaintext_bytes,
        patch_offset, patch, patch_bytes, &candidate);
    if (result != DATA_AEAD_OK) goto done;
    for (unsigned index = 0U; index < candidate.segment_count; ++index) {
        const struct data_aead_segment *next = &candidate.segments[index];
        const bool replacing = index < active.segment_count;
        const struct data_aead_segment *previous = replacing ?
            &active.segments[index] : NULL;
        if (replacing &&
                same_id(next->revision_id, previous->revision_id))
            continue;
        const uint64_t base = (uint64_t)index * DATA_AEAD_SEGMENT_BYTES;
        uint64_t local_offset = 0U;
        const uint8_t *part = NULL;
        size_t part_bytes = 0U;
        patch_part(base, next->plaintext_bytes, patch_offset, patch,
            patch_bytes, &local_offset, &part, &part_bytes);
        struct migration_context migration = {0};
        migration.storage.backend = backend;
        migration.storage.volume = volume;
        migration.storage.key = key;
        result = data_aead_segment_storage_path(key, next->revision_id,
            migration.shadow_path);
        uint64_t old_physical_bytes = 0U;
        if (result == DATA_AEAD_OK && replacing) {
            char old_path[DATA_AEAD_PATH_MAX + 1U];
            result = data_aead_segment_storage_path(key,
                previous->revision_id, old_path);
            if (result == DATA_AEAD_OK) {
                ...
                const enum openrfsfs_status found =
                    ...
                    backend->lstat_path(volume, old_path, &stat) :
                    backend->stat_path(volume, old_path, &stat);
                old_physical_bytes = data_aead_physical_size(
                    previous->plaintext_bytes);
                if (found != OPENRFSFS_STATUS_OK || stat.directory ||
                        is_symlink(&stat) ||
                        stat.size != old_physical_bytes ||
                        backend->open(volume, old_path,
        ...
        if (result == DATA_AEAD_OK) {
            ...
            if (result == DATA_AEAD_OK) {
                ...
                result = data_aead_rewrite_shadow_paths_identified(key,
                    binding, binding, old_physical_bytes,
                    next->plaintext_bytes, local_offset, part, part_bytes,
                    active.stable_id, next->revision_id,
                    next->generation, &io, workspace, workspace_bytes,
                    &new_physical_bytes);
                if (result == DATA_AEAD_OK &&
                        (!migration.shadow_open ||
                         migration.shadow_offset != new_physical_bytes ||
