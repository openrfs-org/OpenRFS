 ...
 * installs would be advisory for ring 0 - which is the only ring Trait OS has.
 */
 ...
 * Trait OS mappings select entry 0 for RAM and entry 3 for device registers, so
...
_Static_assert(
    TRAIT_EARLY_PHYSICAL_LIMIT % PAGING_HUGE_PAGE_SIZE == 0U,
    "the identity window is not a whole number of 2 MiB pages"
);
_Static_assert(
    PAGING_PROBE_ADDRESS >= TRAIT_EARLY_PHYSICAL_LIMIT,
...
/*
 * Every table frame is below TRAIT_EARLY_PHYSICAL_LIMIT and that whole range
...
)
{
     ...
     */
    if ((uint64_t)frame >= TRAIT_EARLY_PHYSICAL_LIMIT) {
        return PAGING_STATUS_PHYSICAL_TOO_WIDE;
    }
...
)
{
    ...
    for (uint64_t base = 0U; base < TRAIT_EARLY_PHYSICAL_LIMIT;
        ...
        for (uint64_t offset = 0U; offset < PAGING_HUGE_PAGE_SIZE;
            ...
            /*
             * The null page stays absent. Nothing in Trait OS reads physical
...
static bool test_table_supply(void)
{
    ...
    hierarchy.arena_base = TRAIT_EARLY_PHYSICAL_LIMIT;
