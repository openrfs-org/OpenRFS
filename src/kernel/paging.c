 ...
 * installs would be advisory for ring 0 - which is the only ring OpenGAT has.
 */
 ...
 * OpenGAT mappings select entry 0 for RAM and entry 3 for device registers, so
...
_Static_assert(
    OPENGAT_EARLY_PHYSICAL_LIMIT % PAGING_HUGE_PAGE_SIZE == 0U,
    "the identity window is not a whole number of 2 MiB pages"
);
_Static_assert(
    PAGING_PROBE_ADDRESS >= OPENGAT_EARLY_PHYSICAL_LIMIT,
...
/*
 * Every table frame is below OPENGAT_EARLY_PHYSICAL_LIMIT and that whole range
...
)
{
     ...
     */
    if ((uint64_t)frame >= OPENGAT_EARLY_PHYSICAL_LIMIT) {
        return PAGING_STATUS_PHYSICAL_TOO_WIDE;
    }
...
)
{
    ...
    for (uint64_t base = 0U; base < OPENGAT_EARLY_PHYSICAL_LIMIT;
        ...
        for (uint64_t offset = 0U; offset < PAGING_HUGE_PAGE_SIZE;
            ...
            /*
             * The null page stays absent. Nothing in OpenGAT reads physical
...
static bool test_table_supply(void)
{
    ...
    hierarchy.arena_base = OPENGAT_EARLY_PHYSICAL_LIMIT;
