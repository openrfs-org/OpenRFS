...
//! Checked, journaled ext4 operations over Trait OS's native block boundary.
...
impl Display for BlockReadError {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        formatter.write_str("Trait OS block read failed")
    }
}
...
impl Display for BlockStorageError {
    fn fmt(&self, formatter: &mut Formatter<'_>) -> fmt::Result {
        formatter.write_str("Trait OS block write or flush failed")
    }
}

impl Error for BlockStorageError {}

struct TraitOSReader {
    context: usize,
}

impl Ext4Read for TraitOSReader {
    fn read(
        &self,
        start_byte: u64,
        destination: &mut [u8],
...
struct TraitOSJournalStorage {
    context: usize,
}

impl JournalStorage for TraitOSJournalStorage {
...
fn execute_storage_plan(
    ...
    execute_commit_operations(&mut TraitOSJournalStorage { context }, operations).map_err(|error| {
        match error {
...
fn load_staged_view(
    ...
    let stage = Rc::new(
        JournalMutationStage::new(Box::new(TraitOSReader { context }), image_bytes)
...
/// Load and validate the exact Trait OS ext4 profile and reachable namespace.
pub(crate) fn mount(context: usize, media_bytes: u64) -> Result<(Box<Mounted>, Identity), Status> {
    ...
    let mut filesystem = Ext4::load(Box::new(TraitOSReader { context })).map_err(map_error)?;
    ...
    if journal.filesystem_needs_recovery() {
        ...
        filesystem = Ext4::load(Box::new(TraitOSReader { context })).map_err(map_error)?;
...
pub(crate) enum Status {
    ...
    NullArgument = 1,
    /// Reserved ABI value retained for existing native callers.
    #[allow(dead_code)]
    Volume = 2,
