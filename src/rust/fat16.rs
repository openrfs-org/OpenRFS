...
/// Canonical on-disk 8.3 bytes for `OPENGAT.BIN`.
pub const OPENGAT_NAME: [u8; 11] = *b"OPENGAT BIN";
...
pub fn make_query(name: &[u8]) -> Result<RootQuery, Status> {
    if !canonical_name(name) || name != OPENGAT_NAME {
...
pub fn find_root(
    ...
    if !canonical_name(&query.canonical_name)
        || query.canonical_name != OPENGAT_NAME
    {
        return Err(Status::NameMalformed);
    }
...
pub fn validate_extent(
    ...
    validate_geometry(geometry)?;
    if entry.canonical_name != OPENGAT_NAME || entry.attribute != ATTR_ARCHIVE {
        return Err(Status::UnsupportedEntry);
    }
    let maximum_cluster = geometry.cluster_count
        .checked_add(1)
...
fn make_bpb(block: &mut [u8; BLOCK_BYTES]) {
    ...
    block[3..11].copy_from_slice(b"OPENGAT ");
...
fn make_root(block: &mut [u8; BLOCK_BYTES]) {
    block.fill(0);
    block[..11].copy_from_slice(&OPENGAT_NAME);
...
pub fn self_test() -> u32 {
    ...
    let query = match make_query(&OPENGAT_NAME) { Ok(value) => value, Err(_) => return 0 };
    make_root(&mut block);
    ...
    make_root(&mut block);
    block[32..43].copy_from_slice(&OPENGAT_NAME);
