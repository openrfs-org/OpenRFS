...
//! The parts of OpenRFS written in Rust.
//!
//! OpenRFS is a C kernel. Rust is used here for one specific job: parsing input
...
//! OpenRFS's bounded kernel heap through the allocator boundary in [`abi`]; the
...
///
/// OpenRFS-authored fallible paths return a status, while compiler-inserted
