...
//! The parts of OpenGAT written in Rust.
//!
//! OpenGAT is a C kernel. Rust is used here for one specific job: parsing input
...
//! OpenGAT's bounded kernel heap through the allocator boundary in [`abi`]; the
...
///
/// OpenGAT-authored fallible paths return a status, while compiler-inserted
