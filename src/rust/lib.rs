...
//! The parts of Trait OS written in Rust.
//!
//! Trait OS is a C kernel. Rust is used here for one specific job: parsing input
...
//! Trait OS's bounded kernel heap through the allocator boundary in [`abi`]; the
...
///
/// Trait-OS-authored fallible paths return a status, while compiler-inserted
