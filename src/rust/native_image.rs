...
//! Safe validators for the OpenRFS application manifest and static ELF64 image.
...
const PAGE: u64 = 4096;
const MANIFEST_MAGIC: &[u8; 8] = b"OPENRFS1";
...
mod tests {
    ...
    fn manifest() -> [u8; MANIFEST_BYTES] {
        ...
        bytes[..8].copy_from_slice(b"OPENRFS1");
    ...
    fn manifest_accepts_printable_url_argument() {
        let mut bytes = manifest();
        let url = b"http://openrfs.test/welcome.txt\0";
