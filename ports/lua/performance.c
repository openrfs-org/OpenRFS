...
void trait_application_entry_probe(void);
void __real_luaL_openlibs(lua_State *state);
void __wrap_luaL_openlibs(lua_State *state);

void trait_application_entry_probe(void)
{
    startup_started = trait_monotonic_ns();
}

void __wrap_luaL_openlibs(lua_State *state)
{
    __real_luaL_openlibs(state);
    printf("TRAIT PERF lua startup_ns=%llu\n",
        (unsigned long long)(trait_monotonic_ns() - startup_started));
}
