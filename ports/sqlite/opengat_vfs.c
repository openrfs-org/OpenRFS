...
/* SQLite VFS over the public OpenGAT SDK. WAL and cross-process locks are not exposed. */
...
#define OPENGAT_SQLITE_OPEN_MAX 16

struct opengat_sqlite_file {
    sqlite3_file base;
    int descriptor;
    int lock_level;
    int delete_on_close;
...
static struct opengat_sqlite_file *open_files[OPENGAT_SQLITE_OPEN_MAX];
...
static int register_file(struct opengat_sqlite_file *file)
{
    for (size_t index = 0U; index < OPENGAT_SQLITE_OPEN_MAX; ++index) {
...
static void unregister_file(struct opengat_sqlite_file *file)
{
    for (size_t index = 0U; index < OPENGAT_SQLITE_OPEN_MAX; ++index) {
...
static int opengat_close(sqlite3_file *base)
{
    struct opengat_sqlite_file *file = (struct opengat_sqlite_file *)base;
...
static int opengat_read(sqlite3_file *base, void *buffer, int amount,
...
static int opengat_write(sqlite3_file *base, const void *buffer, int amount,
    sqlite3_int64 offset)
{
    struct opengat_sqlite_file *file = (struct opengat_sqlite_file *)base;
...
static int opengat_truncate(sqlite3_file *base, sqlite3_int64 size)
{
    const struct opengat_sqlite_file *file =
        (const struct opengat_sqlite_file *)base;
    return size >= 0 && ftruncate(file->descriptor, size) == 0 ?
        SQLITE_OK : SQLITE_IOERR_TRUNCATE;
}

static int opengat_sync(sqlite3_file *base, int flags)
{
    const struct opengat_sqlite_file *file =
        (const struct opengat_sqlite_file *)base;
    (void)flags;
...
static int opengat_file_size(sqlite3_file *base, sqlite3_int64 *size)
{
    const struct opengat_sqlite_file *file =
        (const struct opengat_sqlite_file *)base;
...
static int lock_conflict(const struct opengat_sqlite_file *file, int requested)
{
    for (size_t index = 0U; index < OPENGAT_SQLITE_OPEN_MAX; ++index) {
        const struct opengat_sqlite_file *other = open_files[index];
...
static int opengat_lock(sqlite3_file *base, int requested)
{
    struct opengat_sqlite_file *file = (struct opengat_sqlite_file *)base;
...
static int opengat_unlock(sqlite3_file *base, int requested)
{
    struct opengat_sqlite_file *file = (struct opengat_sqlite_file *)base;
...
static int opengat_check_reserved(sqlite3_file *base, int *result)
{
    const struct opengat_sqlite_file *file =
        (const struct opengat_sqlite_file *)base;
    *result = 0;
...
static int opengat_file_control(sqlite3_file *base, int operation, void *argument)
{
    (void)base;
    (void)operation;
    (void)argument;
...
static int opengat_sector_size(sqlite3_file *base)
{
    (void)base;
    return 512;
}

static int opengat_device_characteristics(sqlite3_file *base)
{
    (void)base;
    return 0;
}

static const sqlite3_io_methods opengat_io = {
    .iVersion = 1,
    .xClose = opengat_close,
    .xRead = opengat_read,
    .xWrite = opengat_write,
    .xTruncate = opengat_truncate,
    .xSync = opengat_sync,
    .xFileSize = opengat_file_size,
    .xLock = opengat_lock,
    .xUnlock = opengat_unlock,
    .xCheckReservedLock = opengat_check_reserved,
    .xFileControl = opengat_file_control,
    .xSectorSize = opengat_sector_size,
    .xDeviceCharacteristics = opengat_device_characteristics
};

static int opengat_open(sqlite3_vfs *vfs, const char *logical,
    sqlite3_file *base, int flags, int *output_flags)
{
    ...
    file->base.pMethods = &opengat_io;
    if (output_flags != NULL) *output_flags = flags;
    return SQLITE_OK;
}

static int opengat_delete(sqlite3_vfs *vfs, const char *logical, int sync_dir)
{
    char path[FILENAME_MAX];
    (void)vfs;
    if (map_path(logical, path) != SQLITE_OK) return SQLITE_IOERR_DELETE;
    if (unlink(path) != 0 && errno != ENOENT) return SQLITE_IOERR_DELETE;
    if (sync_dir && opengat_syscall1(OPENGAT_SYS_VOLUME_SYNC,
            OPENGAT_VOLUME_DATA) < 0) {
        return SQLITE_IOERR_DIR_FSYNC;
    }
    return SQLITE_OK;
}

static int opengat_access(sqlite3_vfs *vfs, const char *logical, int flags,
    int *result)
{
    char path[FILENAME_MAX];
    int mode = F_OK;
...
static int opengat_full_path(sqlite3_vfs *vfs, const char *logical,
...
static int opengat_randomness(sqlite3_vfs *vfs, int amount, char *output)
{
    const long result = amount > 0 ? opengat_random(output, (size_t)amount) : 0;
    (void)vfs;
    return result < 0 ? 0 : (int)result;
}

static int opengat_sleep(sqlite3_vfs *vfs, int microseconds)
{
    (void)vfs;
...
static int opengat_current_time(sqlite3_vfs *vfs, double *result)
{
    (void)vfs;
    *result = 2440587.5 + (double)opengat_monotonic_ns() /
        (86400.0 * 1000000000.0);
    return SQLITE_OK;
}

static int opengat_last_error(sqlite3_vfs *vfs, int capacity, char *output)
{
...
static sqlite3_vfs opengat_vfs = {
    .iVersion = 1,
    .szOsFile = sizeof(struct opengat_sqlite_file),
    .mxPathname = FILENAME_MAX,
    .zName = "opengat",
    .xOpen = opengat_open,
    .xDelete = opengat_delete,
    .xAccess = opengat_access,
    .xFullPathname = opengat_full_path,
    .xRandomness = opengat_randomness,
    .xSleep = opengat_sleep,
    .xCurrentTime = opengat_current_time,
    .xGetLastError = opengat_last_error
};

int sqlite3_os_init(void)
{
    (void)memset(open_files, 0, sizeof(open_files));
    return sqlite3_vfs_register(&opengat_vfs, 1);
}
