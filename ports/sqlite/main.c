...
static int first_phase(void)
{
    ...
    printf("OPENGAT PERF sqlite transaction_ns=%llu\n",
        (unsigned long long)transaction_elapsed);
    puts("OPENGAT SQLITE PHASE1 PASS rows=3 locking=busy");
    return 0;
...
static int second_phase(void)
{
    ...
    reopen_started = opengat_monotonic_ns();
    ...
    database = NULL;
    reopen_elapsed = opengat_monotonic_ns() - reopen_started;
    result = fopen("RESULT.TXT", "w");
    if (result == NULL) return 1;
    write_ok = fwrite(output, 1U, sizeof(output) - 1U, result) ==
        sizeof(output) - 1U;
    if (fclose(result) != 0 || !write_ok) return 1;
    printf("OPENGAT PERF sqlite reopen_query_ns=%llu\n",
        (unsigned long long)reopen_elapsed);
    puts("OPENGAT SQLITE PHASE2 PASS rows=3 sum=66 integrity=ok");
    return 0;
