...
/* SQLite VFS over the public OpenRFS SDK. WAL and cross-process locks are not exposed. */
...
#define OPENRFS_SQLITE_OPEN_MAX 16

struct openrfs_sqlite_file {
    sqlite3_file base;
    int descriptor;
    int lock_level;
    int delete_on_close;
...
static struct openrfs_sqlite_file *open_files[OPENRFS_SQLITE_OPEN_MAX];
...
static int register_file(struct openrfs_sqlite_file *file)
{
    for (size_t index = 0U; index < OPENRFS_SQLITE_OPEN_MAX; ++index) {
...
static void unregister_file(struct openrfs_sqlite_file *file)
{
    for (size_t index = 0U; index < OPENRFS_SQLITE_OPEN_MAX; ++index) {
...
static int openrfs_close(sqlite3_file *base)
{
    struct openrfs_sqlite_file *file = (struct openrfs_sqlite_file *)base;
...
static int openrfs_read(sqlite3_file *base, void *buffer, int amount,
...
static int openrfs_write(sqlite3_file *base, const void *buffer, int amount,
    sqlite3_int64 offset)
{
    struct openrfs_sqlite_file *file = (struct openrfs_sqlite_file *)base;
...
static int openrfs_truncate(sqlite3_file *base, sqlite3_int64 size)
{
    const struct openrfs_sqlite_file *file =
        (const struct openrfs_sqlite_file *)base;
    return size >= 0 && ftruncate(file->descriptor, size) == 0 ?
        SQLITE_OK : SQLITE_IOERR_TRUNCATE;
}

static int openrfs_sync(sqlite3_file *base, int flags)
{
    const struct openrfs_sqlite_file *file =
        (const struct openrfs_sqlite_file *)base;
    (void)flags;
...
static int openrfs_file_size(sqlite3_file *base, sqlite3_int64 *size)
{
    const struct openrfs_sqlite_file *file =
        (const struct openrfs_sqlite_file *)base;
...
static int lock_conflict(const struct openrfs_sqlite_file *file, int requested)
{
    for (size_t index = 0U; index < OPENRFS_SQLITE_OPEN_MAX; ++index) {
        const struct openrfs_sqlite_file *other = open_files[index];
...
static int openrfs_lock(sqlite3_file *base, int requested)
{
    struct openrfs_sqlite_file *file = (struct openrfs_sqlite_file *)base;
...
static int openrfs_unlock(sqlite3_file *base, int requested)
{
    struct openrfs_sqlite_file *file = (struct openrfs_sqlite_file *)base;
...
static int openrfs_check_reserved(sqlite3_file *base, int *result)
{
    const struct openrfs_sqlite_file *file =
        (const struct openrfs_sqlite_file *)base;
    *result = 0;
...
static int openrfs_file_control(sqlite3_file *base, int operation, void *argument)
{
    (void)base;
    (void)operation;
    (void)argument;
...
static int openrfs_sector_size(sqlite3_file *base)
{
    (void)base;
    return 512;
}

static int openrfs_device_characteristics(sqlite3_file *base)
{
    (void)base;
    return 0;
}

static const sqlite3_io_methods openrfs_io = {
    .iVersion = 1,
    .xClose = openrfs_close,
    .xRead = openrfs_read,
    .xWrite = openrfs_write,
    .xTruncate = openrfs_truncate,
    .xSync = openrfs_sync,
    .xFileSize = openrfs_file_size,
    .xLock = openrfs_lock,
    .xUnlock = openrfs_unlock,
    .xCheckReservedLock = openrfs_check_reserved,
    .xFileControl = openrfs_file_control,
    .xSectorSize = openrfs_sector_size,
    .xDeviceCharacteristics = openrfs_device_characteristics
};

static int openrfs_open(sqlite3_vfs *vfs, const char *logical,
    sqlite3_file *base, int flags, int *output_flags)
{
    ...
    file->base.pMethods = &openrfs_io;
    if (output_flags != NULL) *output_flags = flags;
    return SQLITE_OK;
}

static int openrfs_delete(sqlite3_vfs *vfs, const char *logical, int sync_dir)
{
    char path[FILENAME_MAX];
    (void)vfs;
    if (map_path(logical, path) != SQLITE_OK) return SQLITE_IOERR_DELETE;
    if (unlink(path) != 0 && errno != ENOENT) return SQLITE_IOERR_DELETE;
    if (sync_dir && openrfs_syscall1(OPENRFS_SYS_VOLUME_SYNC,
            OPENRFS_VOLUME_DATA) < 0) {
        return SQLITE_IOERR_DIR_FSYNC;
    }
    return SQLITE_OK;
}

static int openrfs_access(sqlite3_vfs *vfs, const char *logical, int flags,
    int *result)
{
    char path[FILENAME_MAX];
    int mode = F_OK;
...
static int openrfs_full_path(sqlite3_vfs *vfs, const char *logical,
...
static int openrfs_randomness(sqlite3_vfs *vfs, int amount, char *output)
{
    const long result = amount > 0 ? openrfs_random(output, (size_t)amount) : 0;
    (void)vfs;
    return result < 0 ? 0 : (int)result;
}

static int openrfs_sleep(sqlite3_vfs *vfs, int microseconds)
{
    (void)vfs;
...
static int openrfs_current_time(sqlite3_vfs *vfs, double *result)
{
    (void)vfs;
    *result = 2440587.5 + (double)openrfs_monotonic_ns() /
        (86400.0 * 1000000000.0);
    return SQLITE_OK;
}

static int openrfs_last_error(sqlite3_vfs *vfs, int capacity, char *output)
{
...
static sqlite3_vfs openrfs_vfs = {
    .iVersion = 1,
    .szOsFile = sizeof(struct openrfs_sqlite_file),
    .mxPathname = FILENAME_MAX,
    .zName = "openrfs",
    .xOpen = openrfs_open,
    .xDelete = openrfs_delete,
    .xAccess = openrfs_access,
    .xFullPathname = openrfs_full_path,
    .xRandomness = openrfs_randomness,
    .xSleep = openrfs_sleep,
    .xCurrentTime = openrfs_current_time,
    .xGetLastError = openrfs_last_error
};

int sqlite3_os_init(void)
{
    (void)memset(open_files, 0, sizeof(open_files));
    return sqlite3_vfs_register(&openrfs_vfs, 1);
}
