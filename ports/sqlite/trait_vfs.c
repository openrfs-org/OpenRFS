...
/* SQLite VFS over the public Trait OS SDK. WAL and cross-process locks are not exposed. */
...
#define TRAIT_SQLITE_OPEN_MAX 16

struct trait_sqlite_file {
    sqlite3_file base;
    int descriptor;
    int lock_level;
    int delete_on_close;
...
static struct trait_sqlite_file *open_files[TRAIT_SQLITE_OPEN_MAX];
...
static int register_file(struct trait_sqlite_file *file)
{
    for (size_t index = 0U; index < TRAIT_SQLITE_OPEN_MAX; ++index) {
...
static void unregister_file(struct trait_sqlite_file *file)
{
    for (size_t index = 0U; index < TRAIT_SQLITE_OPEN_MAX; ++index) {
...
static int trait_close(sqlite3_file *base)
{
    struct trait_sqlite_file *file = (struct trait_sqlite_file *)base;
...
static int trait_read(sqlite3_file *base, void *buffer, int amount,
...
static int trait_write(sqlite3_file *base, const void *buffer, int amount,
    sqlite3_int64 offset)
{
    struct trait_sqlite_file *file = (struct trait_sqlite_file *)base;
...
static int trait_truncate(sqlite3_file *base, sqlite3_int64 size)
{
    const struct trait_sqlite_file *file =
        (const struct trait_sqlite_file *)base;
    return size >= 0 && ftruncate(file->descriptor, size) == 0 ?
        SQLITE_OK : SQLITE_IOERR_TRUNCATE;
}

static int trait_sync(sqlite3_file *base, int flags)
{
    const struct trait_sqlite_file *file =
        (const struct trait_sqlite_file *)base;
    (void)flags;
...
static int trait_file_size(sqlite3_file *base, sqlite3_int64 *size)
{
    const struct trait_sqlite_file *file =
        (const struct trait_sqlite_file *)base;
...
static int lock_conflict(const struct trait_sqlite_file *file, int requested)
{
    for (size_t index = 0U; index < TRAIT_SQLITE_OPEN_MAX; ++index) {
        const struct trait_sqlite_file *other = open_files[index];
...
static int trait_lock(sqlite3_file *base, int requested)
{
    struct trait_sqlite_file *file = (struct trait_sqlite_file *)base;
...
static int trait_unlock(sqlite3_file *base, int requested)
{
    struct trait_sqlite_file *file = (struct trait_sqlite_file *)base;
...
static int trait_check_reserved(sqlite3_file *base, int *result)
{
    const struct trait_sqlite_file *file =
        (const struct trait_sqlite_file *)base;
    *result = 0;
...
static int trait_file_control(sqlite3_file *base, int operation, void *argument)
{
    (void)base;
    (void)operation;
    (void)argument;
...
static int trait_sector_size(sqlite3_file *base)
{
    (void)base;
    return 512;
}

static int trait_device_characteristics(sqlite3_file *base)
{
    (void)base;
    return 0;
}

static const sqlite3_io_methods trait_io = {
    .iVersion = 1,
    .xClose = trait_close,
    .xRead = trait_read,
    .xWrite = trait_write,
    .xTruncate = trait_truncate,
    .xSync = trait_sync,
    .xFileSize = trait_file_size,
    .xLock = trait_lock,
    .xUnlock = trait_unlock,
    .xCheckReservedLock = trait_check_reserved,
    .xFileControl = trait_file_control,
    .xSectorSize = trait_sector_size,
    .xDeviceCharacteristics = trait_device_characteristics
};

static int trait_open(sqlite3_vfs *vfs, const char *logical,
...
static int trait_access(sqlite3_vfs *vfs, const char *logical, int flags,
    int *result)
{
    char path[FILENAME_MAX];
    int mode = F_OK;
...
static int trait_full_path(sqlite3_vfs *vfs, const char *logical,
...
static int trait_randomness(sqlite3_vfs *vfs, int amount, char *output)
{
    const long result = amount > 0 ? trait_random(output, (size_t)amount) : 0;
    (void)vfs;
    return result < 0 ? 0 : (int)result;
}

static int trait_sleep(sqlite3_vfs *vfs, int microseconds)
{
    (void)vfs;
...
static int trait_current_time(sqlite3_vfs *vfs, double *result)
{
    (void)vfs;
    *result = 2440587.5 + (double)trait_monotonic_ns() /
        (86400.0 * 1000000000.0);
    return SQLITE_OK;
}

static int trait_last_error(sqlite3_vfs *vfs, int capacity, char *output)
{
...
static sqlite3_vfs trait_vfs = {
    .iVersion = 1,
    .szOsFile = sizeof(struct trait_sqlite_file),
    .mxPathname = FILENAME_MAX,
    .zName = "trait",
    .xOpen = trait_open,
    .xDelete = trait_delete,
    .xAccess = trait_access,
    .xFullPathname = trait_full_path,
    .xRandomness = trait_randomness,
    .xSleep = trait_sleep,
    .xCurrentTime = trait_current_time,
    .xGetLastError = trait_last_error
};

int sqlite3_os_init(void)
{
    (void)memset(open_files, 0, sizeof(open_files));
    return sqlite3_vfs_register(&trait_vfs, 1);
}
